...
extern void WMFMultilineFunctionDeclaration(int arg1, NSString *arg2,
                                            WMFCodingStyle arg3) {
  // body
}
...
- (BOOL)isEqualToCodingStyleModel:(WMFCodingStyleModel *)other {
  return self.modelIdentifier == other.modelIdentifier &&
         [self.modelIdentifier isEqualToString:other.modelIdentifier] &&
...
- (void)multiLineMacroExample {
  NSAssert(YES, @"Testing a macro invocation across multiple lines. %@ %@ %@",
           @"Another line.", @"Then another.", @"And yet, another.");
}

- (BOOL)multiLineAssignmentExample {
  BOOL foo = [[self multiLineArrayExample]
      containsObject:[NSString
                         stringWithFormat:@"Some really long assigment %d", 0]];
  return foo;
}
