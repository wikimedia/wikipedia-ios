...
- (void)testFormatOfMobileviewSectionHTMLHasNotChangedOnBeta {
    NSArray *obama349208SectionsFromFixtureFile = [self loadJSON:@"obama-beta-cluster-revision-349208"][@"mobileview"][@"sections"];
    NSURL *obama349208BetaClusterURL = [NSURL URLWithString:@"https://en.m.wikipedia.beta.wmflabs.org/w/api.php?action=mobileview&format=json&page=Barack_Obama&sections=all&prop=text%7Csections%7Crevision&revision=349208"];
    XCTestExpectation *expectation = [self expectationWithDescription:@"Wait for article fetch"];
    NSURLSessionDataTask *dataTask =
    [[NSURLSession sharedSession] dataTaskWithURL:obama349208BetaClusterURL
                                completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                                    NSDictionary *json = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
                                    NSArray *sectionsFromBetaCluster = json[@"mobileview"][@"sections"];
                                    
                                    XCTAssertEqualObjects(@(sectionsFromBetaCluster.count), @(obama349208SectionsFromFixtureFile.count), @"Section count mismatch! See test for details.");
                                    
                                    for (NSInteger i = 0; i < sectionsFromBetaCluster.count; i++) {
                                        NSDictionary *sectionFromFixtureFile = obama349208SectionsFromFixtureFile[i];
