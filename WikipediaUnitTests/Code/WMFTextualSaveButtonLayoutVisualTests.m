...
- (void)setUp {
  ...
  // these tests don't care about device (or window size), only OS version &
  // writing direction
  self.deviceAgnostic = NO;
  // record snapshots at fixed size (regardless of window size) which must be
  // larger than the content itself
  // in order to verify alignment with leading edge in LTR & RTL
  self.saveButton = [[WMFLeadingImageTrailingTextButton alloc]
