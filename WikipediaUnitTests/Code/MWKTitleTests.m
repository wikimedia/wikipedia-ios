...
- (void)testUnderscoresAndSpaces {
  NSArray *inputs = @[
    [siteURL wmf_URLWithTitle:@"Fancy title with spaces"],
    [siteURL wmf_URLWithTitle:@"Fancy_title with_spaces"]
  ];
  for (NSURL *title in inputs) {
    XCTAssertEqualObjects(title.wmf_title, @"Fancy title with spaces",
...
- (void)testFragment {
  NSURL *title = [NSURL wmf_URLWithSiteURL:siteURL
       escapedDenormalizedTitleAndFragment:@"foo#bar"];
...
- (void)testPercentEscaped {
  NSURL *title = [NSURL wmf_URLWithSiteURL:siteURL
       escapedDenormalizedTitleAndFragment:@"foo%20baz#bar"];
  assertThat(title.wmf_siteURL, is(siteURL));
