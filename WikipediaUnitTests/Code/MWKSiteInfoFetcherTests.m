...
- (void)testENWikiFixture {
  [self
      runSuccessfulCallbackTestWithFixture:@"ENWikiSiteInfo"
                                   siteURL:
                                       [NSURL wmf_URLWithDefaultSiteAndlanguage:
                                                  @"en"]];
}

- (void)testNOWikiFixture {
  [self
      runSuccessfulCallbackTestWithFixture:@"NOWikiSiteInfo"
                                   siteURL:
                                       [NSURL wmf_URLWithDefaultSiteAndlanguage:
                                                  @"no"]];
}
...
- (void)runSuccessfulCallbackTestWithFixture:(NSString *)fixture
                                     siteURL:(NSURL *)testSiteURL {
  NSString *json =
      [[self wmf_bundle] wmf_stringFromContentsOfFile:fixture ofType:@"json"];
  NSDictionary *jsonDictionary =
      [[self wmf_bundle] wmf_jsonFromContentsOfFile:fixture];

  NSRegularExpression *anyRequestFromTestSite = [NSRegularExpression
      regularExpressionWithPattern:
          [NSString
              stringWithFormat:@"%@.*",
                               [[NSURL wmf_desktopAPIURLForURL:testSiteURL]
                                   absoluteString]]
                           options:0
                             error:nil];
  ...
  [self.fetcher fetchSiteInfoForSiteURL:testSiteURL].then(
      ^(MWKSiteInfo *result) {
        assertThat(result.siteURL, is(equalTo(testSiteURL)));
        assertThat(result.mainPageTitleText,
                   is(equalTo([jsonDictionary
...
- (void)testDesktopFallback {
  ...
  NSString *json =
      [[self wmf_bundle] wmf_stringFromContentsOfFile:@"ENWikiSiteInfo"
                                               ofType:@"json"];
  NSDictionary *jsonDictionary =
      [[self wmf_bundle] wmf_jsonFromContentsOfFile:@"ENWikiSiteInfo"];
  ...
  NSRegularExpression *anyRequestFromTestSiteDesktop = [NSRegularExpression
  ...
  stubRequest(@"GET", @"https://en.m.wikipedia.org/w/"
                      @"api.php?action=query&format=json&meta=siteinfo&siprop="
                      @"general")
