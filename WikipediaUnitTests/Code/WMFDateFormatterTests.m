...
- (void)testIso8601Example {
  ...
  NSDate *decodedDate =
      [[NSDateFormatter wmf_iso8601Formatter] dateFromString:testTimestamp];
  assertThat(decodedDate, is(notNilValue()));
  assertThat(
      [[NSDateFormatter wmf_iso8601Formatter] stringFromDate:decodedDate],
      is(equalTo(testTimestamp)));
}
...
- (void)testShortTimeFormatterIsValidForAllLocales {
  ...
  [self wmf_runParallelTestsWithLocales:[NSLocale availableLocaleIdentifiers]
                                  block:^(NSLocale *locale,
                                          XCTestExpectation *e) {
                                    // need to parse date using the "regular"
                                    // formatter
                                    NSDate *decodedDate =
                                    ...
                                    // TODO: check for "AM" for corresponding
                                    // time locales
                                    assertThat(
                                        [[NSDateFormatter
                                        ...
                                        describedAs(@"expected non-nil for "
                                                    @"locale: %0 from "
                                                    @"timestamp %1",
                                                    notNilValue(),
...
- (void)testShortTimeFormatterExamples {
  ...
  NSDateFormatter *usFormatter =
      [NSDateFormatter wmf_shortTimeFormatterWithLocale:
