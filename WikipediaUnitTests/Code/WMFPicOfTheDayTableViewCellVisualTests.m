...
- (void)setUp {
  ...
  self.cell.potdImageView.wmf_imageController =
      [WMFImageController temporaryController];
}
...
- (void)testShowsCaptionWhenImageIsFinallyDownloaded {
  ...
  stubRequest(@"GET", testURL.absoluteString).andReturnRawResponse(imageData);

  [self.cell setImageURL:testURL];

  [self expectationForPredicate:[NSPredicate
                                    predicateWithBlock:^BOOL(
                                        UIImageView *_Nonnull potdImageView,
