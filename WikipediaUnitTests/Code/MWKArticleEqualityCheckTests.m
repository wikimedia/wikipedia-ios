...
- (void)testTwoArticlesWithSameTitlesAreEqual {
  ...
  MWKArticle *foo = [[MWKArticle alloc] initWithURL:url dataStore:nil];
  MWKArticle *foo2 = [[MWKArticle alloc] initWithURL:url dataStore:nil];
  assertThat(foo, is(equalTo(foo2)));
}

- (void)testTwoArticlesWithSameTitleButDifferentFragmentAreEqual {
  MWKArticle *foo = [[MWKArticle alloc]
      initWithURL:[NSURL URLWithString:@"//en.wikipedia.org/wiki/Foo"]
        dataStore:nil];
  MWKArticle *fooWithBarFragment = [[MWKArticle alloc]
      initWithURL:[NSURL URLWithString:@"//en.wikipedia.org/wiki/Foo#bar"]
        dataStore:nil];
...
- (void)testTwoArticlesWithDifferentTitlesAreNotEqual {
  MWKArticle *foo = [[MWKArticle alloc]
      initWithURL:[NSURL URLWithString:@"//en.wikipedia.org/wiki/Foo"]
        dataStore:nil];
  MWKArticle *bar = [[MWKArticle alloc]
      initWithURL:[NSURL URLWithString:@"//en.wikipedia.org/wiki/Bar"]
        dataStore:nil];
