...
- (void)wmf_runParallelTestsWithLocales:(NSArray *)localeIdentifiers
                                timeout:(NSTimeInterval)timeout
                                  block:(WMFLocaleTest)block {
  NSDictionary *expectationsForLocale = [localeIdentifiers
      bk_reduce:[NSMutableDictionary
  ...
  dispatch_apply(
      localeIdentifiers.count, concurrentBackgroundQueue, ^(size_t i) {
  ...
  NSLog(@"Waiting %f seconds to run a test with %lu locales...", timeout,
        (unsigned long)localeIdentifiers.count);
