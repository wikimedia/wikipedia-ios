...
- (void)testContainsAllUniqueEntriesPassedToInit {
  ...
  assertThat(list.entries,
             containsItemsInCollectionInAnyOrder(self.testObjects));
  assertThat(@(list.dirty), isFalse());
}
...
- (void)testContainsAddedEntry {
  ...
  assertThat(@([list containsEntryForListIndex:[firstEntry listIndex]]),
             isTrue());
...
- (void)testKVO {
  ...
  assertThat(firstChangeDictionary[NSKeyValueChangeKindKey],
             is(@(NSKeyValueChangeInsertion)));
  assertThat(firstChangeDictionary[NSKeyValueChangeIndexesKey],
             is(equalTo([NSIndexSet indexSetWithIndex:0])));

  NSDictionary *secondChangeDictionary = observations[1];
  assertThat(secondChangeDictionary[NSKeyValueChangeKindKey],
             is(@(NSKeyValueChangeRemoval)));
  assertThat(secondChangeDictionary[NSKeyValueChangeIndexesKey],
             is(equalTo([NSIndexSet indexSetWithIndex:0])));
