...
- (void)setUp {
  ...
  self.articleFetcher =
      [[WMFArticleFetcher alloc] initWithDataStore:self.tempDataStore];
...
+ (NSArray<NSInvocation *> *)testInvocations {
  return [[NSProcessInfo processInfo] wmf_isTravis] ? @[]
                                                    : [super testInvocations];
}
...
- (void)testSuccessfulFetchWritesArticleToDataStoreWithoutDuplicatingData {
  ...
  NSData *json =
      [[self wmf_bundle] wmf_dataFromContentsOfFile:@"Obama" ofType:@"json"];

  // TODO: refactor into convenience method
  NSRegularExpression *anyRequestFromTestSite = [NSRegularExpression
  ...
  expectResolutionWithTimeout(10, ^AnyPromise * {
    return [fetcher fetchArticleForURL:dummyArticleURL progress:NULL].then(^id(
        MWKArticle *article) {
      savedArticleAfterFirstFetch =
          [self.tempDataStore articleWithURL:dummyArticleURL];
      firstFetchResult = article;
      return [fetcher fetchArticleForURL:dummyArticleURL progress:NULL].then(
          ^(MWKArticle *article) {
  ...
  assertThat(
      @([firstFetchResult isDeeplyEqualToArticle:savedArticleAfterFirstFetch]),
      isTrue());

  XCTAssertTrue(
      secondFetchResult != firstFetchResult,
  ...
  MWKArticle *savedArticleAfterSecondFetch =
      [self.tempDataStore articleFromDiskWithURL:dummyArticleURL];
  assertThat(
      @([savedArticleAfterSecondFetch isDeeplyEqualToArticle:firstFetchResult]),
      isTrue());
}

- (NSDictionary *)requestHeaders {
  return self.articleFetcher.operationManager.requestSerializer
      .HTTPRequestHeaders;
}
