...
- (void)setUp {
  [super setUp];
  self.dataStore =
      [self wmf_temporaryCopyOfDataStoreFixtureAtPath:@"4.1.7/Valid"];
}
...
- (void)testReordersLegacySavedPageList {
  NSArray<MWKSavedPageEntry *> *legacyEntries =
      [[self.dataStore savedPageListData][MWKSavedPageExportedEntriesKey]
          bk_map:^id(NSDictionary *entryData) {
            MWKSavedPageEntry *entry;
            XCTAssertNoThrow(
                (entry = [[MWKSavedPageEntry alloc] initWithDict:entryData]),
                @"not expecting invalid entries for this test");
            return entry;
          }];

  XCTAssert(legacyEntries.count > 1,
            @"Need more than 1 legacy entry for this test.");
  ...
  assertThat(
      [list.entries valueForKeyPath:@"url.wmf_title"],
      contains(@"Freemanbreen", @"Glacier", @"Crevasse", @"Ice sheet", nil));
  ...
  MWKDataStore *dataStore2 =
      [[MWKDataStore alloc] initWithBasePath:self.dataStore.basePath];
  assertThat(dataStore2.savedPageListData,
             hasEntry(MWKSavedPageExportedSchemaVersionKey,
