...
- (void)testMatchesCorrespondingPowerOfTwo {
  for (NSUInteger rotation; rotation < NSUINT_BIT; rotation++) {
    ...
    XCTAssertEqual(
        ...
        (unsigned long)rotation, (unsigned long)exponent,
        (unsigned long)expectedResult, (unsigned long)actualResult);
  }
}
...
- (void)testSymmetrical {
  for (NSUInteger i; i < 50; i++) {
    ...
    for (NSUInteger rotation; rotation < NSUINT_BIT; rotation++) {
      ...
      XCTAssertEqual(
          original, symmetrical, @"Rotating %lu by %lu should be the same as "
                                 @"rotating by %lu + NSUINT_BIT (%lu)."
                                  "Got %lu expected %lu",
          (unsigned long)testValue, (unsigned long)rotation,
          (unsigned long)rotation, (unsigned long)symmetricalRotation,
          (unsigned long)symmetrical, (unsigned long)original);
    }
  }
}
