...
- (void)testShouldHaveAllChildlessSectionsAtTopLevelForFlatHierarchy {
  ...
  assertThat(flatList.entries,
             everyItem(hasProperty(WMF_SAFE_KEYPATH(MWKSection.new, children),
                                   isEmpty())));
...
- (void)testShouldHaveParentlessSectionsAsTopLevelSections {
  ...
  assertThat(
      ...
      is(equalTo(@[ list.entries[0], list.entries[1], list.entries[3] ])));
}
...
- (void)testShouldHaveAllSubSectionsAsChildOfSharedParent {
  ...
  assertThat(topLevelSection.children,
             is(equalTo(@[ list.entries[1], list.entries[2] ])));
}
...
- (void)testShouldHaveNestedSectionsAsChildrenOfTheirImmediateParent {
  ...
  assertThat([topLevelSection.children.firstObject children],
             is(equalTo(@[ list.entries[2] ])));
}
...
- (void)testShouldNotHavePrecedingSubsectionsAsChildren {
  ...
  NSArray *topLevelSections = list.topLevelSections;
  assertThat(topLevelSections,
             is(equalTo(@[ list.entries[0], list.entries[1] ])));
...
- (void)testShouldConsiderSectionsWithoutALevelAsChildlessOrphans {
  MWKSectionList *list =
      [self sectionListWithLevels:@[ @2, [NSNull null], @4 ]];
  NSArray *topLevelSections = list.topLevelSections;
...
- (void)testShouldParseSectionsNormallyIfFirstSectionDoesNotHaveLevel {
  MWKSectionList *list =
      [self sectionListWithLevels:@[ [NSNull null], @3, @2 ]];
  NSArray *topLevelSections = list.topLevelSections;
  assertThat(topLevelSections, is(equalTo(list.entries)));
  assertThat(topLevelSections,
             everyItem(hasProperty(WMF_SAFE_KEYPATH(MWKSection.new, children),
...
- (MWKSectionList *)sectionListWithLevels:(NSArray *)levels {
  MWKSectionList *list = [[MWKSectionList alloc]
      initWithArticle:self.dummyArticle
             sections:[levels bk_map:^MWKSection *(id levelOrNull) {
               NSMutableDictionary *dict = [
                   @{ @"id" : @(self.sectionIdCounter++) } mutableCopy];
               if (![[NSNull null] isEqual:levelOrNull]) {
                 dict[@"level"] = levelOrNull;
               }
               return [[MWKSection alloc] initWithArticle:self.dummyArticle
                                                     dict:dict];
             }]];
  return list;
}
