...
@interface WMFImageInfoControllerTests
    : WMFAsyncTestCase <WMFImageInfoControllerDelegate>
...
- (void)setUp {
  ...
  self.mockDelegate =
      MKTMockProtocol(@protocol(WMFImageInfoControllerDelegate));

  NSURL *testTitle =
      [[NSURL wmf_URLWithDefaultSiteAndlanguage:@"en"] wmf_URLWithTitle:@"foo"];

  self.testArticle =
      [[MWKArticle alloc] initWithURL:testTitle dataStore:self.tmpDataStore];

  NSArray<MWKImage *> *testImages =
      [[self generateSourceURLs:10] bk_map:^MWKImage *(NSString *urlString) {
        return [[MWKImage alloc] initWithArticle:self.testArticle
                                 sourceURLString:urlString];
      }];

  self.controller =
      [[WMFImageInfoController alloc] initWithDataStore:self.tmpDataStore
                                              batchSize:2
                                            infoFetcher:self.mockInfoFetcher];

  [self.controller setUniqueArticleImages:testImages
                            forArticleURL:self.testArticle.url];
...
- (void)testReadsFromDataStoreLazilyAndPopulatesFetchedIndices {
  ...
  NSURL *testURL =
      [[NSURL wmf_URLWithDefaultSiteAndCurrentLocale] wmf_URLWithTitle:@"foo"];

  MWKArticle *dummyArticle =
      [[MWKArticle alloc] initWithURL:testURL dataStore:mockDataStore];

  NSArray *testImages =
      [[self generateSourceURLs:5] bk_map:^MWKImage *(NSString *sourceURL) {
        return [[MWKImage alloc] initWithArticle:dummyArticle
  ...
  [MKTGiven([mockDataStore imageInfoForArticleWithURL:testURL])
      willReturn:expectedImageInfo];

  WMFImageInfoController *controller =
      [[WMFImageInfoController alloc] initWithDataStore:mockDataStore
                                              batchSize:2
                                            infoFetcher:self.mockInfoFetcher];
  [controller setUniqueArticleImages:testImages forArticleURL:dummyArticle.url];

  assertThat(controller.indexedImageInfo.allValues,
             containsItemsInCollectionInAnyOrder(expectedImageInfo));
  assertThat(controller.uniqueArticleImages, is(testImages));
  assertThat(
      controller.fetchedIndices,
      is(equalTo([NSIndexSet indexSetWithIndexesInRange:preFetchedRange])));
}

- (void)testBatchRange {
  for (NSUInteger i = 0; i < 10; i++) {
    NSRange batchRange = [self.controller batchRangeForTargetIndex:i];
    assertThat(@(batchRange.length),
               is(equalToUnsignedInteger(self.controller.infoBatchSize)));
    assertThat(@(batchRange.location), is(lessThanOrEqualTo(@(i))));
    assertThat(@(batchRange.location + batchRange.length),
               is(greaterThanOrEqualTo(@(i))));
  }
}
...
- (void)testFetchBatchRanges {
  ...
  [indexesToFetch enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
    ...
    [MKTVerifyCount(self.mockInfoFetcher, MKTTimes(1))
        fetchGalleryInfoForImageFiles:expectedTitles
...
- (void)testFetchBatchAlongWithNeighborReturnsOneRequestForEachFetch {
  [MKTGiven([self.mockInfoFetcher
      fetchGalleryInfoForImageFiles:
          [self expectedTitlesForRange:[self.controller
                                           batchRangeForTargetIndex:0]]
...
- (void)
    testFetchBatchAlongWithNeighborIndexesInTheSameBatchOnlyResultsInOneFetch {
  [self.controller fetchBatchContainingIndex:0
                             withNthNeighbor:self.controller.infoBatchSize - 1];
  [MKTVerifyCount(self.mockInfoFetcher, MKTTimes(1))
      fetchGalleryInfoForImageFiles:anything()
                        fromSiteURL:anything()
                            success:anything()
                            failure:anything()];
}
...
- (void)testFetchingItemsThatWereAlreadyFetchedHasNoEffect {
  ...
  [MKTVerifyCount(self.mockInfoFetcher, MKTTimes(fetchedBatches.count))
      fetchGalleryInfoForImageFiles:anything()
                        fromSiteURL:self.testArticle.url.wmf_siteURL
                            success:anything()
                            failure:anything()];
...
- (void)testErrorHandling {
  ...
  assertThat(
      @([self.controller.fetchedIndices containsIndexesInRange:attemptedBatch]),
  ...
  assertThat(
      @([self.controller.fetchedIndices containsIndexesInRange:attemptedBatch]),
      describedAs(@"batch to be reset after error handling, allowing it to be "
                  @"fetched again",
                  isFalse(), nil));
...
- (void)verifyInfoFetcherCallForIndexes:(NSIndexSet *)indexes {
  [indexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
    NSRange expectedRange = [self.controller batchRangeForTargetIndex:idx];
    assertThat(@(WMFRangeIsNotFoundOrEmpty(expectedRange)), isFalse());
    [self verifyInfoFetcherCallForRange:expectedRange
                            withSuccess:nil
                                failure:nil];
  }];
}
...
- (void)verifySuccessfulFetchesForRanges:(NSArray *)ranges {
  ...
  NSMutableArray *accumulatedFetchedImageInfos =
      [NSMutableArray arrayWithCapacity:numImages];
  ...
  assertThat(
      [self.tmpDataStore imageInfoForArticleWithURL:self.testArticle.url],
      containsItemsInCollectionInAnyOrder(accumulatedFetchedImageInfos));
  ...
  for (NSValue *boxedRange in ranges) {
    [MKTVerify(self.mockDelegate) imageInfoController:self.controller
                                        didFetchBatch:boxedRange.rangeValue];
  }

  assertThat(accumulatedFetchedImageInfos,
             hasCountOf(self.controller.uniqueArticleImages.count));
}
...
- (void)fetchRangeSuccessfully:(NSRange)range
  ...
  assertThat(@([self.controller.fetchedIndices containsIndexesInRange:range]),
             describedAs(@"Ranges should be marked as fetched the first time "
                         @"they're requested",
                         isTrue(), nil));

  NSArray *imageInfoForCurrentBatch =
      [MWKImageInfo mappedFromImages:[testImages subarrayWithRange:range]];
...
- (void)imageInfoController:(WMFImageInfoController *)controller
  ...
  [self popExpectationAfter:^{
    [self.mockDelegate imageInfoController:controller
                        failedToFetchBatch:range
                                     error:error];
  }];
}

- (void)imageInfoController:(WMFImageInfoController *)controller
              didFetchBatch:(NSRange)range {
  [self popExpectationAfter:^{
...
- (void)mockInfoFetcherSuccess:(NSRange)range {
  ...
  [self verifyInfoFetcherCallForRange:range
                          withSuccess:[successBlockCaptor capture]
                              failure:nil];
  void (^successBlock)(NSArray *) = [successBlockCaptor value];
  successBlock(
      [MWKImageInfo mappedFromImages:[self.controller.uniqueArticleImages
                                         subarrayWithRange:range]]);
}

- (void)mockInfoFetcherFailure:(NSError *)error
              forTitlesInRange:(NSRange)range {
...
- (NSArray *)createAllExpectedBatches {
  NSUInteger numBatches =
      self.controller.uniqueArticleImages.count / self.controller.infoBatchSize;
...
- (NSArray *)generateSourceURLs:(NSUInteger)count {
  ...
  for (NSUInteger i = 0; i < count; i++) {
    NSString *sourceURL = MWKCreateImageURLWithPath(
        [NSString stringWithFormat:@"/foobar/foo%lu.jpg/440px-foo%lu.jpg",
