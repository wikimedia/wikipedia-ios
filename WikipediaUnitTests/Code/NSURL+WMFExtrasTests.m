...
static NSURL *dummyURLWithExtension(NSString *extension) {
  return [NSURL
      URLWithString:[@"http://foo.org/bar." stringByAppendingString:extension]];
}
...
- (void)testMimeTypeWithPNGExtension {
  NSArray *testURLMimeTypes =
      [@[ dummyURLWithExtension(@"png"), dummyURLWithExtension(@"PNG") ]
          valueForKey:WMF_SAFE_KEYPATH(NSURL.new, wmf_mimeTypeForExtension)];
  assertThat(testURLMimeTypes, everyItem(is(@"image/png")));
}

- (void)testMimeTypeWithJPEGExtension {
  NSArray *testURLMimeTypes = [@[
    dummyURLWithExtension(@"jpg"), dummyURLWithExtension(@"jpeg"),
    dummyURLWithExtension(@"JPG"), dummyURLWithExtension(@"JPEG")
  ] valueForKey:WMF_SAFE_KEYPATH(NSURL.new, wmf_mimeTypeForExtension)];
...
- (void)testMimeTypeWithGIFExtension {
  NSArray *testURLMimeTypes =
      [@[ dummyURLWithExtension(@"gif"), dummyURLWithExtension(@"GIF") ]
          valueForKey:WMF_SAFE_KEYPATH(NSURL.new, wmf_mimeTypeForExtension)];
...
- (void)testPrependSchemeAddHTTPSToSchemelessURL {
  assertThat([[NSURL URLWithString:@"//foo.org/bar.jpg"]
                 wmf_urlByPrependingSchemeIfSchemeless],
...
- (void)testPrependSchemeReturnsOriginalURLWithScheme {
  ...
  XCTAssertTrue(urlWithScheme ==
                [urlWithScheme wmf_urlByPrependingSchemeIfSchemeless]);
}
