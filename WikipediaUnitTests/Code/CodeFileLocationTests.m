...
- (void)test_code_files_are_not_in_root_folder {
  ...
  NSArray *extensionsToKeepOutOfRoot = @[
    @"h", @"m", @"c", @"mm", @"cpp", @"swift", @"xib", @"json", @"storyboard",
    @"plist", @"xcdatamodeld"
  ];

  NSPredicate *extensionsPredicate = [NSPredicate
      predicateWithFormat:@"pathExtension IN %@", extensionsToKeepOutOfRoot];

  NSArray *filesWhichShouldNotBeInRoot =
      [[[NSFileManager defaultManager] contentsOfDirectoryAtPath:SOURCE_ROOT_DIR
                                                           error:nil]
          filteredArrayUsingPredicate:extensionsPredicate];
