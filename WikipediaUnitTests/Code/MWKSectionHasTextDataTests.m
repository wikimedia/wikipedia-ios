...
- (MWKArticle *)getTestingArticle {
  NSURL *url = [NSURL wmf_URLWithDomain:@"wikipedia.org"
                               language:@"en"
                                  title:@"Barack Obama"
                               fragment:nil];
  MWKArticle *article =
      [self articleWithMobileViewJSONFixture:@"Obama"
                                     withURL:url
...
- (void)testHasTextDataMethodReturnsYESforZeroLengthSectionHTML {
  ...
  // Ensure "[MWKSection hasTextData]" returns YES if section html isn't nil -
  // even if it's a zero length string.
  for (MWKSection *section in article.sections) {
    /*
       Reminder: zero length strings are *valid* section text data!
       Some sections have zero length strings - such as sections having
