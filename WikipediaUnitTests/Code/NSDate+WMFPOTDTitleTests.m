...
static inline NSString *expectedPOTDTitleStringForYearMonthDay(NSInteger year,
                                                               NSInteger month,
                                                               NSInteger day) {
  return
      [NSString stringWithFormat:@"%@/%ld-%02ld-%02ld_(en)", WMFPOTDTitlePrefix,
    ...
    describe(@"potd date formatting", ^{
      it(@"should always have two digits for the month & day", ^{
        ...
        expect([testDateComponents.date wmf_picOfTheDayPageTitle])
            .to(equal(expectedPOTDTitleStringForYearMonthDay(
                testDateComponents.year, testDateComponents.month,
