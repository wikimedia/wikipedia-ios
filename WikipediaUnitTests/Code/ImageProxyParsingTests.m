...
@property (nonatomic, copy) NSString *schemeOriginalSrcPrefix;
@property (nonatomic, copy) NSString *galleryAttribute;
@property (nonatomic, strong) WMFURLSchemeHandler *schemeServer;
...
- (void)setUp {
    ...
    self.schemeServer = [WMFURLSchemeHandler shared];
    self.baseURLString = self.schemeServer.baseURL.absoluteString;
    self.schemeOriginalSrcPrefix = [NSString stringWithFormat:@"%@/imageProxy?originalSrc=", self.baseURLString];
    self.imageSize = 640;
...
- (void)testImgTagSrcUrlIsConvertedToSchemeFormat {
    ...
    string = [self.schemeServer stringByReplacingImageURLsWithAppSchemeURLsInHTMLString:string withBaseURL:nil targetImageWidth:self.imageSize];
...
- (void)testObamaArticleImageSchemeSubstitutionCount {
    NSString *allObamaHTMLWithImageTagsChanged = [self.schemeServer stringByReplacingImageURLsWithAppSchemeURLsInHTMLString:[self allObamaHTML] withBaseURL:self.obamaBaseURL targetImageWidth:self.imageSize];
    NSArray *allObamaHTMLSplitOnImageScheme = [allObamaHTMLWithImageTagsChanged componentsSeparatedByString:@"imageProxy"];
...
- (void)testPerformanceOfImageTagParsing {
    ...
    [self.schemeServer stringByReplacingImageURLsWithAppSchemeURLsInHTMLString:@"Small sample string <img alt=\"Example.jpg\" src=\"//upload.wikimedia.org/wikipedia/en/thumb/a/a9/Example.jpg/20px-Example.jpg\" width=\"20\" height=\"22\" srcset=\"//upload.wikimedia.org/wikipedia/en/thumb/a/a9/Example.jpg/30px-Example.jpg 1.5x, //upload.wikimedia.org/wikipedia/en/thumb/a/a9/Example.jpg/40px-Example.jpg 2x\" data-file-width=\"275\" data-file-height=\"297\"> and stuff." withBaseURL:self.obamaBaseURL targetImageWidth:self.imageSize];
    [self measureBlock:^{
        [self.schemeServer stringByReplacingImageURLsWithAppSchemeURLsInHTMLString:[self allObamaHTML] withBaseURL:self.obamaBaseURL targetImageWidth:self.imageSize];
    }];
}
