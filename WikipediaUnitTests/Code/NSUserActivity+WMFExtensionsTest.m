...
@interface NSUserActivity_WMFExtensions_wmf_activityForWikipediaScheme_Test
    : XCTestCase
@end
...
- (void)testSearchURL {
  NSURL *url = [NSURL
      URLWithString:@"wikipedia://en.wikipedia.org/w/index.php?search=dog"];
  NSUserActivity *activity =
      [NSUserActivity wmf_activityForWikipediaScheme:url];
  XCTAssertEqual(activity.wmf_type, WMFUserActivityTypeSearchResults);
  XCTAssertEqualObjects(activity.webpageURL.absoluteString,
                        @"https://en.wikipedia.org/w/"
                        @"index.php?search=dog&title=Special:Search&fulltext="
                        @"1");
}

@end

@interface NSUserActivity_WMFExtensions_articleViewActivityWithArticle_Test
    : XCTestCase
@end
...
- (void)testArticleViewActivityWithArticle {
  MWKArticle *article = [[MWKArticle alloc]
      initWithURL:[NSURL URLWithString:@"https://en.wikipedia.org/wiki/Foo"]];
  NSUserActivity *activity =
      [NSUserActivity wmf_articleViewActivityWithArticle:article];
  XCTAssertEqual(activity.wmf_type, WMFUserActivityTypeArticle);
  XCTAssertEqualObjects(activity.webpageURL.absoluteString,
                        @"https://en.wikipedia.org/wiki/Foo");
}
