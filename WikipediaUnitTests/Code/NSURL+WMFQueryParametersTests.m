...
- (void)testValueForQueryKeyForURLWithMultipleQueryParameters {
  NSURL *url = [NSURL
      URLWithString:@"https://foo.org/bar?key=value&otherkey=othervalue"];
...
- (void)testValueForQueryKeyForURLWithKeyButNoValueForIt {
  NSURL *url =
      [NSURL URLWithString:@"https://foo.org/bar?key=&otherkey=othervalue"];
...
- (void)testChangingValueForQueryKeyWithoutChangingOtherKeysOrValues {
  ...
  assertThat(
      [url absoluteString],
      is(@"http://localhost:8080?KEY=NEWVALUE&originalSrc=http://this.jpg"));
}
...
- (void)testRemovingValueForQueryKeyWithoutChangingOtherKeysOrValues {
  ...
  assertThat([url absoluteString],
             is(@"http://localhost:8080?originalSrc=http://this.jpg"));
}
