...
static NSString *const MediumSearchResultTitle =
    @"This is a moderately long title that should take up two lines.";

static NSString *const LongSearchResultTitle =
    @"This is an excessively lengthy title that I wrote for testing, which "
    @"should take up three lines of text"
     " (at the very least).";

static NSString *const ShortSearchResultDescription = @"One line description";

static NSString *const LongSearchResultDescription =
    @"This description describes a search result, and should take "
    @"approximately three lines to display.";
...
- (void)testShouldTruncateAndShrinkTitle {
  NSString *reallyLongString =
      [LongSearchResultTitle stringByAppendingString:LongSearchResultTitle];
  [self populateTitleLabelWithString:reallyLongString searchQuery:nil];
  [self.searchResultCell setDescriptionText:ShortSearchResultDescription];
  [self wmf_verifyView:self.searchResultCell width:320.f];
}
...
- (void)testShouldHighlightMatchingSubstring {
  NSString *mediumTitleSubstring = [MediumSearchResultTitle
      substringToIndex:MediumSearchResultTitle.length * 0.3];
  [self populateTitleLabelWithString:MediumSearchResultTitle
                         searchQuery:mediumTitleSubstring];
  [self.searchResultCell setDescriptionText:ShortSearchResultDescription];
  [self wmf_verifyView:self.searchResultCell width:320.f];
}
...
- (void)populateTitleLabelWithString:(NSString *)titleText
