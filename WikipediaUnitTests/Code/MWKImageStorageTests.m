...
- (void)setUp {
  [super setUp];
  self.goldenGateImageURL =
      @"https://upload.wikimedia.org/wikipedia/commons/thumb/c/c2/"
      @"Golden_Gate_Bridge%2C_SF_%28cropped%29.jpg/"
      @"500px-Golden_Gate_Bridge%2C_SF_%28cropped%29.jpg";
...
- (void)testLoadNonexistentImage {
  ...
  XCTAssertNotNil([[MWKImage alloc] initWithArticle:self.article
                                    sourceURLString:self.goldenGateImageURL]);
}

- (void)testLoadNonexistentImageData {
  MWKImage *image = [[MWKImage alloc] initWithArticle:self.article
                                      sourceURLString:self.goldenGateImageURL];
