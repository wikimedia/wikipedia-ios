...
- (void)testSnippetFromTextWithCitaiton {
  assertThat(
      [@"March 2011.[9][10] It was the first spacecraft to orbit Mercury.[7]"
...
- (void)testNestedParenthesesRemoval {
  ...
  XCTAssertEqualObjects(
      [string wmf_stringByRecursivelyRemovingParenthesizedContent], @"Hello");
}

- (void)testBracketedContentRemoval {
  NSString *string = @"J[aeio]ump";
  XCTAssertEqualObjects([string wmf_stringByRemovingBracketedContent], @"Jump");
}
...
- (void)testRemovalOfLeadingOrTrailingSpacesNewlinesOrColons {
  NSString *string = @"\n          Syncopation:\n:";
  XCTAssertEqualObjects(
      [string wmf_stringByRemovingLeadingOrTrailingSpacesNewlinesOrColons],
      @"Syncopation");
}
...
- (void)testPunctuationAwareWhitespaceCollapsingCommasSemicolonsAndPeriods {
  ...
  XCTAssertEqualObjects(
      [string wmf_getCollapsedWhitespaceStringAdjustedForTerminalPunctuation],
      @"trim space before commas, semicolons; and periods.");
}
...
- (void)testPunctuationAwareWhitespaceCollapsingLeadingAndTrailingWhitespace {
  ...
  XCTAssertEqualObjects(
      [string wmf_getCollapsedWhitespaceStringAdjustedForTerminalPunctuation],
      @"trim leading and trailing whitespace ok?");
}

- (void)
    testPunctuationAwareWhitespaceCollapsingReduceWhitespaceAroundParenthesisOrBrackets {
  NSString *string = @"collapse but do not (   no!   ) completely remove space "
                     @"around parenthesis or brackets [ \t brackets!\t]";
  XCTAssertEqualObjects(
      [string wmf_getCollapsedWhitespaceStringAdjustedForTerminalPunctuation],
      @"collapse but do not ( no! ) completely remove space around parenthesis "
...
- (void)testAllWhitepacesCollapsing {
  ...
  XCTAssertEqualObjects(
      [string wmf_stringByCollapsingAllWhitespaceToSingleSpaces],
