...
- (void)testNonEmptyPrefixResponse {
  ...
  stubRequest(@"GET",
              [NSRegularExpression
                  regularExpressionWithPattern:@"generator=prefixsearch.*foo.*"
                                       options:0
                                         error:nil])
      .andReturn(200)
      .withJSON(json);
    ...
    return [self.fetcher fetchArticlesForSearchTerm:@"foo"
        ...
        .then(^(WMFSearchResults *result) {
          assertThat(result.results,
                     hasCountOf([[json valueForKeyPath:@"query.pages"] count]));
        });
  });
}
...
- (void)testEmptyPrefixResponse {
  id json = [[self wmf_bundle]
      wmf_jsonFromContentsOfFile:@"NoSearchResultsWithSuggestion"];
  NSParameterAssert(json);
  ...
  expectResolutionWithTimeout(10, ^{
    return [self.fetcher fetchArticlesForSearchTerm:@"foo"
                                        ...
                                        resultLimit:15]
        .then(^(WMFSearchResults *result) {
          assertThat(result.searchSuggestion,
                     is([json valueForKeyPath:@"query.searchinfo.suggestion"]));
...
- (void)testAppendingToPrefixResults {
  NSData *prefixResponseData = [[self wmf_bundle]
      wmf_dataFromContentsOfFile:@"NoSearchResultsWithSuggestion"
                          ofType:@"json"];
  NSParameterAssert(prefixResponseData);

  WMFSearchResults *prefixResults =
      [self.fetcher.operationManager.responseSerializer
          responseObjectForResponse:nil
                               data:prefixResponseData
                              error:nil];
  prefixResults.searchTerm = @"foo";

  XCTAssertNotNil(prefixResults,
                  @"Failed to serialize prefix response fixture.");

  NSData *fullTextSearchJSONData =
      [[self wmf_bundle] wmf_dataFromContentsOfFile:@"MonetFullTextSearch"
                                             ofType:@"json"];
  ...
  XCTAssertNotNil(fullTextResults,
                  @"Failed to serialize full-text response fixture");
  ...
  stubRequest(@"GET", [NSRegularExpression
                          regularExpressionWithPattern:@".*generator=search.*"
                                               options:0
                                                 error:nil])
      .andReturn(200)
  ...
  // expect KVO notification, must be done outside of "expect" since you can't
  // add expectations once waiting has started
  [self keyValueObservingExpectationForObject:prefixResults
                                      keyPath:WMF_SAFE_KEYPATH(prefixResults,
                                                               results)
                                expectedValue:nil];
  ...
  expectResolutionWithTimeout(10, ^{
    return [self.fetcher
               fetchArticlesForSearchTerm:expectedMergedResults.searchTerm
                                  siteURL:[NSURL wmf_randomSiteURL]
                              resultLimit:15
                           fullTextSearch:YES
