...
- (void)setUp {
  ...
  self.infoPlistFilePaths =
      [self.lprojFiles bk_map:^NSString *(NSString *lprojFileName) {
        return [[LOCALIZATIONS_DIR stringByAppendingPathComponent:lprojFileName]
...
- (NSArray *)bundledLprogFiles {
  return
      [[[NSFileManager defaultManager] contentsOfDirectoryAtPath:self.bundleRoot
                                                           error:nil]
          filteredArrayUsingPredicate:
              [NSPredicate predicateWithFormat:@"pathExtension='lproj'"]];
}

- (NSArray *)allLprogFiles {
  return [[[NSFileManager defaultManager]
      contentsOfDirectoryAtPath:LOCALIZATIONS_DIR
                          error:nil]
      filteredArrayUsingPredicate:
          [NSPredicate predicateWithFormat:@"pathExtension='lproj'"]];
}

- (NSDictionary *)getTranslationStringsDictFromLprogAtPath:
    (NSString *)lprojPath {
  NSString *stringsFilePath =
      [lprojPath stringByAppendingPathComponent:@"Localizable.strings"];
...
- (void)test_incoming_translation_string_for_reversed_substitution_shortcuts {
  for (NSString *lprojFileName in self.lprojFiles) {
    NSDictionary *stringsDict = [self
        getTranslationStringsDictFromLprogAtPath:
            [self.bundleRoot stringByAppendingPathComponent:lprojFileName]];
...
- (void)test_incoming_translation_string_for_html {
  for (NSString *lprojFileName in self.lprojFiles) {
    ...
    for (NSString *key in stringsDict) {
      NSString *localizedString = stringsDict[key];
      assertThat(localizedString,
                 isNot(stringContainsInOrder(@"<", @">", nil)));
...
- (NSArray *)unbundledLprojFilesWithTranslations {
  ...
  return [self.unbundledLprojFiles bk_select:^BOOL(NSString *lprojFileName) {
    BOOL isDirectory = NO;
    NSString *localizableStringsFilePath =
        [[LOCALIZATIONS_DIR stringByAppendingPathComponent:lprojFileName]
            stringByAppendingPathComponent:@"Localizable.strings"];
    return [[NSFileManager defaultManager]
        fileExistsAtPath:localizableStringsFilePath
             isDirectory:&isDirectory];
  }];
}
...
- (void)test_all_translated_languages_were_added_to_project_localizations {
  NSMutableArray *files =
      [self.unbundledLprojFilesWithTranslations mutableCopy];
  [files removeObjectsInArray:[self languagesUnsureHowToMapToWikiCodes]];

  // Fails if any lproj languages have translations (in "Localizable.strings")
  // but are
  // not yet bundled in the project.

  // So, if this test fails, the languages listed will need to be added these to
...
- (NSArray *)languagesUnsureHowToMapToWikiCodes {
  ...
  // TODO: ^ revisit these
  return @[
    @"azb.lproj", @"be-tarask.lproj", @"bgn.lproj", @"cnh.lproj",
    @"ku-latn.lproj", @"mai.lproj",
    @"pt-br.lproj", // for some reason Brazilian Portugese is still showing up
                    // as not bundled, but I added it... hmm...
    @"sa.lproj", @"sd.lproj", @"tl.lproj", @"vec.lproj", @"xmf.lproj",
    @"ba.lproj"
  ];
}
...
- (void)test_each_InfoPlist_strings_file_contains_CFBundleDisplayName_key {
  for (NSString *path in [self infoPlistFilePaths]) {
    if (![[[self getDictFromPListAtPath:path] allKeys]
            containsObject:@"CFBundleDisplayName"]) {
