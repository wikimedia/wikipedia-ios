...
configureTempDataStoreForEach(tempDataStore, ^{
  controllerCache = [[WMFExploreSectionControllerCache alloc]
      initWithDataStore:tempDataStore];
});

describe(@"cache invalidation", ^{
  __block WMFExploreSection *cachedSection =
      [WMFExploreSection pictureOfTheDaySectionWithDate:[NSDate date]];
  __block __weak id<WMFExploreSectionController> weakCachedController;

  beforeEach(^{
    id<WMFExploreSectionController> controller =
        [controllerCache getOrCreateControllerForSection:cachedSection
                                           creationBlock:nil];
    weakCachedController = controller;
  });

  it(@"it should not have a section for that controller when the controller is "
     @"removed from cache",
     ^{
       ...
       expect(weakCachedController)
           .toEventuallyWithDescription(
               beNil(), @"Purged controller should have been deallocated!");

       expect([controllerCache.reverseLookup objectForKey:cachedSection])
           .toEventuallyWithDescription(beNil(), @"Internal reverse map of "
                                                 @"sections to controllers "
