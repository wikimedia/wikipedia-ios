...
- (void)testExample {
  ...
  testDateComponents.calendar =
      [NSCalendar calendarWithIdentifier:NSCalendarIdentifierISO8601];
  testDateComponents.month = 11;
  testDateComponents.day = 10;
  testDateComponents.year = 2015;

  NSString *testDatePattern =
      @"https://en\\.wikipedia\\.org.*TFA_title/November%2010%2C%202015.*";
  ...
  NSRegularExpression *previewRequest = [NSRegularExpression
      regularExpressionWithPattern:@"https://"
                                   @"en\\.wikipedia\\.org.*titles=Mackensen-"
                                   @"class%20battlecruiser.*"
                           options:0
                             error:nil];
  ...
  stubRequest(@"GET", tfaTitleRequest)
      ...
      .withBody([[self wmf_bundle]
          wmf_stringFromContentsOfFile:@"TFATitleExtract"
                                ofType:@"json"]);
  ...
  stubRequest(@"GET", previewRequest)
      ...
      .withHeader(@"Content-Type", @"application/json")
      .withBody([[self wmf_bundle]
          wmf_stringFromContentsOfFile:@"TitlePreviewQuery"
                                ofType:@"json"]);

  expectResolution(^AnyPromise * {
    return [self.fetcher
               fetchFeaturedArticlePreviewForDate:[testDateComponents date]]
        .then(^(MWKSearchResult *result) {
          XCTAssertEqualObjects(result.displayTitle,
