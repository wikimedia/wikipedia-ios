...
- (NSString *)wmf_pathForFixtureResource:(NSString *)filename ofType:(NSString *)type {
    NSString *resourceName = type.length > 0 ? [filename stringByAppendingPathExtension:type] : filename;
    return [[[self resourcePath] stringByAppendingPathComponent:@"Fixtures"] stringByAppendingPathComponent:resourceName];
}

- (NSString *)wmf_stringFromContentsOfFile:(NSString *)filename ofType:(NSString *)type {
    NSError *error;
    NSString *string = [NSString stringWithContentsOfFile:[self wmf_pathForFixtureResource:filename ofType:type]
...
- (NSData *)wmf_dataFromContentsOfFile:(NSString *)filename ofType:(NSString *)type {
    NSError *error;
    NSData *data = [NSData dataWithContentsOfFile:[self wmf_pathForFixtureResource:filename ofType:type]
                                          options:0
                                            error:&error];
    NSAssert(!error, @"Unexpected error reading test fixture: %@.%@, %@", filename, type, error);
    return data;
