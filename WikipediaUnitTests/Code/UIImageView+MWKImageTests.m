...
- (void)setUp {
  ...
  NSURL *dummyURL = [NSURL wmf_URLWithDomain:@"wikipedia.org"
                                    language:@"en"
                                       title:@"Foo"
                                    fragment:nil];
  ...
  stubRequest(@"GET", self.testURL.absoluteString)
      .andReturnRawResponse(self.stubbedData);
}
...
- (void)testSuccessfullySettingImageFromMetadataWithCenterFaces {
  MWKImage *testMetadata = [[MWKImage alloc] initWithArticle:self.dummyArticle
  ...
  XCTAssert(self.imageView.wmf_imageController == self.imageController,
            @"Image controller should be set immediately after the method is "
            @"called so it can be cancelled.");

  XCTAssert(self.imageView.wmf_imageMetadata == testMetadata,
            @"Image metadata should be set immediately after the method is "
            @"called so it can be checked & cancelled.");
  ...
  XCTAssert([[UIImageView faceDetectionCache]
                imageRequiresFaceDetection:testMetadata] == NO,
...
- (void)testSuccessfullySettingImageFromURLWithCenterFaces {
  ...
  XCTAssert(self.imageView.wmf_imageURL == self.testURL,
            @"Image url should be set immediately after the method is called "
            @"so it can be checked & cancelled.");

  WaitForExpectations();

  XCTAssert([[UIImageView faceDetectionCache]
                imageAtURLRequiresFaceDetection:self.testURL] == NO,
            @"Face detection should have ran.");
...
- (void)testSuccessfullySettingImageFromMetadataWithoutCenterFaces {
  ...
  XCTAssert([[UIImageView faceDetectionCache]
                imageRequiresFaceDetection:testMetadata] == YES,
...
- (void)testSuccessfullySettingImageFromURLWithoutCenterFaces {
  ...
  XCTAssert([[UIImageView faceDetectionCache]
                imageAtURLRequiresFaceDetection:self.testURL] == YES,
            @"Face detection should NOT have ran.");
...
- (void)testSuccessfullySettingCachedImageWithoutCenterFaces {
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"wait for download"];
...
- (void)testFailureToSetUncachedImageWithFetchError {
  ...
  [self.imageView wmf_setImageWithMetadata:testMetadata
      ...
      success:^{
        XCTFail(@"Promise fullfilled erroneously with url %@",
                [self.testURL description]);
      }];
...
- (void)testFailureOfImageCacheToDetectFacesOfImageWithNoFaces {
  ...
  [self.faceDetectionCache detectFaceBoundsInImage:[UIImage new]
      ...
      success:^(NSValue *value) {
        XCTAssert(CGRectIsEmpty([value CGRectValue]),
                  @"Bounds should be null since the image has no data");
        [promiseExpectation fulfill];
      }];
  ...
  XCTAssertFalse(
      [self.faceDetectionCache imageAtURLRequiresFaceDetection:self.testURL]);
