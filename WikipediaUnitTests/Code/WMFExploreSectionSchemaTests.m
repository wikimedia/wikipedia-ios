...
// Convenience setup method which creates a schema for the given site, injecting
// mocks & temporary stores for other fields.
void (^setupSchemaWithSiteURL)(NSURL *) = ^(NSURL *siteURL) {
  // TODO: setup w/ temp blacklist
  schema = [WMFExploreSectionSchema
      schemaWithSiteURL:siteURL
        ...
        locationManager:mockLocationManager
                   file:[NSURL fileURLWithPath:WMFRandomTemporaryFileOfType(
                                                   @"plist")]];
};
...
afterEach(^{
  ...
  [[NSFileManager defaultManager] removeItemAtPath:schema.fileURL.path
                                             error:nil];
});
...
describe(@"initial state", ^{
  ...
  context(@"en wiki", ^{
    ...
    context(@"location allowed", ^{
      beforeEach(^{
        [mockLocationManager
            setLocation:[[CLLocation alloc] initWithLatitude:0 longitude:0]];
      });
      ...
      it(@"should contain everything except 'because you read' sections", ^{
        ...
        expect([schema.sections
                   valueForKey:WMF_SAFE_KEYPATH([WMFExploreSection new], type)])
            .withTimeout(5)
            .toEventually(equal(@[
              @(WMFExploreSectionTypeFeaturedArticle),
              @(WMFExploreSectionTypeMostRead),
              @(WMFExploreSectionTypePictureOfTheDay),
              @(WMFExploreSectionTypeMainPage), @(WMFExploreSectionTypeRandom),
              @(WMFExploreSectionTypeNearby)
            ]));

        WMFExploreSection *featuredArticleSection = schema.sections[0];
        expect(@(featuredArticleSection.type))
            .to(equal(@(WMFExploreSectionTypeFeaturedArticle)));
        ...
        expect(@(mostReadSection.type))
            .to(equal(@(WMFExploreSectionTypeMostRead)));
        ...
        expect(@(potdSection.type))
            .to(equal(@(WMFExploreSectionTypePictureOfTheDay)));
        expect(@([potdSection.dateCreated isToday])).to(beTrue());

        WMFExploreSection *mainPageSection = schema.sections[3];
        expect(@(mainPageSection.type))
            .to(equal(@(WMFExploreSectionTypeMainPage)));
    ...
    context(@"no location", ^{
      it(@"should contain everything except 'because you read', continue "
         @"reading, and location sections",
         ^{
           setupSchemaWithSiteURL(siteURL);
           expect(@([schema.lastUpdatedAt isToday])).to(beTrue());
           expect(
           ...
           WMFExploreSection *randomSection = schema.sections[4];
           expect(@(randomSection.type))
               .to(equal(@(WMFExploreSectionTypeRandom)));
           expect(@([randomSection.dateCreated isToday])).to(beTrue());
         });
    });
  });
...
describe(@"persistence", ^{
  ...
  it(@"should be equal to a copy read from data serialized to disk", ^{
    ...
    WMFExploreSectionSchema *schema2 =
        [WMFExploreSectionSchema schemaWithSiteURL:schema.siteURL
                                        savedPages:schema.savedPages
                                           history:schema.historyPages
                                         blackList:schema.blackList
                                   locationManager:mockLocationManager
                                              file:schema.fileURL];
    // lastUpdatedAt times will be slightly different, since it will forcibly
    // update on creation
...
describe(@"updates", ^{
  ...
  pending(
      @"should add new location sections when location changes significantly",
      ^{
      });
  pending(@"should add new 'daily' sections every day", ^{
          });
  pending(@"should add recommended titles for saved & recent pages not already "
          @"in the schema",
          ^{
          });
});
