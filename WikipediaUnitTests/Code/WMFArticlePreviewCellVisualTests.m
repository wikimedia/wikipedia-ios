...
- (void)testLayoutWithShortExtractWithoutImage {
  ...
  FBSnapshotVerifyViewWithOptions(
      ...
      FBSnapshotTestCaseDefaultSuffixes(), 0.1);
}
...
- (void)configureCellWithTitleText:(NSString *)titleText
  ...
  NSURL *url = [[NSURL wmf_URLWithDefaultSiteAndCurrentLocale]
      wmf_URLWithTitle:titleText];

  [self.cell setSaveableURL:url
              savedPageList:self.dataStore.userDataStore.savedPageList];
  ...
  if (imageURL) {
    stubRequest(@"GET", imageURL.absoluteString)
        .andReturn(200)
        .withBody([[self wmf_bundle] wmf_dataFromContentsOfFile:@"golden-gate"
                                                         ofType:@".jpg"]);

    XCTestExpectation *expectation =
        [self expectationWithDescription:@"waiting for image set"];
    @weakify(self)[self.cell setImageURL:imageURL
...
- (NSString *)longExtract {
  NSMutableString *longExtract =
      [NSMutableString stringWithString:@"This extract is "];
  for (int i = 0; i < 20; i++) {
