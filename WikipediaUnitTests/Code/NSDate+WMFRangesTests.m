...
describe(@"later dates", ^{
  it(@"should return later today when asking for the dates between earlier "
     @"today and later today",
     ^{
       expect([startOfToday wmf_datesUntilDate:today]).to(equal(@[ today ]));
     });
  it(@"should return expected dates from yesterday until tomorrow in "
     @"descending order",
     ^{
       expect([yesterday wmf_datesUntilDate:tomorrow])
           .to(equal(@[ tomorrow, today, yesterday ]));
     });
});

describe(@"earlier dates", ^{
  it(@"should return later today when asking for the dates between later today "
     @"and earlier today",
     ^{
       expect([today wmf_datesUntilDate:startOfToday]).to(equal(@[ today ]));
     });
  it(@"should return expected dates from tomorrow until yesterday in "
     @"descending order",
     ^{
       expect([tomorrow wmf_datesUntilDate:yesterday])
           .to(equal(@[ tomorrow, today, yesterday ]));
     });
});
