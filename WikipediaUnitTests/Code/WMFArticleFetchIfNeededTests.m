...
describe(@"fetchLatestVersionOfTitleIfNeeded", ^{
  context(@"title is cached with revision ID", ^{
    it(@"should fetch the latest if cache is out of date", ^{
      NSMutableDictionary *latestArticleJSON =
          [[[self wmf_bundle] wmf_jsonFromContentsOfFile:@"Obama"] mutableCopy];
      [latestArticleJSON setValue:@3 forKeyPath:@"mobileview.revision"];
      MWKArticle *cachedArticle =
          [[MWKArticle alloc] initWithURL:[NSURL wmf_randomArticleURL]
      ...
      stubRequest(@"GET",
                  [NSRegularExpression regularExpressionWithPattern:@"rvprop"
                                                            options:0
                                                              error:nil])
          .andReturn(200)
      ...
      stubRequest(@"GET", [NSRegularExpression
                              regularExpressionWithPattern:@"mobileview"
                                                   options:0
                                                     error:nil])
          .andReturn(200)
          .withJSON(latestArticleJSON);

      AnyPromise *fetchedArticlePromise = [articleFetcher
          fetchLatestVersionOfArticleWithURLIfNeeded:cachedArticle.url
                                            progress:nil];
      expect(@([fetchedArticlePromise resolved]))
          .withTimeout(WMFDefaultExpectationTimeout)
          .toEventually(beTrue());
      MWKArticle *fetchedArticle = [fetchedArticlePromise value];
      expect(fetchedArticle)
          .toNotWithDescription(equal(cachedArticle),
                                @"Should have fetched latest revision.");
      expect(fetchedArticle.entityDescription)
          .to(equal(cachedArticle.entityDescription));
    ...
    it(@"should ignore the cache if force is passed", ^{
      ...
      AnyPromise *fetchedArticlePromise =
          [articleFetcher fetchLatestVersionOfArticleWithURL:cachedArticle.url
                                               forceDownload:YES
                                                    progress:nil];
      expect(@([fetchedArticlePromise resolved]))
          .withTimeout(WMFDefaultExpectationTimeout)
      ...
      MWKArticle *fetchedArticle = [fetchedArticlePromise value];
      expect(fetchedArticle)
          .toNotWithDescription(equal(cachedArticle), @"Should have fetched.");
      expect(fetchedArticle.entityDescription)
          .to(equal(cachedArticle.entityDescription));
      expect(fetchedArticle.sections).to(equal(cachedArticle.sections));
    ...
    whenOffline(^{
      it(@"should fall back to the cached article", ^{
        NSMutableDictionary *latestArticleJSON = [[[self wmf_bundle]
            wmf_jsonFromContentsOfFile:@"Obama"] mutableCopy];
        [latestArticleJSON setValue:@3 forKeyPath:@"mobileview.revision"];
        MWKArticle *cachedArticle =
            [[MWKArticle alloc] initWithURL:[NSURL wmf_randomArticleURL]
                                  dataStore:tempDataStore
        ...
        AnyPromise *fetchedArticlePromise = [articleFetcher
            fetchLatestVersionOfArticleWithURLIfNeeded:cachedArticle.url
                                              progress:nil];
        expect(@([fetchedArticlePromise rejected]))
            .withTimeout(WMFDefaultExpectationTimeout)
            .toEventually(beTrue());
