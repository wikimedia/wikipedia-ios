...
describe(@"most read date", ^{
  it(@"should always be 1 or 2 days before the current date", ^{
    [NSTimeZone wmf_forEachKnownTimeZoneAsDefault:^{
      ...
      expect(@([bestDate timeIntervalSinceNow]))
          .to(beGreaterThan(@(-86400 * 3)));
    }];
  });

  it(@"should be yesterday if the hour is >= threshold, otherwise day before "
     @"yesterday",
     ^{
       for (int i = 0; i < 24; i++) {
         NSDate *nowAtHour = [[NSCalendar wmf_utcGregorianCalendar]
             dateBySettingHour:i
                        minute:0
                        second:0
                        ofDate:[NSDate date]
         ...
         NSInteger bestDateDaysSinceNowAtHour =
             [[NSCalendar wmf_utcGregorianCalendar]
                 components:NSCalendarUnitDay
                   fromDate:nowAtHour
                    ...
                    options:NSCalendarMatchStrictly]
                 .day;
         NSNumber *expectedDayDelta =
             i >= WMFPageviewDataAvailabilityThreshold ? @(-1) : @(-2);
