...
id HC_containsItemsInCollectionInAnyOrder(id itemsOrMatchers) {
  return [HCIsCollectionContainingInAnyOrder
      wmf_isCollectionContainingItemsInAnyOrder:itemsOrMatchers];
}

@implementation HCIsCollectionContainingInAnyOrder (WMFCollectionMatcherUtils)

+ (instancetype)wmf_isCollectionContainingItemsInAnyOrder:
    (NSArray *)itemsOrMatchers {
  return [self
      isCollectionContainingInAnyOrder:[itemsOrMatchers bk_map:^id(id obj) {
