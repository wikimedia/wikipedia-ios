...
configureTempDataStoreForEach(tempDataStore, ^{
  ...
  recentPagesDataSource =
      [[WMFRecentPagesDataSource alloc] initWithRecentPagesList:historyList];
});
  ...
  context(@"history contains items from today", ^{
    ...
    describe(@"first section", ^{
      it(@"should have items from today", ^{
        NSArray<NSURL *> *firstSectionTitles = [[SSBaseDataSource
            indexPathArrayWithRange:NSMakeRange(0, todaysTitles.count)
                          inSection:0] bk_map:^NSURL *(NSIndexPath *indexPath) {
      ...
      it(@"should have a 'TODAY' header", ^{
        expect([recentPagesDataSource titleForHeaderInSection:0])
            .to(contain(@"TODAY"));
      });
    });
    ...
    context(@"history also contains items from yesterday", ^{
      ...
      beforeEach(^{
        yesterdaysTitles =
            [historyList injectWithStubbedEntriesFromDate:yesterday];
      });

      describe(@"yesterday section", ^{
        it(@"should have items from yesterday", ^{
          NSArray<NSURL *> *secondSectionTitles = [[SSBaseDataSource
              indexPathArrayWithRange:NSMakeRange(0, yesterdaysTitles.count)
                            inSection:1]
        ...
        it(@"should have a 'YESTERDAY' header", ^{
          expect([recentPagesDataSource titleForHeaderInSection:1])
      ...
      context(@"history also contains items from last week", ^{
        ...
        beforeEach(^{
          lastWeeksTitles =
              [historyList injectWithStubbedEntriesFromDate:lastWeek];
        });

        it(@"should have a single section with all entries from last week", ^{
          NSArray<NSURL *> *thirdSectionTitles = [[SSBaseDataSource
              indexPathArrayWithRange:NSMakeRange(0, yesterdaysTitles.count)
                            inSection:2]
              bk_map:^NSURL *(NSIndexPath *indexPath) {
                return [recentPagesDataSource urlForIndexPath:indexPath];
        ...
        it(@"should have a header with last week's formatted day", ^{
          expect([recentPagesDataSource titleForHeaderInSection:2])
              .to(contain([[NSDateFormatter wmf_mediumDateFormatterWithoutTime]
                  stringFromDate:lastWeek]));
        });
      });
    });
  });
...
QuickSpecEnd
  ...
  entryFromLaterThatDay.date =
      [NSDate dateWithTimeInterval:10 sinceDate:entryFromDate.date];

  [self addEntry:entryFromDate];
  [self addEntry:entryFromLaterThatDay];

  NSArray<NSURL *> *orderedTitlesFromDate = [self.entries
      wmf_mapAndRejectNil:^id _Nullable(MWKHistoryEntry *_Nonnull obj) {
