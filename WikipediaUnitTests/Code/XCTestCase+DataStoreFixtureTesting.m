...
- (nullable MWKDataStore *)wmf_temporaryCopyOfDataStoreFixtureAtPath:
    (NSString *)relativeFolderPath {
  NSString *absFolderPath = [[[self wmf_bundle] resourcePath]
      stringByAppendingPathComponent:relativeFolderPath];
  NSString *tmpCopyPath = WMFRandomTemporaryPath();
  NSError *error;
  if (![[NSFileManager defaultManager] copyItemAtPath:absFolderPath
                                               toPath:tmpCopyPath
                                                error:&error]) {
