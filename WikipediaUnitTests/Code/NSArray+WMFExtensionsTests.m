...
- (void)
    test_wmf_arrayByTrimmingToLengthFromEnd_arraySmallerThanRequestedLength {
...
- (void)testSafeSubarrayShouldLimitToCount {
  ...
  assertThat([original wmf_safeSubarrayWithRange:NSMakeRange(0, 5)],
             is(original));
}

- (void)testSafeSubarrayShouldReturnEmptyArrayIfRangeLocationOutOfBounds {
  assertThat(([@[ @0, @1 ] wmf_safeSubarrayWithRange:NSMakeRange(2, 1)]),
             isEmpty());
}

- (void)testSafeSubarrayShouldReturnEmptyIfRangeIsNotFound {
  assertThat(
      ([@[ @0, @1 ] wmf_safeSubarrayWithRange:NSMakeRange(NSNotFound, 1)]),
      isEmpty());
}

- (void)testSafeSubarrayShouldReturnEmptyIfRangeIsEmpty {
  assertThat(([@[ @0, @1 ] wmf_safeSubarrayWithRange:NSMakeRange(0, 0)]),
             isEmpty());
}
...
- (void)testArrayByRemovingFirstElement_shouldReturnAllButTheFirstElement {
  assertThat(([@[ @0, @1 ] wmf_arrayByRemovingFirstElement]),
             is(equalTo(@[ @1 ])));
}

- (void)
    testArrayByRemovingFirstElement_shouldReturnEmptyArrayFromSingletonList {
...
- (void)testArrayByInterleaving_firstObject {
  assertThat(
      [self.array wmf_arrayByInterleavingElementsFromArray:self.otherArray][0],
      is(@"one"));
}

- (void)testArrayByInterleaving_secondObject {
  assertThat(
      [self.array wmf_arrayByInterleavingElementsFromArray:self.otherArray][1],
      is(@1));
}

- (void)testArrayByInterleaving_lastObject {
  assertThat(
      [[self.array wmf_arrayByInterleavingElementsFromArray:self.otherArray]
          lastObject],
      is(@3));
}
...
- (void)testArrayByInterleaving_firstObjectOtherArray {
  assertThat(
      [self.otherArray wmf_arrayByInterleavingElementsFromArray:self.array][0],
      is(@1));
}

- (void)testArrayByInterleaving_secondObjectOtherArray {
  assertThat(
      [self.otherArray wmf_arrayByInterleavingElementsFromArray:self.array][1],
      is(@"one"));
}

- (void)testArrayByInterleaving_lastObjectOtherArray {
  assertThat(
      [[self.otherArray wmf_arrayByInterleavingElementsFromArray:self.array]
          lastObject],
      is(@3));
}
