...
- (void)testReduce {
    ...
    assertThat([indexes bk_reduce:[NSMutableArray new] withBlock:^id (NSMutableArray* acc, NSUInteger idx) {
        [acc addObject:@(idx)];
        return acc;
...
- (void)testBadInput {
    assertThat([[NSIndexSet indexSet] bk_reduce:nil withBlock:^id (id _, NSUInteger __) { return nil; }],
               is(nilValue()));
