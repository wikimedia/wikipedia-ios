...
QuickSpecBegin(WMFSearchResultMergeTests)
    ...
    describe(@"merging results", ^{
      void (^verifyKVOInsertionOfResults)(
          WMFSearchResults *, WMFSearchResults *) = ^(WMFSearchResults *r1,
                                                      WMFSearchResults *r2) {
        NSArray *originalResults = r1.results;
        [self
            keyValueObservingExpectationForObject:r1
                                          keyPath:WMF_SAFE_KEYPATH(r1, results)
                                          handler:^BOOL(
                                              id _Nonnull observedObject,
      ...
      it(@"should preserve result ordering, putting original results first", ^{
        NSArray *originalResults = @[
          dummySearchResultWithIndex(0), dummySearchResultWithIndex(1),
        ...
        WMFSearchResults *r1 =
            [[WMFSearchResults alloc] initWithSearchTerm:@"foo"
                                        ...
                                        redirectMappings:@[]];
        WMFSearchResults *r2 = [[WMFSearchResults alloc]
            initWithSearchTerm:@"baz"
                       results:@[
        ...
        expect([r1.results
                   subarrayWithRange:NSMakeRange(0, originalResults.count)])
            .toWithDescription(
                equal(originalResults),
                @"put original results first, in the same order");

        expect([r1.results subarrayWithRange:NSMakeRange(originalResults.count,
                                                         r2.results.count)])
            .toWithDescription(equal(r2.results),
                               @"put merged results last, in the same order");
      });
      ...
      it(@"should not insert results already present in the original", ^{
        ...
        WMFSearchResults *r2 = [[WMFSearchResults alloc]
            initWithSearchTerm:@"baz"
                       results:[originalResults
                                   arrayByAddingObjectsFromArray:newResults]
              searchSuggestion:@"buz"
              redirectMappings:@[]];
...
describe(@"merging redirect mappings", ^{
  it(@"should omit mappings already present in the original", ^{
    NSArray *originalMappings =
        @[ [MWKSearchRedirectMapping mappingFromTitle:@"foo" toTitle:@"bar"] ];
    NSArray *newMappings =
        @[ [MWKSearchRedirectMapping mappingFromTitle:@"baz" toTitle:@"buz"] ];
    NSArray *joinedMappings =
        [originalMappings arrayByAddingObjectsFromArray:newMappings];
    ...
    expect(@(r1.redirectMappings.count))
        .to(equal(@(originalMappings.count + newMappings.count)));
...
describe(@"merging search suggestion", ^{
  it(@"should preserve the original suggestion if there was one", ^{
    ...
    WMFSearchResults *r2 = [[WMFSearchResults alloc] initWithSearchTerm:@"foo"
                                                                results:@[]
  ...
  it(@"should set the new suggestion if the original was nil", ^{
    WMFSearchResults *r1 = [[WMFSearchResults alloc] initWithSearchTerm:@"foo"
                                                                results:@[]
