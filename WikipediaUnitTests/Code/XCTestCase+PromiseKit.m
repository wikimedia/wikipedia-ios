...
- (void)expectAnyPromiseToResolve:(AnyPromise * (^)(void))testBlock
  ...
  __block XCTestExpectation *expectation =
      [self expectationForMethod:method line:line];
  ...
  [self waitForExpectationsWithTimeout:timeout
                               handler:^(NSError *_Nullable error) {
                                 if (error) {
                                   // don't fulfill the expectation after the
                                   // timeout expires, XCTest will raise an
                                   // assertion and wreak all sorts of havoc
                                   DDLogError(@"Timeout expired with error: %@",
                                              error);
                                   expectation = nil;
                                 }
                               }];
