...
- (void)setUp {
  ...
  NSAssert([historyList countOfEntries] == 0,
           @"History list must be empty before tests begin.");
}
...
- (void)testStatePersistsWhenSaved {
  MWKHistoryEntry *losAngeles =
      [[MWKHistoryEntry alloc] initWithURL:titleURLLAEn];
  MWKHistoryEntry *sanFrancisco =
      [[MWKHistoryEntry alloc] initWithURL:titleURLSFFr];

  /*
     HAX: dates are not precisely stored, so the difference must be >1s for the
     order to be persisted accurately.
     this shouldn't be a huge problem in practice because users (probably) won't
     save multiple pages in <1s
   */
  ...
  __block XCTestExpectation *expectation =
      [self expectationWithDescription:@"Should resolve"];
  [self->historyList save]
      .then(^{
        XCTAssertFalse(self->historyList.dirty,
                       @"Dirty flag should be reset after saving.");
        MWKHistoryList *persistedList =
            [[MWKHistoryList alloc] initWithDataStore:self->dataStore];

        // HAX: dates aren't exactly persisted, so we need to compare manually
        [persistedList.entries
            enumerateObjectsUsingBlock:^(MWKHistoryEntry *actualEntry,
                                         NSUInteger idx, BOOL *_) {
              MWKHistoryEntry *expectedEntry = self->historyList.entries[idx];
              assertThat(actualEntry.url, is(expectedEntry.url));
              assertThat(@([actualEntry.date
                             timeIntervalSinceDate:expectedEntry.date]),
...
- (void)testAddingEquivalentObjectUpdatesExistingEntryDate {
  ...
  MWKHistoryEntry *copyOfEntry1 =
      [[MWKHistoryEntry alloc] initWithURL:copyOfTitle1];
