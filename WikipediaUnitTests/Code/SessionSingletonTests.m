...
configureTempDataStoreForEach(tempDataStore, ^{
  ...
  WMF_TECH_DEBT_TODO(
      refactor sendUsageReports to use a notification to make it easier to test)
  /*
     ^ this only works now because the queues singleton grabs its values
     directly from the shared instance
     AND the shared instance doesn't "cache" the sendUsageReports value in
     memory, so setting it from a different
...
describe(@"send usage reports", ^{
  ...
  void (
      ^expectAllManagersToHaveExpectedAnalyticsHeaderForCurrentUsageReportsValue)(
      NSArray *managers) = ^(NSArray *managers) {
    NSString *expectedHeaderValue = [[ReadingActionFunnel new] appInstallID];
    NSArray *headerValues = [managers
        valueForKeyPath:@"requestSerializer.HTTPRequestHeaders.X-WMF-UUID"];
    id<NMBMatcher> allEqualExpectedValueOrNull =
        allPass(equal(testSession.shouldSendUsageReports ? expectedHeaderValue
                                                         : [NSNull null]));
    ...
    it(@"should reset the global request managers", ^{
      ...
      [testSession
          setShouldSendUsageReports:!testSession.shouldSendUsageReports];
    ...
    it(@"should be idempotent", ^{
      ...
      NSArray *managersAfterRedundantSet =
          [[QueuesSingleton sharedInstance] allManagers];
      expect(managersAfterRedundantSet).to(equal(oldManagers));
      expectAllManagersToHaveExpectedAnalyticsHeaderForCurrentUsageReportsValue(
          managersAfterRedundantSet);
    });
});
...
QuickSpecEnd
  ...
  sharedExamples(
      @"a persistent property", ^(QCKDSLSharedExampleContext getContext) {
        ...
        it(@"a persistent property", ^{
          ...
          SessionSingleton *newSession = [[SessionSingleton alloc]
