...
- (void)testCreatingSectionListWithNoData {
  ...
  MWKSectionList *emptySectionList =
      [[MWKSectionList alloc] initWithArticle:mockArticle];
  assertThat(@(emptySectionList.count), is(equalToInt(0)));
  [MKTVerifyCount(mockArticle.dataStore, MKTNever()) sectionWithId:anything()
                                                           article:anything()];
}
...
- (void)testSectionListInitializationExeptionHandling {
  ...
  [self addEmptyFolderForSection:0
                             url:anything()
  ...
  [[MKTGiven([self.dataStore sectionWithId:0 article:mockArticle])
      ...
      forArgument:0]
      willThrow:[NSException
                    exceptionWithName:@"MWKSectionListTestException"
...
- (void)addEmptyFolderForSection:(int)sectionId
  ...
  // create an empty section directory, so that our section list will reach the
  // code path
  // where an exception will be thrown when trying to read the section data
  NSString *randomDirectory = WMFRandomTemporaryPath();
  NSString *randomPath =
      [randomDirectory stringByAppendingPathComponent:@"sections/0"];
  BOOL didCreateRandomPath =
      [[NSFileManager defaultManager] createDirectoryAtPath:randomPath
                                withIntermediateDirectories:YES
                                                 attributes:nil
                                                      error:nil];
  XCTAssert(didCreateRandomPath);
  [MKTGiven([mockDataStore pathForArticleURL:anything()])
