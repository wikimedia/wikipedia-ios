...
- (void)setUp {
  ...
  self.dummyArticle = [[MWKArticle alloc]
...
- (void)testInitialStateShouldIndicateNoDetectionOrFaces {
  self.image = [[MWKImage alloc] initWithArticle:self.dummyArticle
...
- (void)testSerializationOfImageWithoutDetection {
  ...
  self.image =
      [[MWKImage alloc] initWithArticle:self.dummyArticle dict:testData];
...
- (void)testDeserializedImageWithDetectionButNoFaces {
  NSDictionary *testData = @{ @"focalRects" : @[], @"sourceURL" : @"foo" };
  self.image =
      [[MWKImage alloc] initWithArticle:self.dummyArticle dict:testData];
...
- (void)testDeserializedImageWithDetectedFaces {
  ...
  assertThat(self.image.allNormalizedFaceBounds,
             is(equalTo(@[ [NSValue valueWithCGRect:testRect] ])));
...
- (void)testShouldSetDidDetectFacesIfPassedNilFeatures {
  ...
  XCTAssertTrue(
      self.image.didDetectFaces,
      @"Need to be able to handle cases where CIDetector passes nil.");
}
