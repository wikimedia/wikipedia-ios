...
- (void)setUp {
  [super setUp];
  self.mosconeCenter =
      [[CLLocation alloc] initWithLatitude:37.783821 longitude:-122.400264];
  self.goldenGateBridge =
      [[CLLocation alloc] initWithLatitude:37.809616 longitude:-122.476784];
  self.coitTower =
      [[CLLocation alloc] initWithLatitude:37.801904 longitude:-122.405713];
  self.twinPeaksSummit =
      [[CLLocation alloc] initWithLatitude:37.752959 longitude:-122.445854];
  self.dogpatchBoulders =
      [[CLLocation alloc] initWithLatitude:37.756303 longitude:-122.387848];
}

- (void)testShouldHaveNoBearingToSelf {
  XCTAssertEqual([self.mosconeCenter wmf_bearingToLocation:self.mosconeCenter],
                 0);
}

- (void)testShouldHaveANorthwestBearingFromMosconeToGoldenGate {
  CLLocationDegrees value =
      [self.mosconeCenter wmf_bearingToLocation:self.goldenGateBridge];
  XCTAssert(275.0 <= value && value <= 360.0, @"Expected %f to be W by NW.",
            value);
...
- (void)testShouldHaveNorthBearingFromMosconeToCoitTower {
  CLLocationDegrees value =
      [self.mosconeCenter wmf_bearingToLocation:self.coitTower];
  XCTAssert(315.0 <= value || value <= 360.0, @"Expected %f to be N by NW.",
            value);
...
- (void)testShouldHaveSouthwestBearingFromMosconeToTwinPeaks {
  CLLocationDegrees value =
      [self.mosconeCenter wmf_bearingToLocation:self.twinPeaksSummit];
...
- (void)testShouldHaveSouthEastBearingFromMosconeToDogpatch {
  CLLocationDegrees value =
      [self.mosconeCenter wmf_bearingToLocation:self.dogpatchBoulders];
