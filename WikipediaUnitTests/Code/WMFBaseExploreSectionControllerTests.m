...
- (NSString *)description {
  // override description to prevent base explore section impl, which assumes
  // conformance to WMFExploreSectionController
...
QuickConfigurationBegin(WMFSharedSectionControllerTests)
  ...
  sharedExamples(@"a fetching section controller", ^(
                     QCKDSLSharedExampleContext getContext) {
    ...
    beforeEach(^{
      ...
      kvoController =
          [[FBKVOController alloc] initWithObserver:self retainObserved:NO];
      ...
      sectionController =
          [[SectionControllerClass alloc] initWithDataStore:tempDataStore];
      expectedSuccessItems = context[@"expectedSuccessItems"];
      [kvoController
          observe:sectionController
          keyPath:WMF_SAFE_KEYPATH(sectionController, items)
          options:0
            block:^(id observer, WMFBaseExploreSectionController *controller,
                    NSDictionary *change) {
...
/*
    `expect()` takes a block—implicitly—which is repeatedly invoked to get the
   value of the given expression.
    therefore, we need to be smart about `expect`-ing the value of a promise
   returned by a function to prevent
  ...
  ^{                                                                           \
    ...
    context(@"initial state", ^{
      it(@"should be filled with placeholders, if it supports placeholders", ^{
        NSUInteger numberOfPlaceholders =
            [sectionController numberOfPlaceholderCells];
        if (numberOfPlaceholders > 0 &&
          ...
          expect(sectionController.items)
              .to(haveCount(@(numberOfPlaceholders)));
        } else {
    ...
    context(@"previous request failed", ^{
      ...
      beforeEach(^{
        ...
        // restart nocilla stubbing to remove failure stub & fail on any
        // unexpected requests
      ...
      it(@"should retry when asked to fetch by user", ^{
        ...
        expectValueOfPromiseReturnedBySectionControllerCall(
            fetchDataUserInitiated)
            .withTimeout(5)
...
QuickConfigurationEnd
    ...
    //             @"expectedSuccessItems": [WMFDummyExploreSectionController
    //             dummyItems]};
    //});
    //
    // QuickSpecEnd
