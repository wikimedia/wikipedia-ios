...
- (void)setUp {
  ...
  // force language link controller to grab device language, not previous values
  // set by another test
  ...
  NSAssert([[NSLocale preferredLanguages] containsObject:@"en-US"] ||
           ...
           @"For simplicity these tests assume the simulator has 'English' has "
           @"one of its preferred languages."
  ...
  self.filter =
      [[MWKLanguageFilter alloc] initWithLanguageDataSource:self.controller];
  [self.controller resetPreferredLanguages];
}

- (void)testReadPreviouslySelectedLanguagesReturnsEmpty {
  assertThat(
      [self.controller readPreferredLanguageCodesWithoutOSPreferredLanguages],
      hasCountOf(0));
}

- (void)testDefaultsToDevicePreferredLanguages {
  /*
     since we've asserted above that "en" or "en-US" is one of the OS preferred
     languages, we can assert that our
...
- (void)
    testSaveSelectedLanguageUpdatesTheControllersFilteredPreferredLanguages {
  NSAssert(![[self preferredLanguageCodes] containsObject:@"test"],
           @"'test' shouldn't be a default member of preferred languages!");

  MWKLanguageLink *link =
      [[MWKLanguageLink alloc] initWithLanguageCode:@"test"
                                      pageTitleText:@"test"
                                               name:@"test"
                                      localizedName:@"test"];
...
- (void)testBasicFiltering {
  ...
  assertThat([self.filter.filteredLanguages bk_reject:^BOOL(
                                                MWKLanguageLink *langLink) {
               return [langLink.name wmf_caseInsensitiveContainsString:@"en"] ||
                      [langLink.localizedName
                          wmf_caseInsensitiveContainsString:@"en"];
             }],
             describedAs(@"All filtered languages have a name or localized "
                         @"name containing filter ignoring case",
                         isEmpty(), nil));
  [self verifyAllLanguageArrayProperties];
}
...
- (void)verifyPreferredAndOtherAreDisjoint {
  XCTAssertFalse(
      [[NSSet setWithArray:self.controller.preferredLanguages]
          intersectsSet:[NSSet setWithArray:self.controller.otherLanguages]],
...
- (void)verifyPreferredAndOtherSumIsAllLanguages {
  NSSet *joinedLanguages = [NSSet
      setWithArray:[self.controller.preferredLanguages
                       arrayByAddingObjectsFromArray:self.controller
                                                         .otherLanguages]];
