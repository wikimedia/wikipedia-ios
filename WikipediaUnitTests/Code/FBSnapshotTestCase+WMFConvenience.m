...
- (void)wmf_verifyMultilineLabelWithText:(id)stringOrAttributedString
                                   width:(CGFloat)width {
  WMFSnapshotVerifyView([self
      wmf_getLabelSizedToFitWidth:width
              configuredWithBlock:^(UILabel *label) {
                ...
                } else if ([stringOrAttributedString
...
- (void)wmf_verifyCellWithIdentifier:(NSString *)identifier
  ...
  WMFSnapshotVerifyView([self
      wmf_getCellWithIdentifier:identifier
...
- (void)wmf_verifyViewAtWindowWidth:(UIView *)view {
  UIWindow *window =
      view.window ?: [[UIApplication sharedApplication] keyWindow];
