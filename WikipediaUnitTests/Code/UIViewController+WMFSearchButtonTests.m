...
#pragma mark - Utils
    ...
    /**
     *  Dismiss the shared search view controller and wait for its view to leave
     * the window.
     *
     *  The added wait is necessary because the view doesn't disappear
     * synchronously even though the dismissal is not animated.
     */
      ...
      [self expectationForPredicate:
                [NSPredicate
                    predicateWithBlock:^BOOL(
                        UIViewController *_Nonnull evaluatedObject,
                        NSDictionary<NSString *, id> *_Nullable bindings) {
                      return evaluatedObject.view.window == nil;
                    }]
                evaluatedWithObject:[UIViewController
                                        wmf_sharedSearchViewController]
                            handler:nil];
      [self waitForExpectationsWithTimeout:10 handler:nil];
    };

/**
 *  Present the shared search view controller from a given presenting view
 * controller and wait for its view to enter the window.
 *
 *  The added wait is necessary because the view doesn't appear synchronously
 * even though the presentation is not animated.
 */
void (^presentSearchFromVCAndWait)(UIViewController *presentingVC) = ^(
    UIViewController *presentingVC) {
  [presentingVC wmf_showSearchAnimated:NO];

  [self
      expectationForPredicate:[NSPredicate predicateWithBlock:^BOOL(
                                               UIViewController
 ...
 *
 *  @warning Do not call @c wmf_showSearch: directly on this or other view
 * controllers during tests. Use the utility
 *           functions provided above to ensure the search view is in the window
 * before proceeding.
 */
...
describe(@"search button", ^{
  it(@"should be presentable from different view controllers", ^{
    ...
    WMFSearchViewController *oldSearchVC =
        [UIViewController wmf_sharedSearchViewController];
    ...
    expect([UIViewController wmf_sharedSearchViewController])
        .to(equal(oldSearchVC));
  });
});

describe(@"global searchVC", ^{
  void (^verifyGlobalVCOutOfWindowResetAfterNotificationNamed)(NSString *) = ^(
  ...
  it(@"should be reset on memory warnings when it is not in the window", ^{
    verifyGlobalVCOutOfWindowResetAfterNotificationNamed(
        UIApplicationDidReceiveMemoryWarningNotification);
  });

  it(@"should reset when the app enters the background if it's not in the "
     @"window",
     ^{
       verifyGlobalVCOutOfWindowResetAfterNotificationNamed(
