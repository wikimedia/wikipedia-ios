...
- (void)setUp {
  ...
  self.fetcher =
      [[MWKLanguageLinkFetcher alloc] initWithManager:self.mockManager
  ...
  [[MKTVerify(self.mockDelegate)
      ...
      fetchFinished:self.fetcher
        fetchedData:nil
             status:0
              error:[NSError
                        errorWithDomain:WMFNetworkingErrorDomain
...
- (void)testFetchingEmptyTitle {
  NSURL *url =
      [[NSURL wmf_URLWithDefaultSiteAndlanguage:@"en"] wmf_URLWithTitle:@""];
