...
+ (void)wmf_start {
  ...
  [PiwikTracker
      sharedInstanceWithSiteID:appID
                       baseURL:[NSURL URLWithString:piwikHostURLString]];
  [[PiwikTracker wmf_configuredInstance]
      setDispatchInterval:WMFDispatchInterval];
  [PiwikTracker wmf_configuredInstance].sampleRate = 10;
#endif
}

+ (instancetype)wmf_configuredInstance {
  return [[NSBundle mainBundle] wmf_isPiwikEnabledAndConfigured]
             ? [self sharedInstance]
             : nil;
}
...
- (void)wmf_sendEventWithCategory:(NSString *)category
...
- (void)
wmf_logActionTapThroughInContext:(id<WMFAnalyticsContextProviding>)context
                     contentType:(nullable id<WMFAnalyticsContentTypeProviding>)
                                     contentType {
  [self wmf_sendEventWithCategory:[context analyticsContext]
                           action:@"Tap Through"
                             name:[contentType analyticsContentType]
...
- (void)
wmf_logActionImpressionInContext:(id<WMFAnalyticsContextProviding>)context
                     contentType:(nullable id<WMFAnalyticsContentTypeProviding>)
                                     contentType {
  [self wmf_sendEventWithCategory:[context analyticsContext]
                           action:@"Impression"
...
- (void)
wmf_logActionTapThroughMoreInContext:(id<WMFAnalyticsContextProviding>)context
                         contentType:
                             (nullable id<WMFAnalyticsContentTypeProviding>)
                                 contentType {
  [self wmf_sendEventWithCategory:[context analyticsContext]
...
- (void)
wmf_logActionSwitchLanguageInContext:(id<WMFAnalyticsContextProviding>)context
                         contentType:
                             (nullable id<WMFAnalyticsContentTypeProviding>)
                                 contentType {
  [self wmf_sendEventWithCategory:[context analyticsContext]
