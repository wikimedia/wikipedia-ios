...
extension FileManager {
	...
	func listFiles(path: String, withSuffix suffix: String) -> [URL] {
		return listFiles(path: path).filter({$0.absoluteString.hasSuffix(suffix)})
	}
}
...
let snapshotsFlattenedPathString = "\(FileManager.default.currentDirectoryPath)/SnapshotsFlattened/"

let removeFile = { (file: URL) in
    do {
        print(file)
...
// Clear out pngs from "SnapshotsFlattened"
let filesToRemove = FileManager.default.listFiles(path: snapshotsFlattenedPathString, withSuffix: ".png")
...
// Copy the file from its subdir to the "SnapshotsFlattened" dir in same dir as this script.
...
let renameAndMoveFileToDirOfThisScript = { (file: URL) in
	...
	do {
        try FileManager.default.copyItem(at: file, to: NSURL.fileURL(withPath: "\(snapshotsFlattenedPathString)/\(newFileNameWithoutSpacesOrSlashes)"))
	}
	catch let error as NSError {
	    print("Something went wrong: \(error)")
	}
