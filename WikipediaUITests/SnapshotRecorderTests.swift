...
- run the `bundle exec fastlane snapshot` command to kick off the screenshot taking tests below
...
- i think this lets `SnapshotHelper.swift` configure our build in that simulator? anyway breakpoints set in the test below will work as expected after running this once.
- then when you select the `iPhone 5s` simulator (in Xcode) and hit the play button on a test below it will behave as expected (ie not fail in unexplainable ways because it's configured for non-EN)
- once your test is navigating to the parts of the app you want to record pics, add `snapshot("SomeRelevantString")` where you want pics to be taken (note: no pic will actually be recorded when you're manually running the tests here - that only happens when you run `bundle exec fastlane snapshot`)
...
- run `bundle exec fastlane snapshot` to take snapshots for ALL langs and sims


TIPS:
 
- setting a breakpoint in your test, running it, and using `print(XCUIApplication().debugDescription)` in the console when the breakpoint hits is handy for seeing what buttons/elements we can interact with or tap on the current screen. Find the button with the label string you are looking for (in the tree printed by `print(XCUIApplication().debugDescription)`) then find the key for that label's localized string and use it.
...
- it appears to not work correctly if you try to have more than one test kick off screenshot recording. weird freezes, etc. so below we just use a single test method: `testRecordAppScreenshots()`
...
- use control-option-command-U to re-run last test you ran!
...
- the Xcode Accessiblity Inspector "Xcode > Open Developer Tool > Accessibility Inspector" is also VERY useful for seeing what accessibility label strings are associated with image-only buttons.
