...
- (instancetype)init {
  ...
  if (self) {
    ...
    WMFSearchResponseSerializer *serializer =
        [WMFSearchResponseSerializer serializer];
...
- (WMFLocationSearchRequestSerializer *)nearbySerializer {
  return (WMFLocationSearchRequestSerializer *)(self.operationManager
                                                    .requestSerializer);
}
...
- (id<Cancellable>)fetchNearbyArticlesWithSiteURL:(NSURL *)siteURL
  ...
  NSURL *url = useDeskTopURL ? [NSURL wmf_desktopAPIURLForURL:siteURL]
                             : [NSURL wmf_mobileAPIURLForURL:siteURL];

  WMFLocationSearchRequestParameters *params =
      [WMFLocationSearchRequestParameters new];
  ...
  return [self.operationManager GET:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id response) {
        ...
        WMFLocationSearchResults *results =
            [[WMFLocationSearchResults alloc] initWithSearchSiteURL:siteURL
                                                           location:location
                                                            results:response];
        resolve(results);
      }
      failure:^(NSURLSessionDataTask *operation, NSError *error) {
        if ([url isEqual:[NSURL wmf_mobileAPIURLForURL:siteURL]] &&
...
requestBySerializingRequest:(NSURLRequest *)request
  ...
  NSDictionary *serializedParams =
      [self serializedParams:(WMFLocationSearchRequestParameters *)parameters];
...
- (NSDictionary *)serializedParams:
    (WMFLocationSearchRequestParameters *)params {
  NSString *coords =
      [NSString stringWithFormat:@"%f|%f", params.location.coordinate.latitude,
                                 params.location.coordinate.longitude];
  NSString *numberOfResults =
      [NSString stringWithFormat:@"%lu", (unsigned long)params.numberOfResults];
