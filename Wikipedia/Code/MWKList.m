...
+ (MTLPropertyStorage)storageBehaviorForPropertyWithKey:
  ...
  if ([propertyKey
          isEqualToString:WMF_SAFE_KEYPATH([MWKList new], mutableEntries)]) {
...
- (NSString *)description {
  return
      [NSString stringWithFormat:@"%@ %@", [super description], self.entries];
}
...
- (void)addEntry:(id<MWKListObject>)entry {
  NSAssert([entry conformsToProtocol:@protocol(MWKListObject)],
           @"attempting to add object that does not implement MWKListObject");
...
- (void)insertEntry:(id<MWKListObject>)entry atIndex:(NSUInteger)index {
  NSAssert(
      [entry conformsToProtocol:@protocol(MWKListObject)],
      @"attempting to insert object that does not implement MWKListObject");
...
- (void)updateEntryWithListIndex:(id)listIndex
                          update:(BOOL (^)(id<MWKListObject> entry))update {
...
- (NSArray *)pruneToMaximumCount:(NSUInteger)maximumCount {
  if (self.mutableEntries.count > maximumCount) {
    NSRange range =
        NSMakeRange(maximumCount, self.mutableEntries.count - maximumCount);
...
- (void)performSaveWithCompletion:(dispatch_block_t)completion
  ...
  if (errorHandler) {
    errorHandler([NSError wmf_unableToSaveErrorWithReason:
                              @"Save is unimplemented for this list"]);
  }
}
