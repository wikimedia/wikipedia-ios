...
- (instancetype)init {
  ...
  if (self) {
    ...
    self.requestManager.responseSerializer = [WMFMantleJSONResponseSerializer
        serializerForArrayOf:[WMFRevisionQueryResults class]
...
- (AnyPromise *)fetchLatestRevisionsForArticleURL:(NSURL *)articleURL
  ...
  return [self.requestManager
             wmf_GETAndRetryWithURL:articleURL
                         parameters:@{
                           @"format" : @"json",
                           @"continue" : @"",
                           @"formatversion" : @2,
                           ...
                           @"rvendid" : @(revisionId),
                           @"rvprop" : WMFJoinedPropertyParameters(
                               @[ @"ids", @"size", @"flags" ])
                         }]
