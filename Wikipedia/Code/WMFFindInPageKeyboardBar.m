...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    self.tintColor =
        [UIColor colorWithRed:0.3373 green:0.3373 blue:0.3373 alpha:1.0];
  }
  return self;
}
...
- (void)updateForCurrentMatchIndex:(NSInteger)index
                      matchesCount:(NSUInteger)count {
...
- (void)updateLabelTextForCurrentMatchIndex:(NSInteger)index
                               matchesCount:(NSUInteger)count {
  ...
  if (self.textField.text.length == 0) {
    ...
    labelText =
        [NSString stringWithFormat:@"%lu / %lu", (unsigned long)(index + 1),
                                   (unsigned long)count];
  }
