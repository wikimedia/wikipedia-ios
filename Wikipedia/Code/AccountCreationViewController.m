...
@property(weak, nonatomic)
    IBOutlet NSLayoutConstraint *passwordConfirmUnderlineHeight;
@property(weak, nonatomic) IBOutlet NSLayoutConstraint *emailUnderlineHeight;
@property(weak, nonatomic)
    IBOutlet NSLayoutConstraint *spaceBeneathCaptchaContainer;
...
- (void)adjustScrollLimitForCaptchaVisiblity {
  // Reminder: spaceBeneathCaptchaContainer constraint is space *below* captcha
  // container - that's why below
  // for the show case we don't have to "convertPoint".
  self.spaceBeneathCaptchaContainer.constant =
      (self.showCaptchaContainer)
          ? (self.view.frame.size.height -
             (self.captchaContainer.frame.size.height / 2))
          : (self.view.frame.size.height -
             [self.loginButton convertPoint:CGPointZero toView:self.scrollView]
                 .y);
  ;
}
...
- (void)viewDidLoad {
  ...
  self.titleLabel.font =
      [UIFont boldSystemFontOfSize:23.0f * MENUS_SCALE_MULTIPLIER];
  self.usernameField.font =
      [UIFont boldSystemFontOfSize:18.0f * MENUS_SCALE_MULTIPLIER];
  self.passwordField.font =
      [UIFont boldSystemFontOfSize:18.0f * MENUS_SCALE_MULTIPLIER];
  self.passwordRepeatField.font =
      [UIFont boldSystemFontOfSize:18.0f * MENUS_SCALE_MULTIPLIER];
  self.emailField.font =
      [UIFont boldSystemFontOfSize:18.0f * MENUS_SCALE_MULTIPLIER];
  self.loginButton.font =
      [UIFont boldSystemFontOfSize:14.0f * MENUS_SCALE_MULTIPLIER];

  [self adjustConstraintsScaleForViews:@[
    self.createAccountContainerView, self.captchaContainer, self.titleLabel,
    self.usernameField, self.passwordField, self.passwordRepeatField,
    self.emailField, self.loginButton
  ]];
  ...
  self.usernameField.attributedPlaceholder =
      [self getAttributedPlaceholderForString:
                MWLocalizedString(@"account-creation-username-placeholder-text",
                                  nil)];

  self.passwordField.attributedPlaceholder =
      [self getAttributedPlaceholderForString:
                MWLocalizedString(@"account-creation-password-placeholder-text",
                                  nil)];

  self.passwordRepeatField.attributedPlaceholder = [self
      getAttributedPlaceholderForString:
          MWLocalizedString(
              @"account-creation-password-confirm-placeholder-text", nil)];

  self.emailField.attributedPlaceholder = [self
      getAttributedPlaceholderForString:
          MWLocalizedString(@"account-creation-email-placeholder-text", nil)];
  ...
  self.passwordConfirmUnderlineHeight.constant =
      self.usernameUnderlineHeight.constant;
  ...
  [self.loginButton
      addGestureRecognizer:[[UITapGestureRecognizer alloc]
                               initWithTarget:self
                                       action:@selector(loginButtonPushed:)]];
    ...
    [self
        dismissViewControllerAnimated:YES
                           completion:^{
                             ...
                             [presenter presentViewController:nc
                                                     animated:YES
                                                   completion:nil];
                           }];
  }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
  if (textField == self.usernameField) {
    ...
    NSAssert(((textField == self.emailField) ||
              (textField == self.captchaViewController.captchaTextBox)),
...
- (void)setShowCaptchaContainer:(BOOL)showCaptchaContainer {
  ...
  self.rightButton.title = showCaptchaContainer
                               ? MWLocalizedString(@"button-done", nil)
                               : MWLocalizedString(@"button-next", nil);
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
      ...
      [UIView animateWithDuration:duration
          ...
          completion:^(BOOL done) {
            ...
            // Pretent a text field changed so the progressive button state gets
            // updated.
...
- (void)refreshCaptchaImage {
  self.captchaViewController.captchaTextBox.text = @"";
  [self.captchaViewController.captchaImageView
      sd_setImageWithURL:self.captchaUrl];
...
- (void)login {
  [[WMFAlertManager sharedInstance]
                  showAlert:MWLocalizedString(@"account-creation-logging-in",
                                              nil)
                     sticky:YES
      dismissPreviousAlerts:YES
                tapCallBack:NULL];
  [[WMFAuthenticationManager sharedInstance]
      loginWithUsername:self.usernameField.text
      ...
      success:^{
        ...
        [[WMFAlertManager sharedInstance] showSuccessAlert:loggedInMessage
                                                    sticky:NO
                                     dismissPreviousAlerts:YES
                                               tapCallBack:NULL];
        [self dismissViewControllerAnimated:YES completion:nil];
...
- (NSArray *)requiredInputFields {
  NSAssert([self isViewLoaded], @"This method is only intended to be called "
                                @"when view is loaded, since they'll all be "
                                @"nil otherwise");
...
- (BOOL)areRequiredFieldsPopulated {
  return NSNotFound == [[self requiredInputFields]
                           indexOfObjectPassingTest:^BOOL(
                               UITextField *field, NSUInteger idx, BOOL *stop) {
...
- (void)save {
  if (![self areRequiredFieldsPopulated]) {
    [[WMFAlertManager sharedInstance]
        showErrorAlertWithMessage:MWLocalizedString(
                                      @"account-creation-missing-fields", nil)
                           sticky:YES
            dismissPreviousAlerts:YES
                      tapCallBack:NULL];
    return;
  ...
  if (![self isPasswordConfirmationCorrect]) {
    [[WMFAlertManager sharedInstance]
        showErrorAlertWithMessage:MWLocalizedString(
                                      @"account-creation-passwords-mismatched",
                                      nil)
                           sticky:YES
            dismissPreviousAlerts:YES
                      tapCallBack:NULL];
  ...
  if ([self.captchaViewController.captchaTextBox.text length] == 0) {
    [[WMFAuthenticationManager sharedInstance]
        getAccountCreationCaptchaWithUsername:self.usernameField.text
        password:self.passwordField.text
        ...
        captcha:^(NSURL *_Nonnull captchaURL) {
          ...
          [[WMFAlertManager sharedInstance]
                   showWarningAlert:MWLocalizedString(
                                        @"account-creation-captcha-required",
                                        nil)
                             sticky:NO
              dismissPreviousAlerts:YES
                        tapCallBack:NULL];
    ...
    [[WMFAuthenticationManager sharedInstance]
        createAccountWithCaptchaText:self.captchaViewController.captchaTextBox
                                         .text
        success:^{
          NSString *loggedInMessage =
              MWLocalizedString(@"main-menu-account-title-logged-in", nil);
