...
static NSString *const WMFPlaceholderImageInfoTitle =
    @"WMFPlaceholderImageInfoTitle";
...
- (NSAttributedString *)headerTitle {
  return [[NSAttributedString alloc]
      initWithString:MWLocalizedString(@"explore-potd-heading", nil)
          attributes:@{
...
- (NSAttributedString *)headerSubTitle {
  return [[NSAttributedString alloc]
      initWithString:[[NSDateFormatter
                         ...
                         stringFromDate:self.fetchedDate]
          attributes:@{
            NSForegroundColorAttributeName :
...
- (void)configureCell:(WMFPicOfTheDayCollectionViewCell *)cell
             withItem:(MWKImageInfo *)item
...
- (AnyPromise *)fetchData {
  @weakify(self);
  return [self.fetcher
             fetchPicOfTheDaySectionInfoForDate:self.fetchedDate
                               metadataLanguage:
...
- (UIViewController *)detailViewControllerForItemAtIndexPath:
  ...
  NSArray<NSDate *> *dates =
      [[self.fetchedDate dateBySubtractingDays:WMFDefaultNumberOfPOTDDates]
          wmf_datesUntilDate:self.fetchedDate];
  WMFPOTDImageGalleryViewController *vc =
      [[WMFPOTDImageGalleryViewController alloc] initWithDates:dates
                                             selectedImageInfo:self.imageInfo];
  vc.referenceViewDelegate = self;
  return vc;
}

- (UIImageView *)referenceViewForImageController:
    (WMFImageGalleryViewController *)controller {
