...
@interface PageHistoryViewController () <UITableViewDataSource,
                                         UITableViewDelegate>

@property(strong, nonatomic)
    __block NSMutableArray<PageHistorySection *> *pageHistoryDataArray;
...
- (void)viewDidLoad {
  ...
  self.historyFetcherParams = [[PageHistoryRequestParameters alloc]
      initWithTitle:self.article.url.wmf_title];
  ...
  self.tableView.tableFooterView = [[UIView alloc]
      initWithFrame:CGRectMake(0, 0, 10.0 * MENUS_SCALE_MULTIPLIER,
                               10.0 * MENUS_SCALE_MULTIPLIER)];
  self.tableView.tableFooterView.backgroundColor = [UIColor whiteColor];

  [self.tableView registerNib:
                      [UINib nibWithNibName:@"PageHistoryResultPrototypeView"
                                     bundle:nil]
       forCellReuseIdentifier:TABLE_CELL_ID];

  // Single off-screen cell for determining dynamic cell height.
  self.offScreenSizingCell = (PageHistoryResultCell *)[self.tableView
...
- (void)getPageHistoryData {
  ...
  [self.pageHistoryFetcher fetchRevisionInfo:self.article.url
      ...
      .then(^(HistoryFetchResults *historyFetchResults) {
        @strongify(self);
        [self.pageHistoryDataArray
            addObjectsFromArray:historyFetchResults.items];
        self.historyFetcherParams = [historyFetchResults
            getPageHistoryRequestParameters:self.article.url];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  PageHistoryResultCell *cell = (PageHistoryResultCell *)[tableView
...
- (void)updateViewsInCell:(PageHistoryResultCell *)cell
...
- (BOOL)shouldLoadNewData {
  ...
  CGFloat maxY =
      self.tableView.contentOffset.y + self.tableView.frame.size.height + 200.0;
  BOOL shouldLoad = NO;
