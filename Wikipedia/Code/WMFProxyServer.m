...
- (NSString *)stringByUpdatingImageTagAttributesForProxyAndScalingInImageTagContents:(NSString *)imageTagContents withTargetImageWidth:(NSUInteger)targetImageWidth {
    ...
    if (src && srcPathComponents.count > 0) {
        ...
        NSRange rangeOfSVG = [filename rangeOfString:@".svg" options:NSCaseInsensitiveSearch]; // check for SVGs because we shouldn't request the original for them
        BOOL isSVG = rangeOfSVG.location != NSNotFound;
        BOOL hasDataFileWidth = dataFileWidth > 0;
        BOOL isWiderThan64 = (width == 0 || width >= 64);
        ...
        if (hasDataFileWidth && isWiderThan64 && srcPathComponents.count > 4 && [[srcPathComponents[srcPathComponents.count - 5] lowercaseString] isEqualToString:@"thumb"]) { //keep the thumb check separate to skip it
            ...
            } else if (!isSVG) { //otherwise the original file is smaller than the target width, and we should just request the original image
