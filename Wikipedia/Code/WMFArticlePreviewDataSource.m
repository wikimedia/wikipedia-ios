...
@property(nonatomic, strong, readwrite, nullable)
    NSArray<MWKSearchResult *> *previewResults;
...
- (void)fetch {
  @weakify(self);
  [self.titlesSearchFetcher
      fetchArticlePreviewResultsForArticleURLs:self.urls
...
- (NSURL *)urlForIndexPath:(NSIndexPath *)indexPath {
  return [self.siteURL
      wmf_URLWithTitle:[self searchResultForIndexPath:indexPath].displayTitle];
}
