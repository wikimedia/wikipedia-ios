...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    ...
    MWKSite *site =
        [self decodeValueForKey:@"site" withCoder:coder modelVersion:0];
    if (site && self.siteURL == nil) {
      self.siteURL = site.URL;
    }
    MWKTitle *title =
        [self decodeValueForKey:@"title" withCoder:coder modelVersion:0];
...
/**
 *  Provides the index of the receiver's item, specifying its ordering with
 * other item types within its @c dateCreated.
 *
 *  @return A positive integer which, when compared with other items' indices,
 * will yield the comparison result describing
 *          their ordering.
 */
...
- (NSComparisonResult)compare:(WMFExploreSection *)section {
  ...
  if (self.type == WMFExploreSectionTypeContinueReading) {
    ...
    // corollary of above, everything else always goes below continue reading,
    // regardless of date
    ...
    // sort all items from different days and/or history/saved items by date,
    // descending
...
+ (nullable instancetype)featuredArticleSectionWithSiteURLIfSupported:
    (NSURL *)url {
  NSParameterAssert(url);
  if (![url.wmf_language isEqualToString:@"en"] ||
      ![url.wmf_domain isEqualToString:@"wikipedia.org"]) {
    /*
       HAX: "Today's Featured Article" template is only available on
       en.wikipedia.org.
     */
    return nil;
  }
...
+ (instancetype)nearbySectionWithLocation:(CLLocation *)location
...
+ (NSUInteger)totalMaxNumberOfSections {
  return
      [self maxNumberOfSectionsForType:WMFExploreSectionTypeHistory] +
