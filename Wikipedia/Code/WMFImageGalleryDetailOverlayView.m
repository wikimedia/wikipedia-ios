...
static NSAttributedString *ConcatOwnerAndLicense(NSString *owner,
                                                 MWKLicense *license) {
  if (!owner && !license) {
    return nil;
  }
  ...
  if (licenseGlyph) {
    // hand-tuning glyph size & baseline offset until all glyphs are positioned
    // & padded in a uniform way
    [result
  ...
  NSAttributedString *attributedOwnerAndSeparator = [[NSAttributedString alloc]
      initWithString:[@" " stringByAppendingString:owner]
          attributes:@{
            NSFontAttributeName :
                [UIFont systemFontOfSize:WMFImageGalleryOwnerFontSize],
            NSForegroundColorAttributeName : [UIColor whiteColor]
          }];
...
- (void)layoutSubviews {
  ...
  if (self.frame.size.height > 0.0) {
    ...
    double const relativeImageDescriptionTop =
        1.0 - imageDescriptionTop / self.frame.size.height;
    self.gradientView.gradientLayer.startPoint =
        CGPointMake(0.5, relativeImageDescriptionTop);
  }
}
...
- (void)setImageDescription:(NSString *)imageDescription {
  if (!imageDescription) {
    ...
    self.imageDescriptionLabel.attributedText = [[NSAttributedString alloc]
        initWithString:imageDescription
            attributes:@{
