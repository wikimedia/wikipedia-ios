...
static NSString *const WMFRelatedSectionBlackListFileName =
...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    ...
    NSArray *fixed =
        [self.entries wmf_mapAndRejectNil:^id _Nullable(id _Nonnull obj) {
...
- (id)decodeValueForKey:(NSString *)key
              withCoder:(NSCoder *)coder
           modelVersion:(NSUInteger)modelVersion {
  if ([key isEqualToString:WMF_SAFE_KEYPATH(self, entries)] &&
      modelVersion == 0) {
    NSArray *titles = [self decodeValueForKey:WMF_SAFE_KEYPATH(self, entries)
                                    withCoder:coder
                                 modelVersion:0];
    ...
    return
        [super decodeValueForKey:key withCoder:coder modelVersion:modelVersion];
  }
}
...
+ (NSURL *)fileURL {
  return [NSURL
      fileURLWithPath:
          [[documentsDirectory()
              stringByAppendingPathComponent:WMFRelatedSectionBlackListFileName]
              stringByAppendingPathExtension:
                  WMFRelatedSectionBlackListFileExtension]];
}
...
- (void)performSaveWithCompletion:(dispatch_block_t)completion
  ...
  @synchronized(self) {
    if (![NSKeyedArchiver archiveRootObject:self
                                     toFile:[[[self class] fileURL] path]]) {
      DDLogError(@"Failed to save sections to disk!");
      if (errorHandler) {
        errorHandler(
            [NSError wmf_unableToSaveErrorWithReason:
                         @"NSKeyedArchiver failed to save blacklist to disk"]);
      }
    } else {
      if (completion) {
        completion();
