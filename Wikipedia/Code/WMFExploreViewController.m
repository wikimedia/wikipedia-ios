...
- (void)viewWillAppear:(BOOL)animated {
    ...
    _readingListsToolbarViewController.delegate = self;
}
...
-(void)setReadingListsToolbarVisible:(BOOL)readingListsToolbarVisible {
    ...
    if (readingListsToolbarVisible) {
        ...
        [_readingListsToolbarViewController didMoveToParentViewController:self];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 8 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            self.readingListsToolbarVisible = NO;
        });
    } else {
        [_readingListsToolbarViewController willMoveToParentViewController:nil];
        [_readingListsToolbarViewController.view removeFromSuperview];
        [_readingListsToolbarViewController removeFromParentViewController];
        [_readingListsToolbarViewController reset];
    }
}

-(void)didSaveArticle:(BOOL)didSave article:(WMFArticle *)article {
    BOOL didSaveOtherArticle = (didSave && self.isReadingListsToolbarVisible && article != _readingListsToolbarViewController.article);
    BOOL didUnsaveOtherArticle = (!didSave && self.isReadingListsToolbarVisible && article != _readingListsToolbarViewController.article);
    ...
    if (didSaveOtherArticle) {
        _readingListsToolbarViewController.article = article;
        return;
    }
