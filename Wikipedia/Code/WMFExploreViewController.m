...
@interface WMFExploreViewController () <
    WMFExploreSectionSchemaDelegate, UIViewControllerPreviewingDelegate,
    WMFAnalyticsContextProviding, WMFAnalyticsViewNameProviding,
    UINavigationControllerDelegate, WMFColumnarCollectionViewLayoutDelegate>
...
@property(nonatomic, strong, nullable) id<WMFExploreSectionController>
    sectionOfPreviewingTitle;
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    ...
    self.navigationItem.titleView.accessibilityLabel =
        MWLocalizedString(@"home-accessibility-label", nil);
...
- (WMFExploreSectionControllerCache *)sectionControllerCache {
  ...
  if (!_sectionControllerCache) {
    _sectionControllerCache = [[WMFExploreSectionControllerCache alloc]
...
- (BOOL)isDisplayingCellsForSectionController:
...
- (BOOL)isDisplayingCellsForSection:(NSInteger)section {
  // NOTE: numberOfSectionsInCollectionView returns 0 when
  // isWaitingForNetworkToReconnect == YES
  ...
  NSParameterAssert(
      section < [self numberOfSectionsInCollectionView:self.collectionView]);
  return [self.collectionView.indexPathsForVisibleItems
      bk_any:^BOOL(NSIndexPath *indexPath) {
        return indexPath.section == section;
      }];
}

- (BOOL)itemAtIndexPathIsOnlyItemVisibleInSection:(NSIndexPath *)indexPath {
  NSArray<NSIndexPath *> *visibleIndexPathsInSection =
      [self.collectionView.indexPathsForVisibleItems
          bk_select:^BOOL(NSIndexPath *i) {
            return i.section == indexPath.section;
          }];

  if ([visibleIndexPathsInSection count] == 1 &&
      [[visibleIndexPathsInSection firstObject] isEqual:indexPath]) {
    return YES;
  } else {
    return NO;
  }
 ...
 *
 *  @param indexPath The index path of the item which will or did end
 * displaying.
 *
 *  @return @c YES if that section isn't displaying or the given item is the
 * only one visible in its section, otherwise @c NO.
 */
...
- (NSArray *)visibleSectionControllers {
  NSIndexSet *visibleSectionIndexes =
      [[self.collectionView indexPathsForVisibleItems]
          bk_reduce:[NSMutableIndexSet indexSet]
  ...
  return [[self.schemaManager.sections objectsAtIndexes:visibleSectionIndexes]
      wmf_mapAndRejectNil:^id(WMFExploreSection *obj) {
...
/**
 * Sends `willDisplaySection` to controllers whose sections are currently
 * visible in the receiver's `collectionView`.
 *
 * Must be called when the view (re)appears: `viewDidApppear` and when the
 * application is resumed (will enter foreground).
 *
 * `collectionView:willDisplayCell:forItemAtIndexPath:` will not trigger a
 * `willDisplaySection` message, since it's only
 * designed to trigger when sections are *scrolled* in and out of view.  This is
 * mostly because we only want to call
 * `willDisplaySection` _once_ for each section as its (potentially multiple)
 * cells scroll into view.
 *
 * This was manifested in the following issue:
 * https://phabricator.wikimedia.org/T128217
 *
 * @see isVisibilityTransitioningForItemAtIndexPath:
 */
- (void)sendWillDisplayToVisibleSectionControllers {
  [[self visibleSectionControllers]
      bk_each:^(id<WMFExploreSectionController> _Nonnull controller) {
        if ([controller respondsToSelector:@selector(willDisplaySection)]) {
          DDLogInfo(@"Manually sending willDisplaySection to controller %@",
                    controller);
...
- (void)showSettings {
  UINavigationController *settingsContainer = [[UINavigationController alloc]
      initWithRootViewController:
          [WMFSettingsViewController
              settingsViewControllerWithDataStore:self.dataStore]];
...
- (BOOL)canScrollToTop {
  ...
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:0];
...
- (void)viewDidLoad {
  ...
  [self.collectionView registerNib:[WMFExploreSectionHeader wmf_classNib]
        forSupplementaryViewOfKind:UICollectionElementKindSectionHeader
               withReuseIdentifier:[WMFExploreSectionHeader wmf_nibName]];

  [self.collectionView registerNib:[WMFExploreSectionFooter wmf_classNib]
        forSupplementaryViewOfKind:UICollectionElementKindSectionFooter
               withReuseIdentifier:[WMFExploreSectionFooter wmf_nibName]];

  [self.collectionView registerClass:[UICollectionReusableView class]
          forSupplementaryViewOfKind:UICollectionElementKindSectionFooter
                 withReuseIdentifier:WMFExploreEmptyFooterReuseIdentifier];
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  /*
     NOTE: Section should only be _created_ on `viewDidAppear`, which is not the
     same as updating.  Updates only happen
  ...
  [NSUserActivity
      wmf_makeActivityActive:[NSUserActivity wmf_exploreViewActivity]];
#if ENABLE_RANDOM_DEBUGGING
  WMFFirstRandomViewController *vc = [[WMFFirstRandomViewController alloc]
      initWithSite:[MWKSite siteWithLanguage:@"en"]
...
- (void)viewDidDisappear:(BOOL)animated {
  ...
  [[self visibleSectionControllers]
      bk_each:^(id<WMFExploreSectionController> _Nonnull obj) {
        if ([obj respondsToSelector:@selector(didEndDisplayingSection)]) {
          DDLogDebug(@"Sending didEndDisplayingSection to controller %@ on "
                     @"view disappearance",
                     obj);
...
- (void)appLanguageDidChangeWithNotification:(NSNotification *)note {
  [self createSectionSchemaIfNeeded];
  [self.schemaManager updateSiteURL:[[[MWKLanguageLinkController sharedInstance]
                                        appLanguage] siteURL]];
}
...
- (void)showOfflineEmptyViewAndReloadWhenReachable {
  ...
  [self.reachabilityManager setReachabilityStatusChangeBlock:^(
    ...
    case AFNetworkReachabilityStatusReachableViaWiFi: {
      ...
      [self.collectionView reloadData];
      [self.collectionView setContentOffset:self.preNetworkTroubleScrollPosition
                                   animated:NO];
      self.preNetworkTroubleScrollPosition = CGPointZero;
      ...
      [[self visibleSectionControllers]
          enumerateObjectsUsingBlock:^(
              id<WMFExploreSectionController> _Nonnull obj, NSUInteger idx,
              BOOL *_Nonnull stop) {
            @weakify(self);
            [obj resetData];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView
  ...
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:section];
  NSParameterAssert(controller);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:indexPath.section];
  NSParameterAssert(controller);
  UICollectionViewCell *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:
                          [controller cellIdentifierForItemIndexPath:indexPath]
...
- (CGFloat)collectionView:(UICollectionView *)collectionView
    estimatedHeightForItemAtIndexPath:(NSIndexPath *)indexPath
                       forColumnWidth:(CGFloat)columnWidth {
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:indexPath.section];
  NSParameterAssert(controller);
...
- (CGFloat)collectionView:(UICollectionView *)collectionView
  ...
  if ([controller conformsToProtocol:@protocol(WMFMoreFooterProviding)] &&
      (![controller respondsToSelector:@selector(isFooterEnabled)] ||
       [(id<WMFMoreFooterProviding>)controller isFooterEnabled])) {
    return 50;
  } else {
    return 0;
  }
...
- (BOOL)collectionView:(UICollectionView *)collectionView
    prefersWiderColumnForSectionAtIndex:(NSUInteger)index {
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:index];
  return [controller respondsToSelector:@selector(prefersWiderColumn)] &&
         [controller prefersWiderColumn];
}

- (void)configureHeader:(WMFExploreSectionHeader *)header
    withStylingFromController:(id<WMFExploreSectionController>)controller {
  header.image = [[controller headerIcon]
  ...
  [title addAttribute:NSFontAttributeName
                value:[UIFont wmf_exploreSectionHeaderTitleFont]
                range:NSMakeRange(0, title.length)];
  header.title = title;

  NSMutableAttributedString *subTitle =
      [[controller headerSubTitle] mutableCopy];
  [subTitle addAttribute:NSFontAttributeName
                   value:[UIFont wmf_exploreSectionHeaderSubTitleFont]
                   range:NSMakeRange(0, subTitle.length)];
  header.subTitle = subTitle;
}

- (nonnull UICollectionReusableView *)collectionView:
                                          (UICollectionView *)collectionView
                   viewForSupplementaryElementOfKind:(NSString *)kind
                                         atIndexPath:(NSIndexPath *)indexPath {
  if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
    return [self collectionView:collectionView
        viewForSectionHeaderAtIndexPath:indexPath];
  } else if ([kind isEqualToString:UICollectionElementKindSectionFooter]) {
    return [self collectionView:collectionView
        viewForSectionFooterAtIndexPath:indexPath];
  } else {
    assert(false);
...
- (nonnull UICollectionReusableView *)collectionView:
                                          (UICollectionView *)collectionView
                     viewForSectionHeaderAtIndexPath:(NSIndexPath *)indexPath {
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:indexPath.section];
  ...
  WMFExploreSectionHeader *header = (id)[collectionView
      dequeueReusableSupplementaryViewOfKind:
          UICollectionElementKindSectionHeader
                         withReuseIdentifier:[WMFExploreSectionHeader
                                                 wmf_nibName]
                                forIndexPath:indexPath];
  ...
  if ([controller conformsToProtocol:@protocol(WMFHeaderMenuProviding)]) {
    ...
    [[header rightButton] setImage:[UIImage imageNamed:@"overflow-mini"]
                          forState:UIControlStateNormal];
    [header.rightButton
        bk_removeEventHandlersForControlEvents:UIControlEventTouchUpInside];
    @weakify(controller);
    [header.rightButton bk_addEventHandler:^(id sender) {
      ...
      if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        UIAlertController *menuActionSheet =
            [(id<WMFHeaderMenuProviding>)controller menuActionSheet];
        ...
        menuActionSheet.popoverPresentationController.sourceRect =
            [sender bounds];
        menuActionSheet.popoverPresentationController.permittedArrowDirections =
            UIPopoverArrowDirectionAny;
        [self presentViewController:menuActionSheet
                           animated:YES
                         completion:nil];
      } else {
        UIAlertController *menuActionSheet =
            [(id<WMFHeaderMenuProviding>)controller menuActionSheet];
        menuActionSheet.popoverPresentationController.sourceView =
            self.navigationController.tabBarController.tabBar.superview;
        menuActionSheet.popoverPresentationController.sourceRect =
        ...
        [self presentViewController:menuActionSheet
                 ...
                 conformsToProtocol:@protocol(WMFHeaderActionProviding)] &&
             (![controller
                  respondsToSelector:@selector(isHeaderActionEnabled)] ||
              [(id<WMFHeaderActionProviding>)
                      controller isHeaderActionEnabled])) {
    header.rightButtonEnabled = YES;
    [[header rightButton]
        setImage:[(id<WMFHeaderActionProviding>)controller headerButtonIcon]
        forState:UIControlStateNormal];
    [header.rightButton
        bk_removeEventHandlersForControlEvents:UIControlEventTouchUpInside];
    @weakify(controller);
...
- (nonnull UICollectionReusableView *)collectionView:
                                          (UICollectionView *)collectionView
                     viewForSectionFooterAtIndexPath:(NSIndexPath *)indexPath {
  id<WMFExploreSectionController> controller =
      [self sectionControllerForSectionAtIndex:indexPath.section];
  if (!controller) {
    return nil;
  ...
  if ([controller conformsToProtocol:@protocol(WMFMoreFooterProviding)] &&
      (![controller respondsToSelector:@selector(isFooterEnabled)] ||
       [(id<WMFMoreFooterProviding>)controller isFooterEnabled])) {
    WMFExploreSectionFooter *footer = (id)[collectionView
        dequeueReusableSupplementaryViewOfKind:
            UICollectionElementKindSectionFooter
                           withReuseIdentifier:[WMFExploreSectionFooter
                                                   wmf_nibName]
                                  forIndexPath:indexPath];
    ...
    return [collectionView
        dequeueReusableSupplementaryViewOfKind:
            UICollectionElementKindSectionFooter
                           withReuseIdentifier:
                               WMFExploreEmptyFooterReuseIdentifier
                                  forIndexPath:indexPath];
  }
}
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if ([controller respondsToSelector:@selector(willDisplaySection)]) {
    DDLogDebug(@"Sending willDisplaySection for controller %@ at indexPath %@",
               controller, indexPath);
    [controller willDisplaySection];
  }

  [self performSelector:@selector(fetchSectionIfShowing:)
  ...
  if ([controller conformsToProtocol:@protocol(WMFTitleProviding)] &&
    ...
    NSURL *articleURL =
        [(id<WMFTitleProviding>)controller urlForItemAtIndexPath:indexPath];
    if (articleURL) {
      [[PiwikTracker wmf_configuredInstance]
          wmf_logActionImpressionInContext:self
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if ([controller respondsToSelector:@selector(didEndDisplayingSection)]) {
    if ([self isVisibilityTransitioningForItemAtIndexPath:indexPath]) {
      DDLogDebug(
          @"Sending didEndDisplayingSection for controller %@ at indexPath %@",
          controller, indexPath);
      [controller didEndDisplayingSection];
    } else {
      DDLogDebug(@"Skipping calling didEndDisplaySection for controller %@ "
                 @"indexPath %@",
                 controller, indexPath);
    }
  }
  ...
  visibleIndexPathsInSection =
      [visibleIndexPathsInSection bk_reject:^BOOL(id obj) {
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  [[PiwikTracker wmf_configuredInstance]
      wmf_logActionTapThroughInContext:self
                           contentType:controller];
  UIViewController *vc =
      [controller detailViewControllerForItemAtIndexPath:indexPath];
  if ([vc isKindOfClass:[WMFArticleViewController class]]) {
    [self wmf_pushArticleViewController:(WMFArticleViewController *)vc
                               animated:YES];
  } else {
...
- (NSString *)lastUpdatedString {
  ...
  return [MWLocalizedString(@"home-last-update-label", nil)
      stringByReplacingOccurrencesOfString:@"$1"
                                withString:[formatter stringFromDate:
                                                          self.schemaManager
                                                              .lastUpdatedAt]];
}

- (void)resetRefreshControlWithCompletion:
...
- (void)createSectionSchemaIfNeeded {
  ...
  self.schemaManager = [WMFExploreSectionSchema
      schemaWithSiteURL:[[[MWKLanguageLinkController sharedInstance]
                            appLanguage] siteURL]
...
- (void)fetchSectionIfShowing:(id<WMFExploreSectionController>)controller {
  if ([self isDisplayingCellsForSectionController:controller]) {
    ...
    DDLogInfo(
        @"Section for controller %@ is no longer visible, skipping fetch.",
        controller);
  }
}
...
- (id<WMFExploreSectionController>)sectionControllerForSection:
    (WMFExploreSection *)section {
  id<WMFExploreSectionController> sectionController = [self.sectionControllerCache
      getOrCreateControllerForSection:section
                        creationBlock:^(
                            id<WMFExploreSectionController> _Nonnull newController) {
                          [self registerSectionForSectionController:
                                    newController];
                        }];
  return sectionController;
}

- (nullable id<WMFExploreSectionController>)sectionControllerForSectionAtIndex:
    (NSInteger)index {
...
- (NSInteger)indexForSectionController:
    (id<WMFExploreSectionController>)controller {
  return [self.schemaManager.sections
      indexOfObject:[self.sectionControllerCache
                        sectionForController:controller]];
}

#pragma mark - Section Loading

- (void)loadSectionControllersForCurrentSectionSchema {
  [self.schemaManager.sections
      enumerateObjectsUsingBlock:^(WMFExploreSection *obj, NSUInteger idx,
                                   BOOL *stop) {
        [self sectionControllerForSection:obj];
      }];
}

- (void)registerSectionForSectionController:
    (id<WMFExploreSectionController>)controller {
  if (!controller) {
    return;
  }
      ...
      options:0
        block:^(WMFExploreViewController *observer,
          ...
          NSUInteger sectionIndex =
              [observer indexForSectionController:observedController];
          if (sectionIndex != NSNotFound &&
              [observer isDisplayingCellsForSection:sectionIndex]) {
            DDLogDebug(@"Reloading table to display results in controller %@",
                       observedController);
            [observer.collectionView
...
- (void)didTapFooterInSection:(NSUInteger)section {
  id<WMFExploreSectionController> controllerForSection =
      [self sectionControllerForSectionAtIndex:section];
  NSParameterAssert(controllerForSection);
  if (!controllerForSection) {
    DDLogError(@"Unexpected footer tap for missing section %lu.",
               (unsigned long)section);
    return;
  }
  if (![controllerForSection
          respondsToSelector:@selector(moreViewController)]) {
    return;
  }
  id<WMFExploreSectionController, WMFMoreFooterProviding>
      articleSectionController =
          (id<WMFExploreSectionController, WMFMoreFooterProviding>)
              controllerForSection;

  UIViewController *moreVC = [articleSectionController moreViewController];
  [[PiwikTracker wmf_configuredInstance]
      wmf_logActionTapThroughMoreInContext:self
                               contentType:controllerForSection];
...
- (void)didTapHeaderInSection:(NSUInteger)section {
  ...
  } else if (homeSection.type == WMFExploreSectionTypeHistory ||
             homeSection.type == WMFExploreSectionTypeSaved) {
    [self wmf_pushArticleWithURL:homeSection.articleURL
                       dataStore:self.dataStore
                        animated:YES];
  } else {
...
viewControllerForLocation:(CGPoint)location {
  ...
  id<WMFExploreSectionController> sectionController =
      [self sectionControllerForSectionAtIndex:previewIndexPath.section];
  ...
  previewingContext.sourceRect =
      [self.collectionView cellForItemAtIndexPath:previewIndexPath].frame;

  UIViewController *vc = [sectionController
      detailViewControllerForItemAtIndexPath:previewIndexPath];
  self.sectionOfPreviewingTitle = sectionController;
  [[PiwikTracker wmf_configuredInstance]
      wmf_logActionPreviewInContext:self
                        contentType:sectionController];
  return vc;
}
...
- (void)previewingContext:(id<UIViewControllerPreviewing>)previewingContext
  ...
  [[PiwikTracker wmf_configuredInstance]
      wmf_logActionTapThroughInContext:self
                           contentType:self.sectionOfPreviewingTitle];
...
- (void)navigationController:(UINavigationController *)navigationController
  ...
  [navigationController
      wmf_hideToolbarIfViewControllerHasNoToolbarItems:viewController];
}
