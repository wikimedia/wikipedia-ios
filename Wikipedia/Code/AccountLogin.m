  ...
  [manager POST:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (![responseObject isDict]) {
          responseObject =
              @{ @"error" : @{@"info" : @"Account login info not found."} };
        }

        NSError *error = nil;
        NSDictionary *output = @{};
        if ([responseObject[@"clientlogin"][@"status"]
                isEqualToString:@"FAIL"]) {
          NSMutableDictionary *errorDict =
              [responseObject[@"clientlogin"] mutableCopy];
        ...
        [self finishWithError:error fetchedData:output];
      }
...
- (NSDictionary *)getSanitizedResponse:(NSDictionary *)rawResponse {
  NSMutableDictionary *mutableResponse =
      [NSMutableDictionary dictionaryWithDictionary:rawResponse];
...
- (NSError *)getErrorForResult:(NSString *)result {
  ...
  NSString *errorMessage =
      [NSString stringWithFormat:@"Unknown login error. Code '%@'", result];
  ...
  } else if ([result isEqualToString:@"WrongPass"] ||
             [result isEqualToString:@"WrongPluginPass"]) {
  ...
  return [NSError errorWithDomain:WMFAccountLoginErrorDomain
                             code:errorType
