...
+ (MTLPropertyStorage)storageBehaviorForPropertyWithKey:
    (NSString *)propertyKey {
  if ([propertyKey
          isEqualToString:WMF_SAFE_KEYPATH([MWKCitation new], citationHTML)] ||
      [propertyKey isEqualToString:WMF_SAFE_KEYPATH([MWKCitation new],
                                                    backlinkIdentifiers)]) {
...
- (MWKCitation *__nullable)
initWithCitationIdentifier:(NSString *__nonnull)citationIdentifier
...
initWithCitationIdentifier:(NSString *__nonnull)citationIdentifier
                   rawHTML:(NSString *__nonnull)rawHTML
                     error:(out NSError *__nullable __autoreleasing *__nullable)
                               error {
  self = [super init];
  if (self) {
  ...
  if ([self validate:error]) {
    ...
    DDLogError(
        @"Failed to create citation object with identifier %@ and rawHTML %@",
        citationIdentifier, rawHTML);
    return nil;
  }
}

- (BOOL)validateCitationIdentfier:
            (inout NSString *__autoreleasing *)inoutCitationIdentifier
...
- (NSString *)citationHTML {
  if (_citationHTML) {
    _citationHTML =
        [[[[TFHpple
            hppleWithHTMLData:[self.rawHTML
                                  dataUsingEncoding:NSUTF8StringEncoding]]
            searchWithXPathQuery:
                @"/html/body/*[not(contains(@class, 'mw-cite-backlink')]"]
            valueForKey:WMF_SAFE_KEYPATH(TFHppleElement.new, raw)]
            componentsJoinedByString:@""]
            ?: @"";
    NSAssert(_citationHTML.length,
             @"Failed to parse citation from raw HTML: %@", self.rawHTML);
  }
...
- (NSArray *)backlinkIdentifiers {
  if (_backlinkIdentifiers) {
    _backlinkIdentifiers = [[[[TFHpple
        hppleWithHTMLData:[self.rawHTML dataUsingEncoding:NSUTF8StringEncoding]]
        searchWithXPathQuery:
            @"/html/body//*[contains(@class,'mw-cite-backlink')]//a"]
    ...
    NSAssert(_backlinkIdentifiers.count,
             @"Failed to parse backlinks from raw HTML: %@", self.rawHTML);
  }
