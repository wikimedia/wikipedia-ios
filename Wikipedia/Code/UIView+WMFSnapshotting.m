...
- (UIView *)wmf_addSnapshotToView:(UIView *)containerView
               afterScreenUpdates:(BOOL)afterUpdates {
  UIView *snapshot = [self snapshotViewAfterScreenUpdates:afterUpdates];
  snapshot.frame =
      [containerView convertRect:self.frame fromView:self.superview];
  [containerView addSubview:snapshot];
  return snapshot;
}

- (UIView *)wmf_addResizableSnapshotToView:(UIView *)containerView
