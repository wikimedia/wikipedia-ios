...
#define PREVIEW_BLUE_COLOR                                                     \
  [UIColor colorWithRed:0.2 green:0.4784 blue:1.0 alpha:1.0]
...
- (void)didMoveToSuperview {
  ...
  self.licenseTitleLabel.font =
      [UIFont systemFontOfSize:11.0 * MENUS_SCALE_MULTIPLIER];
  self.licenseLoginLabel.font =
      [UIFont systemFontOfSize:11.0 * MENUS_SCALE_MULTIPLIER];
  ...
  [self adjustConstraintsScaleForViews:@[
    self.licenseCCLabel, self.licenseTitleLabel, self.licenseLoginLabel
  ]];

  self.bottomDividerHeight.constant =
      self.hideBottomDivider ? 0.0 : 1.0f / [UIScreen mainScreen].scale;

  self.topDividerHeight.constant =
      self.hideTopDivider ? 0.0 : 1.0f / [UIScreen mainScreen].scale;

  // self.licenseTitleLabel.text = [@" abc " randomlyRepeatMaxTimes:100];
}

- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder {
  BOOL isPlaceholder = ([[self subviews] count] ==
                        0); // From: https://blog.compeople.eu/apps/?p=142
  if (!isPlaceholder) {
    return self;
  }

  UINib *previewLicenseViewNib =
      [UINib nibWithNibName:@"PreviewLicenseView" bundle:nil];
...
- (void)styleLinks:(UILabel *)label {
  ...
  label.attributedText = [label.text
      attributedStringWithAttributes:baseAttributes
                 substitutionStrings:@[
...
- (void)termsLicenseLabelTapped:(UITapGestureRecognizer *)recognizer {
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    [self.previewLicenseViewDelegate
        previewLicenseViewTermsLicenseLabelWasTapped:self];
  }
}
...
- (void)underlineSignIn:(UILabel *)label {
  ...
  label.attributedText = [label.text
      attributedStringWithAttributes:baseAttributes
                 substitutionStrings:@[ MWLocalizedString(
                                         @"wikitext-upload-save-sign-in", nil) ]
              substitutionAttributes:@[ substitutionAttributes ]];
}

- (NSAttributedString *)getCCIconAttributedString {
  return [[NSAttributedString alloc]
      initWithString:WIKIGLYPH_CC
          attributes:@{
            NSFontAttributeName :
                [UIFont wmf_glyphFontOfSize:42.0 * MENUS_SCALE_MULTIPLIER],
            NSForegroundColorAttributeName : PREVIEW_BLUE_COLOR,
