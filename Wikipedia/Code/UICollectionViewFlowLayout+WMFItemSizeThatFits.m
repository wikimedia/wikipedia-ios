...
- (CGSize)wmf_itemSizeThatFits:(CGSize)size {
  return CGSizeMake(
      fmaxf(size.width - self.sectionInset.left - self.sectionInset.right -
                self.minimumInteritemSpacing,
            0.f),
      fmaxf(size.height - self.sectionInset.top - self.sectionInset.bottom -
...
- (void)wmf_strictItemSizeToFit {
  CGSize sizeThatFits =
      [self wmf_itemSizeThatFits:self.collectionView.bounds.size];
