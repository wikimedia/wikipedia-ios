...
- (CGRect)wmf_contentFrame {
  return UIEdgeInsetsInsetRect(
      CGRectOffset(self.frame, 0, self.contentOffset.y), self.contentInset);
}

- (void)wmf_safeSetContentOffset:(CGPoint)offset animated:(BOOL)animated {
  if (!isnan(offset.x) && !isinf(offset.x) && !isnan(offset.y) &&
      !isinf(offset.y)) {
#if DEBUG
    ...
    if (self.contentSize.width < offset.x ||
      ...
      DDLogDebug(@"Attempting to scroll to offset %@ which exceeds contentSize "
                 @"scroll view %@",
    ...
    DDLogError(@"Ignoring invalid offset %@ for scroll view %@",
               NSStringFromCGPoint(offset), self);
  }
}
