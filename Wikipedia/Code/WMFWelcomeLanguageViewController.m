...
@property(strong, nonatomic)
    IBOutlet WelcomeLanguagesAnimationView *animationView;
...
- (void)viewDidLoad {
  ...
  self.subTitleLabel.text =
      MWLocalizedString(@"welcome-languages-sub-title", nil);

  [self.moreLanguagesButton
      setTitle:MWLocalizedString(@"welcome-languages-add-button", nil)
      forState:UIControlStateNormal];

  [self.nextStepButton wmf_configureAsWelcomeNextButton];

  self.dividerAboveNextStepButton.backgroundColor =
      [UIColor wmf_welcomeNextButtonDividerBackgroundColor];
...
- (void)viewDidLayoutSubviews {
  ...
  // HAX: to maintain fairly consistent margins above and below the welcome
  // panels
  // their content is constrained to vertically center the image and labels as a
  // whole. This is trickier to do with a table which has a variable number of
  // cells.
  // This hack vertically centers this panel's table contents. Note the image
  // and
  // labels are part of the table so the table's scroll view can be used to
  // scroll
  // the image, labels, and table cells as a group if needed (such as when many
  // language cells are added) - this is expecially important on small screens
  // such
  // as a 4s.
...
- (void)addTopInsetToVerticallyCenterLanguagesTableContentIfNeeded {
  CGFloat topInsetRequiredToCenterTableContent =
      (self.languageTableView.frame.size.height -
       self.languageTableView.contentSize.height) /
      2.f;
  if (topInsetRequiredToCenterTableContent > 0) {
    self.languageTableView.contentInset =
        UIEdgeInsetsMake(topInsetRequiredToCenterTableContent, 0, 0, 0);
  }
}
...
- (void)updateDeleteButtonsVisibility {
  for (WMFWelcomeLanguageTableViewCell *cell in self.languageTableView
           .visibleCells) {
    cell.deleteButton.hidden =
        ([MWKLanguageLinkController sharedInstance].preferredLanguages.count ==
         1)
            ? YES
            : NO;
  }
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  WMFWelcomeLanguageTableViewCell *cell = (id)[tableView
      dequeueReusableCellWithIdentifier:[WMFWelcomeLanguageTableViewCell
                                            wmf_nibName]
                           forIndexPath:indexPath];
  MWKLanguageLink *langLink = [MWKLanguageLinkController sharedInstance]
                                  .preferredLanguages[indexPath.row];
...
- (BOOL)tableView:(UITableView *)tableView
  ...
  return
      [[MWKLanguageLinkController sharedInstance].preferredLanguages count] > 1;
}
...
- (void)tableView:(UITableView *)tableView
  ...
  [self.languageTableView moveRowAtIndexPath:sourceIndexPath
                                 toIndexPath:destinationIndexPath];
  [self useFirstPreferredLanguageAsSearchLanguage];
}

- (void)useFirstPreferredLanguageAsSearchLanguage {
  MWKLanguageLink *firstPreferredLanguage =
      [[MWKLanguageLinkController sharedInstance] appLanguage];

  [[NSUserDefaults standardUserDefaults]
      wmf_setCurrentSearchLanguageDomain:firstPreferredLanguage.siteURL];
}
