...
- (void)enumerateContentGroupsWithBlock:(void (^)(WMFContentGroup *_Nonnull section, BOOL *stop))block{
    ...
    [self readWithBlock:^(YapDatabaseReadTransaction *_Nonnull transaction) {
        [transaction enumerateKeysAndObjectsInCollection:[WMFContentGroup databaseCollectionName] usingBlock:^(NSString * _Nonnull key, id  _Nonnull object, BOOL * _Nonnull stop) {
            block(object, stop);
        }];
    }];
}
...
- (void)enumerateContentGroupsOfKind:(NSString*)kind withBlock:(void (^)(WMFContentGroup *_Nonnull group, BOOL *stop))block{
    ...
    [self readWithBlock:^(YapDatabaseReadTransaction *_Nonnull transaction) {
        [transaction enumerateKeysAndObjectsInCollection:[WMFContentGroup databaseCollectionName] usingBlock:^(NSString * _Nonnull key, WMFContentGroup*  _Nonnull object, BOOL * _Nonnull stop) {
...
- (void)addContentGroup:(WMFContentGroup*)group associatedContent:(nullable NSArray<NSURL*>*)content{
