...
NSError *WMFErrorForApiErrorObject(NSDictionary *apiError) {
  ...
  // build the dictionary this way to avoid early nil termination caused by
  // missing keys in the error obj
  NSMutableDictionary *userInfoBuilder =
      [NSMutableDictionary dictionaryWithCapacity:3];
  void (^maybeMapApiToUserInfo)(NSString *, NSString *) =
      ^(NSString *userInfoKey, NSString *apiErrorKey) {
  ...
  return [NSError errorWithDomain:WMFNetworkingErrorDomain
                             code:WMFNetworkingError_APIError
                         userInfo:userInfoBuilder];
}

NSString *WMFWikimediaRestAPIURLStringWithVersion(NSUInteger restAPIVersion) {
  return [NSString stringWithFormat:@"https://wikimedia.org/api/rest_v%lu",
                                    (unsigned long)restAPIVersion];
}
