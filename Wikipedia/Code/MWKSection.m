...
NSString *const MWKSectionShareSnippetXPath =
    @"/html/body/p[not(.//span[@id='coordinates'])][1]//text()";
...
@property(readwrite, assign, nonatomic) BOOL
...
- (instancetype)initWithArticle:(MWKArticle *)article
  ...
  if (self) {
    ...
    self.level =
        [self optionalNumber:@"level" dict:dict]; // may be a numeric string
    self.line = [self optionalString:@"line" dict:dict];
    self.number =
        [self optionalString:@"number"
                        dict:dict]; // deceptively named, this must be a string
    self.index =
        [self optionalString:@"index"
                        dict:dict]; // deceptively named, this must be a string

    if ([dict[@"fromtitle"] length] > 0) {
      self.fromURL = [NSURL wmf_URLWithSiteURL:self.url
          unescapedDenormalizedTitleAndFragment:dict[@"fromtitle"]];
    }
...
- (nullable NSString *)text {
  if (_text == nil) {
    _text = [self.article.dataStore sectionTextWithId:self.sectionId
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@ id: %d line: %@ level: %@",
                                    [super description], self.sectionId,
...
- (NSString *)shareSnippet {
  return [[self textForXPath:MWKSectionShareSnippetXPath]
...
- (nullable NSArray *)elementsInTextMatchingXPath:(NSString *)xpath {
  ...
  if (!self.text) {
    DDLogWarn(@"Trying to query section text before downloaded. Section: %@",
              self);
    return nil;
  }
  return [[TFHpple
...
- (void)addChild:(MWKSection *)child {
  ...
  NSAssert([child.level compare:self.level] == NSOrderedDescending,
           @"Illegal attempt to add %@ to sibling or descendant %@.", child,
           self);
...
- (NSString *)summary {
  NSArray *textNodes =
      [self elementsInTextMatchingXPath:WMFSectionSummaryXPathSelector];
...
static NSString *const WMFSectionDisambiguationTitlesXPathSelector =
    @"//div[@class='hatnote']//a/@href";

- (nullable NSArray<NSURL *> *)disambiguationURLs {
  NSArray *textNodes = [self
      elementsInTextMatchingXPath:WMFSectionDisambiguationTitlesXPathSelector];
  return [textNodes wmf_mapAndRejectNil:^id(TFHppleElement *node) {
    if (node.text.length == 0 || [node.text containsString:@"redlink=1"] ||
        ![node.text containsString:WMFInternalLinkPathPrefix]) {
      return nil;
    }
    return [NSURL wmf_URLWithSiteURL:self.url
        escapedDenormalizedInternalLink:node.text];
  }];
}
...
/*
   ...
   extensive bias or disproportional coverage towards one or more specific
   regions. (May 2011)"
 */
static NSString *const WMFSectionPageIssuesXPathSelector =
    @"//td[(contains(@class,'mbox-text') or contains(@class,'ambox-text')) and "
    @"not(count(descendant::td) > 0)]";

static NSString *const WMFSectionPageIssueUnhiddenXPathSelector =
    @"//*[not(ancestor-or-self::*[@class = 'hide-when-compact' or "
    @"contains(@class, 'collapsed')])]/text()";

- (nullable NSArray<NSString *> *)pageIssues {
  NSArray *issueNodes =
      [self elementsInTextMatchingXPath:WMFSectionPageIssuesXPathSelector];
  return [issueNodes wmf_mapAndRejectNil:^id(TFHppleElement *node) {
    if (node.raw.length == 0) {
      return nil;
    }
    NSArray *unhiddenIssueNodes = [[TFHpple
        hppleWithHTMLData:[node.raw dataUsingEncoding:NSUTF8StringEncoding]]
        searchWithXPathQuery:WMFSectionPageIssueUnhiddenXPathSelector];

    NSArray *issuesStrings =
        [unhiddenIssueNodes wmf_mapAndRejectNil:^id(TFHppleElement *node) {
          return ([[node.content
    ...
    return issuesStrings.count > 0
               ? [issuesStrings componentsJoinedByString:@""]
               : nil;
  }];
}
