...
- (void)setDataSource:(WMFSearchDataSource *)dataSource {
  ...
  dataSource.cellConfigureBlock =
      ^(WMFArticleListTableViewCell *cell, MWKSearchResult *result,
        UITableView *tableView, NSIndexPath *indexPath) {
        @strongify(self);
        NSURL *articleURL = [self.dataSource urlForIndexPath:indexPath];
        [cell wmf_setTitleText:articleURL.wmf_title
              highlightingText:self.searchResults.searchTerm];
        cell.titleLabel.accessibilityLanguage =
            self.dataSource.searchSiteURL.wmf_language;
        cell.descriptionText = [self descriptionForSearchResult:result];
        // TODO: In "Redirected from: $1", "$1" can be in any language; need to
        // handle that too, currently (continuing) doing nothing for such cases
        cell.descriptionLabel.accessibilityLanguage =
            [self redirectMappingForResult:result] == nil
                ? self.dataSource.searchSiteURL.wmf_language
                : nil;
...
- (MWKSearchRedirectMapping *)redirectMappingForResult:
    (MWKSearchResult *)result {
  return [self.searchResults.redirectMappings
      bk_match:^BOOL(MWKSearchRedirectMapping *obj) {
...
- (NSString *)descriptionForSearchResult:(MWKSearchResult *)result {
  ...
  NSString *redirectedResultMessage =
      [MWLocalizedString(@"search-result-redirected-from", nil)
          stringByReplacingOccurrencesOfString:@"$1"
                                    withString:mapping.redirectFromTitle];

  if (!result.wikidataDescription) {
    return redirectedResultMessage;
  } else {
    return [NSString
        stringWithFormat:@"%@\n%@", redirectedResultMessage,
                         [result.wikidataDescription
