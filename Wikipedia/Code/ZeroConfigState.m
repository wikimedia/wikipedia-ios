...
+ (void)load {
  [super load];
  FBTweakAction(
      @"Networking", @"Wikipedia Zero", @"Reset ZeroOnDialogShownOnce", ^{
...
- (BOOL)zeroOnDialogShownOnce {
  return
      [[NSUserDefaults standardUserDefaults] boolForKey:ZeroOnDialogShownOnce];
}

- (void)setWarnWhenLeaving:(BOOL)warnWhenLeaving {
  [[NSUserDefaults standardUserDefaults]
      setObject:[NSNumber numberWithBool:warnWhenLeaving]
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    promise.then(^{
      ...
      [[NSNotificationCenter defaultCenter]
          postNotificationName:WMFZeroDispositionDidChange
                        object:self];
    });
  });
}
...
- (AnyPromise *)fetchZeroMessage {
  ...
  return [self.zeroMessageFetcher
      fetchZeroMessageForSiteURL:[[[MWKLanguageLinkController sharedInstance]
                                     appLanguage] siteURL]];
}
...
- (void)showFirstTimeZeroOnAlertIfNeeded {
  ...
  UIAlertView *dialog = [[UIAlertView alloc]
          initWithTitle:self.zeroMessage.message
  ...
  [dialog bk_setHandler:^{
    [[UIApplication sharedApplication]
        openURL:[NSURL
                    URLWithString:MWLocalizedString(@"zero-webpage-url", nil)]];
  }
...
- (void)showZeroOffAlert {
  UIAlertView *dialog = [[UIAlertView alloc]
          initWithTitle:MWLocalizedString(@"zero-charged-verbiage", nil)
                message:MWLocalizedString(@"zero-charged-verbiage-extended",
                                          nil)
               delegate:nil
      cancelButtonTitle:MWLocalizedString(@"zero-learn-more-no-thanks", nil)
