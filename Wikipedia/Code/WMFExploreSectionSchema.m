...
static NSTimeInterval const WMFHomeMinimumAutomaticReloadTime =
    600.0; // 10 minutes
static NSTimeInterval const WMFTimeBeforeDisplayingLastReadArticle =
    24 * 60 * 60; // 24 hours
static NSTimeInterval const WMFTimeBeforeRefreshingRandom =
    60 * 60 * 24 * 7; // 7 days
...
- (dispatch_queue_t)saveQueue {
  if (_saveQueue == nil) {
    const char *queueName =
        [NSString
            stringWithFormat:@"org.wikimedia.wikipedia.explore.schema.save.%p",
                             self]
            .UTF8String;
...
- (NSString *)description {
  // HAX: prevent this from logging all its properties in its description, as
  // this causes recursion to
...
+ (instancetype)schemaWithSiteURL:(NSURL *)siteURL
  ...
  WMFExploreSectionSchema *schema =
      [self schemaFromFileAtURL:fileURL]
          ?: [[WMFExploreSectionSchema alloc] init];
...
- (void)setBlackList:(WMFRelatedSectionBlackList *)blackList {
  ...
  [self.KVOController
      ...
      options:0
        block:^(WMFExploreSectionSchema *observer,
                WMFRelatedSectionBlackList *object, NSDictionary *change) {
 ...
 *
 *  Omits certain sections which are not guaranteed to be available (e.g.
 * featured articles & nearby).
 *
 *  @return An array of sections that can be used to start the "feed" from
 * scratch.
 */
- (NSArray<WMFExploreSection *> *)startingSchema {
  return @[
    [WMFExploreSection mainPageSectionWithSiteURL:self.siteURL],
...
- (void)updateSections:(NSArray<WMFExploreSection *> *)sections {
  if (self.sections == sections) {
    // not bothering with equality check here since it could be expensive when
    // list is long
    return;
  }
  ...
  if (sections) {
    self.sections = [sections sortedArrayWithOptions:NSSortStable
                                     usingComparator:^NSComparisonResult(
                                         WMFExploreSection *_Nonnull obj1,
                                         WMFExploreSection *_Nonnull obj2) {
...
- (BOOL)update:(BOOL)force {
  ...
  if (!FBTweakValue(@"Explore", @"General", @"Always update on launch", NO) &&
      !force && self.lastUpdatedAt &&
      [[NSDate date] timeIntervalSinceDate:self.lastUpdatedAt] <
          WMFHomeMinimumAutomaticReloadTime) {
    return [self updateContinueReading];
  }

  // Get updated static sections
  NSMutableArray<WMFExploreSection *> *sections =
      [[self staticSections] mutableCopy];
...
- (void)insertNearbySectionWithLocationIfNeeded:(CLLocation *)location {
  ...
  NSMutableArray<WMFExploreSection *> *existingNearbySections =
      [[self nearbySections] mutableCopy];

  WMFExploreSection *closeEnough =
      [existingNearbySections bk_match:^BOOL(WMFExploreSection *oldNearby) {
        // Don't add a new one if we have one that is minimum distance
        if (oldNearby.location &&
            [location distanceFromLocation:oldNearby.location] <
                WMFMinimumDistanceBeforeUpdatingNearby &&
  ...
  [self.locationManager reverseGeocodeLocation:location]
      .then(^(CLPlacemark *_Nullable placemark) {
        ...
        [existingNearbySections
            addObject:[self nearbySectionWithLocation:location
                                            placemark:placemark]];

        NSUInteger max = [WMFExploreSection
            maxNumberOfSectionsForType:WMFExploreSectionTypeNearby];
        ...
        NSArray *trimmedExistingNearbySections =
...
- (void)updateWithChangesInBlackList:(WMFRelatedSectionBlackList *)blackList {
  ...
  [[blackList.entries wmf_mapAndRejectNil:^id(NSURL *obj) {
    return [self existingSectionForArticleURL:obj];
                       ...
                       usingBlock:^(WMFExploreSection *_Nonnull obj,
...
/**
 *  Sections which should always be present in the "feed" (i.e. everything that
 * isn't site specific).
 *
...
- (WMFExploreSection *)randomSection {
  WMFExploreSection *random =
      [self.sections bk_match:^BOOL(WMFExploreSection *obj) {
        if (obj.type == WMFExploreSectionTypeRandom &&
            [obj.siteURL isEqual:self.siteURL]) {
          return YES;
        }
        return NO;
      }];
  ...
  if (lastEntry &&
      [[NSDate date] timeIntervalSinceDate:lastEntry.date] >
          WMFTimeBeforeRefreshingRandom) {
...
- (NSArray<WMFExploreSection *> *)nearbySections {
  NSArray<WMFExploreSection *> *nearby =
      [self.sections bk_select:^BOOL(WMFExploreSection *obj) {
        if (obj.type == WMFExploreSectionTypeNearby && obj.location != nil &&
            obj.siteURL != nil) {
          return YES;
        }
        return NO;
...
- (nullable WMFExploreSection *)
nearbySectionWithLocation:(CLLocation *)location
 ...
 *
 *  Selects all "most read" sections from the receiver and, if possible, appends
 * an additional section for the most
 *  recent data from the current site.
 *
 *  @return An array of "most read" sections that should be in an updated
 * version of the receiver.
 */
- (NSArray<WMFExploreSection *> *)mostReadSectionsWithUpdateIfNeeded {
  NSMutableArray<WMFExploreSection *> *mostReadSections =
      [[self.sections bk_select:^BOOL(WMFExploreSection *section) {
        return section.type == WMFExploreSectionTypeMostRead &&
               section.siteURL != nil && section.mostReadFetchDate != nil;
      }] mutableCopy];

  WMFExploreSection *latestMostReadSection =
      [self newMostReadSectionWithLatestPopulatedDate];

  BOOL containsLatestSectionEquivalent =
      [mostReadSections bk_any:^BOOL(WMFExploreSection *mostReadSection) {
        BOOL const matchesDay =
        ...
        BOOL const matchesSite =
            [mostReadSection.siteURL isEqual:latestMostReadSection.siteURL];
  ...
  NSUInteger max = FBTweakValue(
      @"Explore", @"Sections", @"Max number of most read",
      [WMFExploreSection
          maxNumberOfSectionsForType:WMFExploreSectionTypeMostRead]);

  // Sort by date
  [mostReadSections
      sortWithOptions:NSSortStable
      usingComparator:^NSComparisonResult(WMFExploreSection *_Nonnull obj1,
                                          WMFExploreSection *_Nonnull obj2) {
        return -[obj1.dateCreated compare:obj2.dateCreated];
      }];
...
- (nullable WMFExploreSection *)newMostReadSectionWithLatestPopulatedDate {
  WMFExploreSection *section = [WMFExploreSection
      mostReadSectionForDate:[NSDate
                                 wmf_latestMostReadDataWithLikelyAvailableData]
                     siteURL:self.siteURL];
...
- (NSArray<WMFExploreSection *> *)featuredSections {
  NSArray *existingFeaturedArticleSections =
      [self.sections bk_select:^BOOL(WMFExploreSection *obj) {
  ...
  BOOL const containsTodaysFeaturedArticle =
      [featured bk_any:^BOOL(WMFExploreSection *obj) {
        NSAssert(obj.type == WMFExploreSectionTypeFeaturedArticle,
                 @"List should only contain featured sections, got %@",
                 featured);
  ...
  if (!containsTodaysFeaturedArticle) {
    [featured
        wmf_safeAddObject:
            [WMFExploreSection
                featuredArticleSectionWithSiteURLIfSupported:self.siteURL]];
  }

  NSUInteger max = FBTweakValue(
      @"Explore", @"Sections", @"Max number of featured",
      [WMFExploreSection
          maxNumberOfSectionsForType:WMFExploreSectionTypeFeaturedArticle]);
...
- (WMFExploreSection *)mainPageSection {
  WMFExploreSection *main =
      [self.sections bk_match:^BOOL(WMFExploreSection *obj) {
        if (obj.type == WMFExploreSectionTypeMainPage &&
            [obj.siteURL isEqual:self.siteURL]) {
          return YES;
        }
        return NO;
  ...
  // If it's a new day and we havent created a new main page section, create it
  // now
...
- (NSArray<WMFExploreSection *> *)pictureOfTheDaySections {
  NSMutableArray<WMFExploreSection *> *existingSections =
      [[self.sections bk_select:^BOOL(WMFExploreSection *obj) {
  ...
  WMFExploreSection *todaySection =
      [existingSections bk_match:^BOOL(WMFExploreSection *existingSection) {
  ...
  if (todaySection == nil) {
    [existingSections
        addObject:[WMFExploreSection
                      pictureOfTheDaySectionWithDate:[NSDate date]]];
  }

  NSUInteger max = [WMFExploreSection
      maxNumberOfSectionsForType:WMFExploreSectionTypePictureOfTheDay];
...
- (nullable WMFExploreSection *)continueReadingSection {
  NSDate *resignActiveDate =
      [[NSUserDefaults standardUserDefaults] wmf_appResignActiveDate];
  BOOL const shouldShowContinueReading =
      FBTweakValue(@"Explore", @"Continue Reading", @"Always Show", NO) ||
      fabs([resignActiveDate timeIntervalSinceNow]) >=
          WMFTimeBeforeDisplayingLastReadArticle;

  // Only return if
  if (shouldShowContinueReading) {
    NSURL *lastRead =
        [[NSUserDefaults standardUserDefaults] wmf_openArticleURL];
    if (lastRead) {
...
- (nullable WMFExploreSection *)existingSectionForArticleURL:
    (NSURL *)articleURL {
  return [self.sections bk_match:^BOOL(WMFExploreSection *obj) {
...
- (NSArray<WMFExploreSection *> *)historyAndSavedPageSections {
  ...
  NSUInteger max = FBTweakValue(
      @"Explore", @"Sections", @"Max number of history/saved",
      [WMFExploreSection
          maxNumberOfSectionsForType:WMFExploreSectionTypeSaved] +
          [WMFExploreSection
              maxNumberOfSectionsForType:WMFExploreSectionTypeHistory]);

  NSArray<WMFExploreSection *> *saved =
      [self sectionsFromSavedEntriesExcludingExistingTitlesInSections:nil
                                                            maxLength:max];
  NSArray<WMFExploreSection *> *history =
      [self sectionsFromHistoryEntriesExcludingExistingTitlesInSections:saved
                                                              maxLength:max];
  ...
  // Sort by date
  [sections
      sortWithOptions:NSSortStable | NSSortConcurrent
      usingComparator:^NSComparisonResult(WMFExploreSection *_Nonnull obj1,
                                          WMFExploreSection *_Nonnull obj2) {
        return -[obj1.dateCreated compare:obj2.dateCreated];
      }];
...
- (NSArray<WMFExploreSection *> *)
sectionsFromHistoryEntriesExcludingExistingTitlesInSections:
    (nullable NSArray<WMFExploreSection *> *)existingSections
                                                  maxLength:
                                                      (NSUInteger)maxLength {
  NSArray<NSURL *> *existingTitles = [existingSections
      valueForKeyPath:WMF_SAFE_KEYPATH([WMFExploreSection new], articleURL)];

  NSArray<MWKHistoryEntry *> *entries =
      [self.historyPages.entries bk_select:^BOOL(MWKHistoryEntry *obj) {
  ...
  entries = [entries
      wmf_arrayByTrimmingToLength:maxLength + [existingSections count]];

  entries = [entries bk_reject:^BOOL(MWKHistoryEntry *obj) {
    return [self urlIsForMainArticle:obj.url] ||
           [existingTitles containsObject:obj.url];
  }];
...
- (NSArray<WMFExploreSection *> *)
sectionsFromSavedEntriesExcludingExistingTitlesInSections:
    (nullable NSArray<WMFExploreSection *> *)existingSections
                                                maxLength:
                                                    (NSUInteger)maxLength {
  NSArray<NSURL *> *existingTitles = [existingSections
  ...
  NSArray<MWKHistoryEntry *> *entries =
      [self.savedPages.entries bk_reject:^BOOL(MWKHistoryEntry *obj) {
        return [self.blackList articleURLIsBlackListed:obj.url];
      }];
...
+ (NSDictionary *)encodingBehaviorsByPropertyKey {
  NSMutableDictionary *behaviors =
      [[super encodingBehaviorsByPropertyKey] mutableCopy];

#define WMFExploreSectionSchemaKey(key)                                        \
  WMF_SAFE_KEYPATH([WMFExploreSectionSchema new], key)

  behaviors[WMFExploreSectionSchemaKey(siteURL)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(savedPages)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(historyPages)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(delegate)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(locationManager)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(blackList)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(fileURL)] =
      @(MTLModelEncodingBehaviorExcluded);
  behaviors[WMFExploreSectionSchemaKey(saveQueue)] =
      @(MTLModelEncodingBehaviorExcluded);
...
- (AnyPromise *)save {
  /*
     NOTE: until this class is made immutable, it cannot safely be passed
     between threads.
   */
  ...
  return [AnyPromise promiseWithResolverBlock:^(PMKResolver _Nonnull resolve) {
    dispatch_async(self.saveQueue, ^{
      ...
      NSKeyedArchiver *archiver =
          [[NSKeyedArchiver alloc] initForWritingWithMutableData:result];

      @try {
        [archiver encodeObject:backgroundCopy
        ...
        error = [NSError
            errorWithDomain:NSInvalidArchiveOperationException
                       code:-1
                   userInfo:@{
...
+ (NSURL *)defaultSchemaURL {
  ...
  NSString *path =
      [documents stringByAppendingPathComponent:WMFExploreSectionsFilePath];
...
+ (instancetype)schemaFromFileAtURL:(NSURL *)fileURL {
  ...
  [NSKeyedUnarchiver setClass:[WMFExploreSectionSchema class]
                 forClassName:@"WMFHomeSectionSchema"];
  [NSKeyedUnarchiver setClass:[WMFExploreSection class]
                 forClassName:@"WMFHomeSection"];
  NSError *error;
  NSData *data =
      [[NSData alloc] initWithContentsOfURL:fileURL options:0 error:&error];
  if (!data) {
    NSAssert([error.domain isEqualToString:NSCocoaErrorDomain] &&
  ...
  if ([[NSProcessInfo processInfo]
          wmf_isOperatingSystemMajorVersionAtLeast:9]) {
    schema =
    ...
    @try {
      ...
      error =
          [NSError errorWithDomain:NSInvalidArchiveOperationException
                              code:-1
                          userInfo:@{
                            NSLocalizedDescriptionKey : exception.name,
                            NSLocalizedFailureReasonErrorKey : exception.reason
