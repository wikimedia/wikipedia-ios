...
@property(nonatomic, strong, readwrite) id<MWKLanguageFilterDataSource>
    dataSource;
...
- (instancetype)initWithLanguageDataSource:
  ...
  if (self) {
    ...
    [self.KVOController observe:dataSource
                        ...
                        options:0
                          block:^(MWKLanguageFilter *observer, id object,
                                  NSDictionary *change) {
  ...
  if ([self.languageFilter length] == 0) {
    ...
    self.filteredLanguages = [self.dataSource.allLanguages
        bk_select:^BOOL(MWKLanguageLink *langLink) {
          return [langLink.name
                     wmf_caseInsensitiveContainsString:self.languageFilter] ||
                 [langLink.localizedName
                     wmf_caseInsensitiveContainsString:self.languageFilter] ||
                 [langLink.languageCode
                     wmf_caseInsensitiveContainsString:self.languageFilter];
        }];
    self.filteredPreferredLanguages = [self.dataSource.preferredLanguages
        bk_select:^BOOL(MWKLanguageLink *langLink) {
