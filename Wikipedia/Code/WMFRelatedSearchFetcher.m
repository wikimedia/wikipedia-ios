...
- (AnyPromise *)fetchArticlesRelatedArticleWithURL:(NSURL *)URL
  ...
  return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
    WMFRelatedSearchRequestParameters *params =
        [WMFRelatedSearchRequestParameters new];
    ...
    [self.operationManager wmf_GETAndRetryWithURL:URL
        ...
        success:^(NSURLSessionDataTask *operation, id responseObject) {
          ...
          resolve([[WMFRelatedSearchResults alloc] initWithURL:URL
...
- (void)setNumberOfResults:(NSUInteger)numberOfResults {
  if (numberOfResults > WMFMaxRelatedSearchResultLimit) {
    DDLogError(@"Illegal attempt to request %lu articles, limiting to %lu.",
               ...
               (unsigned long)WMFMaxRelatedSearchResultLimit);
...
requestBySerializingRequest:(NSURLRequest *)request
  ...
  NSDictionary *serializedParams =
      [self serializedParams:(WMFRelatedSearchRequestParameters *)parameters];
...
- (NSDictionary *)serializedParams:(WMFRelatedSearchRequestParameters *)params {
  ...
  [baseParams setValuesForKeysWithDictionary:@{
    ...
    // search
    @"gsrsearch" :
        [NSString stringWithFormat:@"morelike:%@", params.articleURL.wmf_title],
    @"gsrnamespace" : @0,
    @"gsrwhat" : @"text",
    @"gsrinfo" : @"",
