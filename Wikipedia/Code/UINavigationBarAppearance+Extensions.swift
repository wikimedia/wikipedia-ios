...
@objc extension UINavigationBarAppearance {
    ...
    @objc static func scrollEdgeAppearanceForTheme(_ theme: Theme, style: WMFThemeableNavigationControllerStyle) -> UINavigationBarAppearance {
        let appearance = UINavigationBarAppearance()
        appearance.configureWithOpaqueBackground()
        appearance.backgroundColor = theme.colors.chromeBackground
        appearance.titleTextAttributes = theme.scrollEdgeTitleTextAttributes
        switch style {
        case .editor:
            appearance.backgroundImage = theme.editorNavigationBarBackgroundImage
        case .sheet:
