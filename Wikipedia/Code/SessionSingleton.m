...
- (instancetype)init {
  return [self initWithDataStore:[[MWKDataStore alloc]
                                     initWithBasePath:[[MWKDataStore class]
                                                          mainDataStorePath]]];
}
...
- (instancetype)initWithDataStore:(MWKDataStore *)dataStore {
  ...
  if (self) {
    ...
    WMFURLCache *urlCache =
        [[WMFURLCache alloc] initWithMemoryCapacity:MegabytesToBytes(64)
...
- (void)setCurrentArticleSiteURL:(NSURL *)currentArticleSiteURL {
  ...
  if (!currentArticleSiteURL ||
      [_currentArticleSiteURL isEqual:currentArticleSiteURL]) {
    return;
  }
  _currentArticleSiteURL = [currentArticleSiteURL wmf_siteURL];
  [[NSUserDefaults standardUserDefaults]
      setObject:currentArticleSiteURL.wmf_language
...
- (void)setCurrentArticleURL:(NSURL *)currentArticleURL {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:currentArticleURL.wmf_title
...
- (NSURL *)lastKnownSite {
  return [NSURL wmf_URLWithDefaultSiteAndlanguage:
                    [[NSUserDefaults standardUserDefaults]
...
- (NSURL *)urlForLanguage:(NSString *)language {
  return self.fallback
             ? [NSURL wmf_desktopAPIURLForURL:
                          [NSURL wmf_URLWithDefaultSiteAndlanguage:language]]
             : [NSURL wmf_mobileAPIURLForURL:
                          [NSURL wmf_URLWithDefaultSiteAndlanguage:language]];
}
...
- (void)setShouldSendUsageReports:(BOOL)sendUsageReports {
  ...
  [[NSUserDefaults standardUserDefaults]
      wmf_setSendUsageReports:sendUsageReports];
