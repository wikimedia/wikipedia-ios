...
typedef NS_ENUM(NSInteger, WMFFaceDectionError) {
  ...
  WMFFaceDectionErrorAppInBackground =
      1 // face detection on GPU not allowed in the background
};
...
/**
 * Asynchronously detect faces in `image`, without doing extra processing for
 * smiles or eyes.
 */
