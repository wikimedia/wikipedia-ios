...
#define WMFVerifyCacheConsistency(sectionOrController)                         \
  [self verifyCacheConsistency:(sectionOrController)]
#else
#define WMFVerifyCacheConsistency(sectionOrController)
#endif
...
- (void)verifyCacheConsistencyForController:
    (id<WMFExploreSectionController>)controller {
  WMFExploreSection *section =
      [self.reverseLookup objectForKey:@([controller hash])];
  if (!section) {
    if ([self.sectionControllersBySection.allValues
            containsObject:controller]) {
      DDLogWarn(@"Reverse map is missing a section for controller: %@",
                controller);
    }
    return;
  }
  id<WMFExploreSectionController> cacheController =
      [self.sectionControllersBySection objectForKey:section];
  if (!cacheController) {
    DDLogWarn(@"Reverse map contains section for controller which is no longer "
              @"cached: %@",
              section);
  }
}
...
- (void)verifyCacheConsistencyForSection:(WMFExploreSection *)section {
  ...
  if (!cacheController) {
    if ([self.reverseLookup.allValues containsObject:section]) {
      DDLogWarn(@"Reverse map contains section for controller which is no "
                @"longer cached: %@",
                section);
    }
    return;
  }
  WMFExploreSection *reverseSection =
      [self.reverseLookup objectForKey:@([cacheController hash])];
  if (![reverseSection isEqual:section]) {
    DDLogWarn(@"Mismatch between cached controllers & reverse map! Reverse "
              @"map: %@ cache: %@",
...
getOrCreateControllerForSection:(WMFExploreSection *)section
                  creationBlock:(nullable void (^)(
                                    id<WMFExploreSectionController> _Nonnull))
                                    creationBlock {
  id<WMFExploreSectionController> controller =
      [self controllerForSection:section];
  if (controller) {
    return controller;
  }
...
- (id<WMFExploreSectionController>)newControllerForSection:
    (WMFExploreSection *)section {
  NSAssert(![self.sectionControllersBySection objectForKey:section],
           @"Invalid request to create a new section controller for section %@ "
  ...
  case WMFExploreSectionTypeMostRead:
    ...
    /*
       !!!: do not add a default case, it is intentionally omitted so an
       error/warning is triggered when
       a new case is added to the enum, enforcing that all sections are handled
       here.
     */
  }
...
- (WMFMostReadSectionController *)mostReadSectionControllerForSection:
    (WMFExploreSection *)section {
  return [[WMFMostReadSectionController alloc]
      initWithDate:section.mostReadFetchDate
           siteURL:section.siteURL
         dataStore:self.dataStore];
}

- (WMFRelatedSectionController *)relatedSectionControllerForSectionSchemaItem:
    (WMFExploreSection *)item {
  return [[WMFRelatedSectionController alloc]
      initWithArticleURL:item.articleURL
               blackList:[WMFRelatedSectionBlackList sharedBlackList]
               dataStore:self.dataStore];
}

- (WMFContinueReadingSectionController *)
continueReadingSectionControllerForSchemaItem:(WMFExploreSection *)item {
  return [[WMFContinueReadingSectionController alloc]
      initWithArticleURL:item.articleURL
               dataStore:self.dataStore];
}

- (WMFNearbySectionController *)nearbySectionControllerForSchemaItem:
    (WMFExploreSection *)item {
...
- (WMFRandomSectionController *)randomSectionControllerForSchemaItem:
    (WMFExploreSection *)item {
  return
      [[WMFRandomSectionController alloc] initWithSearchSiteURL:item.siteURL
                                                      dataStore:self.dataStore];
}

- (WMFMainPageSectionController *)mainPageSectionControllerForSchemaItem:
    (WMFExploreSection *)item {
  return [[WMFMainPageSectionController alloc] initWithSiteURL:item.siteURL
                                                     dataStore:self.dataStore];
}

- (WMFPictureOfTheDaySectionController *)
picOfTheDaySectionControllerForSchemaItem:(WMFExploreSection *)item {
  return [[WMFPictureOfTheDaySectionController alloc]
      initWithDataStore:self.dataStore
                   date:item.dateCreated];
}

- (WMFFeaturedArticleSectionController *)
featuredArticleSectionControllerForSchemaItem:(WMFExploreSection *)item {
  return [[WMFFeaturedArticleSectionController alloc]
      initWithSiteURL:item.siteURL
                 date:item.dateCreated
            dataStore:self.dataStore];
}
...
- (void)removeAllSectionsExcept:(NSArray<WMFExploreSection *> *)sections {
  ...
  NSMutableSet *sectionsToRemove =
      [NSMutableSet setWithArray:self.sectionControllersBySection.allKeys];
