...
/// Required extmetadata keys, don't forget to add new ones to
/// +requiredExtMetadataKeys!
...
static CGSize MWKImageInfoSizeFromJSON(NSDictionary *json, NSString *widthKey,
                                       NSString *heightKey) {
...
+ (NSArray *)galleryExtMetadataKeys {
  return @[
    ExtMetadataLicenseKey, ExtMetadataLicenseUrlKey,
    ExtMetadataLicenseShortNameKey, ExtMetadataImageDescriptionKey,
...
- (id)responseObjectForResponse:(NSURLResponse *)response
  ...
  NSMutableArray *itemListBuilder =
      [NSMutableArray arrayWithCapacity:[[indexedImages allKeys] count]];
  [indexedImages enumerateKeysAndObjectsUsingBlock:^(
                     NSString *key, NSDictionary *image, BOOL *stop) {
    ...
    MWKLicense *license = [[MWKLicense alloc]
            initWithCode:extMetadata[ExtMetadataLicenseKey][@"value"]
        shortDescription:extMetadata[ExtMetadataLicenseShortNameKey][@"value"]
                     URL:[NSURL wmf_optionalURLWithString:
                                    extMetadata[ExtMetadataLicenseUrlKey]
                                               [@"value"]]];

    MWKImageInfo *item = [[MWKImageInfo alloc]
        initWithCanonicalPageTitle:image[@"title"]
                  canonicalFileURL:[NSURL wmf_optionalURLWithString:imageInfo[
                                                                        @"url"]]
                  imageDescription:
                      [[extMetadata[ExtMetadataImageDescriptionKey][@"value"]
                          wmf_joinedHtmlTextNodes]
                          wmf_getCollapsedWhitespaceStringAdjustedForTerminalPunctuation]
                           license:license
                       filePageURL:[NSURL wmf_optionalURLWithString:
                                              imageInfo[@"descriptionurl"]]
                     imageThumbURL:[NSURL wmf_optionalURLWithString:
                                              imageInfo[@"thumburl"]]
                             owner:
                                 [[extMetadata[ExtMetadataArtistKey][@"value"]
                                     wmf_joinedHtmlTextNodes]
                                     wmf_getCollapsedWhitespaceStringAdjustedForTerminalPunctuation]
                         imageSize:MWKImageInfoSizeFromJSON(imageInfo, @"width",
                                                            @"height")
                         thumbSize:MWKImageInfoSizeFromJSON(imageInfo,
                                                            @"thumbwidth",
                                                            @"thumbheight")];
