...
- (void)setTabularSubviews:(NSArray *)tabularSubviews {
  // Reminder: iOS 6 can crash (tap "Login" then tap "X" to go back to left
  // menu) for
  // some reason if we don't delay execution of this stuff to next run loop
  // iteration.
  ...
  [[NSRunLoop currentRunLoop]
      performSelector:@selector(actuallySetTabularSubviews:)
               target:self
...
- (void)viewTapped:(id)sender {
  ...
  NSMutableDictionary *userInfo =
      [NSMutableDictionary dictionaryWithObject:tappedView
                                         forKey:@"tappedItem"];
  if (tappedChildView) {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"TabularScrollViewItemTapped"
                    object:self
                  userInfo:userInfo];
}
...
- (void)constrainContainerViewSubviewsHorizontally {
  ...
  for (UIView *view in self.containerView.subviews) {
    ...
    [constraints
        addObject:
            [NSLayoutConstraint
                constraintsWithVisualFormat:@"V:[view(>=height)]"
                                    options:0
                                    metrics:@{
                                      @"height" : @(self.minSubviewHeight)
                                    }
                                      views:NSDictionaryOfVariableBindings(
                                                view)]];
    prevView = view;
