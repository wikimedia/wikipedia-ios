...
- (NSDictionary *)dictionary {
  if (!_dictionary && self.data) {
    NSError *error = nil;
    _dictionary = [NSJSONSerialization JSONObjectWithData:self.data
                                                  options:0
                                                    error:&error];
...
- (NSArray *)array {
  if (!_array && self.data) {
    NSError *error = nil;
    _array = [NSJSONSerialization JSONObjectWithData:self.data
                                             options:0
                                               error:&error];
    NSAssert([_array isKindOfClass:[NSArray class]], @"Expected array, got %@",
             _array);
...
- (NSURL *)url {
  ...
  switch (self.fileType) {
  case WMFAssetsFileTypeConfig:
    urlString = @"https://meta.wikimedia.org/static/current/extensions/"
                @"MobileApp/config/ios.json";
    break;
...
- (BOOL)isOlderThan:(NSTimeInterval)maxAge {
  ...
  [url getResourceValue:&lastModified
                 forKey:NSURLContentModificationDateKey
                  error:&error];
  if (!error) {
    NSTimeInterval currentAge =
        [[NSDate date] timeIntervalSinceDate:lastModified];
