...
- (CGRect)getPrimaryFocalRectFromCanonicalLeadImage {
  NSAssert([self.leadImage isVariantOfImage:self.leadImage.article.image],
           @"Primary focal rect sought on non-lead image.");

  // Focal rect info is parked on the article.image which is the originally
  // retrieved lead image.
  // self.leadImage is potentially a larger variant, which is why here the focal
  ...
  if (focalRects.count > 0) {
    focalRect = WMFDenormalizeRectUsingSize(
        [[focalRects firstObject] CGRectValue], self.leadImage.size);
  }
  return focalRect;
}
...
- (void)drawRect:(CGRect)rect {
  ...
  CGRect focalBounds = self.leadImage.isDownloaded
                           ? [self getPrimaryFocalRectFromCanonicalLeadImage]
                           : CGRectZero;
...
- (void)drawGradientBackground {
  ...
  void (^drawGradient)(CGFloat, CGFloat, CGRect) = ^void(
      CGFloat upperAlpha, CGFloat bottomAlpha, CGRect rect) {
    CGFloat locations[] = {
    ...
    CGGradientRef gradient = CGGradientCreateWithColorComponents(
        colorSpace, colorComponents, locations, 2);
