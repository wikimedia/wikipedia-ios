...
- (UIWindow *)window {
  if (!_window) {
    if ([[[NSProcessInfo processInfo] environment][@"FBTweakShakeWindowEnabled"]
            boolValue]) {
...
- (void)updateDynamicIconShortcutItems {
  if (![[UIApplication sharedApplication]
          respondsToSelector:@selector(shortcutItems)]) {
    return;
  }

  NSMutableArray<UIApplicationShortcutItem *> *shortcutItems =
      [[NSMutableArray alloc]
          initWithObjects:[UIApplicationShortcutItem wmf_random],
                          [UIApplicationShortcutItem wmf_nearby], nil];

  [shortcutItems
      wmf_safeAddObject:[UIApplicationShortcutItem wmf_continueReading]];
...
- (BOOL)application:(UIApplication *)application
  ...
  [[NSUserDefaults standardUserDefaults]
      wmf_setAppInstallDateIfNil:[NSDate date]];

  [self.statsFunnel logAppNumberOfDaysSinceInstall];

  WMFAppViewController *vc =
...
- (void)applicationDidBecomeActive:(UIApplication *)application {
  ...
  [[NSUserDefaults standardUserDefaults]
      wmf_setAppBecomeActiveDate:[NSDate date]];
}
...
- (BOOL)application:(UIApplication *)app
  ...
  NSUserActivity *activity =
      [NSUserActivity wmf_activityForWikipediaScheme:url];
  if (activity) {
...
- (void)applicationWillResignActive:(UIApplication *)application {
  ...
  [[NSUserDefaults standardUserDefaults]
      wmf_setAppResignActiveDate:[NSDate date]];
}
