...
- (instancetype)initAndFetchHtmlForWikiText:(NSString *)wikiText
  ...
  if (self) {
    ...
    [self fetchPreviewForWikiText:wikiText
...
- (void)fetchPreviewForWikiText:(NSString *)wikiText
  ...
  NSURL *url = [[SessionSingleton sharedInstance]
      urlForLanguage:articleURL.wmf_language];

  NSDictionary *params =
      [self getParamsForArticleURL:articleURL wikiText:wikiText];
  ...
  [manager POST:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (responseObject[@"error"]) {
          ...
          error = [NSError errorWithDomain:@"Preview HTML Fetcher"
                                      code:001
                                  userInfo:errorDict];
        }
...
- (NSDictionary *)getParamsForArticleURL:(NSURL *)articleURL
