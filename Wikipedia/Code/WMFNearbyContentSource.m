...
- (void)locationManager:(WMFLocationManager *)controller didUpdateLocation:(CLLocation *)location {
    ...
    self.isFetchingInitialLocation = NO;
    [self getGroupForLocation:location completion:^(WMFLocationContentGroup *group) {
...
- (nullable WMFLocationContentGroup*)contentGroupCloseToLocation:(CLLocation*)location{
    ...
    [self.contentStore enumerateContentGroupsOfKind:[WMFLocationContentGroup kind] withBlock:^(WMFContentGroup * _Nonnull group, BOOL * _Nonnull stop) {
...
- (void)getGroupForLocation:(CLLocation *)location completion:(void (^) (WMFLocationContentGroup* group))completion
    ...
    [self.currentLocationManager reverseGeocodeLocation:location completion:^(CLPlacemark * _Nonnull placemark) {
...
- (void)fetchResultsForLocationGroup:(WMFLocationContentGroup*)group completion:(nullable dispatch_block_t)completion{
    ...
    @weakify(self);
    [self.locationSearchFetcher fetchArticlesWithSiteURL:self.siteURL location:group.location resultLimit:20 completion:^(WMFLocationSearchResults * _Nonnull results) {
        @strongify(self);
...
- (void)removeOldSectionsForDate:(NSDate*)date{
    ...
    [self.contentStore enumerateContentGroupsOfKind:[WMFLocationContentGroup kind] withBlock:^(WMFContentGroup * _Nonnull section, BOOL * _Nonnull stop) {
