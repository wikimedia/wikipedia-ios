 ...
 *
 * This is *mostly* links to the same topic/entity in another language, but not
 * always. See the comments
 * in LanguageLinksFetcher. Be sure to double check that you add special
 * handling when necessary. For example, main
...
@property(readonly, copy, nonatomic)
    NSString *thumbnailURL; // optional; generated from imageURL
@property(readwrite, copy, nonatomic)
    NSString *imageURL; // optional; pulled in article request

- (NSString *)bestThumbnailImageURL;

@property(readonly, copy, nonatomic) NSString
    *entityDescription; // optional; currently pulled separately via wikidata
@property(readonly, copy, nonatomic)
    NSString *searchSnippet; // Snippet returned from search results
 ...
 *
 *  Might be `nil` if the section containing the reference list hasn't been
 * downloaded, be sure to check `isCached`
 *  and fetch the full article contents if necessary.  Might also be `nil` if an
 ...
 *
 *  This method is meant to be a good compromise between comprehensive equality
 * checking and speed. For a more detailed
 *  check which takes into account the full content of the article (e.g. section
 * text), use `isDeeplyEqualToArticle:`.
 *
 ...
 *
 *  Only use this method when you both 1) need to check the articles' content
 * and 2) can afford to load all the section
 *  text into memory (i.e. ideally not on the main thread, and definitely not in
 * a tight loop).
 *
 *  @param article Another `MWKArticle`.
 *
