...
@property(strong, nonatomic)
    IBOutlet WelcomeAnalyticsAnimationView *animationView;
...
- (void)viewDidLoad {
  [super viewDidLoad];
  self.titleLabel.text = [MWLocalizedString(@"welcome-volunteer-title", nil)
      uppercaseStringWithLocale:[NSLocale currentLocale]];
  self.subTitleLabel.text =
      MWLocalizedString(@"welcome-volunteer-sub-title", nil);
  ...
  // Set state of the toggle. Also make sure crash manager setting is in sync
  // with this setting - likely to happen on first launch or for previous users.
...
- (void)updateToggleLabelTitleForUsageReportsIsOn:(BOOL)isOn {
  NSString *title =
      isOn ? [MWLocalizedString(@"welcome-volunteer-thanks", nil)
                 stringByReplacingOccurrencesOfString:@"$1"
                                           withString:@"😀"]
           : MWLocalizedString(@"welcome-volunteer-send-usage-reports", nil);
  self.toggleLabel.text = title;
  self.toggleLabel.textColor =
      isOn ? [UIColor wmf_green] : [UIColor darkGrayColor];
}
