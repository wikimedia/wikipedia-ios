...
@property(nonatomic, assign, readwrite)
    WMFSettingsMenuItemDisclosureType disclosureType;
...
+ (WMFSettingsMenuItem *)itemForType:(WMFSettingsMenuItemType)type {
        ...
        (userName)
            ? [MWLocalizedString(@"main-menu-account-title-logged-in", nil)
                  stringByReplacingOccurrencesOfString:@"$1"
                                            withString:userName]
            : MWLocalizedString(@"main-menu-account-login", nil);

    return [[WMFSettingsMenuItem alloc]
          initWithType:type
                 title:loginString
              iconName:@"settings-user"
             iconColor:[UIColor
                           wmf_colorWithHex:(userName ? 0xFF8E2B : 0x9AA0A7)
                                      alpha:1.0]
  ...
  case WMFSettingsMenuItemType_SearchLanguage: {
    return [[WMFSettingsMenuItem alloc]
        ...
        disclosureText:[[[MWKLanguageLinkController sharedInstance] appLanguage]
                               .languageCode uppercaseString]
            isSwitchOn:NO];
  }
  case WMFSettingsMenuItemType_SearchLanguageBarVisibility: {
    return [[WMFSettingsMenuItem alloc]
        ...
        disclosureText:nil
            isSwitchOn:[[NSUserDefaults standardUserDefaults]
                           wmf_showSearchLanguageBar]];
  }
    ...
    return [[WMFSettingsMenuItem alloc]
          initWithType:type
                 title:MWLocalizedString(
                           @"preference_title_eventlogging_opt_in", nil)
              iconName:@"settings-analytics"
             iconColor:[UIColor wmf_colorWithHex:0x95D15A alpha:1.0]
        disclosureType:WMFSettingsMenuItemDisclosureType_Switch
        disclosureText:nil
            isSwitchOn:[SessionSingleton sharedInstance]
                           .shouldSendUsageReports];
  }
  case WMFSettingsMenuItemType_ZeroWarnWhenLeaving: {
    return [[WMFSettingsMenuItem alloc]
        ...
        disclosureText:nil
            isSwitchOn:[SessionSingleton sharedInstance]
                           .zeroConfigState.warnWhenLeaving];
  }
