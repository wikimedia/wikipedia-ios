...
+ (WMFAssetsFile *)mainPages {
  ...
  if (!mainPages) {
    mainPages =
        [[WMFAssetsFile alloc] initWithFileType:WMFAssetsFileTypeMainPages];
  }
  return mainPages;
}
...
+ (nullable NSURL *)wmf_mainPageURLForLanguage:(NSString *)language {
  ...
  return [NSURL wmf_URLWithDomain:WMFDefaultSiteDomain
                         language:language
                            title:titleText
                         fragment:nil];
}
...
+ (nullable NSURL *)wmf_wikimediaCommonsURL {
  ...
  URLComponents.host = [NSURLComponents wmf_hostWithDomain:@"wikimedia.org"
                                                 subDomain:@"commons"
                                                  isMobile:NO];
...
+ (NSURL *)wmf_URLWithDefaultSiteAndLocale:(NSLocale *)locale {
  return [self wmf_URLWithDomain:WMFDefaultSiteDomain
...
+ (NSURL *)wmf_URLWithDomain:(NSString *)domain
  ...
  return
      [[NSURLComponents wmf_componentsWithDomain:domain language:language] URL];
}
...
+ (NSURL *)wmf_URLWithDomain:(NSString *)domain
  ...
  return [[NSURLComponents wmf_componentsWithDomain:domain
                                           language:language
                                              title:title
...
+ (NSRegularExpression *)invalidPercentEscapesRegex {
  ...
  dispatch_once(&onceToken, ^{
    percentEscapesRegex = [NSRegularExpression
        regularExpressionWithPattern:@"%[^0-9A-F]|%[0-9A-F][^0-9A-F]"
...
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
    unescapedDenormalizedTitleAndFragment:(NSString *)path {
  ...
  if ([path wmf_isWikiResource]) {
    return [NSURL wmf_URLWithSiteURL:siteURL
        unescapedDenormalizedInternalLink:path];
  } else {
    NSArray *bits = [path componentsSeparatedByString:@"#"];
    NSString *fragment =
        [[bits wmf_safeObjectAtIndex:1] precomposedStringWithCanonicalMapping];
    return
        [NSURL wmf_URLWithSiteURL:siteURL
...
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
    escapedDenormalizedTitleAndFragment:(NSString *)path {
  NSAssert(![path wmf_isWikiResource],
           @"Didn't expect %@ to be an internal link. Use "
           @"initWithInternalLink:site: instead.",
           path);
  NSAssert([[NSURL invalidPercentEscapesRegex]
               matchesInString:path
                       options:0
                         range:NSMakeRange(0, path.length)]
                   .count == 0,
           @"%@ should only have valid percent escapes", path);
  if ([path wmf_isWikiResource]) {
    return
        [NSURL wmf_URLWithSiteURL:siteURL escapedDenormalizedInternalLink:path];
  } else {
    NSArray *bits = [path componentsSeparatedByString:@"#"];
    return [NSURL
        wmf_URLWithSiteURL:siteURL
...
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
    unescapedDenormalizedInternalLink:(NSString *)internalLink {
  NSAssert(internalLink.length == 0 || [internalLink wmf_isWikiResource],
           @"Expected string with internal link prefix but got: %@",
           internalLink);
  return [self wmf_URLWithSiteURL:siteURL
      unescapedDenormalizedTitleAndFragment:[internalLink
                                                wmf_pathWithoutWikiPrefix]];
}
...
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
  ...
  NSAssert(internalLink.length == 0 || [internalLink wmf_isWikiResource],
           ...
           internalLink);
  return [self wmf_URLWithSiteURL:siteURL
      escapedDenormalizedTitleAndFragment:[internalLink
                                              wmf_pathWithoutWikiPrefix]];
}

- (NSURL *)wmf_URLWithTitle:(NSString *)title {
  NSURLComponents *components =
      [NSURLComponents componentsWithURL:self resolvingAgainstBaseURL:NO];
...
- (NSURL *)wmf_URLWithPath:(NSString *)path isMobile:(BOOL)isMobile {
  ...
  if (isMobile != self.wmf_isMobile) {
    components.host = [NSURLComponents wmf_hostWithDomain:self.wmf_domain
                                                 language:self.wmf_language
                                                 isMobile:isMobile];
  }
  return components.URL;
}
...
+ (NSURL *)wmf_mobileURLForURL:(NSURL *)url {
  if (url.wmf_isMobile) {
    ...
    components.host = [NSURLComponents wmf_hostWithDomain:url.wmf_domain
                                                 language:url.wmf_language
                                                 isMobile:YES];
...
- (NSString *)wmf_domain {
  ...
  if (hostComponents.count < 3) {
    ...
    NSArray *subarray = [hostComponents
        subarrayWithRange:NSMakeRange(firstIndex,
                                      hostComponents.count - firstIndex)];
...
- (NSString *)wmf_title {
  ...
  NSString *title =
      [[self.path wmf_pathWithoutWikiPrefix] wmf_normalizedPageTitle];
  if (title == nil) {
    title = @"";
  }
  return title;
...
- (NSString *)wmf_titleWithUnderScores {
  ...
  NSString *title =
      [[self.path wmf_pathWithoutWikiPrefix] wmf_denormalizedPageTitle];
  if (title == nil) {
    title = @"";
  }
  return title;
...
- (UIUserInterfaceLayoutDirection)wmf_layoutDirection {
  switch (CFLocaleGetLanguageCharacterDirection(
      (__bridge CFStringRef)self.wmf_language)) {
