...
- (void)setupTable {
  [self.table registerNib:[UINib nibWithNibName:@"RecentSearchCell" bundle:nil]
  ...
  /*
     HAX: Used grouped table layout to get, for free, separators above the first
     cell
     and below the last cell, but grouped layout adds a background which causes
     the
...
- (void)setupHeadingLabel {
  // Reminder: TWN has in the past rejected all-caps strings because there are
  // complications
  // with translation/meaning of all-caps in other languages. The recommendation
  // was to submit strings to TWN with non-all-caps, and at display time force
  // the string
  // to all caps.
...
- (void)setupTrashButton {
  @weakify(self) self.trashButton =
      [UIButton buttonWithType:UIButtonTypeCustom];
  [self.trashButton setImage:[UIImage imageNamed:@"clear-mini"]
                    forState:UIControlStateNormal];
  [self.trashButton bk_addEventHandler:^(UIButton *sender) {
    @strongify(self)[self showDeleteAllDialog];
  }
  ...
  self.trashButton.accessibilityLabel =
...
- (void)updateTrashButtonEnabledState {
  self.trashButton.enabled =
      ([self.recentSearches countOfEntries] > 0) ? YES : NO;
}

- (void)updateHeaderVisibility {
  self.headerContainer.hidden =
      ([self.recentSearches countOfEntries] > 0) ? NO : YES;
}
...
- (void)showDeleteAllDialog {
  UIAlertView *dialog = [[UIAlertView alloc]
          initWithTitle:MWLocalizedString(
                            @"search-recent-clear-confirmation-heading", nil)
                message:MWLocalizedString(
                            @"search-recent-clear-confirmation-sub-heading",
                            nil)
               delegate:self
      cancelButtonTitle:MWLocalizedString(@"search-recent-clear-cancel", nil)
      otherButtonTitles:MWLocalizedString(@"search-recent-clear-delete-all",
                                          nil),
                        nil];
  [dialog show];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  RecentSearchCell *cell = (RecentSearchCell *)[tableView
      dequeueReusableCellWithIdentifier:cellId
                           forIndexPath:indexPath];
  NSString *term =
      [[self.recentSearches entryAtIndex:indexPath.row] searchTerm];
...
- (void)tableView:(UITableView *)tableView
  ...
  [self.delegate
      recentSearchController:self
         didSelectSearchTerm:[self.recentSearches entryAtIndex:indexPath.row]];
}
