...
static NSString *const WMFSettingsURLZeroFAQ =
    @"https://m.wikimediafoundation.org/wiki/Wikipedia_Zero_App_FAQ";
static NSString *const WMFSettingsURLTerms =
    @"https://m.wikimediafoundation.org/wiki/Terms_of_Use";
static NSString *const WMFSettingsURLRate =
    @"itms-apps://itunes.apple.com/app/id324715238";
static NSString *const WMFSettingsURLDonation =
    @"https://donate.wikimedia.org/"
    @"?utm_medium=WikipediaApp&utm_campaign=iOS&utm_source=<app-version>&"
    @"uselang=<langcode>";

@interface WMFSettingsViewController () <
    UITableViewDelegate, WMFPreferredLanguagesViewControllerDelegate,
...
- (void)viewDidLoad {
  ...
  [self.tableView registerNib:[WMFSettingsTableViewCell wmf_classNib]
       forCellReuseIdentifier:[WMFSettingsTableViewCell identifier]];
  ...
  [self.KVOControllerNonRetaining
      observe:[WMFAuthenticationManager sharedInstance]
      keyPath:WMF_SAFE_KEYPATH([WMFAuthenticationManager sharedInstance],
                               loggedInUsername)
      options:NSKeyValueObservingOptionInitial
        block:^(WMFSettingsViewController *observer, id object,
                NSDictionary *change) {
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  [NSUserActivity
      wmf_makeActivityActive:[NSUserActivity wmf_settingsViewActivity]];
}
...
- (void)configureTableDataSource {
  ...
  self.elementDataSource.tableActionBlock = ^BOOL(
      SSCellActionType action, UITableView *tableView, NSIndexPath *indexPath) {
    return NO;
  };

  @weakify(self) self.elementDataSource.cellConfigureBlock =
      ^(WMFSettingsTableViewCell *cell, WMFSettingsMenuItem *menuItem,
        ...
        cell.selectionStyle = (menuItem.disclosureType ==
                               WMFSettingsMenuItemDisclosureType_Switch)
                                  ? UITableViewCellSelectionStyleNone
                                  : UITableViewCellSelectionStyleDefault;

        [cell.disclosureSwitch
            bk_removeEventHandlersForControlEvents:UIControlEventValueChanged];
        [cell.disclosureSwitch bk_addEventHandler:^(UISwitch *sender) {
          @strongify(self) menuItem.isSwitchOn = sender.isOn;
          [self updateStateForMenuItemType:menuItem.type
                           isSwitchOnValue:sender.isOn];
        }
...
- (void)updateStateForMenuItemType:(WMFSettingsMenuItemType)type
                   isSwitchOnValue:(BOOL)isOn {
  switch (type) {
...
- (void)tableView:(UITableView *)tableView
  ...
  switch ([(WMFSettingsMenuItem *)[self.elementDataSource
      itemAtIndexPath:indexPath] type]) {
  ...
  case WMFSettingsMenuItemType_SendFeedback: {
    WMFHelpViewController *vc =
        [[WMFHelpViewController alloc] initWithDataStore:self.dataStore];
  ...
  case WMFSettingsMenuItemType_DevSettings: {
    FBTweakViewController *tweaksVC = [[FBTweakViewController alloc]
        initWithStore:[FBTweakStore sharedInstance]];
...
- (NSURL *)donationURL {
  ...
  url = [url stringByReplacingOccurrencesOfString:@"<langcode>"
                                       withString:languageCode];
  url = [url stringByReplacingOccurrencesOfString:@"<app-version>"
...
- (void)showLoginOrLogout {
  NSString *userName =
      [WMFAuthenticationManager sharedInstance].loggedInUsername;
  if (userName) {
...
- (void)showLogoutActionSheet {
  UIAlertController *sheet = [UIAlertController
      alertControllerWithTitle:MWLocalizedString(
                                   @"main-menu-account-logout-are-you-sure",
                                   nil)
                       message:nil
  ...
  @weakify(self)
      [sheet addAction:[UIAlertAction
                           actionWithTitle:MWLocalizedString(
                                               @"main-menu-account-logout", nil)
                                     style:UIAlertActionStyleDestructive
                                   handler:^(UIAlertAction *_Nonnull action) {
                                     @strongify(self)[self logout];
                                     [self reloadVisibleCellOfType:
                                               WMFSettingsMenuItemType_Login];
                                   }]];
  [sheet
      addAction:[UIAlertAction
                    actionWithTitle:MWLocalizedString(
                                        @"main-menu-account-logout-cancel", nil)
...
- (void)tweakViewControllerPressedDone:
    (FBTweakViewController *)tweakViewController {
  [[NSNotificationCenter defaultCenter]
      postNotificationName:FBTweakShakeViewControllerDidDismissNotification
...
- (nullable NSIndexPath *)indexPathForVisibleCellOfType:
    (WMFSettingsMenuItemType)type {
  return [self.tableView.indexPathsForVisibleRows
      bk_match:^BOOL(NSIndexPath *indexPath) {
        return ((WMFSettingsMenuItem *)[self.elementDataSource
...
- (void)reloadVisibleCellOfType:(WMFSettingsMenuItemType)type {
  ...
  if (indexPath) {
    [self.elementDataSource
        replaceItemAtIndexPath:indexPath
                      withItem:[WMFSettingsMenuItem itemForType:type]];
  }
}
...
- (SSSection *)section_1 {
  SSSection *section = [SSSection sectionWithItems:@[
...
- (SSSection *)section_2 {
  SSSection *section = [SSSection sectionWithItems:@[
    ...
    [WMFSettingsMenuItem
        itemForType:WMFSettingsMenuItemType_SearchLanguageBarVisibility]
  ]];
  section.header = nil;
  section.footer = nil;
  return section;
...
- (SSSection *)section_4 {
  SSSection *section = [SSSection sectionWithItems:@[
    [WMFSettingsMenuItem
        itemForType:WMFSettingsMenuItemType_ZeroWarnWhenLeaving],
