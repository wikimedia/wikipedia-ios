...
// Returns an instance of the receiver from the initial view controller of a
// storyboard matching it's class storyboard.
+ (instancetype)wmf_initialViewControllerFromClassStoryboard {
  id vc = [[self wmf_classStoryboard] instantiateInitialViewController];
  NSAssert(vc,
           @"Instantiating view controller %@ from storyboard %@ returned nil!",
           vc, [self wmf_classStoryboardName]);
  NSAssert([vc isMemberOfClass:self], @"Expected %@ to be instance of class %@",
           vc, self);
  return vc;
}
...
+ (UIStoryboard *)wmf_classStoryboard {
  id sb = [UIStoryboard storyboardWithName:[self wmf_classStoryboardName]
                                    bundle:[NSBundle bundleForClass:self]];
  NSAssert(sb, @"Instantiating storyboard %@ returned nil!",
           [self wmf_classStoryboardName]);
  return sb;
}
...
+ (instancetype)wmf_viewControllerWithIdentifier:(NSString *)identifier
  ...
  NSAssert([instance isMemberOfClass:self],
           @"Unexpected view controller for identifier %@. Expected instance "
           @"of %@, got %@",
