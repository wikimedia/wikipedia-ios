...
- (AnyPromise *)fetchArticlesForSearchTerm:(NSString *)searchTerm
  ...
  return [self fetchArticlesForSearchTerm:searchTerm
                                  siteURL:siteURL
                              resultLimit:resultLimit
                           fullTextSearch:NO
                  appendToPreviousResults:nil];
}
...
- (AnyPromise *)fetchArticlesForSearchTerm:(NSString *)searchTerm
                   ...
                   appendToPreviousResults:
                       (nullable WMFSearchResults *)results {
  return [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
    [self fetchArticlesForSearchTerm:searchTerm
                             siteURL:siteURL
                         resultLimit:resultLimit
      ...
      failure:^(NSURLSessionDataTask *operation, NSError *error) {
        if ([url isEqual:[NSURL wmf_mobileAPIURLForURL:siteURL]] &&
          ...
          [self fetchArticlesForSearchTerm:searchTerm
                               ...
                               resultLimit:resultLimit
                            fullTextSearch:fullTextSearch
                   appendToPreviousResults:previousResults
                             useDesktopURL:YES
                                  resolver:resolve];
        } else {
          [[MWNetworkActivityIndicatorManager sharedManager] pop];
...
- (void)setNumberOfResults:(NSUInteger)numberOfResults {
  if (numberOfResults > WMFMaxSearchResultLimit) {
    DDLogError(@"Illegal attempt to request %lu articles, limiting to %lu.",
               ...
               (unsigned long)WMFMaxSearchResultLimit);
...
requestBySerializingRequest:(NSURLRequest *)request
  ...
  NSDictionary *serializedParams =
      [self serializedParams:(WMFSearchRequestParameters *)parameters];
