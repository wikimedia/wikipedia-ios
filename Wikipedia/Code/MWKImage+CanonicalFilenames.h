 ...
 *
 * @warning The returned array will contain @c NSNull instances for each image
 * which could not parse its canonical
 *          filename. This is intentional.
 *
 * Uses the images' @c canonicalFilename property, which is parsed from its @c
 * sourceURL.
 *
 * @return An array of @c NSString objects for successfully constructed
 * "File:..." titles or <code>[NSNull null]</code>
