...
/**
 * Image tag src urls are modified to point to the localhost proxy web server so
 *we can intercept image requests.
 * WKWebView requests are out of process so other methods - NSURLProtocol etc -
 *do not work.
 *
 ...
 *
 * Note that the original image url is added as the value for the "originalSrc"
 *query parameter.
 *
 * WMFProxyImageOriginalSrcKey controls the "originalSrc" string in the example
 *above.
 *
 ...
 *
 * @return  Returns the original non-proxy src url. Returns nil if no
 *WMFProxyImageOriginalSrcKey value is found in the underlying NSURL.
 **/
- (nullable NSURL *)wmf_imageProxyOriginalSrcURL;

/**
 * Adds a WMFProxyImageOriginalSrcKey key to the underlying NSURL set to the
 *value passed to the WMFProxyImageOriginalSrcKey parameter.
 *
