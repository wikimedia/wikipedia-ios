...
@property(nonatomic, strong, nullable, readwrite)
    WMFMostReadTitlesResponseItem *mostReadArticlesResponse;
@property(nonatomic, strong, nullable, readwrite)
    NSArray<MWKSearchResult *> *previews;
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@ site = %@, date = %@",
                                    [super description], self.siteURL,
                                    [self englishUTCDateString]];
}
 ...
 *
 *  "Most read" articles are computed for UTC dates. UTC time zone is used
 * because converting to the user's time zone
 *  might accidentally change the "day" the app displays based on the the offset
 * between UTC & the device's default time
 *  zone.  For example: 02/12/2016 01:26 UTC converted to EST is 02/11/2016
 * 20:26, one day off!
 *
...
- (NSString *)localDateDisplayString {
  if (!_localDateDisplayString) {
    _localDateDisplayString =
        [[NSDateFormatter wmf_utcDayNameMonthNameDayOfMonthNumberDateFormatter]
            stringFromDate:self.date];
  }
...
- (NSString *)localDateShortDisplayString {
  if (!_localDateShortDisplayString) {
    _localDateShortDisplayString = [[NSDateFormatter
...
/**
 *  Stable string for the receiver's date that doesn't vary by current calendar,
 * locale, or time zone.
 *
 *  This is necessary to ensure that cached sections are retrievable when the
 * locale, calendar, etc. changes.
 *
 *  @return An english-formatted string of the receiver's date in the UTC time
 * zone.
 */
- (NSString *)englishUTCDateString {
  return [[NSDateFormatter wmf_englishUTCSlashDelimitedYearMonthDayFormatter]
      stringFromDate:self.date];
}
...
- (NSString *)sectionIdentifier {
  return [NSString stringWithFormat:@"%@_%@", self.siteURL.host,
                                    [self englishUTCDateString]];
}
...
- (NSAttributedString *)headerTitle {
  ...
  NSString *language = [[NSLocale currentLocale]
      wmf_localizedLanguageNameForCode:self.siteURL.wmf_language];

  NSString *heading = nil;

  // crash protection if language is nil
  if (language) {
    heading = [MWLocalizedString(@"explore-most-read-heading", nil)
  ...
  NSDictionary *attributes = @{
    NSForegroundColorAttributeName :
        [UIColor wmf_exploreSectionHeaderTitleColor]
  };
  return
      [[NSAttributedString alloc] initWithString:heading attributes:attributes];
}
...
- (NSString *)footerText {
  return [MWLocalizedString(@"explore-most-read-footer-for-date", nil)
      stringByReplacingOccurrencesOfString:@"$1"
                                withString:self.localDateShortDisplayString];
}

- (UIViewController *)moreViewController {
  return [[WMFMostReadListTableViewController alloc]
      initWithPreviews:self.previews
...
- (UIViewController *)detailViewControllerForItemAtIndexPath:
  ...
  return [[WMFArticleViewController alloc]
      initWithArticleURL:[self urlForItemAtIndexPath:indexPath]
               dataStore:self.dataStore];
}
...
- (AnyPromise *)fetchData {
  @weakify(self);
  return [self.mostReadTitlesFetcher fetchMostReadTitlesForSiteURL:self.siteURL
                                                              date:self.date]
      .then(^id(WMFMostReadTitlesResponseItem *mostReadResponse) {
        ...
        NSArray<NSURL *> *titlesToPreview = [mostReadResponse.articles
            bk_map:^NSURL *(WMFMostReadTitlesResponseItemArticle *article) {
              // HAX: must normalize title otherwise it won't match fetched
              // previews. this is why pageid > title
        ...
        return [self.previewFetcher
            fetchArticlePreviewResultsForArticleURLs:titlesToPreview
                                      ...
                                      thumbnailWidth:
                                          [[UIScreen mainScreen]
                                              wmf_listThumbnailWidthForScale]
                                              .unsignedIntegerValue];
      })
      .then(
          ^NSArray<MWKSearchResult *> *(NSArray<MWKSearchResult *> *previews) {
            @strongify(self);

            // Now that we have preview data we can check for articleID. If
            // articleID is zero
            // it's not a regular article. Rejecting these hides most special
            // pages.
            ...
            // only return first 5 previews to the section, store the rest
            // internally for the full list view
