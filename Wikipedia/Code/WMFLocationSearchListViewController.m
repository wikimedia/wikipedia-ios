...
- (instancetype)initWithSearchSiteURL:(NSURL *)url
  ...
  if (self) {
    ...
    self.dataSource = [[WMFNearbyTitleListDataSource alloc]
        initWithSearchSiteURL:self.searchSiteURL];
    self.dataSource.cellClass = [WMFNearbyArticleTableViewCell class];

    @weakify(self);
    self.dataSource.cellConfigureBlock = ^(
        WMFNearbyArticleTableViewCell *nearbyCell,
        MWKLocationSearchResult *result, UITableView *tableView,
      ...
      [nearbyCell setDistanceProvider:[self.compassViewModel
                                          distanceProviderForResult:result]];
      [nearbyCell setBearingProvider:[self.compassViewModel
                                         bearingProviderForResult:result]];
    };
  }
  return self;
}
...
- (void)viewDidLoad {
  ...
  [self.tableView registerNib:[WMFNearbyArticleTableViewCell wmf_classNib]
       forCellReuseIdentifier:[WMFNearbyArticleTableViewCell identifier]];
  self.tableView.estimatedRowHeight =
      [WMFNearbyArticleTableViewCell estimatedRowHeight];
}
