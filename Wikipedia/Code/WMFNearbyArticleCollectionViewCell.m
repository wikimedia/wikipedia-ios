...
- (void)awakeFromNib {
  ...
  self.articleImageView.layer.cornerRadius =
      self.articleImageView.bounds.size.width / 2;
  self.articleImageView.layer.borderWidth = 1.0 / [UIScreen mainScreen].scale;
  self.articleImageView.layer.borderColor =
      [UIColor colorWithWhite:0.9 alpha:1.0].CGColor;
  self.distanceLabelBackground.layer.cornerRadius = 2.0;
  self.distanceLabelBackground.layer.borderWidth =
      1.0 / [UIScreen mainScreen].scale;
  self.distanceLabelBackground.layer.borderColor =
      [UIColor wmf_customGray].CGColor;
...
- (void)updateTitleLabel {
  NSMutableAttributedString *attributedTitleAndDescription =
  ...
  NSAttributedString *searchResultDescription =
      [self attributedDescriptionText];
  if ([searchResultDescription length] > 0) {
    [attributedTitleAndDescription
        appendAttributedString:[[NSMutableAttributedString alloc]
                                   initWithString:@"\n"]];
    [attributedTitleAndDescription
        appendAttributedString:searchResultDescription];
  }
...
- (NSAttributedString *)attributedDescriptionText {
  ...
  return [[NSAttributedString alloc]
      initWithString:self.descriptionText
          attributes:@{
...
- (void)setDistance:(CLLocationDistance)distance {
  if (FBTweakValue(@"Explore", @"Nearby", @"Show raw distance", NO)) {
    ...
    self.distanceLabel.text =
        [NSString wmf_localizedStringForDistance:distance];
  }
}
...
- (void)setImageURL:(NSURL *)imageURL
  ...
  [self.articleImageView wmf_setImageWithURL:imageURL
                                 detectFaces:YES
...
- (void)setImage:(MWKImage *)image
         failure:(WMFErrorHandler)failure
         success:(WMFSuccessHandler)success {
  [self.articleImageView wmf_setImageWithMetadata:image
                                      detectFaces:YES
                                          failure:WMFIgnoreErrorHandler
                                          success:WMFIgnoreSuccessHandler];
}
...
- (NSString *)accessibilityLabel {
  ...
  if (self.descriptionText) {
    titleAndDescription = [NSString
        stringWithFormat:@"%@, %@", self.titleText, self.descriptionText];
  } else {
    titleAndDescription = self.titleText;
  }
  return [NSString stringWithFormat:@"%@, %@ %@", titleAndDescription,
                                    self.distanceLabel.text,
                                    self.compassView.accessibilityLabel];
}
