...
- (NSCachedURLResponse *)cachedResponseForRequest:(NSURLRequest *)request {
  ...
  if ([self isMIMETypeImage:mimeType] &&
      [[WMFImageController sharedInstance]
          hasDataOnDiskForImageWithURL:request.URL]) {
    WMFTypedImageData *typedData = [[WMFImageController sharedInstance]
        typedDiskDataForImageWithURL:request.URL];
...
- (void)storeCachedResponse:(NSCachedURLResponse *)cachedResponse
  ...
  if ([self isJsonResponse:cachedResponse fromMDotRequest:request]) {
    ...
    // TODO: should refactor a lot of this into ZeroConfigState itself and make
    // it thread safe so we can do its work off the main thread.
    [self processZeroHeaders:cachedResponse.response];
  }
}

- (BOOL)isJsonResponse:(NSCachedURLResponse *)cachedResponse
       fromMDotRequest:(NSURLRequest *)request {
  return ([[request URL].host hasSuffix:WMFURLCacheWikipediaHost] &&
          [cachedResponse.response.MIMEType
              isEqualToString:WMFURLCacheJsonMIMEType]);
}
...
- (void)processZeroHeaders:(NSURLResponse *)response {
  ...
  BOOL zeroProviderChanged =
      zeroRatedHeaderPresent && ![xZeroRatedHeader isEqualToString:xcs];
  BOOL zeroDisposition =
      [SessionSingleton sharedInstance].zeroConfigState.disposition;

  // enable this tweak to make the cache pretend it found W0 headers in the
  // response
