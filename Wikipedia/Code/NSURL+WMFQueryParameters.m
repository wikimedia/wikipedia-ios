...
- (nullable NSString *)wmf_valueForQueryKey:(NSString *)key {
  NSURLQueryItem *matchingItem =
      [[[NSURLComponents componentsWithURL:self resolvingAgainstBaseURL:YES]
          queryItems] bk_match:^BOOL(NSURLQueryItem *item) {
...
- (NSURL *)wmf_urlWithValue:(NSString *)value forQueryKey:(NSString *)key {
  ...
  if ([self wmf_valueForQueryKey:key]) {
    ...
    NSArray<NSURLQueryItem *> *queryItems =
        [components.queryItems bk_map:^id(NSURLQueryItem *item) {
    ...
    if (newItem) {
      ...
      if (components.queryItems) {
        components.queryItems =
            [components.queryItems arrayByAddingObjectsFromArray:newItemArray];
      } else {
...
- (NSURL *)wmf_urlWithoutQueryKey:(NSString *)key {
  NSURLComponents *components =
      [NSURLComponents componentsWithURL:self resolvingAgainstBaseURL:YES];
  NSArray<NSURLQueryItem *> *queryItems =
      [components.queryItems bk_select:^BOOL(NSURLQueryItem *item) {
