  ...
  [manager POST:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (![responseObject isDict]) {
          responseObject =
              @{ @"error" : @{@"info" : @"Account creation data not found."} };
        }
        ...
        if ([responseObject[@"createaccount"][@"status"]
                isEqualToString:@"FAIL"] &&
            ![responseObject[@"createaccount"][@"message"]
                isEqualToString:@"Incorrect or missing CAPTCHA."]) {
          NSMutableDictionary *errorDict =
              [responseObject[@"createaccount"] mutableCopy];
        ...
        if (!error) {
          if ([responseObject[@"createaccount"][@"status"]
            ...
            errorDict[NSLocalizedDescriptionKey] =
                MWLocalizedString(@"account-creation-captcha-required", nil);

            error =
                [NSError errorWithDomain:@"Account Creation Fetcher"
                                    code:ACCOUNT_CREATION_ERROR_NEEDS_CAPTCHA
                                userInfo:errorDict];
          }
        }

        [self finishWithError:error fetchedData:result];
      }
