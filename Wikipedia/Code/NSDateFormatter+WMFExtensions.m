...
+ (NSDateFormatter *)wmf_iso8601Formatter {
  ...
  dispatch_once(&onceToken, ^{
    // need to use "en" locale, otherwise the timestamp will fail to parse when
    // the current locale is arabic on iOS 6
...
+ (NSDateFormatter *)wmf_shortTimeFormatter {
  ...
  if (!shortTimeFormatter) {
    shortTimeFormatter =
        [self wmf_shortTimeFormatterWithLocale:[NSLocale currentLocale]];
  }
...
+ (instancetype)wmf_englishHyphenatedYearMonthDayFormatter {
  ...
  dispatch_once(&onceToken, ^{
    _dateFormatter =
        [self wmf_newEnglishYearMonthDayFormatterWithSeparator:@"-"];
  });
  return _dateFormatter;
}
...
+ (instancetype)wmf_newEnglishYearMonthDayFormatterWithSeparator:
    (NSString *)separator {
  NSString *quotedSeparator =
      [@[ @"'", separator, @"'" ] componentsJoinedByString:@""];
  NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
  dateFormatter.dateFormat =
      [@[ @"yyyy", @"MM", @"dd" ] componentsJoinedByString:quotedSeparator];
...
+ (instancetype)wmf_utcDayNameMonthNameDayOfMonthNumberDateFormatter {
  ...
  dispatch_once(&onceToken, ^{
    _dateFormatter =
        [[self wmf_dayNameMonthNameDayOfMonthNumberDateFormatter] copy];
    _dateFormatter.timeZone = [NSTimeZone timeZoneWithAbbreviation:@"UTC"];
  });
  return _dateFormatter;
}
...
+ (instancetype)wmf_utcShortDayNameShortMonthNameDayOfMonthNumberDateFormatter {
  ...
  dispatch_once(&onceToken, ^{
    _dateFormatter =
        [[self wmf_shortDayNameShortMonthNameDayOfMonthNumberDateFormatter]
            copy];
    _dateFormatter.timeZone = [NSTimeZone timeZoneWithAbbreviation:@"UTC"];
  });
  return _dateFormatter;
