 ...
 * @warning This might be a guess based on the dimensions specified in the <img>
 * tag the receiver was
 *          parsed from.
 */
...
/**
 * Width of the original image file. For example, if this image is a thumbnail,
 * this is the width of the original image.
 */
@property(copy, nonatomic) NSNumber *originalFileWidth;

/**
 * Height of the original image file. For example, if this image is a thumbnail,
 * this is the height of the original image.
 */
...
/**
 * Initializes the receiver with the given article, and sets its properties with
 * data from `dict`.
 * @param article  The article to associate this image with.
 * @param dict     A dictionary which contains data from a previous call to
 * `-[MWKImage dataExport]`.
 */
...
/**
 * Array of NSValue-wrapped unit rectangles, in the coordinate space of the
 * receiver's image (y-origin on the bottom).
 *
...
@property(copy, nonatomic /*, nullable*/)
    NSArray<NSValue *> *allNormalizedFaceBounds;

/**
 * Convenience accessor for the bounds of the first face in
 * `allNormalizedFaceBounds`.
 * @return The normalized bounds of the first face, or `CGRectZero` if
 * allNormalizedFaceBounds is empty or `nil`.
 */
