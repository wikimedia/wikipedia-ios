...
- (NSAttributedString *)wmf_trim {
  ...
  NSUInteger lastNonWhitespaceLocation =
      [self locationOfLastNonWhitespaceCharacter];
  if (lastNonWhitespaceLocation == NSNotFound) {
    return [[NSAttributedString alloc] init];
  }

  NSUInteger firstNonWhitespaceLocation =
      [self locationOfFirstNonWhitespaceCharacter];
  if (firstNonWhitespaceLocation == NSNotFound) {
    return [[NSAttributedString alloc] init];
  }

  return [self
      attributedSubstringFromRange:NSMakeRange(firstNonWhitespaceLocation,
                                               lastNonWhitespaceLocation -
...
- (NSUInteger)locationOfLastNonWhitespaceCharacter {
  return
      [self.string
          rangeOfCharacterFromSet:[NSCharacterSet wmf_invertedWhitespaceCharSet]
