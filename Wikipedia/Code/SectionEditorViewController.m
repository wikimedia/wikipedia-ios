...
- (void)viewDidLoad {
    ...
    buttonX.accessibilityLabel = NSLocalizedStringWithDefaultValue(@"back-button-accessibility-label", nil, NSBundle.wmf_localizationBundle, @"Back", "Accessibility label for a button to navigate back.\n{{Identical|Back}}");
    self.navigationItem.leftBarButtonItem = buttonX;

    self.rightButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringWithDefaultValue(@"button-next", nil, NSBundle.wmf_localizationBundle, @"Next", "Button text for next button used in various places.\n{{Identical|Next}}")
...
- (void)rightButtonPressed {
    if (![self changesMade]) {
        [[WMFAlertManager sharedInstance] showAlert:NSLocalizedStringWithDefaultValue(@"wikitext-preview-changes-none", nil, NSBundle.wmf_localizationBundle, @"No changes were made to be previewed.", "Alert text shown if no changes were made to be previewed.") sticky:NO dismissPreviousAlerts:YES tapCallBack:NULL];
    } else {
        [self preview];
    }
}
            ...
            case FETCH_FINAL_STATUS_SUCCEEDED: {
                ...
                if (protectionStatus && [[protectionStatus allowedGroupsForAction:@"edit"] count] > 0) {
                    ...
                    if ([groups indexOfObject:@"autoconfirmed"] != NSNotFound) {
                        msg = NSLocalizedStringWithDefaultValue(@"page-protected-autoconfirmed", nil, NSBundle.wmf_localizationBundle, @"This page has been semi-protected.", "Brief description of Wikipedia 'autoconfirmed' protection level, shown when editing a page that is protected.");
                    ...
                    //[self showAlert:NSLocalizedStringWithDefaultValue(@"wikitext-download-success", nil, NSBundle.wmf_localizationBundle, @"Content loaded.", "Alert text shown when latest revision of the section being edited has been retrieved") type:ALERT_TYPE_TOP duration:1];
...
- (void)loadLatestWikiTextForSectionFromServer {
    [[WMFAlertManager sharedInstance] showAlert:NSLocalizedStringWithDefaultValue(@"wikitext-downloading", nil, NSBundle.wmf_localizationBundle, @"Loading content...", "Alert text shown when obtaining latest revision of the section being edited") sticky:YES dismissPreviousAlerts:YES tapCallBack:NULL];
