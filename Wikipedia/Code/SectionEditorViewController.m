...
#define EDIT_TEXT_VIEW_FONT                                                    \
  [UIFont systemFontOfSize:16.0f * MENUS_SCALE_MULTIPLIER]
...
- (void)viewDidLoad {
  ...
  @weakify(self) UIBarButtonItem *buttonX = [UIBarButtonItem
      wmf_buttonType:WMFButtonTypeX
             handler:^(id sender) {
               @strongify(
                   self)[self.delegate sectionEditorFinishedEditing:self];
             }];
  buttonX.accessibilityLabel =
      MWLocalizedString(@"back-button-accessibility-label", nil);
...
- (void)fetchFinished:(id)sender
  ...
  if ([sender isKindOfClass:[WikiTextSectionFetcher class]]) {
    ...
    case FETCH_FINAL_STATUS_SUCCEEDED: {
      WikiTextSectionFetcher *wikiTextSectionFetcher =
          (WikiTextSectionFetcher *)sender;
      ...
      self.funnel =
          [[EditFunnel alloc] initWithUserId:[userInfo[@"id"] intValue]];
      [self.funnel logStart];

      MWKProtectionStatus *protectionStatus =
          wikiTextSectionFetcher.section.article.protection;

      if (protectionStatus &&
          [[protectionStatus allowedGroupsForAction:@"edit"] count] > 0) {
        ...
        [[WMFAlertManager sharedInstance] showAlert:msg
                                             sticky:NO
                              dismissPreviousAlerts:YES
                                        tapCallBack:NULL];
      } else {
      ...
      //[self.editTextView performSelector:@selector(becomeFirstResponder)
...
- (void)loadLatestWikiTextForSectionFromServer {
  [[WMFAlertManager sharedInstance]
                  showAlert:MWLocalizedString(@"wikitext-downloading", nil)
                     sticky:YES
      dismissPreviousAlerts:YES
                tapCallBack:NULL];

  [[QueuesSingleton sharedInstance].sectionWikiTextDownloadManager
      wmf_cancelAllTasksWithCompletionHandler:^{
        (void)[[WikiTextSectionFetcher alloc]
            initAndFetchWikiTextForSection:self.section
                               withManager:[QueuesSingleton sharedInstance]
                                               .sectionWikiTextDownloadManager
...
- (void)preview {
  PreviewAndSaveViewController *previewVC = [PreviewAndSaveViewController
...
- (void)previewViewControllerDidSave:
    (PreviewAndSaveViewController *)previewViewController {
...
- (void)keyboardWasShown:(NSNotification *)aNotification {
  ...
  CGRect windowKeyboardRect =
      [[info objectForKey:UIKeyboardFrameEndUserInfoKey] CGRectValue];

  CGRect viewKeyboardRect =
      [self.view.window convertRect:windowKeyboardRect toView:self.view];

  self.viewKeyboardRect = viewKeyboardRect;

  // This makes it so you can always scroll to the bottom of the text view's
  // text
  // even if the keyboard is onscreen.
  UIEdgeInsets contentInsets =
      UIEdgeInsetsMake(0.0, 0.0, viewKeyboardRect.size.height, 0.0);
  self.editTextView.contentInset = contentInsets;
  self.editTextView.scrollIndicatorInsets = contentInsets;

  // Mark the text view as needing a layout update so the inset changes above
  // will
...
- (void)scrollTextViewSoCursorNotUnderKeyboard:(UITextView *)textView {
  // If cursor is hidden by keyboard, scroll the text view so cursor is
  // onscreen.
  if (!CGRectIsNull(self.viewKeyboardRect)) {
    CGRect cursorRectInTextView =
        [textView caretRectForPosition:textView.selectedTextRange.start];
    CGRect cursorRectInView =
        [textView convertRect:cursorRectInTextView toView:self.view];
    if (CGRectIntersectsRect(self.viewKeyboardRect, cursorRectInView)) {
      CGFloat margin = -20;
      // Margin here is the amount the cursor will be scrolled above the top of
      // the keyboard.
