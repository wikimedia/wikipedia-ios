...
- (NSAttributedString *)headerTitle {
  return [[NSAttributedString alloc]
      initWithString:MWLocalizedString(@"explore-nearby-heading", nil)
          attributes:@{
  ...
  if (self.placemark) {
    return [[NSAttributedString alloc]
        initWithString:[NSString stringWithFormat:@"%@, %@",
            ...
            attributes:@{
              NSForegroundColorAttributeName :
                  [UIColor wmf_exploreSectionHeaderSubTitleColor]
            }];
  } else if (self.searchResults.results.count > 0) {
    return [[NSAttributedString alloc]
        initWithString:[[self.searchResults.results firstObject] displayTitle]
            attributes:@{
              NSForegroundColorAttributeName :
                  [UIColor wmf_exploreSectionHeaderSubTitleColor]
            }];
...
- (void)configureCell:(WMFNearbyArticleCollectionViewCell *)cell
             withItem:(MWKLocationSearchResult *)item
          atIndexPath:(nonnull NSIndexPath *)indexPath {
  NSParameterAssert([item isKindOfClass:[MWKLocationSearchResult class]]);
  NSParameterAssert(
      [cell isKindOfClass:[WMFNearbyArticleCollectionViewCell class]]);
  cell.titleText = item.displayTitle;
  cell.descriptionText = item.wikidataDescription;
  [cell setImageURL:item.thumbnailURL];
  [cell setDistanceProvider:[self.compassViewModel
                                distanceProviderForResult:item]];
  [cell
      setBearingProvider:[self.compassViewModel bearingProviderForResult:item]];
...
- (void)configureEmptyCell:(WMFEmptySectionCollectionViewCell *)cell {
  ...
  [cell.reloadButton setTitle:MWLocalizedString(@"home-nearby-check-again", nil)
...
- (NSString *)footerText {
  return [MWLocalizedString(@"home-nearby-location-footer", nil)
      stringByReplacingOccurrencesOfString:@"$1"
                                withString:self.placemark.name];
}

- (UIViewController *)moreViewController {
  WMFLocationSearchListViewController *vc =
      [[WMFLocationSearchListViewController alloc]
          initWithLocation:self.location
             searchSiteURL:self.searchSiteURL
                 dataStore:self.dataStore];
  return vc;
}
...
- (AnyPromise *)fetchTitlesForLocation:(CLLocation *__nullable)location {
  @weakify(self);
  return [self.locationSearchFetcher
