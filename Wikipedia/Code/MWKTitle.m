...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    if (!self.URL) {
      NSString *text =
          [self decodeValueForKey:@"text" withCoder:coder modelVersion:0];
      MWKSite *site =
          [self decodeValueForKey:@"site" withCoder:coder modelVersion:0];
      NSString *fragment =
          [self decodeValueForKey:@"fragment" withCoder:coder modelVersion:0];
      if (site && text) {
        self.URL =
            [NSURL wmf_URLWithSiteURL:site.URL title:text fragment:fragment];
      } else {
        return nil;
      }
    }
...
- (instancetype)initWithInternalLink:(NSString *)relativeInternalLink
                                site:(MWKSite *)site {
  NSURL *URL = [NSURL wmf_URLWithSiteURL:site.URL
         escapedDenormalizedInternalLink:relativeInternalLink];
  return [self initWithURL:URL];
}

- (instancetype)initWithString:(NSString *)string site:(MWKSite *)site {
  NSURL *URL = [NSURL wmf_URLWithSiteURL:site.URL
      escapedDenormalizedTitleAndFragment:string];
  return [self initWithURL:URL];
}

- (instancetype)initWithUnescapedString:(NSString *)string
                                   site:(MWKSite *)site {
  NSURL *URL = [NSURL wmf_URLWithSiteURL:site.URL
      unescapedDenormalizedTitleAndFragment:string];
  return [self initWithURL:URL];
}
...
// Need to specify storage properties since text & site are readonly, which
// Mantle interprets as transitory.
+ (MTLPropertyStorage)storageBehaviorForPropertyWithKey:
    (NSString *)propertyKey {
#define IS_MWKTITLE_KEY(key)                                                   \
  [propertyKey isEqualToString:WMF_SAFE_KEYPATH([MWKTitle new], key)]
