...
static NSString *const WMFWikipediaZeroHeaderTweakIdentifier =
    @"org.wikimedia.wikipedia.tweaks.networking.zero.fakezeroheaders";

@implementation FBTweak (WikipediaZero)

+ (void)load {
  FBTweakCategory *networkingCategory =
      [[FBTweakStore sharedInstance] tweakCategoryWithName:@"Networking"];
  NSParameterAssert(networkingCategory);

  FBTweakCollection *zeroTweakCollection =
      [[FBTweakCollection alloc] initWithName:@"Wikipedia Zero"];
  [networkingCategory addTweakCollection:zeroTweakCollection];

  FBTweak *zeroHeaders = [[FBTweak alloc]
      initWithIdentifier:WMFWikipediaZeroHeaderTweakIdentifier];
