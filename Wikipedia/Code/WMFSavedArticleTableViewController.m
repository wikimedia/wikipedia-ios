...
- (void)viewDidLoad {
  ...
  self.tableView.estimatedRowHeight =
      [WMFArticleListTableViewCell estimatedRowHeight];

  WMFSavedPagesDataSource *ds = [[WMFSavedPagesDataSource alloc]
      initWithSavedPagesList:[self savedPageList]];

  ds.cellClass = [WMFArticleListTableViewCell class];

  @weakify(self);
  ds.cellConfigureBlock =
      ^(WMFArticleListTableViewCell *cell, MWKSavedPageEntry *entry,
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  [NSUserActivity
      wmf_makeActivityActive:[NSUserActivity wmf_savedPagesViewActivity]];
}
