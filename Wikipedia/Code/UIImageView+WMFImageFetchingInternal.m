...
static const char *const WMFImageControllerAssociationKey =
    "WMFImageController";
...
- (WMFImageController *__nullable)wmf_imageController {
  WMFImageController *controller =
      [self bk_associatedValueForKey:WMFImageControllerAssociationKey];
  if (!controller) {
...
- (void)wmf_setImageController:(nullable WMFImageController *)imageController {
  [self bk_associateValue:imageController
                  withKey:WMFImageControllerAssociationKey];
}
...
- (UIImage *)wmf_cachedImage {
  UIImage *cachedImage = [self.wmf_imageController
      cachedImageInMemoryWithURL:[self wmf_imageURLToFetch]];
  return cachedImage;
}
...
- (BOOL)wmf_imageRequiresFaceDetection {
  if (self.wmf_imageURL) {
    return [[[self class] faceDetectionCache]
        imageAtURLRequiresFaceDetection:self.wmf_imageURL];
  } else {
    return [[[self class] faceDetectionCache]
        imageRequiresFaceDetection:self.wmf_imageMetadata];
  }
}
...
- (NSValue *)wmf_faceBoundsInImage:(UIImage *)image {
  if (self.wmf_imageURL) {
    return
        [[[self class] faceDetectionCache] faceBoundsForURL:self.wmf_imageURL];
  } else {
    return [[[self class] faceDetectionCache]
        faceBoundsForImageMetadata:self.wmf_imageMetadata];
  }
}

- (void)wmf_getFaceBoundsInImage:(UIImage *)image
                         failure:(WMFErrorHandler)failure
                         success:(WMFSuccessNSValueHandler)success {
  if (self.wmf_imageURL) {
    [[[self class] faceDetectionCache] detectFaceBoundsInImage:image
                                                           URL:self.wmf_imageURL
                                                       failure:failure
                                                       success:success];
  } else {
    [[[self class] faceDetectionCache]
        detectFaceBoundsInImage:image
                  imageMetadata:self.wmf_imageMetadata
                        failure:failure
                        success:success];
  }
}
...
- (void)wmf_fetchImageDetectFaces:(BOOL)detectFaces
  ...
  NSAssert(
      ...
      @"Interaction with a UIImageView should only happen on the main thread");
  ...
  [self.wmf_imageController
      fetchImageWithURL:imageURL
                failure:failure
                success:^(WMFImageDownload *_Nonnull download) {
                  dispatch_async(dispatch_get_main_queue(), ^{
                    @strongify(self);
                    if (!WMF_EQUAL([self wmf_imageURLToFetch], isEqual:,
                                   imageURL)) {
                      failure([NSError cancelledError]);
                    } else {
                      [self wmf_setImage:download.image
                             detectFaces:detectFaces
                                animated:YES
                                 failure:failure
                                 success:success];
...
- (void)wmf_setImage:(UIImage *)image
  ...
  if (![self wmf_imageRequiresFaceDetection]) {
    ...
    [self wmf_setImage:image
        faceBoundsValue:faceBoundsValue
               animated:animated
                failure:failure
                success:success];
    return;
  ...
  [self wmf_getFaceBoundsInImage:image
                         ...
                         success:^(NSValue *bounds) {
                           dispatch_async(dispatch_get_main_queue(), ^{
                             if (!WMF_EQUAL([self wmf_imageURLToFetch],
                                            isEqual:, imageURL)) {
                               failure([NSError cancelledError]);
                             } else {
                               [self wmf_setImage:image
                                   faceBoundsValue:bounds
                                          animated:animated
                                           failure:failure
                                           success:success];
                             }
...
- (void)wmf_setImage:(UIImage *)image
  ...
  if (!CGRectIsEmpty(faceBounds)) {
    [self wmf_cropContentsByVerticallyCenteringFrame:
              [image wmf_denormalizeRect:faceBounds]
