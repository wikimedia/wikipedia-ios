...
@property(nonatomic, strong)
    NSMutableDictionary<NSURL *, AnyPromise *> *fetchOperationsByArticleTitle;
@property(nonatomic, strong)
    NSMutableDictionary<NSURL *, NSError *> *errorsByArticleTitle;
...
- (instancetype)initWithSavedPageList:(MWKSavedPageList *)savedPageList
  ...
  if (self) {
    _accessQueue = dispatch_queue_create(
        "org.wikipedia.savedarticlesarticleFetcher.accessQueue",
...
- (instancetype)initWithSavedPageList:(MWKSavedPageList *)savedPageList {
  return
      [self initWithSavedPageList:savedPageList
                   articleFetcher:[[WMFArticleFetcher alloc]
                                      initWithDataStore:savedPageList.dataStore]
...
- (void)fetchUncachedEntries:(NSArray<MWKSavedPageEntry *> *)insertedEntries {
  ...
  [self fetchUncachedArticleURLs:[insertedEntries
                                     valueForKey:WMF_SAFE_KEYPATH(
                                                     [MWKSavedPageEntry new],
                                                     url)]];
}
...
- (void)cancelFetchForEntries:(NSArray<MWKSavedPageEntry *> *)deletedEntries {
  ...
  dispatch_async(self.accessQueue, ^{
    ...
    if (wasFetching) {
      /*
         only notify delegate if deletion occurs during a download session. if
         deletion occurs
...
- (void)fetchUncachedArticleURLs:(NSArray<NSURL *> *)urls {
  dispatch_block_t didFinishLegacyMigration = ^{
    [[NSUserDefaults standardUserDefaults]
        wmf_setDidFinishLegacySavedArticleImageMigration:YES];
  };
...
- (void)fetchArticleURL:(NSURL *)articleURL
                failure:(WMFErrorHandler)failure
                success:(WMFSuccessHandler)success {
  // NOTE: must check isCached to determine that all article data has been
  // downloaded
  MWKArticle *articleFromDisk =
      [self.savedPageList.dataStore articleWithURL:articleURL];
  @weakify(self);
  if (articleFromDisk.isCached) {
    ...
    /*
       don't use "finallyOn" to remove the promise from our tracking dictionary
       since it has to be removed
       immediately in order to ensure accurate progress & error reporting.
     */
    self.fetchOperationsByArticleTitle[articleURL] =
        [self.articleFetcher fetchArticleForURL:articleURL progress:NULL]
            .thenOn(self.accessQueue,
                    ^(MWKArticle *article) {
                      @strongify(self);
                      [self downloadImageDataForArticle:article
                          failure:^(NSError *error) {
                            dispatch_async(self.accessQueue, ^{
                              [self didFetchArticle:article
                                                url:articleURL
                                              error:error];
                              failure(error);
                            });
...
- (void)downloadImageDataForArticle:(MWKArticle *)article
  ...
  if (![[NSUserDefaults standardUserDefaults]
          wmf_didFinishLegacySavedArticleImageMigration]) {
    WMF_TECH_DEBT_TODO(
        This legacy migration can be removed after enough users upgrade
            to 5.0.5)[self migrateLegacyImagesInArticle:article];
  }
  [self fetchAllImagesInArticle:article
      ...
      success:^{
        // NOTE: turning off gallery image fetching as users are potentially
        // downloading large amounts of data up front when upgrading to a new
        // version of the app.
        //        [self fetchGalleryDataForArticle:article failure:failure
        //        success:success];
        if (success) {
          success();
        }
...
- (void)migrateLegacyImagesInArticle:(MWKArticle *)article {
  //  Copies saved article images cached by versions 5.0.4 and older to the
  //  locations where 5.0.5 and newer are looking for them. Previously, the app
  //  cached at the width of the largest image in the srcset. Currently, we
  //  request a thumbnail at wmf_articleImageWidthForScale (or original if it's
  //  narrower than that width). By copying from the old size to the new
  //  expected sizes, we ensure that articles saved with these older versions
  //  will still have images availble offline in the newer versions.
  WMFImageController *imageController = [WMFImageController sharedInstance];
  NSArray *legacyImageURLs =
      [self.savedPageList.dataStore legacyImageURLsForArticle:article];
  NSUInteger articleImageWidth =
      [[UIScreen mainScreen] wmf_articleImageWidthForScale];
    ...
    @autoreleasepool {
      ...
      if (width != articleImageWidth && width != NSNotFound) {
        if (legacyImageURL != nil &&
          ...
          NSURL *cachedFileURL = [NSURL
              fileURLWithPath:[imageController
                                  cachePathForImageWithURL:legacyImageURL]
                  isDirectory:NO];
          if (cachedFileURL != nil) {
            ...
            NSString *imageURLStringAtArticleWidth =
                WMFChangeImageSourceURLSizePrefix(legacyImageURLString,
                                                  articleImageWidth);
            NSURL *imageURLAtArticleWidth =
                [NSURL URLWithString:imageURLStringAtArticleWidth];
            if (imageURLAtArticleWidth != nil &&
                ![imageController
                    hasDataOnDiskForImageWithURL:imageURLAtArticleWidth]) {
              [imageController cacheImageFromFileURL:cachedFileURL
                                              forURL:imageURLAtArticleWidth
                                            MIMEType:imageMIMEType];
            }

            NSString *originalImageURLString =
                WMFOriginalImageURLStringFromURLString(legacyImageURLString);
            NSURL *originalImageURL =
                [NSURL URLWithString:originalImageURLString];
            if (![imageController
                    hasDataOnDiskForImageWithURL:originalImageURL]) {
              [imageController cacheImageFromFileURL:cachedFileURL
                                              forURL:originalImageURL
                                            MIMEType:imageMIMEType];
            }
          }
        }
...
- (void)fetchGalleryDataForArticle:(MWKArticle *)article
                           failure:(WMFErrorHandler)failure
                           success:(WMFSuccessHandler)success {
  WMF_TECH_DEBT_TODO(check whether on -
                     disk image info matches what we are about to fetch)
  @weakify(self);
  ...
  [self fetchImageInfoForImagesInArticle:article
      ...
      success:^(NSArray *info) {
        ...
        [self cacheImagesWithURLsInBackground:URLs
                                      failure:failure
                                      success:success];
      }];
}

- (void)fetchImageInfoForImagesInArticle:(MWKArticle *)article
                                 failure:(WMFErrorHandler)failure
                                 success:(WMFSuccessNSArrayHandler)success {
  @weakify(self);
  NSArray<NSString *> *imageFileTitles =
      [MWKImage mapFilenamesFromImages:[article imagesForGallery]];

  if (imageFileTitles.count == 0) {
    DDLogVerbose(@"No image info to fetch, returning successful promise with "
                 @"empty array.");
  ...
  PMKJoin([[imageFileTitles bk_map:^AnyPromise *(NSString *canonicalFilename) {
    return [self.imageInfoFetcher fetchGalleryInfoForImage:canonicalFilename
                                               fromSiteURL:article.url];
    ...
    [self.savedPageList.dataStore saveImageInfo:infoObjects
                                  forArticleURL:article.url];
    success(infoObjects);
    return infoObjects;
  });
}

- (void)cacheImagesWithURLsInBackground:(NSArray<NSURL *> *)imageURLs
                                failure:(void (^_Nonnull)(
  ...
  [self.imageController cacheImagesWithURLsInBackground:imageURLs
                                                failure:failure
                                                success:success];
}
...
- (void)cancelFetchForArticleURL:(NSURL *)URL {
  ...
  [[[self.savedPageList.dataStore existingArticleWithURL:URL] allImageURLs]
      bk_each:^(NSURL *imageURL) {
...
- (void)savedPageListDidChange:(NSDictionary *)change {
  ...
  default:
    NSAssert(NO, @"Unsupported KVO operation %@ on saved page list %@", change,
             self.savedPageList);
    break;
  }
}
...
- (CGFloat)progress {
  /*
     FIXME: Handle progress when only downloading a subset of saved pages (e.g.
     if some were already downloaded in
     a previous session)?
   */
  ...
  return (CGFloat)([self.savedPageList countOfEntries] -
                   [self.fetchOperationsByArticleTitle count]) /
...
- (void)didFetchArticle:(MWKArticle *__nullable)fetchedArticle
  ...
  if (error) {
    ...
    DDLogError(@"Failed to download saved page %@ due to error: %@", url,
               error);
...
- (void)notifyDelegateIfFinished {
  if ([self.fetchOperationsByArticleTitle count] == 0) {
    ...
    [self finishWithError:reportedError fetchedData:nil];
  }
}
...
+ (instancetype)wmf_savedPageImageDownloadError {
  return [NSError errorWithDomain:WMFSavedPageErrorDomain
                             code:1
                         userInfo:@{
                           NSLocalizedDescriptionKey : MWLocalizedString(
                               @"saved-pages-image-download-error", nil)
                         }];
}
