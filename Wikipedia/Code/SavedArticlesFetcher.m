...
+ (instancetype)wmf_savedPageImageDownloadError {
    return [NSError errorWithDomain:WMFSavedPageErrorDomain
                               code:1
                           userInfo:@{
                               NSLocalizedDescriptionKey: NSLocalizedStringWithDefaultValue(@"saved-pages-image-download-error", nil, NSBundle.wmf_localizationBundle, @"Failed to download images for this saved page.", "Error message shown when one or more images fails to save for offline use.")
                           }];
}
