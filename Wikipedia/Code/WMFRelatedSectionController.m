...
static NSString *const WMFRelatedSectionIdentifierPrefix =
    @"WMFRelatedSectionIdentifier";
...
- (id)sectionIdentifier {
  return [WMFRelatedSectionIdentifierPrefix
      stringByAppendingString:self.url.wmf_title];
}
...
- (NSAttributedString *)headerTitle {
  return [[NSAttributedString alloc]
      initWithString:MWLocalizedString(@"explore-continue-related-heading", nil)
          attributes:@{
...
- (NSAttributedString *)headerSubTitle {
  return [[NSAttributedString alloc]
      initWithString:self.url.wmf_title
          attributes:@{
...
- (void)configureCell:(WMFArticlePreviewCollectionViewCell *)cell
  ...
  [cell setSaveableURL:[self urlForItemAtIndexPath:indexPath]
...
- (BOOL)prefersWiderColumn {
  return FBTweakValue(@"Explore", @"General",
                      @"Put 'Because You Read' in Wider Column", YES);
}
  ...
  return [self.relatedTitleDataSource fetch]
      .then(^(WMFRelatedSearchResults *results) {
        ...
        return [self.searchResults.results
            wmf_safeSubarrayWithRange:NSMakeRange(0,
                                                  WMFRelatedSectionMaxResults)];
      })
      .catch(^(NSError *error) {
        @strongify(self);
...
- (UIAlertController *)menuActionSheet {
  ...
  [sheet addAction:[UIAlertAction
                       actionWithTitle:MWLocalizedString(
                                           @"home-hide-suggestion-prompt", nil)
                                 style:UIAlertActionStyleDestructive
                               handler:^(UIAlertAction *_Nonnull action) {
                                 [self.blackList
                                     addBlackListArticleURL:self.url];
                                 [self.blackList save];
                               }]];
  [sheet addAction:[UIAlertAction
                       actionWithTitle:MWLocalizedString(
                                           @"home-hide-suggestion-cancel", nil)
...
- (NSString *)footerText {
  return [MWLocalizedString(@"home-more-like-footer", nil)
...
- (WMFRelatedTitleListDataSource *)relatedTitleDataSource {
  if (!_relatedTitleDataSource) {
    /*
       HAX: Need to use the "more" data source to fetch data and keep it around
       since morelike: searches for the same
       title don't have the same results in order. might need to look into
       continuation soon
     */
...
- (UIViewController *)moreViewController {
  if (!self.relatedSearchFetcher.isFetching &&
      !self.relatedTitleDataSource.relatedSearchResults) {
    [self.relatedTitleDataSource fetch];
  }
  WMFRelatedTitleViewController *vc =
      [[WMFRelatedTitleViewController alloc] init];
