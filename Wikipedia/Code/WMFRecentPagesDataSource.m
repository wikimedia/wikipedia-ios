...
- (nonnull instancetype)initWithRecentPagesList:(MWKHistoryList *)recentPages {
  ...
  self = [super initWithSections:[WMFRecentPagesDataSource
                                     sectionsFromHistoryList:recentPages]];
  if (self) {
    self.recentPages = recentPages;

    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(rebuildSections)
               name:MWKHistoryListDidUpdateNotification
             object:recentPages];

    self.tableDeletionBlock =
        ^(WMFRecentPagesDataSource *dataSource, UITableView *parentView,
    ...
    [self.KVOController
        observe:self.recentPages
        keyPath:WMF_SAFE_KEYPATH(self.recentPages, entries)
        options:NSKeyValueObservingOptionInitial |
                NSKeyValueObservingOptionPrior
          block:^(WMFRecentPagesDataSource *observer, MWKHistoryList *object,
                  NSDictionary *change) {
            BOOL isPrior =
                [change[NSKeyValueChangeNotificationIsPriorKey] boolValue];
            NSKeyValueChange changeKind =
                [change[NSKeyValueChangeKindKey] unsignedIntegerValue];
            NSIndexSet *indexes = change[NSKeyValueChangeIndexesKey];

            if (isPrior) {
              if (changeKind == NSKeyValueChangeSetting) {
                [observer
                    willChangeValueForKey:WMF_SAFE_KEYPATH(observer, urls)];
              } else {
                [observer willChange:changeKind
                     valuesAtIndexes:indexes
                              forKey:WMF_SAFE_KEYPATH(observer, urls)];
              }
            } else {
              if (changeKind == NSKeyValueChangeSetting) {
                [observer
                    didChangeValueForKey:WMF_SAFE_KEYPATH(observer, urls)];
              } else {
...
+ (NSArray *)sectionsFromHistoryList:(MWKHistoryList *)list {
  ...
  [sortedEntriesToIterate
      enumerateObjectsUsingBlock:^(MWKHistoryEntry *_Nonnull obj,
  ...
  NSMutableArray *sections =
      [NSMutableArray arrayWithCapacity:[entriesBydate count]];
  [[[entriesBydate allKeys] sortedArrayUsingComparator:^NSComparisonResult(
                                NSDate *_Nonnull obj1, NSDate *_Nonnull obj2) {
    return -[obj1 compare:obj2]; // by date decending
  }] enumerateObjectsUsingBlock:^(NSDate *_Nonnull date, NSUInteger idx,
                                  BOOL *_Nonnull stop) {
    NSMutableArray *entries = entriesBydate[date];
    SSSection *section = [SSSection sectionWithItems:entries];

    // HACK: Table views for some reason aren't adding padding to the left of
    // the default headers. Injecting some manually.
    NSString *padding = @"    ";

    if ([date isToday]) {
      section.header = [padding
          stringByAppendingString:[MWLocalizedString(@"history-section-today",
                                                     nil) uppercaseString]];
    } else if ([date isYesterday]) {
      section.header = [padding
          stringByAppendingString:[MWLocalizedString(
                                      @"history-section-yesterday", nil)
                                      uppercaseString]];
    } else {
      section.header = [padding
          stringByAppendingString:[[NSDateFormatter
                                      wmf_mediumDateFormatterWithoutTime]
                                      stringFromDate:date]];
    }
