...
- (void)applyCurrentPage:(BOOL)animated {
  ...
  NSParameterAssert(self.currentPage <
                    [self.collectionView.dataSource
                                collectionView:self.collectionView
                        numberOfItemsInSection:0]);

  // can't use scrollToItem because it doesn't handle post-rotation scrolling
  // well on iOS 6
  ...
  NSAssert(currentPageAttributes, @"Layout attributes for current page were "
                                  @"nil because %@ was called too early!",
           NSStringFromSelector(_cmd));
  [self.collectionView setContentOffset:currentPageAttributes.frame.origin
                               animated:animated];
}
...
- (void)primitiveSetCurrentPage:(NSUInteger)page {
  // assert that page is within bounds for our collection view's data source, or
  // that the view isn't loaded yet
  NSParameterAssert(![self isViewLoaded] ||
                    page < [self.collectionView.dataSource
                                       collectionView:self.collectionView
                               numberOfItemsInSection:0]);
  _currentPage = page;
}
...
- (NSUInteger)mostVisibleItemIndex {
  return [self.collectionViewLayout
                 wmf_indexPathHorizontallyClosestToContentOffset]
      .item;
}
...
- (void)viewWillTransitionToSize:(CGSize)size
  ...
  [coordinator notifyWhenInteractionEndsUsingBlock:^(
                   id<UIViewControllerTransitionCoordinatorContext> _) {
...
- (void)viewDidLayoutSubviews {
  ...
  /*
     only apply visible image index once the collection view has been populated
     with cells, otherwise calls to get
     layout attributes of the item at `currentPageIndex` will return `nil` (on
     iOS 6, at least)
   */
  if (!self.didApplyCurrentPage && self.collectionView.visibleCells.count) {
    [self applyCurrentPage:NO];
    /*
       only set the flag *after* the visible index has been updated, to make
       sure UICollectionViewDelegate callbacks
       don't override it
     */
