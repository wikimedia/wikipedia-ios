...
- (nullable instancetype)initWithSrc:(NSString *)src
  ...
  if ([[src stringByTrimmingCharactersInSet:[NSCharacterSet
                                                whitespaceCharacterSet]]
          length] == 0) {
    return nil;
  }

  NSURLComponents *srcURLComponents =
      [NSURLComponents componentsWithString:src];
  ...
  if (self) {
    ...
    self.height =
        [height isEqual:[NSNull null]] ? nil : @([height integerValue]);
    self.dataFileWidth = [dataFileWidth isEqual:[NSNull null]]
                             ? nil
                             : @([dataFileWidth integerValue]);
    self.dataFileHeight = [dataFileHeight isEqual:[NSNull null]]
                              ? nil
                              : @([dataFileHeight integerValue]);
  }
  return self;
}

- (nullable instancetype)initWithImageTagContents:(NSString *)imageTagContents
                                          baseURL:(nullable NSURL *)baseURL {
  static NSRegularExpression *attributeRegex;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    NSString *attributePattern =
        @"(src|data-file-width|width|data-file-height|height)=[\"']?((?:.(?!["
        @"\"']?\\s+(?:\\S+)=|[>\"']))+.)[\"']?"; // match on the attributes we
                                                  // need to read: src,
                                                  // data-file-width, width, etc
    attributeRegex = [NSRegularExpression
  ...
  __block NSRange srcAttributeRange = NSMakeRange(
      NSNotFound, 0); // the range of the full src attribute - src=blah
  __block NSString *dataFileWidth =
      0;                       // the original file width from data-file-width=
  __block NSString *width = 0; // the width of the image from width=
  __block NSString *dataFileHeight =
      0; // the original file height from data-file-height=
  __block NSString *height = 0; // the height of the image from height=
  NSInteger attributeOffset = 0;
  [attributeRegex
      enumerateMatchesInString:imageTagContents
                       options:0
                         range:NSMakeRange(0, imageTagContents.length)
                    usingBlock:^(
                        NSTextCheckingResult *_Nullable attributeResult,
                        NSMatchingFlags flags, BOOL *_Nonnull stop) {
                      NSString *attributeName = [[attributeRegex
                          replacementStringForResult:attributeResult
                                            inString:imageTagContents
                                              offset:attributeOffset
                                            template:@"$1"] lowercaseString];
                      NSString *attributeValue = [attributeRegex
                          replacementStringForResult:attributeResult
                                            inString:imageTagContents
                                              offset:attributeOffset
                                            template:@"$2"];
...
- (BOOL)isSizeLargeEnoughForGalleryInclusion {
  return
      // Ensure images which are just used as tiny icons are not included in
      // gallery.
      self.width.integerValue >=
          WMFImageTagMinimumSizeForGalleryInclusion.width &&
      self.height.integerValue >=
          WMFImageTagMinimumSizeForGalleryInclusion.height &&
      // Also make sure we only try to show them in the gallery if their
      // canonical size is of sufficient resolution.
      self.dataFileWidth.integerValue >=
          WMFImageTagMinimumSizeForGalleryInclusion.width &&
      self.dataFileHeight.integerValue >=
          WMFImageTagMinimumSizeForGalleryInclusion.height;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@""
                                     ...
                                     "dataFileWidth = %@ \n\t "
                                     "dataFileHeight = %@ \n",
                                    self.src, self.srcset, self.alt, self.width,
                                    self.height, self.dataFileWidth,
                                    self.dataFileHeight];
}
...
- (NSString *)imageTagContents {
  ...
  if (self.originalImageTagContents &&
      self.originalImageTagContentsSrcAttributeRange.location != NSNotFound) {
    newImageTagContents = [self.originalImageTagContents
        stringByReplacingCharactersInRange:
            self.originalImageTagContentsSrcAttributeRange
                                withString:self.src]; // only src is settable
  } else {
    newImageTagContents = @"";
  ...
  for (NSString *attribute in attributes) {
    ...
    if (value) {
      NSString *attributeString =
          [@[ @" ", attribute, @"=", value ] componentsJoinedByString:@""];
      newImageTagContents =
          [newImageTagContents stringByAppendingString:attributeString];
    }
  }
