...
- (void)userContentController:(WKUserContentController *)userContentController
  ...
  if ([message.name isEqualToString:@"anchorClicked"]) {
    [self.previewAnchorTapAlertDelegate
        wmf_showAlertForTappedAnchorHref:message.body[@"href"]];
  }
}
...
- (WKWebViewConfiguration *)configuration {
  ...
  MWLanguageInfo *langInfo =
      [self.previewSectionLanguageInfoDelegate wmf_editedSectionLanguageInfo];
  NSString *uidir =
      ([[UIApplication sharedApplication] wmf_isRTL] ? @"rtl" : @"ltr");

  NSString *earlyJavascriptTransforms = [NSString
      stringWithFormat:
          @""
           ...
           "            "
           "window.webkit.messageHandlers.anchorClicked.postMessage({ 'href': "
           "href });"
           "        }"
           "};"
           "window.wmf.utilities.setLanguage('%@', '%@', '%@');",
          langInfo.code, langInfo.dir, uidir];
...
- (void)forceScrollViewContentSizeToReflectActualHTMLHeight {
  // Only run this if the width has changed. Otherwise it will recurse
  // endlessly.
