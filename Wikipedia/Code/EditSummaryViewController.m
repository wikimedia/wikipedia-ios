...
@property(weak, nonatomic)
    IBOutlet NSLayoutConstraint *bottomLineHeightConstraint;
...
- (void)viewDidLoad {
  ...
  self.buttonDone = [[UIBarButtonItem alloc]
      bk_initWithTitle:MWLocalizedString(@"button-done", nil)
  ...
  self.placeholderLabel.font =
      [UIFont systemFontOfSize:(14.0 * MENUS_SCALE_MULTIPLIER)];
...
- (void)updatePlaceholderLabelState {
  self.placeholderLabel.hidden =
      ([self.summaryTextField.text length] == 0) ? NO : YES;
}
...
- (void)save {
  NSString *trimmedSummary = [self.summaryTextField.text
