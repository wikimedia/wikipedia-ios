...
- (void)addLanguageButtonTapped {
    ...
    WMFComponentNavigationController *navVC = [[WMFComponentNavigationController alloc] initWithRootViewController:languagesVC modalPresentationStyle:UIModalPresentationOverFullScreen customBarBackgroundColor:nil];
...
- (void)notifyDelegateThatPreferredLanguagesDidUpdate {
    if ([self.delegate respondsToSelector:@selector(languagesController:didUpdatePreferredLanguages:)]) {
        ...
        [[WMFTempAccountDataController shared] checkWikiTempAccountAvailabilitySyncBridgeWithLanguage:MWKDataStore.shared.languageLinkController.appLanguage.languageCode isCheckingPrimaryWiki:YES];
    }
}
