...
@property(strong, nonatomic)
    IBOutlet NSLayoutConstraint *languageFilterTopSpaceConstraint;
@property(strong, nonatomic)
    IBOutlet NSLayoutConstraint *filterDividerHeightConstraint;
@property(strong, nonatomic)
    IBOutlet NSLayoutConstraint *filterHeightConstraint;
...
+ (instancetype)languagesViewController {
  WMFLanguagesViewController *languagesVC =
      [WMFLanguagesViewController wmf_initialViewControllerFromClassStoryboard];
...
- (void)viewDidLoad {
  ...
  @weakify(self) UIBarButtonItem *xButton = [UIBarButtonItem
      wmf_buttonType:WMFButtonTypeX
             handler:^(id sender) {
               @strongify(
  ...
  self.languageFilterField.layer.borderColor =
      [[UIColor wmf_settingsBackgroundColor] CGColor];
  self.languageFilterField.layer.borderWidth = 1.f;

  // stylize
  if ([self.languageFilterField
          respondsToSelector:@selector(setReturnKeyType:)]) {
  ...
  self.languageFilterField.placeholder =
      MWLocalizedString(@"article-languages-filter-placeholder", nil);

  self.tableView.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
  self.filterDividerHeightConstraint.constant = 0.5f;

  [self.tableView registerNib:[WMFLanguageCell wmf_classNib]
       forCellReuseIdentifier:[WMFLanguageCell wmf_nibName]];
  [self.tableView registerNib:[WMFArticleLanguagesSectionHeader wmf_classNib]
      forHeaderFooterViewReuseIdentifier:[WMFArticleLanguagesSectionHeader
                                             wmf_nibName]];

  // HAX: force these to take effect if they were set before the VC was
  // presented/pushed.
...
- (MWKLanguageFilter *)languageFilter {
  if (!_languageFilter) {
    _languageFilter = [[MWKLanguageFilter alloc]
        initWithLanguageDataSource:[MWKLanguageLinkController sharedInstance]];
  }
...
- (void)configurePreferredLanguageCell:(WMFLanguageCell *)cell
                                 atRow:(NSUInteger)row {
  cell.isPreferred = YES;
  [self configureCell:cell
          forLangLink:self.languageFilter.filteredPreferredLanguages[row]];
}

- (void)configureOtherLanguageCell:(WMFLanguageCell *)cell
                             atRow:(NSUInteger)row {
  cell.isPreferred = NO;
  [self configureCell:cell
          forLangLink:self.languageFilter.filteredOtherLanguages[row]];
}

- (void)configureCell:(WMFLanguageCell *)cell
          forLangLink:(MWKLanguageLink *)langLink {
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  WMFLanguageCell *cell = (id)
      [tableView dequeueReusableCellWithIdentifier:[WMFLanguageCell wmf_nibName]
...
- (CGFloat)alphaForDeleteButton {
  return !self.tableView.editing || ([MWKLanguageLinkController sharedInstance]
                                         .preferredLanguages.count == 1)
             ? 0.f
             : 1.f;
}
...
- (NSString *)titleForHeaderInSection:(NSInteger)section {
  NSString *title = ([self isPreferredSection:section])
                        ? MWLocalizedString(@"article-languages-yours", nil)
                        : MWLocalizedString(@"article-languages-others", nil);
  return [title uppercaseStringWithLocale:[NSLocale currentLocale]];
}

- (void)configureHeader:(WMFArticleLanguagesSectionHeader *)header
...
- (nullable UIView *)tableView:(UITableView *)tableView
  ...
  if ([self shouldShowHeaderForSection:section]) {
    WMFArticleLanguagesSectionHeader *header =
        (id)[tableView dequeueReusableHeaderFooterViewWithIdentifier:
                           [WMFArticleLanguagesSectionHeader wmf_nibName]];
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  return [self shouldShowHeaderForSection:section] ? WMFLanguageHeaderHeight
                                                   : 0;
}
...
- (void)tableView:(UITableView *)tableView
  ...
  if ([self.delegate respondsToSelector:@selector(languagesController:
                                                    didSelectLanguage:)]) {
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView
  ...
  if ([self isPreferredSection:indexPath.section]) {
    if ([self tableView:tableView numberOfRowsInSection:indexPath.section] >
        1) {
...
- (void)tableView:(UITableView *)tableView
  ...
  case UITableViewCellEditingStyleInsert: {
    MWKLanguageLink *langLink =
        self.languageFilter.filteredOtherLanguages[indexPath.row];
    [[MWKLanguageLinkController sharedInstance]
        appendPreferredLanguage:langLink];
  } break;
  case UITableViewCellEditingStyleDelete: {
    MWKLanguageLink *langLink =
        self.languageFilter.filteredPreferredLanguages[indexPath.row];
    [[MWKLanguageLinkController sharedInstance]
        removePreferredLanguage:langLink];
  } break;
...
@interface WMFPreferredLanguagesViewController () <
...
+ (instancetype)preferredLanguagesViewController {
  WMFPreferredLanguagesViewController *languagesVC =
...
- (void)viewDidLoad {
  ...
  [self.tableView registerNib:[WMFArticleLanguagesSectionFooter wmf_classNib]
      forHeaderFooterViewReuseIdentifier:[WMFArticleLanguagesSectionFooter
                                             wmf_nibName]];
}
...
- (void)setEditing:(BOOL)editing animated:(BOOL)animated {
  ...
  if (animated) {
    [UIView animateWithDuration:0.30
                     animations:^{
                       self.tableView.tableFooterView.alpha =
                           editing ? 1.0 : 0.0;
...
- (IBAction)addLanguages:(id)sender {
  WMFLanguagesViewController *languagesVC =
      [WMFLanguagesViewController nonPreferredLanguagesViewController];
...
- (void)languagesController:(WMFLanguagesViewController *)controller
  ...
  if ([self.delegate respondsToSelector:@selector(languagesController:
                                            didUpdatePreferredLanguages:)]) {
    [self.delegate
                languagesController:self
        didUpdatePreferredLanguages:[MWKLanguageLinkController sharedInstance]
                                        .preferredLanguages];
  }
}
...
- (nullable UIView *)tableView:(UITableView *)tableView
  ...
  if ([self shouldShowFooterForSection:section]) {
    WMFArticleLanguagesSectionFooter *footer =
        (id)[tableView dequeueReusableHeaderFooterViewWithIdentifier:
                           [WMFArticleLanguagesSectionFooter wmf_nibName]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
         cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  WMFLanguageCell *cell = (WMFLanguageCell *)[super tableView:tableView
...
- (void)tableView:(UITableView *)tableView
  ...
  MWKLanguageLink *langLink = [MWKLanguageLinkController sharedInstance]
                                  .preferredLanguages[sourceIndexPath.row];
  [[MWKLanguageLinkController sharedInstance]
      reorderPreferredLanguage:langLink
                       toIndex:destinationIndexPath.row];

  // TODO: reloadData is a bit brute force, but had issues with the "PRIMARY"
  // indicator
  // showing on more than one cell after re-ordering first cell.
  [tableView reloadData];

  [self.delegate languagesController:self
         didUpdatePreferredLanguages:[MWKLanguageLinkController sharedInstance]
                                         .preferredLanguages];
}
...
- (BOOL)tableView:(UITableView *)tableView
  ...
  return [self isPreferredSection:indexPath.section] &&
...
- (void)setArticleURL:(NSURL *)articleURL {
  NSAssert(self.isViewLoaded == NO,
           @"Article Title must be set prior to view being loaded");
...
- (MWKTitleLanguageController *)titleLanguageController {
  NSAssert(
      self.articleURL != nil,
      @"Article Title must be set before accessing titleLanguageController");
  if (!_titleLanguageController) {
    _titleLanguageController = [[MWKTitleLanguageController alloc]
        initWithArticleURL:self.articleURL
        languageController:[MWKLanguageLinkController sharedInstance]];
  }
...
- (MWKLanguageFilter *)languageFilter {
  if (!_languageFilter) {
    _languageFilter = [[MWKLanguageFilter alloc]
        initWithLanguageDataSource:self.titleLanguageController];
  }
  return _languageFilter;
}
...
- (void)downloadArticlelanguages {
  @weakify(self);
  [self.titleLanguageController fetchLanguagesWithSuccess:^{
    @strongify(self)[self reloadDataSections];
  }
