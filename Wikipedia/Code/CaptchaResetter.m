  ...
  [manager POST:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (![responseObject isDict]) {
          responseObject =
              @{ @"error" : @{@"info" : @"Captcha Resetter data not found."} };
        }
...
- (NSDictionary *)getParams {
  return @{ @"action" : @"fancycaptchareload", @"format" : @"json" };
}
...
+ (NSString *)newCaptchaImageUrlFromOldUrl:(NSString *)oldUrl
                                  andNewId:(NSString *)newId {
  NSError *error = nil;
  NSRegularExpression *regex = [NSRegularExpression
      regularExpressionWithPattern:@"wpCaptchaId=([^&]*)"
                           options:NSRegularExpressionCaseInsensitive
                             error:&error];
  if (!error) {
    NSString *newCaptchaUrl = [regex
        stringByReplacingMatchesInString:oldUrl
                                 options:0
                                   range:NSMakeRange(0, [oldUrl length])
                            withTemplate:[NSString
                                             stringWithFormat:@"wpCaptchaId=%@",
                                                              newId]];
