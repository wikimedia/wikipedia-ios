...
- (void)wmf_cropContentsByVerticallyCenteringFrame:(CGRect)rect
  ...
  CGRect verticallyCenteredFrame =
      CGRectMake(0, rect.origin.y, image.size.width, rect.size.height);
  [self wmf_cropContentsToFrame:verticallyCenteredFrame
...
- (void)wmf_cropContentsToFrame:(CGRect)rect
  ...
  CGPoint imageCenter =
      CGPointMake(image.size.width / 2.f, image.size.height / 2.f);
...
- (void)wmf_setContentOffset:(CGPoint)offset image:(UIImage *)image {
  self.layer.contentsRect = [image
      wmf_normalizeRect:CGRectMake(WMFClamp(0, offset.x, image.size.width),
