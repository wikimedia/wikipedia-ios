...
- (NSArray *)sectionsFromDataStore {
  ...
  NSString *path = [[self.article.dataStore pathForArticleURL:self.article.url]
  ...
  @try {
    for (NSString *subpath in files) {
      ...
      NSArray *matches =
          [redigits matchesInString:filename
                            options:0
                              range:NSMakeRange(0, [filename length])];
      if (matches && [matches count]) {
        MWKSection *section =
            [self.article.dataStore sectionWithId:filename.intValue
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
  ...
  NSUInteger start = state->state, count = 0;
...
- (void)buildSectionHierarchy {
  ...
  [self.sections enumerateObjectsUsingBlock:^(MWKSection *currentSection,
    ...
    if ([currentParent isAtLevelAboveSection:currentSection]) {
      ...
      if ([lastChild isAtSameLevelAsSection:currentSection] ||
          ![lastChild isAtLevelAboveSection:currentSection]) {
...
- (NSArray *)topLevelSections {
  __block MWKSection *currentParent = nil;
  return [self.sections
      bk_reduce:[NSMutableArray arrayWithCapacity:self.sections.count]
      withBlock:^NSMutableArray *(NSMutableArray *topLevelSections,
                                  MWKSection *section) {
        if (!section.level) {
          [topLevelSections addObject:section];
          currentParent = nil;
        } else if (currentParent == nil ||
                   ![currentParent isAtLevelAboveSection:section]) {
