...
/**
 *  Lazily computed property containing "id" attributes of backlinks from the
 * receiver.
 *
 *  Used to allow users to jump back to one of the locations where the receiver
 * was cited.
 */
@property(nonatomic, copy, readonly) NSArray *backlinkIdentifiers;

/**
 *  Lazily computed property which returns the HTML inside the citation element,
 * excluding the back-links.
 *
 *  This can be anything from a simple span of text with a link, but could also
 * be a list of links.
 *
 *  @see rawHTML
 */
...
- (MWKCitation *__nullable)initWithCitationIdentifier:
                               (NSString *)citationIdentifier
                                              rawHTML:(NSString *)rawHTML;

- (MWKCitation *__nullable)
initWithCitationIdentifier:(NSString *)citationIdentifier
                   rawHTML:(NSString *)rawHTML
