...
static const NSTimeInterval
    WMFArticleNavigationControllerSecondToolbarAnimationDuration = 0.3;
...
- (void)viewDidLoad {
  ...
  [self layoutSecondToolbarForViewBounds:self.view.bounds
                                  hidden:self.isSecondToolbarHidden
                                animated:NO];
}
...
- (void)layoutSecondToolbarForViewBounds:(CGRect)bounds
                                  hidden:(BOOL)hidden
  ...
  if (animated) {
    [UIView animateWithDuration:
                WMFArticleNavigationControllerSecondToolbarAnimationDuration
...
- (void)setSecondToolbarHidden:(BOOL)secondToolbarHidden
                      animated:(BOOL)animated {
  self.secondToolbarHidden = secondToolbarHidden;
  [self layoutSecondToolbarForViewBounds:self.view.bounds
                                  hidden:secondToolbarHidden
                                animated:animated];
}
...
- (void)navigationController:(UINavigationController *)navigationController
  ...
  // Ideally this observes secondToolbarItems for changes, but this is all we
  // need for our use case at the moment
  ...
  if ([self.navigationDelegate
          respondsToSelector:
              @selector(navigationController:
                  interactionControllerForAnimationController:)]) {
    return [self.navigationDelegate navigationController:navigationController
...
@implementation
    UIViewController (UINavigationControllerContextualSecondToolbarItems)

static const void *SecondToolbarItemsKey = &SecondToolbarItemsKey;

- (void)setSecondToolbarItems:
    (NSArray<__kindof UIBarButtonItem *> *)secondToolbarItems {
  objc_setAssociatedObject(self, SecondToolbarItemsKey, secondToolbarItems,
