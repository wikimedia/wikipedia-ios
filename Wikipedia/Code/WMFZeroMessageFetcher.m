...
- (instancetype)init {
  ...
  if (self) {
    ...
    self.operationManager.responseSerializer = [WMFMantleJSONResponseSerializer
        serializerForInstancesOf:[WMFZeroMessage class]
                     fromKeypath:nil];
  }
  return self;
}
...
- (AnyPromise *)fetchZeroMessageForSiteURL:(NSURL *)siteURL {
  return [AnyPromise promiseWithResolverBlock:^(PMKResolver _Nonnull resolve) {
    ...
    [self.operationManager
        GET:[[NSURL wmf_mobileAPIURLForURL:siteURL] absoluteString]
        parameters:params
        progress:NULL
        success:^(NSURLSessionDataTask *_Nonnull _,
