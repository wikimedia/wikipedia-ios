...
NSString *const WMFArticleFetcherErrorCachedFallbackArticleKey =
    @"WMFArticleFetcherErrorCachedFallbackArticleKey";
...
- (instancetype)init {
  ...
  if (self) {
    ...
    NSString *queueID = [NSString
        stringWithFormat:@"org.wikipedia.articlefetcher.accessQueue.%@",
                         [[NSUUID UUID] UUIDString]];
    self.operationsQueue = dispatch_queue_create(
        [queueID cStringUsingEncoding:NSUTF8StringEncoding],
...
- (void)fetchArticleForURL:(NSURL *)articleURL
  ...
  if (!articleURL.wmf_title) {
    resolve([NSError wmf_errorWithType:WMFErrorTypeStringMissingParameter
                              userInfo:nil]);
  }
  ...
  NSURL *url = useDeskTopURL ? [NSURL wmf_desktopAPIURLForURL:articleURL]
                             : [NSURL wmf_mobileAPIURLForURL:articleURL];

  NSURLSessionDataTask *operation = [self.operationManager
      ...
      failure:^(NSURLSessionDataTask *operation, NSError *error) {
        if ([url isEqual:[NSURL wmf_mobileAPIURLForURL:articleURL]] &&
            [error wmf_shouldFallbackToDesktopURLError]) {
          [self fetchArticleForURL:articleURL
                     useDesktopURL:YES
                          progress:progress
                          resolver:resolve];
        } else {
...
- (void)trackOperation:(NSURLSessionDataTask *)operation
...
- (instancetype)initWithDataStore:(MWKDataStore *)dataStore {
  ...
  if (self) {
    self.operationManager.requestSerializer =
        [WMFArticleRequestSerializer serializer];
    self.operationManager.responseSerializer =
        [WMFArticleResponseSerializer serializer];

    self.dataStore = dataStore;
    self.revisionFetcher = [[WMFArticleRevisionFetcher alloc] init];

    /*
       Setting short revision check timeouts, to ensure that poor connections
       don't drastically impact the case
       when cached article content is up to date.
     */
    FBTweakBind(self.revisionFetcher, timeoutInterval, @"Networking",
                @"Article", @"Revision Check Timeout", 0.8);
  }
  return self;
}
...
- (id)serializedArticleWithURL:(NSURL *)url response:(NSDictionary *)response {
  ...
  @try {
    ...
    DDLogError(@"Failed to import article data. Response: %@. Error: %@",
               response, e);
...
- (AnyPromise *)
fetchLatestVersionOfArticleWithURL:(NSURL *)url
  ...
  if (forceDownload || !cachedArticle || !cachedArticle.revisionId ||
      [cachedArticle isMain]) {
    if (forceDownload) {
      ...
      DDLogInfo(@"Cached article for %@ doesn't have revision ID, fetching "
                @"immediately.",
                url);
    } else {
      // Main pages dont neccesarily have revisions every day. We can't rely on
      // the revision check
      DDLogInfo(@"Cached article for main page: %@, fetching immediately.",
                url);
    }
    promisedArticle = [self fetchArticleForURL:url progress:progress];
  } else {
    promisedArticle =
        [self.revisionFetcher
            fetchLatestRevisionsForArticleURL:url
                                  resultLimit:1
            ...
            .then(^(WMFRevisionQueryResults *results) {
              ...
              if (!self) {
                return [AnyPromise promiseWithValue:[NSError cancelledError]];
              } else if ([results.revisions.firstObject.revisionId
                             isEqualToNumber:cachedArticle.revisionId]) {
...
- (AnyPromise *)
fetchLatestVersionOfArticleWithURLIfNeeded:(NSURL *)url
                                  progress:
                                      (WMFProgressHandler __nullable)progress {
  return [self fetchLatestVersionOfArticleWithURL:url
                                    forceDownload:NO
                                         progress:progress];
}

- (AnyPromise *)fetchArticleForURL:(NSURL *)articleURL
                          progress:(WMFProgressHandler __nullable)progress {
