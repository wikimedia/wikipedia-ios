...
- (instancetype)initWithArticle:(MWKArticle *)article
                sourceURLString:(NSString *)urlString {
  return [self initWithArticle:article
                     sourceURL:[NSURL wmf_optionalURLWithString:urlString]];
}
...
- (instancetype)initWithArticle:(MWKArticle *)article
  ...
  if (self) {
    ...
    // fileNameNoSizePrefix is lazily derived from this property, so be careful
    // if _sourceURL needs to be re-set
...
- (instancetype)initWithArticle:(MWKArticle *)article
  ...
  if (self) {
    ...
    self.originalFileWidth =
        [self optionalNumber:@"originalFileWidth" dict:dict];
    self.originalFileHeight =
        [self optionalNumber:@"originalFileHeight" dict:dict];
    _allNormalizedFaceBounds =
        [dict[@"focalRects"] bk_map:^NSValue *(NSString *rectString) {
...
- (id)dataExport {
  ...
  if (self.allNormalizedFaceBounds) {
    dict[@"focalRects"] =
        [self.allNormalizedFaceBounds bk_map:^id(NSValue *rectValue) {
...
- (CGSize)originalFileSize {
  if ([self hasOriginalFileSize]) {
    return CGSizeMake(self.originalFileWidth.floatValue,
                      self.originalFileHeight.floatValue);
  } else {
    return CGSizeZero;
  }
}
...
- (NSString *)basename {
  NSArray *sourceURLComponents =
      [self.sourceURLString componentsSeparatedByString:@"/"];
...
- (NSString *)fileNameNoSizePrefix {
  if (!_fileNameNoSizePrefix) {
    _fileNameNoSizePrefix =
        WMFParseImageNameFromSourceURL(self.sourceURLString);
  }
...
/// @return @c YES if @c size is within @c points of
/// <code>self.estimatedSize</code>, otherwise @c NO.
...
- (BOOL)isDownloaded {
  return [[WMFImageController sharedInstance]
      hasImageWithURL:[NSURL URLWithString:self.sourceURLString]];
}
...
- (BOOL)isEqualToImage:(MWKImage *)image {
  return self == image ||
         [[self.sourceURLString wmf_schemelessURL]
             isEqualToString:[image.sourceURLString wmf_schemelessURL]];
}
...
- (BOOL)isVariantOfImage:(MWKImage *)otherImage {
  return otherImage.canonicalFilename &&
         [self.canonicalFilename isEqualToString:otherImage.canonicalFilename];
}

- (NSString *)description {
  return [NSString stringWithFormat:@"%@ article: %@ sourceURL: %@",
                                    [super description], self.article.url,
