 ...
 *  @param success          The retry block - called if a retry is executed. Use
 * this to get the new operation.
...
wmf_GETWithMobileURLString:(NSString *)mobileURLString
          desktopURLString:(NSString *)desktopURLString
                parameters:(id)parameters
                     retry:(void (^)(NSURLSessionDataTask *retryOperation,
                                     NSError *error))retry
                   success:(void (^)(NSURLSessionDataTask *operation,
                                     id responseObject))success
                   failure:(void (^)(NSURLSessionDataTask *operation,
                                     NSError *error))failure;

/**
 *  Send a @c GET request to the mobile endpoint (see -[NSURL wmf_mobileAPIURL])
 *  for the given url, falling back to desktop endpoint (-[NSURL
 * wmf_desktopAPIURL])
 *  if the mobile endpoint fails.
 *
 *  @param URL The URL to derive the endpoints from. First mobile, then desktop.
 *  @param parameters       The parameters for the request
 *  @param retry            The retry block - called if a retry is executed. Use
 * this to get the new operation.
 ...
 */
- (NSURLSessionDataTask *)
wmf_GETAndRetryWithURL:(NSURL *)URL
            parameters:(id)parameters
                 retry:(void (^)(NSURLSessionDataTask *retryOperation,
                                 NSError *error))retry
               success:(void (^)(NSURLSessionDataTask *operation,
 ...
 *  if the mobile endpoint fails.
 *
 *  Convenience, promise-based alternative to block-based API.  Omits the retry
 * parameter & operation return for the
 *  simple case which doesn't need cancellation.
 *
 *  @param URL The URL to derive the endpoints from. First mobile, then desktop.
 *
 *  @param parameters The parameters which will be passed to the receiver's @c
 * requestSerializer.
 *
 *  @return A promise which will be resolved with the successful response of the
 * request, or rejected with any errors
 *          that occur.
 *
 *  @see
 * wmf_GETWithMobileURLString:desktopURLString:parameters:retry:success:failure:
 */
 ...
 *  @param mobileURLString  The mobile URL
 *  @param desktopURLString The desktop URL
 *  @param parameters       The parameters for the request (same as normal
 * POST:)
 *  @param success          The retry block - called if a retry is executed. Use
 * this to get the new operation.
 ...
 */
- (NSURLSessionDataTask *)
wmf_POSTWithMobileURLString:(NSString *)mobileURLString
           desktopURLString:(NSString *)desktopURLString
                 parameters:(id)parameters
                      retry:(void (^)(NSURLSessionDataTask *retryOperation,
                                      NSError *error))retry
