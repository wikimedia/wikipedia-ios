...
- (instancetype)initWithCoder:(NSCoder *)coder {
  return [self initWithText:@""
                   fontSize:16.0 * MENUS_SCALE_MULTIPLIER
                       bold:NO
...
- (instancetype)initWithText:(NSString *)text
  ...
  if (self) {
    ...
    self.fontBold = bold;
    self.label = [[MenuLabel alloc] initWithText:text
                                        fontSize:size
                                            bold:bold
                                           color:color
                                         padding:padding];
...
- (void)constrainLabel {
  ...
  NSArray *constraintArrays = @[
    ...
    [NSLayoutConstraint
        constraintsWithVisualFormat:@"H:|-(marginLeft)-[label]-(marginRight)-|"
                            options:0
                            metrics:metrics
                              views:views],

    [NSLayoutConstraint
        constraintsWithVisualFormat:@"V:|-(marginTop)-[label]-(marginBottom)-|"
                            options:0
                            metrics:metrics
                              views:views]
