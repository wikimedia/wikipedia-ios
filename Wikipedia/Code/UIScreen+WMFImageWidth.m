 ...
 *
 *  This prevents us from fragmenting the image thumbnail caches on the
 * back-end.
 */
typedef NS_ENUM(NSUInteger, WMFImageWidth) {
   ...
   *
   *  There's no guarantee about image aspect ratio, so we fetch a little more
   * and use aspect fill.
   */
...
- (NSUInteger)wmf_maxScale {
  NSUInteger scaleMultiplierCeiling =
      FBTweakValue(@"Images", @"Scale multiplier", @"Ceiling", 2, 1, 3);
