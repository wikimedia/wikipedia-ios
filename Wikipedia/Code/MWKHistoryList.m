...
NSString *const MWKHistoryListDidUpdateNotification =
    @"MWKHistoryListDidUpdateNotification";
...
- (instancetype)initWithDataStore:(MWKDataStore *)dataStore {
  NSArray *entries =
      [[dataStore historyListData] wmf_mapAndRejectNil:^id(id obj) {
        @try {
...
- (void)addEntry:(MWKHistoryEntry *)entry {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:MWKHistoryListDidUpdateNotification
                    object:self];
}

- (void)setPageScrollPosition:(CGFloat)scrollposition
       onPageInHistoryWithURL:(NSURL *)url {
  if ([url.wmf_title length] == 0) {
    return;
  }
  [self updateEntryWithListIndex:url
...
- (void)cleanupRemovedEntries:(NSArray<MWKHistoryEntry *> *)entries {
  ...
  NSSet *savedURLs = [NSSet
      setWithArray:[savedPageList.entries
                       valueForKey:WMF_SAFE_KEYPATH([MWKSavedPageEntry new],
                                                    url)]];
  NSMutableSet *removedURLs = [NSMutableSet
      setWithArray:[entries valueForKey:WMF_SAFE_KEYPATH([MWKHistoryEntry new],
                                                         url)]];
...
- (nullable NSArray<NSSortDescriptor *> *)sortDescriptors {
  ...
  dispatch_once(&onceToken, ^{
    sortDescriptors = @[ [NSSortDescriptor
        sortDescriptorWithKey:WMF_SAFE_KEYPATH([MWKHistoryEntry new], date)
                    ascending:NO] ];
  });
