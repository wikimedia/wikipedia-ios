...
- (void)setEditTokens:(NSDictionary *)editTokens {
  ...
  NSError *error = nil;
  NSData *tokenDictJsonData =
      [NSJSONSerialization dataWithJSONObject:editTokens
                                      options:NSJSONWritingPrettyPrinted
                                        error:&error];
  if (!error) {
    NSString *tokenDictJsonString =
        [[NSString alloc] initWithData:tokenDictJsonData
                              encoding:NSUTF8StringEncoding];
    [self setKeychainValue:tokenDictJsonString
                  forEntry:@"org.wikimedia.wikipedia.edittokens"];
  }
}

- (NSMutableDictionary *)editTokens {
  NSString *tokenDictJsonString =
      [self getKeychainValueForEntry:@"org.wikimedia.wikipedia.edittokens"];
  if (!tokenDictJsonString) {
    return [@{} mutableCopy];
  }

  NSError *error = nil;
  NSDictionary *tokenDict = [NSJSONSerialization
      JSONObjectWithData:[tokenDictJsonString
...
- (BOOL)setKeychainValue:(NSString *)value forEntry:(NSString *)entry {
  if (!value) {
    // Makes setting the value to nil cause the item to be removed from the
    // keychain.
  ...
  NSDictionary *dict = @{
    ...
    (__bridge id)kSecAttrService : [[[NSBundle mainBundle] infoDictionary]
...
- (BOOL)updateKeychainValue:(NSString *)value forEntry:(NSString *)entry {
  ...
  NSDictionary *dataDict = @{(__bridge id)kSecValueData : valueData};

  OSStatus status = SecItemUpdate((__bridge CFDictionaryRef)dict,
                                  (__bridge CFDictionaryRef)dataDict);
  if (status == errSecSuccess) {
    return YES;
  } else {
...
- (void)deleteItemFromKeychainWithIdentifier:(NSString *)identifier {
  NSMutableDictionary *searchDictionary =
      [self setupSearchDirectoryForIdentifier:identifier];
...
// From:
// http://www.raywenderlich.com/6475/basic-security-in-ios-5-tutorial-part-1
- (NSMutableDictionary *)setupSearchDirectoryForIdentifier:
    (NSString *)identifier {
  // Setup dictionary to access keychain.
  NSMutableDictionary *searchDictionary = [[NSMutableDictionary alloc] init];
  // Specify we are using a password (rather than a certificate, internet
  // password, etc).
  [searchDictionary setObject:(__bridge id)kSecClassGenericPassword
                       forKey:(__bridge id)kSecClass];
  // Uniquely identify this keychain accessor.
  [searchDictionary setObject:[[[NSBundle mainBundle] infoDictionary]
                                  objectForKey:@"CFBundleIdentifier"]
