        ...
        if displayType == .modal,
           let navVC = (presentingViewController as? WMFAppViewController)?.currentTabNavigationController {
            dismiss(animated: true) { [weak self] in
                ...
                let coordinator = ArticleCoordinator(navigationController: navVC, articleURL: articleURL, dataStore: dataStore, theme: theme, source: .undefined, linkDelegate: self)
                coordinator.start()
            }
        } else {
...
extension ReadingListDetailViewController: UITextViewDelegate {
