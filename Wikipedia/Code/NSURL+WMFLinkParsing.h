...
/**
 * Initialize a new URL with the main page URL for a given language anf the
 *default domain - wikipedia.org.
 *
 * @param language      An optional Wikimedia language code. Should be ISO
 *639-x/IETF BCP 47 @see kCFLocaleLanguageCode - for example: `en`.
 *
...
/**
 * Initialize a new URL with the default Site domain -wikipedia.org - and
 *`language`.
 *
 * @param language      An optional Wikimedia language code. Should be ISO
 *639-x/IETF BCP 47 @see kCFLocaleLanguageCode - for example: `en`.
...
/// @return A URL with the default domain and the language code returned by @c
/// locale.
 ...
 **/
+ (NSURL *)wmf_URLWithDomain:(NSString *)domain
                    language:(nullable NSString *)language;

/**
 * Initialize a new URL with a Wikimedia `domain`, `language`, `title` and
 *`fragment`.
 *
 * @param domain        Wikimedia domain - for example: `wikimedia.org`.
 * @param language      An optional Wikimedia language code. Should be ISO
 *639-x/IETF BCP 47 @see kCFLocaleLanguageCode - for exmaple: `en`.
 * @param title         An optional Wikimedia title. For exmaple: `Main Page`.
 * @param fragment      An optional fragment, for example if you want the URL to
 *contain `#section`, the fragment is `section`.
 *
...
/**
 * Return a new URL constructed from the `siteURL`, replacing the `title` and
 *`fragment` with the given values.
 *
 * @param siteURL       A Wikimedia site URL. For exmaple:
 ...
 *`fragment` with the given values.
 **/
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
                        title:(nullable NSString *)title
                     fragment:(nullable NSString *)fragment;

/**
 * Return a new URL constructed from the `siteURL`, replacing the `path` with
 *the `internalLink`.
 *
 * @param siteURL       A Wikimedia site URL. For exmaple:
 *`https://en.wikipedia.org`.
 * @param internalLink  A Wikimedia internal link path. For exmaple:
 *`/wiki/Main_Page#section`.
 *
 * @return A new URL constructed from the `siteURL`, replacing the `path` with
 *the `internalLink`.
 **/
WMF_TECH_DEBT_TODO(this method should be generecized to "path" and
                   handle the presence of / wiki /)
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
    escapedDenormalizedInternalLink:(NSString *)internalLink;

/**
 * Return a new URL constructed from the `siteURL`, replacing the `path` with
 *the internal link prefix and the `path`.
 *
 * @param siteURL                                       A Wikimedia site URL.
 *For exmaple: `https://en.wikipedia.org`.
 * @param escapedDenormalizedTitleAndFragment           A Wikimedia path and
 *fragment. For exmaple: `/Main_Page#section`.
 *
 * @return A new URL constructed from the `siteURL`, replacing the `path` with
 *the internal link prefix and the `path`.
 **/
WMF_TECH_DEBT_TODO(this method should be folded into the above method and should
                       handle the presence of a #)
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
    escapedDenormalizedTitleAndFragment:
        (NSString *)escapedDenormalizedTitleAndFragment;
 ...
 * @param siteURL                                       A Wikimedia site URL.
 *For exmaple: `https://en.wikipedia.org`.
 * @param unescapedDenormalizedTitleAndFragment           A Wikimedia path and
 *fragment. For exmaple: `/99%_Invisible#section`. Note the % is not escaped.
 *
 * @return A new URL constructed from the `siteURL`, replacing the `path` with
 *the internal link prefix and the `path`.
 **/
+ (NSURL *)wmf_URLWithSiteURL:(NSURL *)siteURL
    unescapedDenormalizedTitleAndFragment:
        (NSString *)escapedDenormalizedTitleAndFragment;
...
/**
 * Return a new URL similar to the URL you call this method on but replace the
 *title.
 *
 * @param title         A Wikimedia title. For exmaple: `Main Page`.
 *
 * @return A new URL based on the URL you call this method on with the given
 *title.
 **/
...
/**
 *  Return a URL with just the domain, language, and mobile subdomain of the
 * reciever.
...
WMF_TECH_DEBT_TODO(These should be renamed and / or
                   moved to indicate that they are based on the language
                       subdomain of the receiver)
