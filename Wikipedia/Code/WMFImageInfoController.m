...
NSDictionary *WMFIndexImageInfo(NSArray *__nullable imageInfo) {
  return [imageInfo
      bk_reduce:[NSMutableDictionary dictionaryWithCapacity:imageInfo.count]
      withBlock:^NSMutableDictionary *(NSMutableDictionary *indexedInfo,
                                       MWKImageInfo *info) {
...
- (instancetype)initWithDataStore:(MWKDataStore *)dataStore
  ...
  return [self
      initWithDataStore:dataStore
              batchSize:batchSize
            infoFetcher:[[MWKImageInfoFetcher alloc] initWithDelegate:nil]];
}

- (instancetype)initWithDataStore:(MWKDataStore *)dataStore
                        batchSize:(NSUInteger)batchSize
                      infoFetcher:(MWKImageInfoFetcher *)fetcher {
  NSAssert(batchSize <= 50,
           @"Only up to 50 titles can be retrieved at a time.");
...
- (void)setUniqueArticleImages:(NSArray<MWKImage *> *)uniqueArticleImages
                 forArticleURL:(NSURL *)url {
  if (_uniqueArticleImages == uniqueArticleImages &&
      WMF_EQUAL(self.articleURL, isEqual:, url)) {
    return;
  }
...
- (NSArray *)imageFilePageTitles {
  if (!_imageFilePageTitles) {
    ...
    _imageFilePageTitles =
        [[MWKImage mapFilenamesFromImages:_uniqueArticleImages] copy];
  }
...
- (NSDictionary *)indexedImageInfo {
  if (!_indexedImageInfo) {
    _indexedImageInfo = WMFIndexImageInfo([self.dataStore
                            imageInfoForArticleWithURL:self.articleURL])
...
- (NSUInteger)indexOfImageAssociatedWithInfo:(MWKImageInfo *)info {
  return [self.uniqueArticleImages
      indexOfObjectPassingTest:^BOOL(MWKImage *img, NSUInteger idx,
                                     BOOL *stop) {
...
- (NSMutableIndexSet *)fetchedIndices {
  if (!_fetchedIndices) {
    _fetchedIndices = [self.indexedImageInfo.allValues
        bk_reduce:[NSMutableIndexSet new]
...
- (BOOL)hasFetchedAllItems {
  return [self.fetchedIndices
      containsIndexesInRange:NSMakeRange(0, self.uniqueArticleImages.count)];
}
...
- (id<MWKImageInfoRequest> __nullable)fetchBatchContainingIndex:
    (NSInteger)index {
  ...
  if (indexes.count == 0 || !self.articleURL) {
    ...
    return [indexes
        ...
        withBlock:^NSMutableArray *(NSMutableArray *acc, NSUInteger index) {
          id<MWKImageInfoRequest> request =
              [self fetchBatchContainingIndex:index];
          if (request) {
...
- (NSArray *__nullable)fetchBatchContainingIndex:(NSInteger)index
...
- (NSRange)batchRangeForTargetIndex:(NSUInteger)index {
  ...
  if (index > self.uniqueArticleImages.count) {
    DDLogWarn(@"Attempted to fetch %lu which is beyond upper bound of %lu",
              (unsigned long)index,
              (unsigned long)self.uniqueArticleImages.count);
    return WMFRangeMakeNotFound();
  }
  NSUInteger const start =
      floorf(index / (float)self.infoBatchSize) * self.infoBatchSize;
  NSRange const range = NSMakeRange(
      start, MIN(self.infoBatchSize, self.uniqueArticleImages.count - start));
...
- (id<MWKImageInfoRequest> __nullable)fetchBatch:(NSRange)batch {
  ...
  if (WMFRangeIsNotFoundOrEmpty(batch)) {
    DDLogWarn(@"Attempted to fetch not found or empty range: %@",
  ...
  // optimistically add batch to fetched indices, then remove it if the request
  // fails
  ...
  return [self.imageInfoFetcher fetchGalleryInfoForImageFiles:titlesToFetch
      ...
      success:^(NSArray *infoObjects) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          ...
          // !!!: we should have already read any pre-existing image info from
          // the data store
          // HAX: we need to re-save the entire array every time, otherwise info
          // will be "dropped"
          [[self dataStore] saveImageInfo:self.indexedImageInfo.allValues
                            forArticleURL:self.articleURL];
        ...
        if (self) {
          dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!([error.domain isEqualToString:NSURLErrorDomain] &&
              ...
              [self.delegate imageInfoController:self
                              failedToFetchBatch:batch
                                           error:error];
            }
          });
        }
