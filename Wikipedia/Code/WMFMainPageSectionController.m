...
static NSString *const WMFMainPageSectionIdentifier =
    @"WMFMainPageSectionIdentifier";
...
- (NSAttributedString *)headerTitle {
  return [[NSAttributedString alloc]
      initWithString:MWLocalizedString(@"explore-main-page-heading", nil)
          attributes:@{
...
- (AnyPromise *)fetchData {
  @weakify(self);
  return [self.siteInfoFetcher fetchSiteInfoForSiteURL:self.siteURL]
      .then(^(MWKSiteInfo *data) {
        @strongify(self);
        if (!self || !data.mainPageURL) {
          return (id)[AnyPromise promiseWithValue:[NSError cancelledError]];
        }
        self.siteInfo = data;
        return (id)[self.titleSearchFetcher
            fetchArticlePreviewResultsForArticleURLs:@[ self.siteInfo
                                                            .mainPageURL ]
