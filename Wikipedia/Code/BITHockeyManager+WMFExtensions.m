...
static NSString *const kHockeyAppQuestionStringsKey =
    @"hockeyapp-alert-question";
static NSString *const kHockeyAppQuestionWithResponseFieldStringsKey =
    @"hockeyapp-alert-question-with-response-field";
static NSString *const kHockeyAppSendStringsKey =
    @"hockeyapp-alert-send-report";
...
- (void)wmf_setupAndStart {
  ...
  if ([[BITHockeyManager sharedHockeyManager] crashManager]
          .crashManagerStatus == BITCrashManagerStatusDisabled) {
  ...
  [[BITHockeyManager sharedHockeyManager] crashManager]
      .enableAppNotTerminatingCleanlyDetection = YES;
...
- (void)wmf_setupCrashNotificationAlert {
  [[BITHockeyManager sharedHockeyManager].crashManager setAlertViewHandler:^() {
    ...
    WMFCrashAlertView *customAlertView = [[WMFCrashAlertView alloc]
            initWithTitle:title
                  message:nil
                 delegate:self
        cancelButtonTitle:nil
        otherButtonTitles:[[self class] crashSendText],
                          [[self class] crashAlwaysSendText],
                          [[self class] crashDoNotSendText],
                          [WMFCrashAlertView wmf_hockeyAppPrivacyButtonText],
                          nil];
    NSString *exceptionReason = [[BITHockeyManager sharedHockeyManager]
                                        .crashManager lastSessionCrashDetails]
                                    .exceptionReason;
    if (exceptionReason) {
      customAlertView.message =
          [MWLocalizedString(kHockeyAppQuestionWithResponseFieldStringsKey, nil)
              stringByReplacingOccurrencesOfString:@"$1"
                                        withString:WMFHockeyAppServiceName];
      customAlertView.alertViewStyle = UIAlertViewStylePlainTextInput;
    } else {
      customAlertView.message =
          [MWLocalizedString(kHockeyAppQuestionStringsKey, nil)
              stringByReplacingOccurrencesOfString:@"$1"
                                        withString:WMFHockeyAppServiceName];
    }
