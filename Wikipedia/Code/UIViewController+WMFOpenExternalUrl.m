...
- (void)wmf_openExternalUrl:(NSURL *)url useSafari:(BOOL)useSafari {
  ...
  // If zero rated, don't open any external (non-zero rated!) links until user
  // consents!
  if ([SessionSingleton sharedInstance].zeroConfigState.disposition &&
      [[NSUserDefaults standardUserDefaults]
          boolForKey:@"ZeroWarnWhenLeaving"]) {
    WMFZeroMessage *zeroMessage =
        [SessionSingleton sharedInstance].zeroConfigState.zeroMessage;
    NSString *exitDialogTitle =
        zeroMessage.exitTitle
            ?: MWLocalizedString(@"zero-interstitial-title", nil);
    NSString *messageWithHost = [NSString
        stringWithFormat:@"%@\n\n%@",
                         zeroMessage.exitWarning
                             ?: MWLocalizedString(
                                    @"zero-interstitial-leave-app", nil),
                         url.host];

    UIAlertView *zeroAlert =
        [UIAlertView bk_alertViewWithTitle:exitDialogTitle
                                   message:messageWithHost];
    [zeroAlert bk_setCancelButtonWithTitle:MWLocalizedString(
                                               @"zero-interstitial-cancel", nil)
                                   handler:nil];
    [zeroAlert
        bk_addButtonWithTitle:MWLocalizedString(@"zero-interstitial-continue",
                                                nil)
                      handler:^{
                        [self wmf_openExternalUrlModallyIfNeeded:url
                                                     forceSafari:useSafari];
                      }];
    if ([self isPartnerInfoConfigValid:zeroMessage]) {
      ...
      [zeroAlert
          bk_addButtonWithTitle:partnerInfoText
                        handler:^{
                          [self
                              wmf_openExternalUrlModallyIfNeeded:partnerInfoUrl
                                                     forceSafari:useSafari];
                        }];
    }
...
- (void)wmf_presentExternalUrlWithinApp:(NSURL *)url {
  ...
  if (scheme.length == 0 || (![scheme isEqualToString:@"https"] &&
...
- (void)wmf_presentExternalUrlAsSVModal:(NSURL *)url {
  [self presentViewController:[[SVModalWebViewController alloc] initWithURL:url]
                     animated:YES
                   completion:nil];
}

- (void)wmf_presentExternalUrlAsSFSafari:(NSURL *)url {
  [self presentViewController:[[SFSafariViewController alloc] initWithURL:url]
                     animated:YES
                   completion:nil];
}
