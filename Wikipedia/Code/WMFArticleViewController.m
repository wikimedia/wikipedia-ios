...
static const CGFloat
    WMFArticleViewControllerExpandedTableOfContentsWidthPercentage = 0.33;
static const CGFloat WMFArticleViewControllerTableOfContentsSeparatorWidth = 1;
static const CGFloat
    WMFArticleViewControllerTableOfContentsSectionUpdateScrollDistance = 10;

@interface WMFArticleViewController () <
    UINavigationControllerDelegate,
    WMFImageGalleryViewControllerReferenceViewDelegate,
    SectionEditorViewControllerDelegate, UIViewControllerPreviewingDelegate,
...
@property(strong, nonatomic, nullable)
    WMFShareOptionsController *shareOptionsController;
...
// Table of Contents
@property(nonatomic, strong)
    UISwipeGestureRecognizer *tableOfContentsCloseGestureRecognizer;
...
@property(nonatomic, weak) id<UIViewControllerPreviewing>
    leadImagePreviewingContext;
...
- (WMFArticleFooterMenuViewController *)footerMenuViewController {
  if (!_footerMenuViewController && [self hasAboutThisArticle]) {
    self.footerMenuViewController =
        [[WMFArticleFooterMenuViewController alloc] initWithArticle:self.article
                                           similarPagesListDelegate:self];
  }
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"%@ %@", [super description], self.articleURL];
}

- (void)setArticle:(nullable MWKArticle *)article {
  NSAssert(self.isViewLoaded,
           @"Expecting article to only be set after the view loads.");
  NSAssert([article.url isEqual:[self.articleURL wmf_URLWithFragment:nil]],
           @"Invalid article set for VC expecting article data for title: %@",
           self.articleURL);
  ...
  // always update webVC & headerGallery, even if nil so they are reset if
  // needed
    ...
    if ([self.article.url wmf_isNonStandardURL]) {
      ...
      [self.headerImageView wmf_setImageWithMetadata:_article.leadImage
                                         detectFaces:YES
                                             ...
                                             success:WMFIgnoreSuccessHandler];
    }
    [self startSignificantlyViewedTimer];
    [self wmf_hideEmptyView];
    [NSUserActivity
        wmf_makeActivityActive:
            [NSUserActivity wmf_articleViewActivityWithArticle:self.article]];
  }
...
- (nullable WMFShareOptionsController *)shareOptionsController {
  ...
  if (!_shareOptionsController) {
    _shareOptionsController =
        [[WMFShareOptionsController alloc] initWithArticle:self.article
...
- (UIView *)headerView {
  if (!_headerView) {
    ...
    UIView *headerBorderView = [[UIView alloc]
        initWithFrame:CGRectMake(0, height - borderHeight, 1, borderHeight)];
    headerBorderView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.2];
    headerBorderView.autoresizingMask =
...
- (WMFReadMoreViewController *)readMoreListViewController {
  if (!_readMoreListViewController) {
    _readMoreListViewController =
        [[WMFReadMoreViewController alloc] initWithURL:self.articleURL
...
- (WMFArticleFetcher *)articleFetcher {
  if (!_articleFetcher) {
    _articleFetcher =
        [[WMFArticleFetcher alloc] initWithDataStore:self.dataStore];
  }
...
- (WebViewController *)webViewController {
  if (!_webViewController) {
    _webViewController =
        [WebViewController wmf_initialViewControllerFromClassStoryboard];
...
- (BOOL)canFindInPage {
  // The presentedViewController check is to prevent the find-in-page icon from
  // responding if
...
- (void)getShareText:(void (^)(NSString *text))completion {
  [self.webViewController.webView
      wmf_getSelectedText:^(NSString *_Nonnull text) {
        if (text.length == 0) {
...
- (NSArray<UIBarButtonItem *> *)articleToolBarItems {
  ...
  for (NSArray *itemGroup in itemGroups) {
    ...
    default:
      [articleToolbarItems
          addObject:[UIBarButtonItem flexibleSpaceToolbarItem]];
      break;
    }
  }
  ...
  case WMFTableOfContentsDisplayModeModal: {
    [articleToolbarItems
        addObject:[UIBarButtonItem wmf_barButtonItemOfFixedWidth:8]];
  ...
  default: {
    ...
    [articleToolbarItems
        insertObject:[UIBarButtonItem wmf_barButtonItemOfFixedWidth:8]
             atIndex:1];
    [articleToolbarItems insertObject:[UIBarButtonItem flexibleSpaceToolbarItem]
                              atIndex:2];
    [articleToolbarItems addObject:[UIBarButtonItem flexibleSpaceToolbarItem]];
    [articleToolbarItems
        addObject:[UIBarButtonItem
                      wmf_barButtonItemOfFixedWidth:tocItem.width + 8]];
...
- (void)updateToolbarItemsIfNeeded {
  if (!self.saveButtonController) {
    self.saveButtonController = [[WMFSaveButtonController alloc]
        initWithBarButtonItem:self.saveToolbarItem
  ...
  if (![self.toolbarItems isEqualToArray:toolbarItems]) {
    // HAX: only update toolbar if # of items has changed, otherwise items will
    // (somehow) get lost
...
- (UIBarButtonItem *)showTableOfContentsToolbarItem {
  if (!_showTableOfContentsToolbarItem) {
    _showTableOfContentsToolbarItem =
        [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"toc"]
                                         style:UIBarButtonItemStylePlain
                                        target:self
                                        action:@selector(showTableOfContents)];
    _showTableOfContentsToolbarItem.accessibilityLabel =
        MWLocalizedString(@"table-of-contents-button-label", nil);
...
- (UIBarButtonItem *)hideTableOfContentsToolbarItem {
  if (!_hideTableOfContentsToolbarItem) {
    ...
    closeButton.frame =
        (CGRect){.origin = CGPointZero, .size = closeImage.size};
    _hideTableOfContentsToolbarItem =
        [[UIBarButtonItem alloc] initWithCustomView:closeButton];
    _hideTableOfContentsToolbarItem.accessibilityLabel =
        MWLocalizedString(@"table-of-contents-button-label", nil);
...
- (UIBarButtonItem *)saveToolbarItem {
  if (!_saveToolbarItem) {
    _saveToolbarItem =
        [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"save"]
                                         style:UIBarButtonItemStylePlain
                                        target:nil
                                        action:nil];
  }
...
- (UIBarButtonItem *)fontSizeToolbarItem {
  if (!_fontSizeToolbarItem) {
    @weakify(self);
    _fontSizeToolbarItem = [[UIBarButtonItem alloc]
        bk_initWithImage:[UIImage imageNamed:@"font-size"]
                   style:UIBarButtonItemStylePlain
                 handler:^(id sender) {
                   @strongify(self);
...
- (UIBarButtonItem *)shareToolbarItem {
  if (!_shareToolbarItem) {
    @weakify(self);
    _shareToolbarItem = [[UIBarButtonItem alloc]
        bk_initWithBarButtonSystemItem:UIBarButtonSystemItemAction
                               handler:^(id sender) {
                                 @strongify(self);
                                 [self shareArticleWithTextSnippet:nil
                                                        fromButton:
                                                            self->
                                                            _shareToolbarItem];
                               }];
...
- (UIBarButtonItem *)findInPageToolbarItem {
  if (!_findInPageToolbarItem) {
    @weakify(self);
    _findInPageToolbarItem = [[UIBarButtonItem alloc]
        bk_initWithImage:[UIImage imageNamed:@"find-in-page"]
                   style:UIBarButtonItemStylePlain
                 handler:^(id sender) {
                   @strongify(self);
                   if ([self canFindInPage]) { // Needed so you can't tap find
                                               // icon when text size adjuster
                                               // is onscreen.
                     [self showFindInPage];
                   }
                 }];
...
- (UIBarButtonItem *)languagesToolbarItem {
  if (!_languagesToolbarItem) {
    _languagesToolbarItem =
        [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"language"]
                                         style:UIBarButtonItemStylePlain
                                        target:self
...
- (void)showLanguagePicker {
  WMFArticleLanguagesViewController *languagesVC =
      [WMFArticleLanguagesViewController
          articleLanguagesViewControllerWithArticleURL:self.articleURL];
  languagesVC.delegate = self;
  [self presentViewController:[[UINavigationController alloc]
                                  initWithRootViewController:languagesVC]
                     animated:YES
                   completion:nil];
}
...
- (void)languagesController:(WMFLanguagesViewController *)controller
  ...
  [[PiwikTracker wmf_configuredInstance]
      wmf_logActionSwitchLanguageInContext:self
                               contentType:nil];
  [self
      dismissViewControllerAnimated:YES
                         completion:^{
                           [self
                               pushArticleViewControllerWithURL:language
                                                                    .articleURL
                                                    contentType:nil
                                                       animated:YES];
                         }];
...
- (void)updateTableOfContentsForFootersIfNeeded {
  ...
  BOOL includeReadMore =
      [self hasReadMore] && [self.readMoreListViewController hasResults];

  [self appendItemsToTableOfContentsIncludingAboutThisArticle:
            [self hasAboutThisArticle]
...
- (void)updateWebviewFootersIfNeeded {
  ...
  /*
     NOTE: only include read more if it has results (don't want an empty
     section). conditionally fetched in `setArticle:`
   */
...
- (void)addProgressView {
  NSAssert(!self.progressView.superview,
           @"Illegal attempt to re-add progress view.");
...
- (void)updateProgress:(CGFloat)progress animated:(BOOL)animated {
  ...
  [self.delegate articleController:self
      didUpdateArticleLoadProgress:progress
                          animated:animated];
}

- (void)completeAndHideProgressWithCompletion:
...
- (void)startSignificantlyViewedTimer {
  ...
  if (!entry.titleWasSignificantlyViewed) {
    self.significantlyViewedTimer = [NSTimer
        scheduledTimerWithTimeInterval:FBTweakValue(
                                           @"Explore", @"Related items",
                                           @"Required viewing time", 30.0)
                                target:self
                              selector:@selector(significantlyViewedTimerFired:)
                              userInfo:nil
                               repeats:NO];
  }
}
...
- (void)setUpTitleBarButton {
  ...
  self.navigationItem.titleView.accessibilityLabel =
      MWLocalizedString(@"home-button-accessibility-label", nil);
  self.navigationItem.titleView.accessibilityTraits |=
...
- (void)viewDidLoad {
  ...
  [self
      updateTableOfContentsDisplayModeWithTraitCollection:self.traitCollection];

  self.tableOfContentsDisplayState =
      self.tableOfContentsDisplayMode == WMFTableOfContentsDisplayModeInline
          ? WMFTableOfContentsDisplayStateInlineVisible
          : WMFTableOfContentsDisplayStateModalHidden;

  [self.navigationController
          .toolbar wmf_applySolidWhiteBackgroundWithTopShadow];
...
- (void)traitCollectionDidChange:
  ...
  if ([self.presentedViewController
          isKindOfClass:[WMFFontSliderViewController class]]) {
...
- (void)layoutForSize:(CGSize)size {
  BOOL isTOCVisible = self.tableOfContentsDisplayState ==
                      WMFTableOfContentsDisplayStateInlineVisible;

  CGFloat separatorWidth =
      WMFArticleViewControllerTableOfContentsSeparatorWidth;
  CGFloat tocWidth =
      round(size.width *
            WMFArticleViewControllerExpandedTableOfContentsWidthPercentage);
  CGFloat webFrameWidth =
      size.width - (isTOCVisible ? (separatorWidth + tocWidth) : 0);
  ...
  case WMFTableOfContentsDisplaySideRight:
    tocOriginX = isTOCVisible ? webFrameWidth + separatorWidth
  ...
  if (self.tableOfContentsDisplayMode != WMFTableOfContentsDisplayModeModal) {
    self.tableOfContentsViewController.view.frame =
        CGRectMake(tocOriginX, origin.y, tocWidth, size.height);
    self.tableOfContentsSeparatorView.frame =
        CGRectMake(separatorOriginX, origin.y, separatorWidth, size.height);
    self.tableOfContentsViewController.view.alpha = isTOCVisible ? 1 : 0;
    self.tableOfContentsSeparatorView.alpha = isTOCVisible ? 1 : 0;
  }

  CGRect webFrame =
      CGRectMake(webFrameOriginX, origin.y, webFrameWidth, size.height);
...
- (void)updateTableOfContentsDisplayModeWithTraitCollection:
    (UITraitCollection *)traitCollection {
  self.tableOfContentsDisplaySide =
      [[UIApplication sharedApplication] wmf_tocShouldBeOnLeft]
          ? WMFTableOfContentsDisplaySideRight
          : WMFTableOfContentsDisplaySideLeft; // inverse of the modal ToC
  self.tableOfContentsDisplayMode =
...
- (void)showTableOfContents {
  ...
  default: {
    ...
    [self presentViewController:self.tableOfContentsViewController
                       animated:YES
                     completion:NULL];
  } break;
  }
  ...
  case WMFTableOfContentsDisplayModeInline: {
    if (self.tableOfContentsViewController.parentViewController != self) {
      ...
      if (self.tableOfContentsSeparatorView == nil) {
        ...
        self.tableOfContentsSeparatorView.backgroundColor =
            [UIColor wmf_lightGrayColor];
      }

      [self createTableOfContentsViewControllerIfNeeded];
      [self addChildViewController:self.tableOfContentsViewController];
      [self.view insertSubview:self.tableOfContentsViewController.view
                  aboveSubview:self.webViewController.view];
      [self.tableOfContentsViewController didMoveToParentViewController:self];

      [self.view insertSubview:self.tableOfContentsSeparatorView
                  aboveSubview:self.tableOfContentsViewController.view];

      self.tableOfContentsCloseGestureRecognizer =
          [[UISwipeGestureRecognizer alloc]
              initWithTarget:self
                      action:@selector(handleTableOfContentsCloseGesture:)];
      ...
      [self.tableOfContentsViewController.view
          addGestureRecognizer:self.tableOfContentsCloseGestureRecognizer];
    }
  } break;
  default:
  case WMFTableOfContentsDisplayModeModal: {
...
- (void)handleTableOfContentsCloseGesture:
    (UISwipeGestureRecognizer *)recognizer {
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    if (self.tableOfContentsDisplayState ==
        WMFTableOfContentsDisplayStateInlineVisible) {
...
- (void)saveWebViewScrollOffset {
  ...
  if (offset > 0) {
    [self.recentPages setPageScrollPosition:offset
                     onPageInHistoryWithURL:self.articleURL];
...
- (void)fetchArticleForce:(BOOL)force {
  ...
  NSAssert(self.isViewLoaded, @"Should only fetch article when view is loaded "
                              @"so we can update its state.");
  ...
  @weakify(self);
  self.articleFetcherPromise =
      [self.articleFetcher
          fetchLatestVersionOfArticleWithURL:self.articleURL
                               forceDownload:force
                                    progress:^(CGFloat progress) {
                                      [self
                                          updateProgress:
                                              [self
                                                  totalProgressWithArticleFetcherProgress:
                                                      progress]
                                                animated:YES];
                                    }]
          .then(^(MWKArticle *article) {
            ...
            [self
                updateProgress:[self
                                   totalProgressWithArticleFetcherProgress:1.0]
                      animated:YES];
            self.article = article;
            /*
               NOTE(bgerstle): add side effects to setArticle, not here. this
               ensures they happen even when falling back to
               cached content
             */
          })
          .catch(^(NSError *error) {
            @strongify(self);
            DDLogError(@"Article Fetch Error: %@",
                       [error localizedDescription]);
            [self.pullToRefresh endRefreshing];
            [self hideProgressViewAnimated:YES];
            [self.delegate articleControllerDidLoadArticle:self];

            MWKArticle *cachedFallback =
                error.userInfo[WMFArticleFetcherErrorCachedFallbackArticleKey];
            if (cachedFallback) {
...
- (void)fetchReadMoreIfNeeded {
  ...
  @weakify(self);
  [self.readMoreListViewController fetchIfNeeded]
      .then(^{
        @strongify(self);
        if (!self) {
          return;
...
- (void)shareAFactWithTextSnippet:(nullable NSString *)text {
  ...
  [self.shareOptionsController
      presentShareOptionsWithSnippet:text
                    inViewController:self
                   fromBarButtonItem:self.shareToolbarItem];
}
...
- (void)shareArticleWithTextSnippet:(nullable NSString *)text
  ...
  [items addObject:[[WMFArticleTextActivitySource alloc]
                       initWithArticle:self.article
                             shareText:text]];
  ...
  if (url) {
    url = [[NSURL alloc]
        initWithString:[NSString stringWithFormat:@"%@?%@", url.absoluteString,
                                                  @"wprov=sfsi1"]];
...
- (void)showFontSizePopup {
  ...
  WMFFontSliderViewController *vc = [[WMFFontSliderViewController alloc]
      initWithNibName:@"WMFFontSliderViewController"
               bundle:nil];
...
- (void)sliderValueChangedInController:(WMFFontSliderViewController *)container
  ...
  [self.webViewController
      setFontSizeMultiplier:self.fontSizeMultipliers[value]];
}

- (NSArray<NSNumber *> *)fontSizeMultipliers {
  return @[
    @(FBTweakValue(@"Article", @"Font Size", @"Step 1", 70)),
...
- (NSUInteger)indexOfCurrentFontSize {
  NSNumber *fontSize =
      [[NSUserDefaults standardUserDefaults] wmf_readingFontSize];

  NSUInteger index = [[self fontSizeMultipliers] indexOfObject:fontSize];

  if (index == NSNotFound) {
    index = [[[self fontSizeMultipliers]
        bk_reduce:@(NSIntegerMax)
        withBlock:^id(NSNumber *current, NSNumber *obj) {
          NSUInteger currentDistance = current.integerValue;
          NSUInteger objDistance =
              abs((int)(obj.integerValue - fontSize.integerValue));
...
- (void)webViewController:(WebViewController *)controller
  ...
  MWKImage *selectedImage =
      [[MWKImage alloc] initWithArticle:self.article sourceURL:imageSourceURL];
  WMFArticleImageGalleryViewController *fullscreenGallery =
      [[WMFArticleImageGalleryViewController alloc]
          initWithArticle:self.article
...
- (void)webViewController:(WebViewController *)controller
           didLoadArticle:(MWKArticle *)article {
  ...
  if (self.tableOfContentsDisplayMode != WMFTableOfContentsDisplayModeModal) {
    [self setupTableOfContentsViewController];
    [self layoutForSize:self.view.bounds.size];
    [self.tableOfContentsViewController selectAndScrollToItemAtIndex:0
                                                            animated:NO];
  }
...
- (void)webViewController:(WebViewController *)controller
     didTapEditForSection:(MWKSection *)section {
  [self showEditorForSection:section];
}

- (void)webViewController:(WebViewController *)controller
    didTapOnLinkForArticleURL:(NSURL *)url {
...
- (void)webViewController:(WebViewController *)controller
    didTapShareWithSelectedText:(NSString *)text {
  [self shareAFactWithTextSnippet:text];
}

- (nullable NSString *)webViewController:(WebViewController *)controller
            titleForFooterViewController:
                (UIViewController *)footerViewController {
  if (footerViewController == self.readMoreListViewController) {
    return
        [MWSiteLocalizedString(self.articleURL, @"article-read-more-title", nil)
            uppercaseStringWithLocale:[NSLocale currentLocale]];
  } else if (footerViewController == self.footerMenuViewController) {
    return [MWSiteLocalizedString(self.articleURL, @"article-about-title", nil)
        uppercaseStringWithLocale:[NSLocale currentLocale]];
  }
  return nil;
}

- (void)updateTableOfContentsHighlightWithScrollView:
    (UIScrollView *)scrollView {
  [self.webViewController getCurrentVisibleSectionCompletion:^(
                              MWKSection *_Nullable section,
                              NSError *_Nullable error) {
    if (section) {
      [self selectAndScrollToTableOfContentsItemForSection:section
                                                  animated:YES];
    } else {
      NSInteger visibleFooterIndex = self.webViewController.visibleFooterIndex;
      if (visibleFooterIndex != NSNotFound) {
        [self
            selectAndScrollToTableOfContentsFooterItemAtIndex:visibleFooterIndex
                                                     animated:YES];
      }
    }
  }];
...
- (void)webViewController:(WebViewController *)controller
  ...
  if (self.isUpdateTableOfContentsSectionOnScrollEnabled &&
      ...
      ABS(self.previousContentOffsetYForTOCUpdate -
          scrollView.contentOffset.y) >
          WMFArticleViewControllerTableOfContentsSectionUpdateScrollDistance) {
...
- (void)imageViewDidTap:(UITapGestureRecognizer *)tap {
  NSAssert(self.article.isCached,
           @"Expected article data to already be downloaded.");
...
- (UIImageView *)referenceViewForImageController:
    (WMFArticleImageGalleryViewController *)controller {
...
- (void)showEditorForSection:(MWKSection *)section {
  if (self.article.editable) {
    SectionEditorViewController *sectionEditVC = [SectionEditorViewController
        wmf_initialViewControllerFromClassStoryboard];
    sectionEditVC.section = section;
    sectionEditVC.delegate = self;
    UINavigationController *nc = [[UINavigationController alloc]
        initWithRootViewController:sectionEditVC];
    [self presentViewController:nc animated:YES completion:NULL];
  } else {
    ProtectedEditAttemptFunnel *funnel =
        [[ProtectedEditAttemptFunnel alloc] init];
    [funnel logProtectionStatus:[[self.article.protection
                                    allowedGroupsForAction:@"edit"]
...
- (void)registerForPreviewingIfAvailable {
  [self wmf_ifForceTouchAvailable:^{
    NSAssert(!self.webViewController.webView.allowsLinkPreview,
             @"WKWebView's built-in link preview forces Safari to open as of "
             @"iOS 9.x. Do not enable.");
    [self unregisterForPreviewing];
    ...
    self.leadImagePreviewingContext = [self
        registerForPreviewingWithDelegate:self
                               sourceView:self.webViewController.headerView];

    for (UIGestureRecognizer *r in previewView.gestureRecognizers) {
      if ([NSStringFromClass([r class])
              isEqualToString:@"_UIPreviewGestureRecognizer"]) {
        [r requireGestureRecognizerToFail:
                self.linkPreviewingContext
...
viewControllerForLocation:(CGPoint)location {
  if (previewingContext == self.linkPreviewingContext) {
    UIViewController *peekVC = [self
        peekViewControllerForPeekElement:self.webViewController.peekElement];
    if (peekVC) {
      [[PiwikTracker wmf_configuredInstance] wmf_logActionPreviewInContext:self
                                                               contentType:nil];
...
- (nullable UIViewController *)peekViewControllerForPeekElement:
    (WMFPeekHTMLElement *)peekElement {
...
- (nullable UIViewController *)viewControllerForImageURL:(nullable NSURL *)url {
  ...
  MWKImage *selectedImage =
      [[MWKImage alloc] initWithArticle:self.article sourceURL:url];
...
- (void)previewingContext:(id<UIViewControllerPreviewing>)previewingContext
  ...
  if ([viewControllerToCommit isKindOfClass:[WMFArticleViewController class]]) {
    [self pushArticleViewController:(WMFArticleViewController *)
...
- (void)
pushArticleViewController:(WMFArticleViewController *)articleViewController
              contentType:
                  (nullable id<WMFAnalyticsContentTypeProviding>)contentType
                 animated:(BOOL)animated {
  [[PiwikTracker wmf_configuredInstance]
      wmf_logActionTapThroughInContext:self
                           contentType:contentType];
  [self wmf_pushArticleViewController:articleViewController animated:YES];
}

- (void)pushArticleViewControllerWithURL:(NSURL *)url
                             contentType:
                                 (nullable id<WMFAnalyticsContentTypeProviding>)
                                     contentType
                                animated:(BOOL)animated {
  WMFArticleViewController *articleViewController =
      [[WMFArticleViewController alloc] initWithArticleURL:url
                                                 dataStore:self.dataStore];
  [self pushArticleViewController:articleViewController
                      contentType:contentType
                         animated:animated];
}
...
- (void)listViewController:(WMFArticleListTableViewController *)listController
       didSelectArticleURL:(NSURL *)url {
  if ([self presentedViewController]) {
    [self dismissViewControllerAnimated:YES completion:NULL];
  }
  id<WMFAnalyticsContentTypeProviding> contentType = nil;
  if ([listController
          conformsToProtocol:@protocol(WMFAnalyticsContentTypeProviding)]) {
    contentType = (id<WMFAnalyticsContentTypeProviding>)listController;
  }
  [self pushArticleViewControllerWithURL:url
                             contentType:contentType
                                animated:YES];
}

- (UIViewController *)listViewController:
                          (WMFArticleListTableViewController *)listController
    viewControllerForPreviewingArticleURL:(NSURL *)url {
  return [[WMFArticleViewController alloc] initWithArticleURL:url
                                                    dataStore:self.dataStore];
}

- (void)listViewController:(WMFArticleListTableViewController *)listController
    didCommitToPreviewedViewController:(UIViewController *)viewController {
  if ([self presentedViewController]) {
    [self dismissViewControllerAnimated:YES completion:NULL];
  }
  if ([viewController isKindOfClass:[WMFArticleViewController class]]) {
    ...
    [self pushArticleViewController:(WMFArticleViewController *)viewController
                        contentType:contentType
                           animated:YES];
  } else {
