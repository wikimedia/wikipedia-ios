 ...
 *
 *  @return The URL returned needs to be as close to equal to the targetWidth as
 * possible. Because the image scaler will not scale raster images above their
 * canonical resolution the URL returned here needs to sometimes be modified to
 * just use the canonical URL.
 */
- (NSURL *)URLForTargetWidth:(NSUInteger)targetWidth {
  ...
  if (sizeFromSrcUrl != NSNotFound) {
    ...
    NSNumber *safeCanonicalWidthAssumption =
        self.dataFileWidth ? self.dataFileWidth : self.width;
    NSString *canonicalExtension = [[[tagSrc stringByDeletingLastPathComponent]
    ...
    NSUInteger width =
        MAX(sizeFromSrcUrl, safeCanonicalWidthAssumption.integerValue);
    width = MIN(width, targetWidth);

    if (width == self.dataFileWidth.integerValue) {
      // The scaler wont scale raster images greater *or equal* to their
      // canonical width.
      // So we need to get rid of "/thumb/" path component and "px-" size prefix
      // last
      // path component, but only if we're actually dealing with a "thumb"
      // scaled variant.
      // This check for "/thumb/" is needed in case the canonical image name
      // happens to
      // start with "XXXpx-", as the last image on "enwiki > Geothermal
      // gradient" does.
