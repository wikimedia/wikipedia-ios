 ...
 *
 * As of iOS 8, the system font doesn't support these languages, e.g. "arc"
 * (Aramaic, Syriac font). [0]
 *
...
- (void)loadLanguagesFromFile {
  WMFAssetsFile *assetsFile =
      [[WMFAssetsFile alloc] initWithFileType:WMFAssetsFileTypeLanguages];
  ...
  self.allLanguages = [assetsFile.array bk_map:^id(NSDictionary *langAsset) {
    ...
    if (![self isCompoundLanguageCode:code]) {
      // iOS will return less descriptive name for compound codes - ie "Chinese"
      // for zh-yue which
      // should be "Cantonese". It looks like iOS ignores anything after the
      // "-".
      NSString *iOSLocalizedName =
          [[NSLocale currentLocale] wmf_localizedLanguageNameForCode:code];
      if (iOSLocalizedName) {
...
- (void)setAllLanguages:(NSArray *)allLanguages {
  ...
  NSArray *supportedLanguageLinks =
      [allLanguages bk_reject:^BOOL(MWKLanguageLink *languageLink) {
        return [unsupportedLanguages containsObject:languageLink.languageCode];
      }];

  supportedLanguageLinks =
      [supportedLanguageLinks sortedArrayUsingSelector:@selector(compare:)];
...
- (void)updateLanguageArrays {
  ...
  self.preferredLanguages =
      [preferredLanguageCodes wmf_mapAndRejectNil:^id(NSString *langString) {
        return [self.allLanguages bk_match:^BOOL(MWKLanguageLink *langLink) {
          return [langLink.languageCode isEqualToString:langString];
        }];
      }];
  self.otherLanguages =
      [self.allLanguages bk_select:^BOOL(MWKLanguageLink *langLink) {
...
- (void)addPreferredLanguage:(MWKLanguageLink *)language {
  ...
  NSMutableArray<NSString *> *langCodes =
      [[self readPreferredLanguageCodes] mutableCopy];
...
- (void)reorderPreferredLanguage:(MWKLanguageLink *)language
...
- (NSArray<NSString *> *)readOSPreferredLanguageCodes {
  NSArray<NSString *> *osLanguages = [[NSLocale preferredLanguages]
      wmf_mapAndRejectNil:^NSString *(NSString *languageCode) {
        NSLocale *locale = [NSLocale localeWithLocaleIdentifier:languageCode];
        // use language code when determining if a langauge is preferred (e.g.
        // "en_US" is preferred if "en" was selected)
...
- (NSArray<NSString *> *)readPreferredLanguageCodes {
  NSMutableArray<NSString *> *preferredLanguages = [
      [self readPreferredLanguageCodesWithoutOSPreferredLanguages] mutableCopy];
  NSArray<NSString *> *osLanguages = [self readOSPreferredLanguageCodes];

  if (preferredLanguages.count == 0) {
    [osLanguages
        enumerateObjectsWithOptions:NSEnumerationReverse
                         usingBlock:^(id _Nonnull obj, NSUInteger idx,
                                      BOOL *_Nonnull stop) {
...
- (void)savePreferredLanguageCodes:(NSArray<NSString *> *)languageCodes {
  [[NSUserDefaults standardUserDefaults] setObject:languageCodes
                                            forKey:WMFPreviousLanguagesKey];
  [[NSUserDefaults standardUserDefaults] synchronize];
  [self updateLanguageArrays];
  [[NSNotificationCenter defaultCenter]
      postNotificationName:WMFPreferredLanguagesDidChangeNotification
                    object:self];
}
