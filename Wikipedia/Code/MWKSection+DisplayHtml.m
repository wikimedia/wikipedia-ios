...
- (NSString *)displayHTML {
  ...
  @try {
    ...
    NSAssert(html, @"html was not created from section %@: %@", self.url,
             self.text);
  }
  ...
  return [NSString
      stringWithFormat:@"<div id='section_heading_and_content_block_%d'>%@<div "
                       @"id='content_block_%d' "
                       @"class='content_block'>%@%@%@</div></div>",
                       self.sectionId,
                       (isMainPage ? @"" : [self getHeaderTag]),
                       self.sectionId, (([self isLeadSection]) && !isMainPage)
                                           ? @"<hr id='content_block_0_hr'>"
                                           : @"",
                       (([self isLeadSection]) && !isMainPage)
  ...
  if ([self isLeadSection]) {
    return [NSString
        stringWithFormat:
            ...
            self.anchorAsElementId, self.sectionId,
            self.article.displaytitle ? self.article.displaytitle
                                      : self.url.wmf_title,
    ...
    return [NSString
        stringWithFormat:
            ...
            headingTagSize, self.sectionId, self.anchor, self.line,
            [self getEditPencilAnchor], headingTagSize];
  }
}

- (NSString *)anchorAsElementId {
  return self.anchor.length > 0
             ? [NSString stringWithFormat:@"id='%@'", self.anchor]
             : @"";
}
...
- (NSString *)articleEntityDescriptionAsParagraph {
  if (self.article.entityDescription.length == 0) {
    ...
    return [NSString
        stringWithFormat:@"<p id='entity_description'>%@</p>",
...
- (NSString *)getEditPencilAnchor {
  return [NSString stringWithFormat:@"<a class='edit_section_button' "
                                    @"data-action='edit_section' data-id='%d' "
                                    @"id='edit_section_button_%d'></a>",
                                    self.sectionId, self.sectionId];
}

- (NSString *)getHTMLWrappedInTablesIfNeeded {
  NSString *tableFormatString =
      @"<table><th>%@</th><tr><td>%@</td></tr></table>";
  NSArray *titlesToWrap = @[
  ...
  for (NSString *sectionTitle in titlesToWrap) {
    if ([self.line isEqualToString:sectionTitle]) {
      return [NSString
          stringWithFormat:tableFormatString, sectionTitle, self.text];
    }
  }
  return self.text;
}
