...
@interface WMFEnglishFeaturedTitleResponseSerializer
...
+ (AFHTTPSessionManager *)featuredTitleOperationManager {
  ...
  featuredTitleOperationManager.requestSerializer =
      [WMFEnglishFeaturedTitleRequestSerializer serializer];
  featuredTitleOperationManager.responseSerializer =
      [WMFEnglishFeaturedTitleResponseSerializer serializer];
...
+ (AFHTTPSessionManager *)titlePreviewOperationManager {
  AFHTTPSessionManager *titlePreviewOperationManager =
      [AFHTTPSessionManager wmf_createDefaultManager];
  titlePreviewOperationManager.requestSerializer =
      [WMFTitlePreviewRequestSerializer serializer];
  titlePreviewOperationManager.responseSerializer =
      [WMFMantleJSONResponseSerializer
          serializerForValuesInDictionaryOfType:[MWKSearchResult class]
...
- (instancetype)init {
  self = [super init];
  if (self) {
    self.featuredTitleOperationManager =
        [WMFEnglishFeaturedTitleFetcher featuredTitleOperationManager];
    self.titlePreviewOperationManager =
        [WMFEnglishFeaturedTitleFetcher titlePreviewOperationManager];
  }
  return self;
}
...
- (BOOL)isFetching {
  return
      [[self.featuredTitleOperationManager operationQueue] operationCount] >
          0 ||
      [[self.titlePreviewOperationManager operationQueue] operationCount] > 0;
}

- (AnyPromise *)fetchFeaturedArticlePreviewForDate:(NSDate *)date {
  @weakify(self);
  NSURL *siteURL = [NSURL wmf_URLWithDefaultSiteAndlanguage:@"en"];
  return [self.featuredTitleOperationManager wmf_GETAndRetryWithURL:siteURL
                                                         parameters:date]
      .thenInBackground(^(NSString *title) {
        ...
        return [self.titlePreviewOperationManager wmf_GETAndRetryWithURL:siteURL
                                                              parameters:title]
...
+ (NSString *)titleForDate:(NSDate *)date {
  ...
  return [@[
    tfaTitleTemplatePrefix, @"/",
    [[self featuredArticleDateFormatter] stringFromDate:date]
...
requestBySerializingRequest:(NSURLRequest *)request
  ...
  return [super
      requestBySerializingRequest:request
                   withParameters:@{
                     @"action" : @"query",
                     @"format" : @"json",
                     @"titles" : [WMFEnglishFeaturedTitleRequestSerializer
                         titleForDate:date],
                     // extracts
                     @"prop" : @"extracts",
                     @"exchars" : @100,
...
- (nullable id)responseObjectForResponse:(nullable NSURLResponse *)response
  ...
  NSDictionary *feedItemPageObj =
      [[json[@"query"][@"pages"] allValues] firstObject];
  NSString *title = [WMFEnglishFeaturedTitleResponseSerializer
      titleFromFeedItemExtract:feedItemPageObj[@"extract"]];

  if (title.length == 0) {
    DDLogError(@"Empty extract for feed item request %@", response.URL);
    NSError *error = [NSError
        wmf_errorWithType:WMFErrorTypeStringLength
...
requestBySerializingRequest:(NSURLRequest *)request
  ...
  NSMutableDictionary *baseParams =
      [NSMutableDictionary wmf_titlePreviewRequestParameters];
  baseParams[@"titles"] = title;
  return [super requestBySerializingRequest:request
                             withParameters:baseParams
                                      error:error];
}
