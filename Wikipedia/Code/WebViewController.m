...
@interface WebViewController () <ReferencesVCDelegate, WKScriptMessageHandler,
                                 ...
                                 WMFFindInPageKeyboardBarDelegate>
...
- (void)userContentController:(WKUserContentController *)userContentController
  ...
  WMFWKScriptMessageType messageType =
      [WKScriptMessage wmf_typeForMessageName:message.name];
...
- (void)handlePeekScriptMessage:(NSDictionary *)messageDict {
  if (messageDict[@"tagName"]) {
    self.peekElement =
        [[WMFPeekHTMLElement alloc] initWithTagName:messageDict[@"tagName"]
                                                src:messageDict[@"src"]
...
- (void)handleClickLinkScriptMessage:(NSDictionary *)messageDict {
  [self hideFindInPageWithCompletion:^{
    ...
    if ([href wmf_isWikiResource]) {
      ...
      if (!url.wmf_domain) {
        url = [NSURL wmf_URLWithSiteURL:self.article.url
            escapedDenormalizedInternalLink:href];
      }
...
- (void)handleClickImageScriptMessage:(NSDictionary *)messageDict {
  WMFImageTag *imageTagClicked =
      [[WMFImageTag alloc] initWithSrc:messageDict[@"src"]
                                srcset:nil
                                   alt:nil
  ...
  if (imageTagClicked == nil) {
    // yes, this would have caught in the if below, but keeping this here in
    // case that check ever goes away
    return;
  }
  ...
  if (!selectedImageSrcURLString.length) {
    DDLogError(@"Image clicked callback invoked with empty src url: %@",
               messageDict);
    return;
  }
  ...
  [self.delegate webViewController:self
          didTapImageWithSourceURL:selectedImageURL];
}
...
- (void)handleClickEditScriptMessage:(NSDictionary *)messageDict {
  [self hideFindInPageWithCompletion:^{
    NSUInteger sectionIndex =
        (NSUInteger)[messageDict[@"sectionId"] integerValue];
    if (sectionIndex < [self.article.sections count]) {
      [self.delegate webViewController:self
                  didTapEditForSection:self.article.sections[sectionIndex]];
    }
  }];
}
...
- (void)handleLateJavascriptTransformScriptMessage:(NSString *)messageString {
  if ([messageString isEqualToString:@"collapseTables"]) {
    ...
    [self.webView
        wmf_setLanguage:[MWLanguageInfo
                            languageInfoForCode:self.article.url.wmf_language]];
  } else if ([messageString isEqualToString:@"setPageProtected"] &&
             !self.article.editable) {
...
- (void)handleArticleStateScriptMessage:(NSString *)messageString {
  if ([messageString isEqualToString:@"articleLoaded"]) {
    ...
    NSAssert(self.article,
             @"Article not set - may need to use the old 0.1 second delay...");
...
- (void)resetFindInPageWithCompletion:(nullable dispatch_block_t)completion {
  [self.webView
      evaluateJavaScript:@"window.wmf.findInPage.removeSearchTermHighlights()"
...
- (void)updateWebContentMarginForSize:(CGSize)size {
  NSString *jsFormat = @"document.body.style.paddingLeft='%ipx';document.body."
                       @"style.paddingRight='%ipx';";
...
- (void)updateFindInPageKeyboardBar {
  [[self findInPageKeyboardBar]
      updateForCurrentMatchIndex:self.findInPageSelectedMatchIndex
                    matchesCount:self.findInPageMatches.count];
}

#pragma FindInPageBar selected match

- (void)setFindInPageSelectedMatchIndex:
    (NSInteger)findInPageSelectedMatchIndex {
  _findInPageSelectedMatchIndex = findInPageSelectedMatchIndex;
  [self updateFindInPageKeyboardBar];
}

- (void)moveFindInPageSelectedMatchIndexInDirection:
    (WMFFindInPageScrollDirection)direction {
...
- (void)scrollToAndFocusOnSelectedMatch {
  ...
  NSString *matchSpanId = [self.findInPageMatches
      wmf_safeObjectAtIndex:self.findInPageSelectedMatchIndex];
  if (matchSpanId == nil) {
    return;
  }
  @weakify(self);
  [self.webView
      getScrollViewRectForHtmlElementWithId:matchSpanId
                                 completion:^(CGRect rect) {
                                   ...
                                   [UIView animateWithDuration:0.3
                                       ...
                                       animations:^{
                                         ...
                                         // TODO: modified to scroll the match
                                         // to the vertical point between top of
                                         // keyboard and top of screen

                                         [self.webView.scrollView
                                             wmf_safeSetContentOffset:
                                                 CGPointMake(
                                                     self.webView.scrollView
                                                         .contentOffset.x,
                                                     fmaxf(rect.origin.y - 80.f,
                                                           0.f))
                                                             animated:NO];
                                       }
                                       completion:^(BOOL done) {
  ...
  [self.webView
      evaluateJavaScript:[NSString stringWithFormat:@"window.wmf.findInPage."
                                                    @"useFocusStyleForHighligh"
                                                    @"tedSearchTermWithId('%@'"
                                                    @")",
                                                    matchSpanId]
...
- (void)keyboardBar:(WMFFindInPageKeyboardBar *)keyboardBar
    searchTermChanged:(NSString *)term {
  [self.webView
      evaluateJavaScript:[NSString stringWithFormat:@"window.wmf.findInPage."
                                                    @"findAndHighlightAllMatch"
                                                    @"esForSearchTerm('%@')",
                                                    term]
...
- (void)keyboardBarPreviousButtonTapped:
    (WMFFindInPageKeyboardBar *)keyboardBar {
  [self moveFindInPageSelectedMatchIndexInDirection:
            WMFFindInPageScrollDirectionPrevious];
...
- (WKWebViewConfiguration *)configuration {
  ...
  [userContentController
      addUserScript:[[WKUserScript alloc]
                          initWithSource:@"window.webkit.messageHandlers."
                                         @"lateJavascriptTransform."
                                         @"postMessage('collapseTables');"
  ...
  [userContentController
      addScriptMessageHandler:[[WeakScriptMessageDelegate alloc]
                                  initWithDelegate:self]
                         name:@"nonAnchorTouchEndedWithoutDragging"];

  [userContentController
      addScriptMessageHandler:[[WeakScriptMessageDelegate alloc]
                                  initWithDelegate:self]
                         name:@"sendJavascriptConsoleLogMessageToXcodeConsole"];
  ...
  NSString *earlyJavascriptTransforms =
      @""
       ...
       "window.wmf.transformer.transform( 'addImageOverflowXContainers', "
       "document );"
       // 'addImageOverflowXContainers' needs to happen before 'widenImages'.
       // See "enwiki > Counties of England > Scope and structure > Local
       // government"
       ...
       ";"
       "console.log = "
       "function(message){window.webkit.messageHandlers."
       "sendJavascriptConsoleLogMessageToXcodeConsole.postMessage({'message': "
       "message});};";

  [userContentController
      addUserScript:[[WKUserScript alloc]
                          initWithSource:earlyJavascriptTransforms
                           injectionTime:WKUserScriptInjectionTimeAtDocumentEnd
                        forMainFrameOnly:YES]];
...
- (void)viewDidLoad {
  ...
  self.webView.scrollView.backgroundColor =
      [UIColor wmf_articleBackgroundColor];
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(updateZeroStateWithNotification:)
           ...
           object:nil];
  // should happen in will appear to prevent bar from being incorrect during
  // transitions
...
- (void)saveOpenArticleTitleWithCurrentlyOnscreenFragment {
  if (self.navigationController.topViewController ==
      self.parentViewController) { // Ensure only the topmost article is
                                   // recorded.
    [self getCurrentVisibleSectionCompletion:^(MWKSection *visibleSection,
                                               NSError *error) {
...
/**
 *  Observe changes to the native footer bounds so we can message back to the
 * html to
 *  add bottom padding to html body tag to make room for the native
 * footerContainerView overlay.
 */
- (void)unobserveFooterContainerViewBounds {
  [self.KVOControllerNonRetaining
      unobserve:self.footerContainerView
        keyPath:WMF_SAFE_KEYPATH(self.footerContainerView, bounds)];
}

- (void)observeFooterContainerViewBounds {
  [self.KVOControllerNonRetaining
      observe:self.footerContainerView
      keyPath:WMF_SAFE_KEYPATH(self.footerContainerView, bounds)
      options:NSKeyValueObservingOptionInitial
        block:^(WebViewController *observer, UIView *view,
                NSDictionary *change) {
          if (view && observer.webView) {
            [observer.webView
                wmf_setBottomPadding:(NSInteger)(
                                         ceil(view.bounds.size.height))];
          }
        }];
}

/**
 *  Observe changes to web view scroll view's content size so we can set the top
 * constraint
 *  of the native footerContainerView. Reminder: we constrain to top of
 * footerContainerView
 *  because constraining its bottom to the WKContentView's bottom is flakey - ie
 * doesn't
 *  always work.
 */
- (void)unobserveScrollViewContentSize {
  [self.KVOControllerNonRetaining
      unobserve:self.webView.scrollView
...
- (void)observeScrollViewContentSize {
  @weakify(self);
  [self.KVOControllerNonRetaining
      observe:self.webView.scrollView
      keyPath:WMF_SAFE_KEYPATH(self.webView.scrollView, contentSize)
      options:NSKeyValueObservingOptionInitial
        block:^(WebViewController *observer, UIScrollView *scrollView,
                NSDictionary *change) {
          @strongify(self);
          [self
              setTopOfFooterContainerViewForContentSize:scrollView.contentSize];
        }];
}

- (void)setTopOfFooterContainerViewForContentSize:(CGSize)contentSize {
  self.footerContainerViewTopConstraint.offset =
      contentSize.height - self.footerContainerView.bounds.size.height;
}
...
- (void)updateZeroState {
  if ([[SessionSingleton sharedInstance] zeroConfigState].disposition) {
    [self showZeroBannerWithMessage:[[[SessionSingleton sharedInstance]
                                        zeroConfigState] zeroMessage]];
  } else {
    self.zeroStatusLabel.text = @"";
  }
}
...
- (void)scrollToFooterAtIndex:(NSInteger)index {
  ...
  CGPoint footerViewOrigin =
      [self.webView.scrollView convertPoint:viewToScrollTo.frame.origin
...
- (NSInteger)visibleFooterIndex {
  CGRect const scrollViewContentFrame =
      self.webView.scrollView.wmf_contentFrame;
  if (!CGRectIntersectsRect(scrollViewContentFrame,
                            self.footerContainerView.frame)) {
    return NSNotFound;
  }

  return [self.footerViewControllers indexOfObjectPassingTest:^BOOL(
                                         UIViewController *_Nonnull vc,
                                         NSUInteger idx, BOOL *_Nonnull stop) {
    CGRect absoluteFooterViewFrame =
        [self.webView.scrollView convertRect:vc.view.frame
                                    fromView:self.footerContainerView];
...
- (void)addHeaderView {
  ...
  [self.headerView mas_makeConstraints:^(MASConstraintMaker *make) {
    // lead/trail must be constained to webview, the scrollview doesn't define a
    // width
...
- (WMFArticleFooterView *)footerLicenseView {
  if (!_footerLicenseView) {
    ...
    @weakify(self);
    [_footerLicenseView.showLicenseButton
        bk_addEventHandler:^(id sender) {
          @strongify(self);
...
- (void)addFooterContainerView {
  ...
  [self.footerContainerView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    self.footerContainerViewTopConstraint =
        make.top.equalTo(self.webView.scrollView);
  }];
}
...
- (void)addFooterContentViews {
  ...
  MASViewAttribute *lastAnchor = [self.footerViewControllers
      bk_reduce:self.footerContainerView.mas_top
      withBlock:^MASViewAttribute *(MASViewAttribute *topAnchor,
                                    UIViewController *childVC) {
        NSString *footerTitle = [self.delegate webViewController:self
                                    titleForFooterViewController:childVC];
        if (footerTitle) {
          WMFArticleFooterViewHeader *header =
              [WMFArticleFooterViewHeader wmf_viewFromClassNib];
          self.footerViewHeadersByIndex[
              @([self.footerViewControllers indexOfObject:childVC])] = header;
...
- (void)setFooterViewControllers:
    (NSArray<UIViewController *> *)footerViewControllers {
  if (WMF_EQUAL(self.footerViewControllers, isEqualToArray:,
                footerViewControllers)) {
    return;
  }
...
- (void)setHeaderView:(UIView *)headerView {
  NSAssert(!self.headerView,
           @"Dynamic/re-configurable header view is not supported.");
  NSAssert(!self.isViewLoaded,
           @"Expected header to be configured before viewDidLoad.");
  _headerView = headerView;
}

- (CGFloat)headerHeightForCurrentArticle {
  if (self.article.isMain || !self.article.imageURL ||
      [self.article.url wmf_isNonStandardURL]) {
    return 0;
  } else {
    return 210;
  }
...
- (void)scrollToFragment:(NSString *)fragment animated:(BOOL)animated {
  if (fragment.length == 0) {
    ...
    [self.webView.scrollView scrollRectToVisible:CGRectMake(0, 1, 1, 1)
                                        animated:animated];
  } else {
    if (!animated) {
    ...
    [self.webView
        getScrollViewRectForHtmlElementWithId:fragment
                                   completion:^(CGRect rect) {
...
- (void)accessibilityCursorToSection:(MWKSection *)section {
  // This might shift the visual scroll position. To prevent it affecting other
  // users,
  // we will only do it when we detect than an assistive technology which
  // actually needs this is running.
  [self.webView wmf_accessibilityCursorToFragment:section.anchor];
}

- (void)getCurrentVisibleSectionCompletion:
    (void (^)(MWKSection *_Nullable, NSError *__nullable error))completion {
  [self.webView
      getIndexOfTopOnScreenElementWithPrefix:
                                  ...
                                  completion:^(id obj, NSError *error) {
                                    if (error) {
                                      ...
                                      NSInteger indexOfFirstOnscreenSection =
                                          ((NSNumber *)obj).integerValue;
                                      completion(
                                          indexOfFirstOnscreenSection == -1
                                              ? nil
...
- (void)scrollToVerticalOffset:(CGFloat)offset {
  [self.webView.scrollView wmf_safeSetContentOffset:CGPointMake(0, offset)
                                           animated:NO];
}
...
- (void)tocScrollWebViewToPoint:(CGPoint)point
  ...
  [UIView animateWithDuration:duration
                      ...
                      options:UIViewAnimationOptionBeginFromCurrentState
                   animations:^{
                     [self.webView.scrollView wmf_safeSetContentOffset:point
                                                              animated:NO];
                   }
                   completion:^(BOOL done){
                   }];
...
- (void)setArticle:(MWKArticle *_Nullable)article
...
- (void)displayArticle {
  ...
  [self.webView loadHTML:[self.article articleHTML]
                 baseURL:self.article.url
          withAssetsFile:@"index.html"
      scrolledToFragment:self.articleURL.fragment
                 padding:UIEdgeInsetsMake(headerHeight, marginWidth, 0,
                                          marginWidth)];

  UIMenuItem *shareSnippet = [[UIMenuItem alloc]
      initWithTitle:MWLocalizedString(@"share-a-fact-share-menu-item", nil)
...
#pragma clang diagnostic pop
  ...
  NSNumber *refsHeight = @(
      (self.view.frame.size.height * MENUS_SCALE_MULTIPLIER) * percentOfHeight);
...
- (void)referencesShow:(NSDictionary *)payload {
  ...
  // reference parsing javascript can't parse until the reference section html
  // has
  // been retrieved. If user taps a reference link while the non-lead sections
  // are
  ...
  self.referencesVC =
      [ReferencesVC wmf_initialViewControllerFromClassStoryboard];
...
- (void)referenceViewController:(ReferencesVC *)referenceViewController
  ...
  [self.webView
      getScreenRectForHtmlElementWithId:linkID
                             completion:^(CGRect rect) {
                               if (!CGRectIsNull(rect)) {
                                 CGFloat vSpaceAboveRefsPanel =
                                     self.view.bounds.size.height -
                                     referenceViewController.panelHeight;
                                 // Only scroll up if the refs link would be
                                 // below the refs panel.
                                 if ((rect.origin.y + rect.size.height) >
                                     (vSpaceAboveRefsPanel)) {
                                   // Calculate the distance needed to scroll
                                   // the refs link to the vertical center of
                                   // the
                                   // part of the article web view not covered
                                   // by the refs panel.
                                   CGFloat distanceFromVerticalCenter =
                                       ((vSpaceAboveRefsPanel) / 2.0) -
...
- (void)referenceViewControllerCloseReferences:
    (ReferencesVC *)referenceViewController {
  [self referencesHide];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  if ([self.delegate respondsToSelector:@selector(webViewController:
                                                scrollViewDidScroll:)]) {
