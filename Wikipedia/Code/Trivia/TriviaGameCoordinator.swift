...
class TriviaGameCoordinator {
    ...
    private func fetchTriviaData(completion: @escaping (TriviaGameData?) -> Void) {
        // Extract data from the datastore - use fullContent for more events to randomize
        ...
        // Select two random events with valid years
        let validEvents = allEvents.compactMap { event -> (WMFFeedOnThisDayEvent, String)? in
            guard let year = event.year?.intValue else { return nil }
            
            // Use the same date formatting pattern as OnThisDayWidget
        ...
        guard validEvents.count >= 2 else {
            DispatchQueue.main.async {
                completion(nil)
            }
            return
        ...
        let selectedEvents = Array(validEvents.shuffled().prefix(2))
    ...
    private func presentTriviaGame(with gameData: TriviaGameData) {
    ...
    private func presentFallbackGame() {
        let fallbackGameData = TriviaGameData(
