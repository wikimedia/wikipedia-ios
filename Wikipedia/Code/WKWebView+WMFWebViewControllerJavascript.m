...
- (void)wmf_setTextSize:(NSInteger)textSize {
  [self evaluateJavaScript:[NSString stringWithFormat:@"document.querySelector("
                                                      @"'body').style['-"
                                                      @"webkit-text-size-"
                                                      @"adjust'] = '%ld%%';",
                                                      (long)textSize]
...
- (void)wmf_collapseTablesForArticle:(MWKArticle *)article {
  [self evaluateJavaScript:[self tableCollapsingJavascriptForArticle:article]
...
- (NSString *)tableCollapsingJavascriptForArticle:(MWKArticle *)article {
  return [NSString
      stringWithFormat:
          @"window.wmf.transformer.transform('hideTables', document, %d, '%@', "
          @"'%@', '%@');",
          article.isMain,
          [self apostropheEscapedArticleLanguageLocalizedStringForKey:
                    @"info-box-title"
                                                              article:article],
          [self apostropheEscapedArticleLanguageLocalizedStringForKey:
                    @"table-title-other"
...
- (NSString *)
apostropheEscapedArticleLanguageLocalizedStringForKey:(NSString *)key
                                              article:(MWKArticle *)article {
  return [MWSiteLocalizedString(article.url, key, nil)
  ...
  [self
      evaluateJavaScript:
          [NSString stringWithFormat:
                        ...
                        languageInfo.code, languageInfo.dir,
                        [[UIApplication sharedApplication] wmf_isRTL] ? @"rtl"
                                                                      : @"ltr"]
       completionHandler:nil];
}

- (void)wmf_setPageProtected {
  [self evaluateJavaScript:@"window.wmf.utilities.setPageProtected()"
         completionHandler:nil];
}
...
- (void)wmf_setBottomPadding:(NSInteger)bottomPadding {
  [self
      evaluateJavaScript:[NSString
                             stringWithFormat:@"document.getElementsByTagName('"
                                              @"BODY')[0].style.paddingBottom "
                                              @"= '%ldpx';",
                                              (long)bottomPadding]
       completionHandler:nil];
}
...
- (void)wmf_scrollToFragment:(NSString *)fragment {
  [self evaluateJavaScript:
            [NSString
                stringWithFormat:@"window.wmf.utilities.scrollToFragment('%@')",
                                 fragment]
         completionHandler:nil];
}
...
- (void)wmf_accessibilityCursorToFragment:(NSString *)fragment {
  if (UIAccessibilityIsVoiceOverRunning()) {
    [self evaluateJavaScript:[NSString stringWithFormat:@"window.wmf.utilities."
                                                        @"accessibilityCursorT"
                                                        @"oFragment('%@')",
                                                        fragment]
           completionHandler:nil];
  }
         ...
         completionHandler:^(id _Nullable obj, NSError *_Nullable error) {
           if ([obj isKindOfClass:[NSString class]]) {
             NSString *selectedText =
                 [(NSString *)obj wmf_shareSnippetFromText];
             selectedText = selectedText.length < kMinimumTextSelectionLength
                                ? @""
                                : selectedText;
