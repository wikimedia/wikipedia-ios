...
- (void)viewDidLoad {
    ...
    self.buttonX.accessibilityLabel = WMFLocalizedStringWithDefaultValue(@"menu-cancel-accessibility-label", nil, NSBundle.wmf_localizationBundle, @"Cancel", @"Accessible label text for toolbar cancel button\n{{Identical|Cancel}}");
    self.buttonCaretLeft.accessibilityLabel = WMFLocalizedStringWithDefaultValue(@"back-button-accessibility-label", nil, NSBundle.wmf_localizationBundle, @"Back", @"Accessibility label for a button to navigate back.\n{{Identical|Back}}");
...
- (NSString *)title {
    if ([self isDisplayingLicense]) {
        return WMFLocalizedStringWithDefaultValue(@"about-libraries-license", nil, NSBundle.wmf_localizationBundle, @"License", @"About page link title that will display a license for a library used in the app\n{{Identical|License}}");
    }
    return WMFLocalizedStringWithDefaultValue(@"about-title", nil, NSBundle.wmf_localizationBundle, @"About", @"Title for credits page\n{{Identical|About}}");
}
...
- (void)injectAboutPageContentIntoWebView:(WKWebView *)webView {
    ...
    setDivHTML(@"wikipedia", WMFLocalizedStringWithDefaultValue(@"about-wikipedia", nil, NSBundle.wmf_localizationBundle, @"Wikipedia", @"Wikipedia\n{{Identical|Wikipedia}}"));
    setDivHTML(@"contributors_title", WMFLocalizedStringWithDefaultValue(@"about-contributors", nil, NSBundle.wmf_localizationBundle, @"Contributors", @"Header text for contributors section of the about page. Is not capitalised for aesthetic reasons, but could be capitalised in translations.\n{{Identical|Contributor}}"));
    setDivHTML(@"contributors_body", self.contributors);
    setDivHTML(@"translators_title", WMFLocalizedStringWithDefaultValue(@"about-translators", nil, NSBundle.wmf_localizationBundle, @"Translators", @"Header text for translators section of the about page. Is not capitalised for aesthetic reasons, but could be capitalised in translations.\n{{Identical|Translator}}"));
    setDivHTML(@"testers_title", WMFLocalizedStringWithDefaultValue(@"about-testers", nil, NSBundle.wmf_localizationBundle, @"Testers", @"Header text for (software) testers section of the about page. Is not capitalised for aesthetic reasons, but could be capitalised in translations."));
    setDivHTML(@"libraries_title", WMFLocalizedStringWithDefaultValue(@"about-libraries", nil, NSBundle.wmf_localizationBundle, @"Libraries used", @"Header text for libraries section (as in a collection of subprograms used to develop software) of the about page. Is not capitalised for aesthetic reasons, but could be capitalised in translations."));
    setDivHTML(@"libraries_body", [[self class] linkHTMLForURLString:@"wmflicense://licenses" title:WMFLocalizedStringWithDefaultValue(@"about-libraries-complete-list", nil, NSBundle.wmf_localizationBundle, @"Complete list", @"Title for link to complete list of libraries use by the app")]);
    setDivHTML(@"repositories_title", WMFLocalizedStringWithDefaultValue(@"about-repositories", nil, NSBundle.wmf_localizationBundle, @"Repositories", @"Header text for repositories section of the about page. Is not capitalised for aesthetic reasons, but could be capitalised in translations. \n{{Identical|Repository}}"));
    setDivHTML(@"repositories_body", self.repositoryLinks);

    setDivHTML(@"repositories_subtitle", [NSString stringWithFormat:WMFLocalizedStringWithDefaultValue(@"about-repositories-app-source-license", nil, NSBundle.wmf_localizationBundle, @"Source code available under the %1$@.", @"Text explaining the app source licensing. %1$@ is the message {{msg-wikimedia|about-repositories-app-source-license-mit}}."), [[self class] linkHTMLForURLString:self.urls[kWMFURLsMITKey] title:WMFLocalizedStringWithDefaultValue(@"about-repositories-app-source-license-mit", nil, NSBundle.wmf_localizationBundle, @"MIT License", @"Name of the \"MIT\" license")]]);

    setDivHTML(@"feedback_body", [[self class] linkHTMLForURLString:self.feedbackURL title:WMFLocalizedStringWithDefaultValue(@"about-send-feedback", nil, NSBundle.wmf_localizationBundle, @"Send app feedback", @"Link text for sending app feedback")]);
    setDivHTML(@"license_title", WMFLocalizedStringWithDefaultValue(@"about-content-license", nil, NSBundle.wmf_localizationBundle, @"Content license", @"Header text for content license section"));

    setDivHTML(@"license_body", [NSString stringWithFormat:WMFLocalizedStringWithDefaultValue(@"about-content-license-details", nil, NSBundle.wmf_localizationBundle, @"Unless otherwise specified, content is available under a %1$@.", @"Text explaining license of app content. %1$@ is the message {{msg-wikimedia|about-content-license-details-share-alike-license}}."), [[self class] linkHTMLForURLString:self.urls[kWMFURLsShareAlikeKey] title:WMFLocalizedStringWithDefaultValue(@"about-content-license-details-share-alike-license", nil, NSBundle.wmf_localizationBundle, @"Creative Commons Attribution-ShareAlike License", @"Name of the \"Creative Commons Attribution-ShareAlike\" license")]]);

    setDivHTML(@"translators_body", [NSString stringWithFormat:WMFLocalizedStringWithDefaultValue(@"about-translators-details", nil, NSBundle.wmf_localizationBundle, @"Translated by volunteers at %1$@", @"Description of volunteer translation. %1$@ is translatewiki url."), [[self class] linkHTMLForURLString:self.urls[kWMFURLsTranslateWikiKey] title:[self.urls[kWMFURLsTranslateWikiKey] substringFromIndex:7]]]);
    setDivHTML(@"testers_body", [NSString stringWithFormat:WMFLocalizedStringWithDefaultValue(@"about-testers-details", nil, NSBundle.wmf_localizationBundle, @"QA tested by %1$@", @"Description of the Quality Assurance (QA) testers. %1$@ is specialistsguild.org, the website of the testing group."), [[self class] linkHTMLForURLString:self.urls[kWMFURLsSpecialistGuildKey] title:[self.urls[kWMFURLsSpecialistGuildKey] substringFromIndex:7]]]);

    setDivHTML(@"footer", [NSString stringWithFormat:WMFLocalizedStringWithDefaultValue(@"about-product-of", nil, NSBundle.wmf_localizationBundle, @"Made by the %1$@ with the help of volunteers like you", @"Description of who produced the app. %1$@ is the message {{msg-wikimedia|wikipedia-ios-about-wikimedia-foundation}}."), [[self class] linkHTMLForURLString:self.urls[kWMFURLsWikimediaKey] title:WMFLocalizedStringWithDefaultValue(@"about-wikimedia-foundation", nil, NSBundle.wmf_localizationBundle, @"Wikimedia Foundation", @"Name of the Wikimedia Foundation. Used by the message {{Msg-wikimedia|wikipedia-ios-about-product-of}}.")]]);
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([[self class] isLicenseURL:requestURL]) {
        ...
        vc.headerText = [NSString localizedStringWithFormat: WMFLocalizedStringWithDefaultValue(@"about-libraries-licenses-title", nil, NSBundle.wmf_localizationBundle, @"We love open source software %1$@", @"Title for list of library licenses. %1$@ will be replaced with an emoji expressing our love for open source software"), @"💖"];
