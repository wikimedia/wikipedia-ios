...
- (void)wmf_setInnerHTML:(NSString *)html ofElementId:(NSString *)elementId {
  ...
  // Rely on NSJSONSerialization for string escaping:
  // http://stackoverflow.com/a/13569786
  ...
  [self evaluateJavaScript:[NSString stringWithFormat:@"document."
                                                      @"getElementById('%@')."
                                                      @"innerHTML = \"%@\";",
                                                      elementId, escapedString]
...
- (void)wmf_setTextFontSize {
  NSString *fontSizeJS =
      [NSString stringWithFormat:@"document.body.style.fontSize = '%f%%'",
...
- (void)wmf_preventTextFromExpandingOnRotation {
  [self evaluateJavaScript:@"document.getElementsByTagName('body')[0].style['-"
...
- (instancetype)init {
  ...
  NSString *licenseText = [NSString
      stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"OFL"
                                                               ofType:@"txt"]
                      encoding:NSUTF8StringEncoding
                         error:&error];
  NSAssert(!error, @"License copy not retrieved");
  return [super initWithTitle:@"TharLon Myanmar Unicode Font" text:licenseText];
}
...
- (void)viewDidLoad {
  ...
  @weakify(self) self.buttonX = [UIBarButtonItem
  ...
  self.buttonCaretLeft = [UIBarButtonItem
      wmf_buttonType:WMFButtonTypeCaretLeft
             handler:^(id sender) {
               @strongify(
                   self)[self.webView loadHTMLFromAssetsFile:kWMFAboutHTMLFile
...
- (void)updateNavigationBar {
  ...
  self.navigationItem.leftBarButtonItem =
      [self isDisplayingLicense] ? self.buttonCaretLeft : self.buttonX;
}
...
- (NSDictionary *)data {
  NSString *plistPath =
      [[NSBundle mainBundle] pathForResource:kWMFAboutPlistName
                                      ofType:@"plist"];
...
- (NSString *)repositoryLinks {
  NSMutableDictionary *repos =
      (NSMutableDictionary *)self.data[kWMFRepositoriesKey];
...
- (NSString *)feedbackURL {
  ...
  feedbackUrl = [feedbackUrl
      stringByReplacingOccurrencesOfString:@"$1"
                                withString:[WikipediaAppUtils
                                               versionedUserAgent]];

  NSString *encodedUrlString = [feedbackUrl
...
- (void)injectAboutPageContentIntoWebView:(WKWebView *)webView {
  void (^setDivHTML)(NSString *, NSString *) =
      ^void(NSString *divId, NSString *twnString) {
        [self.webView wmf_setInnerHTML:twnString ofElementId:divId];
      };

  setDivHTML(@"version",
             [[NSBundle mainBundle] wmf_versionForCurrentBundleIdentifier]);
  setDivHTML(@"wikipedia", MWLocalizedString(@"about-wikipedia", nil));
  setDivHTML(@"contributors_title",
             MWLocalizedString(@"about-contributors", nil));
  setDivHTML(@"contributors_body", self.contributors);
  setDivHTML(@"translators_title",
             MWLocalizedString(@"about-translators", nil));
  ...
  setDivHTML(
      @"libraries_body",
      [[self class]
          linkHTMLForURLString:@"wmflicense://licenses"
                         title:MWLocalizedString(
                                   @"about-libraries-complete-list", nil)]);
  setDivHTML(@"repositories_title",
             MWLocalizedString(@"about-repositories", nil));
  setDivHTML(@"repositories_body", self.repositoryLinks);

  setDivHTML(
      @"repositories_subtitle",
      [self stringFromLocalizationKey:@"about-repositories-app-source-license"
                               urlKey:kWMFURLsMITKey
                   urlLocalizationKey:
                       @"about-repositories-app-source-license-mit"]);
  ...
  setDivHTML(@"license_title",
             MWLocalizedString(@"about-content-license", nil));

  setDivHTML(
      @"license_body",
      [self stringFromLocalizationKey:@"about-content-license-details"
                               urlKey:kWMFURLsShareAlikeKey
                   urlLocalizationKey:
                       @"about-content-license-details-share-alike-license"]);

  setDivHTML(@"translators_body",
             [self stringFromLocalizationKey:@"about-translators-details"
                                      urlKey:kWMFURLsTranslateWikiKey]);
  setDivHTML(@"testers_body",
             [self stringFromLocalizationKey:@"about-testers-details"
                                      urlKey:kWMFURLsSpecialistGuildKey]);

  setDivHTML(@"footer",
             [self stringFromLocalizationKey:@"about-product-of"
                                      urlKey:kWMFURLsWikimediaKey
                          urlLocalizationKey:@"about-wikimedia-foundation"]);
...
- (NSString *)stringFromLocalizationKey:(NSString *)localizationKey
  ...
  return [MWLocalizedString(localizationKey, nil)
      stringByReplacingOccurrencesOfString:@"$1"
                                withString:
                                    [[self class]
                                        linkHTMLForURLString:self.urls[urlKey]
                                                       title:
                                                           MWLocalizedString(
                                                               urlLocalizationKey,
                                                               nil)]];
...
- (BOOL)isDisplayingLicense {
  if ([[[self.webView URL] scheme] isEqualToString:kWMFLicenseRedirectScheme] &&
      [[[self.webView URL] resourceSpecifier]
          isEqualToString:kWMFLicenseRedirectResourceIdentifier]) {
    return YES;
  }
...
- (void)webView:(WKWebView *)webView
  ...
  if ([[self class] isLicenseURL:requestURL]) {
    VTAcknowledgementsViewController *vc =
        [VTAcknowledgementsViewController acknowledgementsViewController];
    vc.headerText = [MWLocalizedString(@"about-libraries-licenses-title", nil)
        stringByReplacingOccurrencesOfString:@"$1"
                                  withString:@"💖"];

    vc.acknowledgements = [vc.acknowledgements
        arrayByAddingObjectsFromArray:@[ [[TharlonFontAcknowledgement alloc]
                                          init] ]];
...
- (void)webView:(WKWebView *)webView
  ...
  if (!([[self class] isLicenseURL:[webView URL]] ||
        [[self class] isLicenseRedirectURL:[webView URL]])) {
