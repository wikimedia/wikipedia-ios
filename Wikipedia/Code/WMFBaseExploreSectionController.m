...
static const DDLogLevel WMFBaseExploreSectionControllerLogLevel =
    DDLogLevelInfo;

#undef LOG_LEVEL_DEF
#define LOG_LEVEL_DEF WMFBaseExploreSectionControllerLogLevel

static NSString *const WMFExploreSectionControllerException =
    @"WMFExploreSectionControllerException";
...
- (NSString *)description {
  NSAssert([self conformsToProtocol:@protocol(WMFExploreSectionController)],
           ...
           [self class]);
  return [NSString stringWithFormat:@"%@ identifier = %@", [super description],
...
- (NSString *)cellIdentifier {
  @throw
      [NSException exceptionWithName:WMFExploreSectionControllerException
...
- (void)configureEmptyCell:(WMFEmptySectionCollectionViewCell *)cell {
  ...
  [cell.reloadButton
      setTitle:MWLocalizedString(@"home-empty-section-check-again", nil)
...
- (void)registerCellsInCollectionView:(UICollectionView *)collectionView {
  [collectionView registerNib:[self cellNib]
      forCellWithReuseIdentifier:[self cellIdentifier]];
  if ([self placeholderCellIdentifier] && [self placeholderCellNib]) {
    [collectionView registerNib:[self placeholderCellNib]
        forCellWithReuseIdentifier:[self placeholderCellIdentifier]];
  }
  [collectionView registerNib:[WMFEmptySectionCollectionViewCell wmf_classNib]
      forCellWithReuseIdentifier:[WMFEmptySectionCollectionViewCell
                                     identifier]];
}
...
- (void)configureCell:(UICollectionViewCell *)cell
  ...
  if ([self shouldShowPlaceholderCell]) {
    ...
    [self configureCell:cell
               withItem:self.items[indexPath.row]
...
- (AnyPromise *)fetchDataIgnoreError:(BOOL)ignoreError
                  ignoreCurrentItems:(BOOL)ignoreCurrentItems {
  if (!ignoreError && [self lastFetchFailed]) {
    ...
    self.fetcherPromise =
        [self fetchData]
            ...
            .catch(^(NSError *error) {
              @strongify(self);
              DDLogError(@"Failed to fetch items for section %@. %@", self,
                         error);
              self.fetcherPromise = nil;
...
- (BOOL)supportsPlaceholders {
  ...
  NSAssert(
      supportsPlaceholders > 0 == ([self placeholderCellNib] != nil),
      @"placeholderCellNib must be nonnull if placeholders are supported.");
  NSAssert(supportsPlaceholders > 0 ==
               ([self placeholderCellIdentifier] != nil),
           @"placeholderCellIdentifier must be nonnull if placeholders are "
           @"supported.");
...
- (BOOL)shouldShowPlaceholderCell {
  return [self supportsPlaceholders] && [self.fetchedItems count] == 0 &&
         self.fetchError == nil;
}
