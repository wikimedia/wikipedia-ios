...
- (void)cleanup {
  [[WMFImageController sharedInstance]
      cancelFetchForURL:[NSURL
                            wmf_optionalURLWithString:self.article.imageURL]];
...
- (void)presentShareOptionsWithSnippet:(NSString *)snippet
...
- (void)fetchImageThenShowShareCard {
  @weakify(self);
  [[WMFImageController sharedInstance]
      fetchImageWithURL:[NSURL wmf_optionalURLWithString:self.article.imageURL]
...
- (void)setupShareOptions {
  WMFShareOptionsView *shareOptionsView =
      [[[NSBundle mainBundle] loadNibNamed:@"ShareOptions"
                                     owner:self
  ...
  shareOptionsView.shareAsCardLabel.text =
      MWLocalizedString(@"share-as-image", nil);
  shareOptionsView.shareAsTextLabel.text =
      MWLocalizedString(@"share-as-text", nil);
...
- (void)setContainerViewControllerActionsEnabled:(BOOL)enabled {
  self.containerViewController.navigationController.navigationBar
      .userInteractionEnabled = enabled;
  [self.containerViewController.navigationItem.leftBarButtonItems
      enumerateObjectsUsingBlock:^(UIBarButtonItem *_Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        obj.enabled = enabled;
      }];
  [self.containerViewController.navigationItem.rightBarButtonItems
      enumerateObjectsUsingBlock:^(UIBarButtonItem *_Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        obj.enabled = enabled;
      }];
  [self.containerViewController.toolbarItems
      enumerateObjectsUsingBlock:^(__kindof UIBarButtonItem *_Nonnull obj,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        obj.enabled = enabled;
      }];
  self.containerViewController.navigationController.navigationBar
      .accessibilityElementsHidden = !enabled;
  self.containerViewController.navigationController.toolbar
      .accessibilityElementsHidden = !enabled;
}
...
- (void)presentShareOptions {
  ...
  [UIView animateWithDuration:0.40
      ...
      completion:^(BOOL finished) {
        UITapGestureRecognizer *tapForCardOnCardImageViewRecognizer =
            [[UITapGestureRecognizer alloc]
                initWithTarget:self
                        action:@selector(respondToTapForCardGesture:)];
        UITapGestureRecognizer *tapForCardOnButtonRecognizer =
            [[UITapGestureRecognizer alloc]
                initWithTarget:self
                        action:@selector(respondToTapForCardGesture:)];
        UITapGestureRecognizer *tapForTextRecognizer =
            [[UITapGestureRecognizer alloc]
                initWithTarget:self
                        action:@selector(respondToTapForTextGesture:)];
        [self.shareOptions.cardImageViewContainer
            addGestureRecognizer:tapForCardOnCardImageViewRecognizer];
        [self.shareOptions.shareAsCardLabel
            addGestureRecognizer:tapForCardOnButtonRecognizer];
        [self.shareOptions.shareAsTextLabel
            addGestureRecognizer:tapForTextRecognizer];
        @weakify(self);
        ...
        UIAccessibilityPostNotification(
            UIAccessibilityScreenChangedNotification, self.shareOptions);
      }];
}
...
- (void)respondToTapForCardGesture:(UITapGestureRecognizer *)recognizer {
  ...
  [self dismissShareOptionsWithCompletion:^{
    [self
        presentActivityViewControllerWithImage:self.shareImage
                                         title:[self titleForActivityWithCard]];
  }];
}
...
- (void)respondToTapForTextGesture:(UITapGestureRecognizer *)recognizer {
  ...
  [self dismissShareOptionsWithCompletion:^{
    [self
        presentActivityViewControllerWithImage:nil
                                         title:[self titleForActivityTextOnly]];
  }];
}
...
- (NSString *)shareTitle {
  return [self.article.url.wmf_title length] > 0
             ? [self.article.url.wmf_title copy]
             : @"";
}
...
- (NSString *)titleForActivityTextOnly {
  if ([self snippetForTextOnlySharing].length == 0) {
    ...
    return [[MWLocalizedString(
        @"share-article-name-on-wikipedia-with-selected-text", nil)
        stringByReplacingOccurrencesOfString:@"$1"
                                  withString:self.shareTitle]
...
- (void)presentActivityViewControllerWithImage:(nullable UIImage *)image
  ...
  NSURL *url = [[NSURL alloc]
      initWithString:[NSString
                         stringWithFormat:@"%@?%@",
                                          [NSURL
                                              wmf_desktopURLForURL:self.article
                                                                       .url]
                                              .absoluteString,
                                          parameter]];
  ...
  UIPopoverPresentationController *presenter =
      [shareActivityVC popoverPresentationController];
  presenter.barButtonItem = self.originButtonItem;

  shareActivityVC.excludedActivityTypes = @[
    UIActivityTypePrint, UIActivityTypeAssignToContact, UIActivityTypeAirDrop,
    UIActivityTypeAddToReadingList
  ];
  ;

  [shareActivityVC setCompletionWithItemsHandler:^(
                       NSString *__nullable activityType, BOOL completed,
  ...
  [self.containerViewController presentViewController:shareActivityVC
                                             animated:YES
                                           completion:nil];
