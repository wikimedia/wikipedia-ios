...
@interface PreviewAndSaveViewController () <
    FetchFinishedDelegate, UITextFieldDelegate, UIScrollViewDelegate,
    WMFOpenExternalLinkDelegate, WMFPreviewSectionLanguageInfoDelegate,
    WMFPreviewAnchorTapAlertDelegate, PreviewLicenseViewDelegate>
...
@property(strong, nonatomic)
    IBOutlet PreviewWebViewContainer *previewWebViewContainer;
@property(weak, nonatomic)
    IBOutlet NSLayoutConstraint *previewWebViewHeightConstraint;
...
@property(strong, nonatomic)
    UIGestureRecognizer *previewLicenseTapGestureRecognizer;
...
- (void)dealloc {
  [self.previewWebViewContainer.webView.scrollView
      removeObserver:self
...
- (void)wmf_showAlertForTappedAnchorHref:(NSString *)href {
  ...
  [alertController
      addAction:[UIAlertAction
                    actionWithTitle:MWLocalizedString(@"button-ok", nil)
...
- (void)viewDidLoad {
  ...
  @weakify(self) self.buttonX =
      [UIBarButtonItem wmf_buttonType:WMFButtonTypeX
                              handler:^(id sender) {
                                @strongify(self)[self goBack];
                              }];

  self.buttonLeftCaret =
      [UIBarButtonItem wmf_buttonType:WMFButtonTypeCaretLeft
                              handler:^(id sender) {
                                @strongify(self)[self goBack];
                              }];

  self.buttonSave = [[UIBarButtonItem alloc]
      bk_initWithTitle:MWLocalizedString(@"button-save", nil)
                 style:UIBarButtonItemStylePlain
               handler:^(id sender) {
                 @strongify(self)[self goForward];
               }];

  self.buttonNext = [[UIBarButtonItem alloc]
      bk_initWithTitle:MWLocalizedString(@"button-next", nil)
                 style:UIBarButtonItemStylePlain
               handler:^(id sender) {
                 @strongify(self)[self goForward];
               }];
  ...
  self.previewLabel.font =
      [UIFont boldSystemFontOfSize:15.0 * MENUS_SCALE_MULTIPLIER];
  ...
  [self.previewLabel
      addGestureRecognizer:[[UITapGestureRecognizer alloc]
                               initWithTarget:self
                                       action:@selector(previewLabelTapped:)]];
  ...
  // Observer the web view's contentSize property to enable the web view to
  // expand to the
  // height of the html content it is displaying so the web view's scroll view
  // doesn't show
  // any scroll bars. (Expand the web view to the full height of its content so
  // it scrolls
  // with this view controller's scroll view rather than its own.) Note that to
  // make this
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ((object == self.previewWebViewContainer.webView.scrollView) &&
      [keyPath isEqual:@"contentSize"]) {
    // Size the web view to the height of the html content it is displaying
    // (gets rid of the web view's scroll bars).
    // Note: the PreviewWebView class has to call
    // "forceScrollViewContentSizeToReflectActualHTMLHeight" in its
    // overridden "layoutSubviews" method for the contentSize to be reported
    // accurately such that it reflects the
    // actual height of the web view content here. Without the web view class
    // calling this method in its
    // layoutSubviews, the contentSize.height wouldn't change if we, say,
    // rotated the device.
    self.previewWebViewHeightConstraint.constant =
        self.previewWebViewContainer.webView.scrollView.contentSize.height;
  }
}
...
- (void)constrainEditSummaryContainerSubviews {
  ...
  CGFloat spaceAboveCC =
      ([UIScreen mainScreen].bounds.size.height != 480) ? 43 : 4;
  ...
  NSArray *constraints = @[
    ...
    [NSLayoutConstraint constraintsWithVisualFormat:@"H:|[cannedSummary04]"
                                            ...
                                            metrics:metrics
                                              views:views],
    [NSLayoutConstraint
        constraintsWithVisualFormat:@"V:|-(40)-[aboutLabel]-(5)-["
                                    @"cannedSummary01(buttonHeight)]["
                                    @"cannedSummary02(buttonHeight)]["
                                    @"cannedSummary03(buttonHeight)]["
                                    @"cannedSummary04(buttonHeight)]-("
                                    @"spaceAboveCC)-|"
                            options:0
                            metrics:metrics
                              views:views]
  ];
  [self.editSummaryContainer
      addConstraints:[constraints valueForKeyPath:@"@unionOfArrays.self"]];

  [self adjustConstraintsScaleForViews:@[
    self.cannedSummary01, self.cannedSummary02, self.cannedSummary03,
    self.cannedSummary04, self.editSummaryContainer, self.aboutLabel
  ]];
}
...
- (void)setupEditSummaryContainerSubviews {
  ...
  MenuButton * (^setupButton)(NSString *, NSInteger) =
      ^MenuButton *(NSString *text, WMFCannedSummaryChoices tag) {
  ...
  self.cannedSummary01 =
      setupButton(MWLocalizedString(@"edit-summary-choice-fixed-typos", nil),
                  CANNED_SUMMARY_TYPOS);
  self.cannedSummary02 =
      setupButton(MWLocalizedString(@"edit-summary-choice-fixed-grammar", nil),
                  CANNED_SUMMARY_GRAMMAR);
  self.cannedSummary03 =
      setupButton(MWLocalizedString(@"edit-summary-choice-linked-words", nil),
                  CANNED_SUMMARY_LINKS);
  self.cannedSummary04 =
      setupButton(MWLocalizedString(@"edit-summary-choice-other", nil),
                  CANNED_SUMMARY_OTHER);

  // Setup the canned edit summaries label.
  self.aboutLabel = [[UILabel alloc] init];
  self.aboutLabel.numberOfLines = 0;
  self.aboutLabel.font =
      [UIFont boldSystemFontOfSize:24.0 * MENUS_SCALE_MULTIPLIER];
...
- (void)showSummaryOverlay {
  EditSummaryViewController *summaryVC =
      [EditSummaryViewController wmf_initialViewControllerFromClassStoryboard];
  ...
  [self presentViewController:[[UINavigationController alloc]
                                  initWithRootViewController:summaryVC]
                     animated:YES
                   completion:nil];
}
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  self.captchaViewController =
      [CaptchaViewController wmf_initialViewControllerFromClassStoryboard];
  [self wmf_addChildController:self.captchaViewController
      andConstrainToEdgesOfContainerView:self.captchaContainer];
  ...
  self.previewLicenseTapGestureRecognizer = [[UITapGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(licenseLabelTapped:)];
  [self.previewLicenseView.licenseLoginLabel
      addGestureRecognizer:self.previewLicenseTapGestureRecognizer];
...
- (void)licenseLabelTapped:(UIGestureRecognizer *)recognizer {
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    ...
    LoginViewController *loginVC =
        [LoginViewController wmf_initialViewControllerFromClassStoryboard];
...
- (void)viewWillDisappear:(BOOL)animated {
  ...
  [self.previewLicenseView.licenseLoginLabel
      removeGestureRecognizer:self.previewLicenseTapGestureRecognizer];
...
- (void)fetchFinished:(id)sender
  ...
  if ([sender isKindOfClass:[PreviewHtmlFetcher class]]) {
    ...
    case FETCH_FINAL_STATUS_SUCCEEDED: {
      [[WMFAlertManager sharedInstance] dismissAlert];
      [self.previewWebViewContainer.webView
                    loadHTML:fetchedData
                     baseURL:[NSURL URLWithString:@"https://wikipedia.org"]
              withAssetsFile:@"preview.html"
    ...
    void (^upload)() = ^void() {
      ...
      (void)[[WikiTextSectionUploader alloc]
          initAndUploadWikiText:tokenFetcher.wikiText
                  forArticleURL:tokenFetcher.articleURL
                    ...
                    captchaWord:tokenFetcher.captchaWord
                          token:editToken
                    withManager:[QueuesSingleton sharedInstance]
                                    .sectionWikiTextUploadManager
    ...
    case FETCH_FINAL_STATUS_FAILED:
      ...
      // Still try the uploadWikiTextOp even if EditTokenFetcher fails to get a
      // token.
      // EditTokenFetcher return an anonymous "+\" edit token if it doesn't find
      // an edit token.
      upload();
    ...
    case FETCH_FINAL_STATUS_FAILED: {
      ...
      case WIKITEXT_UPLOAD_ERROR_ABUSEFILTER_OTHER: {
        ...
        // Hides the license panel. Needed if logged in and a disallow is
        // triggered.
        [[WMFAlertManager sharedInstance] dismissAlert];

        AbuseFilterAlertType alertType =
            (error.code == WIKITEXT_UPLOAD_ERROR_ABUSEFILTER_DISALLOWED)
      ...
      case WIKITEXT_UPLOAD_ERROR_UNKNOWN:
        ...
        [self.funnel
            logError:error.localizedDescription]; // @fixme is this right msg?
        break;
    ...
    case FETCH_FINAL_STATUS_SUCCEEDED: {
      ...
      NSString *newCaptchaUrl =
          [CaptchaResetter newCaptchaImageUrlFromOldUrl:self.captchaUrl
                                               andNewId:self.captchaId];
      if (newCaptchaUrl) {
      ...
      animateWithDuration:0.2f
               animations:^{
                 ...
                 [self.captchaViewController.captchaTextBox
                     performSelector:@selector(becomeFirstResponder)
                          withObject:nil
                          afterDelay:0.4f];
                 ...
                 dispatch_async(
                     dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT,
                                               0),
                     ^(void) {
                       NSURL *captchaImageUrl = [NSURL
                           URLWithString:[NSString stringWithFormat:
                                                       @"https://%@.m.%@%@",
                                                       article.url.wmf_language,
                                                       article.url.wmf_domain,
                                                       self.captchaUrl]];

                       UIImage *captchaImage = [UIImage
                           imageWithData:
                               [NSData dataWithContentsOfURL:captchaImageUrl]];

                       dispatch_async(dispatch_get_main_queue(), ^(void) {
                         self.captchaViewController.captchaTextBox.text = @"";
                         self.captchaViewController.captchaImageView.image =
                             captchaImage;
...
- (void)preview {
  [[WMFAlertManager sharedInstance]
                  showAlert:MWLocalizedString(@"wikitext-preview-changes", nil)
                     sticky:YES
      dismissPreviousAlerts:YES
                tapCallBack:NULL];

  [[QueuesSingleton sharedInstance].sectionPreviewHtmlFetchManager
      wmf_cancelAllTasksWithCompletionHandler:^{
        (void)[[PreviewHtmlFetcher alloc]
            initAndFetchHtmlForWikiText:self.wikiText
                             articleURL:self.section.url
                            withManager:[QueuesSingleton sharedInstance]
                                            .sectionPreviewHtmlFetchManager
                     thenNotifyDelegate:self];
      }];
}

- (void)save {
  // TODO: maybe? if we have credentials, yet the edit token retrieved for an
  // edit
  // is an anonymous token (i think this happens if you try to get an edit token
  // and your login session has expired), need to pop up alert asking user if
  // they
  // want to log in before continuing with their edit

  [[WMFAlertManager sharedInstance]
                  showAlert:MWLocalizedString(@"wikitext-upload-save", nil)
                     sticky:YES
      dismissPreviousAlerts:YES
                tapCallBack:NULL];
  ...
  [[QueuesSingleton sharedInstance].sectionWikiTextUploadManager
      wmf_cancelAllTasksWithCompletionHandler:^{
        // If fromTitle was set, the section was transcluded, so use the title
        // of the page
        // it was transcluded from.
        NSURL *editURL = self.section.fromURL ? self.section.fromURL
                                              : self.section.article.url;
        ...
        (void)[[EditTokenFetcher alloc]
            initAndFetchEditTokenForWikiText:self.wikiText
                                  articleURL:editURL
                                     section:[NSString
                                                 stringWithFormat:
                                                     @"%d",
                                 ...
                                 captchaWord:self.captchaViewController
                                                 .captchaTextBox.text
                                 withManager:[QueuesSingleton sharedInstance]
                                                 .sectionWikiTextUploadManager
                          thenNotifyDelegate:self];
...
- (void)showAbuseFilterAlertOfType:(AbuseFilterAlertType)alertType {
  AbuseFilterAlert *abuseFilterAlert =
      [[AbuseFilterAlert alloc] initWithType:alertType];
  ...
  [self.view
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:@"H:|[abuseFilterAlert]|"
                                             options:0
                                             metrics:nil
                                               views:views]];
  ...
  [self.view
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:@"V:|[abuseFilterAlert]|"
                                             options:0
                                             metrics:nil
                                               views:views]];
}
...
- (void)reloadCaptchaPushed:(id)sender {
  self.captchaViewController.captchaTextBox.text = @"";
  [[WMFAlertManager sharedInstance]
                  showAlert:MWLocalizedString(
                                @"account-creation-captcha-obtaining", nil)
                     sticky:NO
      dismissPreviousAlerts:YES
                tapCallBack:NULL];
  [[QueuesSingleton sharedInstance].sectionWikiTextUploadManager
      wmf_cancelAllTasksWithCompletionHandler:^{
        (void)[[CaptchaResetter alloc]
            initAndResetCaptchaForDomain:[SessionSingleton sharedInstance]
                                             .currentArticleSiteURL.wmf_language
                             withManager:[QueuesSingleton sharedInstance]
                                             .sectionWikiTextUploadManager
                      thenNotifyDelegate:self];
      }];
...
- (void)revealCaptcha {
  ...
  self.captchaScrollView.backgroundColor =
...
- (void)previewLicenseViewTermsLicenseLabelWasTapped:
  ...
  [sheet
      addAction:[UIAlertAction
                    actionWithTitle:MWLocalizedString(
                                        @"wikitext-upload-save-terms-name", nil)
                              style:UIAlertActionStyleDefault
                            handler:^(UIAlertAction *_Nonnull action) {
                              [self wmf_openExternalUrl:
                                        [NSURL URLWithString:TERMS_LINK]];
                            }]];
  [sheet addAction:[UIAlertAction
                       actionWithTitle:MWLocalizedString(
                                           @"wikitext-upload-save-license-name",
                                           nil)
                                 style:UIAlertActionStyleDefault
                               handler:^(UIAlertAction *_Nonnull action) {
                                 [self wmf_openExternalUrl:
                                           [NSURL URLWithString:LICENSE_LINK]];
                               }]];
  [sheet addAction:[UIAlertAction actionWithTitle:MWLocalizedString(
                                                      @"open-link-cancel", nil)
