...
- (WMFSearchResultDistanceProvider *)distanceProviderForResult:
  ...
  WMFSearchResultDistanceProvider *provider =
      [WMFSearchResultDistanceProvider new];
  [provider.KVOController
      observe:self
      keyPath:WMF_SAFE_KEYPATH(self, lastLocation)
      options:NSKeyValueObservingOptionInitial
        block:^(WMFSearchResultDistanceProvider *observer,
                WMFCompassViewModel *compassViewModel, NSDictionary *_) {
          observer.distanceToUser = [result.location
              distanceFromLocation:compassViewModel.lastLocation];
        }];
  return provider;
}
...
- (WMFSearchResultBearingProvider *)bearingProviderForResult:
  ...
  WMFSearchResultBearingProvider *provider =
      [WMFSearchResultBearingProvider new];
  [provider.KVOController
      observe:self
      keyPath:WMF_SAFE_KEYPATH(self, lastHeading)
      options:NSKeyValueObservingOptionInitial
        block:^(WMFSearchResultBearingProvider *observer,
                WMFCompassViewModel *compassViewModel, NSDictionary *_) {
          observer.bearingToLocation = [compassViewModel.lastLocation
              wmf_bearingToLocation:result.location
...
- (void)nearbyController:(WMFLocationManager *)controller
  ...
  //    }
  //    if (![error.domain isEqualToString:kCLErrorDomain] && error.code ==
