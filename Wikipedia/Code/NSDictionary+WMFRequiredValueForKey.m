...
NSString *const WMFInvalidValueForKeyErrorDomain =
    @"WMFInvalidValueForKeyErrorDomain";
NSString *const WMFFailingDictionaryUserInfoKey =
    @"WMFFailingDictionaryUserInfoKey";
...
- (nullable id)wmf_instanceOfClass:(Class)aClass
  ...
  NSError * (^errorWithCode)(WMFInvalidValueForKeyError) = ^(
      WMFInvalidValueForKeyError code) {
    return [NSError errorWithDomain:WMFInvalidValueForKeyErrorDomain
                               code:code
                           userInfo:@{WMFFailingDictionaryUserInfoKey : self}];
  };
  id value = self[key];
  if (!value) {
    ...
    DDLogError(@"Expected instance of %@, but got %@ for key %@", aClass,
               [value class], key);
    WMFSafeAssign(outError,
                  errorWithCode(WMFInvalidValueForKeyErrorIncorrectType));
    return nil;
  } else {
    return value;
  }
