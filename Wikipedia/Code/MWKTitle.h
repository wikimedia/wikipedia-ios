...
/**
 *  The normalized page title string (decoded, no underscores or percent
 * escapes).
 */
...
/**
 * Initialize a new title with a URL, using its path and and host as the title's
 * `text` and `site`.
 *
 * @param url URL pointing to a Wikipedia page (i.e. an internal link).
 *
 * @return A new title with properties parsed from the given URL, or `nil` if an
 * error occurred.
 */
 ...
 *
 * The preferred initializer is @c initWithString:site:, which parses components
 * in the string.
 *
 * @param site      The site to which this title belongs.
 * @param text      The text which makes up the title.
 * @param fragment  An optional fragment, for example if the URL contains
 * `#section`, the fragment is `section`.
 *
 * @return A new title.
 */
...
/**
 * Initialize a new title from the given string, parsing & escaping the title
 * and fragment.
