...
- (void)extracted:(WMFArticle * _Nonnull)article completion:(dispatch_block_t _Nullable)completion date:(NSDate * _Nonnull)date groupURL:(NSURL *)groupURL moc:(NSManagedObjectContext * _Nonnull)moc {
    [self.relatedSearchFetcher fetchRelatedArticlesForArticleWithURL:article.URL completion:^(NSError * _Nullable error, NSDictionary<WMFInMemoryURLKey *, WMFRelatedPage *> * _Nullable summariesByKey) {
        if (error) {
        ...
        if (!summariesByKey) {
            ...
            [moc performBlock:^{
                ...
                NSDictionary<WMFInMemoryURLKey *, WMFArticle *> *articles = [moc wmf_createOrUpdateArticleSummmariesWithRelatedPages:summariesByKey error:&summaryError];
                if (summaryError) {
