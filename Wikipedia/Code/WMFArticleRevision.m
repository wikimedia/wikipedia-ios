...
#define WMFArticleRevisionKey(key)                                             \
  WMF_SAFE_KEYPATH([WMFArticleRevision new], key)

typedef NS_ENUM(NSInteger, WMFArticleRevisionError) {
  WMFArticleRevisionErrorMissingRevisionId = 1
};

static NSString *const WMFArticleRevisionErrorDomain =
    @"WMFArticleRevisionErrorDomain";
...
- (BOOL)validate:(NSError *__autoreleasing *)error {
  if (!self.revisionId) {
    WMFSafeAssign(
        error, [NSError errorWithDomain:WMFArticleRevisionErrorDomain
...
+ (NSDictionary *)JSONKeyPathsByPropertyKey {
  return @{
    WMFArticleRevisionKey(revisionId) : @"revid",
