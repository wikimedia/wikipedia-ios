...
NSString *const WMFFaceDetectionErrorDomain =
    @"org.wikimedia.face-detection-error";
...
+ (instancetype)wmf_sharedFaceDetector {
  ...
  dispatch_once(&onceToken, ^{
    defaultFaceDetector = [CIDetector
...
+ (NSDictionary *)wmf_featurelessFaceOptions {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    featurelessFaceOptions = @{CIDetectorAccuracy : CIDetectorAccuracyLow};
  });
  return featurelessFaceOptions;
}

- (void)wmf_detectFeaturelessFacesInImage:(UIImage *)image
...
- (void)wmf_detectFeaturesInImage:(UIImage *)image
  ...
  dispatch_async(queue, ^{
    if ([[UIApplication sharedApplication] applicationState] ==
      ...
      id features =
          [self featuresInImage:[image wmf_getOrCreateCIImage] options:options];
      success(features);
    } else {
      failure([NSError errorWithDomain:WMFFaceDetectionErrorDomain
                                  code:WMFFaceDectionErrorAppInBackground
                              userInfo:nil]);
    }
  });
}
