...
+ (instancetype)imageInfoWithExportedData:(NSDictionary *)exportedData {
  ...
  return [[MWKImageInfo alloc]
      initWithCanonicalPageTitle:exportedData[MWKImageInfoCanonicalPageTitleKey]
                canonicalFileURL:
                    [NSURL URLWithString:exportedData
                                             [MWKImageInfoCanonicalFileURLKey]]
                imageDescription:exportedData[MWKImageInfoImageDescriptionKey]
                         license:[MWKLicense
                                     licenseWithExportedData:
                                         exportedData[MWKImageInfoLicenseKey]]
                     filePageURL:
                         [NSURL URLWithString:exportedData
                                                  [MWKImageInfoFilePageURLKey]]
                   imageThumbURL:
                       [NSURL URLWithString:exportedData
                                                [MWKImageInfoImageThumbURLKey]]
                           owner:exportedData[MWKImageInfoOwnerKey]
                       imageSize:CGSizeFromString(
                                     exportedData[MWKImageInfoImageSize])
                       thumbSize:CGSizeFromString(
                                     exportedData[MWKImageInfoThumbSize])];
}
...
- (NSDictionary *)dataExport {
  ...
  [dict wmf_maybeSetObject:self.canonicalPageTitle
                    forKey:MWKImageInfoCanonicalPageTitleKey];
  [dict wmf_maybeSetObject:self.canonicalFileURL.absoluteString
                    forKey:MWKImageInfoCanonicalFileURLKey];
  [dict wmf_maybeSetObject:self.imageDescription
                    forKey:MWKImageInfoImageDescriptionKey];
  [dict wmf_maybeSetObject:self.filePageURL.absoluteString
                    forKey:MWKImageInfoFilePageURLKey];
  [dict wmf_maybeSetObject:self.imageThumbURL.absoluteString
                    forKey:MWKImageInfoImageThumbURLKey];
  [dict wmf_maybeSetObject:self.owner forKey:MWKImageInfoOwnerKey];

  [dict wmf_maybeSetObject:[self.license dataExport]
                    forKey:MWKImageInfoLicenseKey];
...
- (NSUInteger)hash {
  return self.canonicalPageTitle.hash ^
         flipBitsWithAdditionalRotation(self.canonicalFileURL.hash, 1);
}

- (NSString *)description {
  return [NSString stringWithFormat:@"%@ %@ %@", [super description],
                                    self.canonicalPageTitle,
                                    self.canonicalFileURL];
}
...
- (id)imageAssociationValue {
  if (!_imageAssociationValue) {
    _imageAssociationValue =
        WMFParseImageNameFromSourceURL(self.canonicalFileURL);
  }
