...
// Designated initializer, can be used to inject a mock request manager while
// testing.
...
- (AnyPromise *)fetchGalleryInfoForImagesOnPages:(NSArray *)pageTitles
  ...
  return [AnyPromise promiseWithResolverBlock:^(PMKResolver _Nonnull resolve) {
    [self
        fetchInfoForTitles:pageTitles
               fromSiteURL:siteURL
            thumbnailWidth:[[UIScreen mainScreen] wmf_galleryImageWidthForScale]
           extmetadataKeys:[MWKImageInfoResponseSerializer
                               galleryExtMetadataKeys]
...
fetchGalleryInfoForImageFiles:(NSArray *)imageTitles
  ...
  return [self
      fetchInfoForTitles:imageTitles
             fromSiteURL:siteURL
          thumbnailWidth:[[UIScreen mainScreen] wmf_galleryImageWidthForScale]
...
- (id<MWKImageInfoRequest>)
fetchInfoForTitles:(NSArray *)titles
       fromSiteURL:(NSURL *)siteURL
  ...
  NSMutableDictionary *params = [@{
    ...
    @"prop" : @"imageinfo",
    @"iiprop" :
        WMFJoinedPropertyParameters(@[ @"url", @"extmetadata", @"dimensions" ]),
  ...
  @weakify(self);
  NSURLSessionDataTask *request = [self.manager wmf_GETAndRetryWithURL:siteURL
      parameters:params
      retry:nil
