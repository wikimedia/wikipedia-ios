...
- (instancetype)initWithArticleURL:(NSURL *)articleURL
                    ...
                    diceButtonItem:(UIBarButtonItem *)diceButtonItem {
...
- (void)setupSecondToolbar {
  if (!self.diceButtonItem) {
    self.diceButton =
        [[WMFRandomDiceButton alloc] initWithFrame:CGRectMake(0, 0, 57, 57)];
    self.diceButtonItem =
        [[UIBarButtonItem alloc] initWithCustomView:self.diceButton];
  }

  UIBarButtonItem *leftFlexibleSpace = [[UIBarButtonItem alloc]
      initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace
                           target:nil
                           action:NULL];

  self.anotherRandomArticleButton =
      [UIButton buttonWithType:UIButtonTypeSystem];
  [self.anotherRandomArticleButton
      setTitle:MWLocalizedString(@"explore-another-random", nil)
      forState:UIControlStateNormal];
  CGSize size = [self.anotherRandomArticleButton
      sizeThatFits:CGSizeMake(CGFLOAT_MAX, 57)];
  self.anotherRandomArticleButton.frame = (CGRect){CGPointZero, size};
  [self.anotherRandomArticleButton
             addTarget:self
                action:@selector(loadAndShowAnotherRandomArticle:)
      forControlEvents:UIControlEventTouchUpInside];
  [self.anotherRandomArticleButton setTintColor:[UIColor wmf_blueTintColor]];
  UIBarButtonItem *anotherRandomItem = [[UIBarButtonItem alloc]
      initWithCustomView:self.anotherRandomArticleButton];

  UIBarButtonItem *rightFlexibleSpace = [[UIBarButtonItem alloc]
      initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace
                           target:nil
                           action:NULL];

  self.secondToolbarItems = @[
    leftFlexibleSpace, self.diceButtonItem, anotherRandomItem,
    rightFlexibleSpace
  ];
}
...
- (void)setupEmptyFadeView {
  ...
  self.emptyFadeView.autoresizingMask =
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.diceButton addTarget:self
                      action:@selector(loadAndShowAnotherRandomArticle:)
            forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)viewWillDisappear:(BOOL)animated {
  ...
  [self.diceButton removeTarget:self
                         action:@selector(loadAndShowAnotherRandomArticle:)
               forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)configureViewsForRandomArticleLoading:(BOOL)isRandomArticleLoading
  ...
  if (animated) {
    [UIView animateWithDuration:WMFRandomAnimationDurationFade
...
- (void)loadAndShowAnotherRandomArticle:(id)sender {
  ...
  [self.randomArticleFetcher fetchRandomArticleWithSiteURL:siteURL
      ...
      success:^(MWKSearchResult *result) {
        ...
        WMFRandomArticleViewController *randomArticleVC =
            [[WMFRandomArticleViewController alloc]
                initWithArticleURL:titleURL
                         dataStore:self.dataStore
                    diceButtonItem:self.diceButtonItem];
...
- (void)webViewController:(WebViewController *)controller
  ...
  WMFArticleNavigationController *articleNavgiationController =
      (WMFArticleNavigationController *)self.navigationController;
  if (![articleNavgiationController
          isKindOfClass:[WMFArticleNavigationController class]]) {
    return;
  }
  ...
  if (articleNavgiationController.secondToolbarHidden) {
    BOOL shouldShowRandomButton =
        newContentOffsetY <= 0 ||
        (!scrollView.tracking && scrollView.decelerating &&
         newContentOffsetY < self.previousContentOffsetY &&
         newContentOffsetY <
             (scrollView.contentSize.height - scrollView.bounds.size.height));
    shouldHideRandomButton = !shouldShowRandomButton;
  } else {
    shouldHideRandomButton = newContentOffsetY > 0 &&
                             newContentOffsetY > self.previousContentOffsetY;
  }

  if (articleNavgiationController.secondToolbarHidden !=
      shouldHideRandomButton) {
    [articleNavgiationController setSecondToolbarHidden:shouldHideRandomButton
                                               animated:YES];
  }
