...
dispatchTimeFromNowWithDelta(NSTimeInterval seconds) {
  return dispatch_time(DISPATCH_TIME_NOW,
                       (int64_t)nanosecondsWithSeconds(seconds));
}
...
static inline void dispatchAfterDelayInSeconds(NSTimeInterval delay,
...
static inline void
dispatchOnMainQueueAfterDelayInSeconds(NSTimeInterval delay,
                                       dispatch_block_t block) {
