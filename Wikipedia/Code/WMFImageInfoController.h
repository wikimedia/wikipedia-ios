...
/// Initialize with @c article, letting the receiver create the default @c
/// fetcher and @c imageFetcher.
...
/**
 * Fetch the image info batch which contains @c index if it hasn't already been
 * fetched.
 *
 * @return The request to fetch the specified batch, or @c nil if it has already
 * been fetched or the index is
 *         out of bounds.
 */
 ...
 *              <pre>[imageInfoController
 * fetchBatchContainingIndex:index+next];</pre>
 *
 * Note that this will either result in one fetch (@c index and <code>index +
 * next</code> is in the same batch or
 * two fetches if @c index and <code>index + next</code> are in different
 * batches.
 */
...
/// @return The @c MWKImageInfo object which is associated with @c image, or @c
/// nil if none exists.
- (MWKImageInfo *__nullable)infoForImage:(MWKImage *)image;

/// @return The index of the @c MWKImage associated with @c info, or @c
/// NSNotFound.
