...
static NSString *const MWKSavedPageEntryDidMigrateImageDataKey =
    @"didMigrateImageData";
...
- (instancetype)initWithURL:(NSURL *)url {
  ...
  if (self) {
    ...
    // defaults to true for instances since new image data will go to the
    // correct location
...
- (instancetype)initWithDict:(NSDictionary *)dict {
  ...
  if ([urlString length]) {
    ...
    url = [NSURL wmf_URLWithDomain:domain
                          language:language
                             title:title
                          fragment:nil];
  } else {
  ...
  if (self) {
    ...
    if (schemaVersion.unsignedIntegerValue >= MWKSavedPageEntrySchemaVersion1) {
      self.didMigrateImageData =
          [[self requiredNumber:MWKSavedPageEntryDidMigrateImageDataKey
                           dict:dict] boolValue];
    } else {
...
- (id)dataExport {
  ...
  [dict wmf_maybeSetObject:@(MWKSavedPageEntrySchemaVersionCurrent)
                    forKey:MWKSavedPageEntrySchemaVersionKey];
  [dict wmf_maybeSetObject:@(self.didMigrateImageData)
                    forKey:MWKSavedPageEntryDidMigrateImageDataKey];
