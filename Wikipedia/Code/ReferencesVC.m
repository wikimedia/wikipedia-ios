...
- (void)setupTopContainer {
  ...
  [self.xButton.label setWikiText:WIKIGLYPH_X
                   ...
                   baselineOffset:0];
  self.xButton.accessibilityLabel =
      MWLocalizedString(@"close-button-accessibility-label", nil);
  ...
  [self.nextButton.label
         setWikiText:isRTL ? WIKIGLYPH_BACKWARD : WIKIGLYPH_FORWARD
               color:[UIColor darkGrayColor]
  ...
  [self.prevButton.label
         setWikiText:isRTL ? WIKIGLYPH_FORWARD : WIKIGLYPH_BACKWARD
               color:[UIColor darkGrayColor]
                size:24.0 * MENUS_SCALE_MULTIPLIER
      baselineOffset:2.0];
  ...
  UITapGestureRecognizer *prevTap =
...
- (void)setupConstraints {
  ...
  [self.topContainerView
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:
                             @"V:|-(vPadding)-[nextButton]-(vPadding)-|"
                                             options:0
                                             metrics:metrics
                                               views:views]];
  ...
  [self.topContainerView
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:
                             @"V:|-(vPadding)-[prevButton]-(vPadding)-|"
                                             options:0
                                             metrics:metrics
                                               views:views]];
  ...
  [self.topContainerView
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:
                             @"H:[prevButton]-(20)-[nextButton]-(hPadding)-|"
                                             options:0
                                             metrics:metrics
                                               views:views]];
  ...
  [self.topContainerView
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:
                             @"V:|-(vPadding)-[topPageControl]-(vPadding)-|"
                                             options:0
                                             metrics:metrics
                                               views:views]];
  ...
  [self.view
      addConstraints:[NSLayoutConstraint
                         constraintsWithVisualFormat:@"H:|[topContainerView]|"
                                             options:0
                                             metrics:metrics
                                               views:views]];

  [self adjustConstraintsScaleForViews:@[
    self.xButton, self.nextButton, self.prevButton, self.topContainerView,
    self.topPageControl
  ]];
}
  ...
  if (isRTL) {
    ...
    if (a.count > 0) {
      ...
      payload = @{
        ...
        @"linkText" :
            [[payload[@"linkText"] reverseObjectEnumerator] allObjects],
...
- (void)setPayload:(NSDictionary *)payload {
  ...
  // Use alpha - other changes to page control properties apparently cause it to
  // be
...
- (void)setViewControllers:(NSArray *)viewControllers
  ...
  // UIPageViewController is ridiculous. Making it jump to a specific page is
  // tricky - it
  // will not then swipe back or forward correctly. This method fixes that. Use
  // it rather
                  ...
                  [weakSelf.pageController
                      setViewControllers:viewControllers
                              ...
                              completion:^(BOOL done) {
                                [weakSelf
                                    refViewDidAppear:weakSelf.topPageControl
                                                         .currentPage];
                              }];
                });
              }];
...
- (ReferenceVC *)viewControllerAtIndex:(NSInteger)index {
  ...
  ReferenceVC *refVC =
      [self.storyboard instantiateViewControllerWithIdentifier:@"ReferenceVC"];
  refVC.delegate = self;
  refVC.index = index;
...
- (void)referenceViewController:(ReferenceVC *)referenceViewController
  ...
  [self.delegate referenceViewController:self
              didShowReferenceWithLinkID:linkID];
}

- (void)referenceViewController:(ReferenceVC *)referenceViewController
    didFinishShowingReferenceWithLinkID:(NSString *)linkID {
  [self.delegate referenceViewController:self
      didFinishShowingReferenceWithLinkID:linkID];
}

- (void)referenceViewController:(ReferenceVC *)referenceViewController
    didSelectInternalReferenceWithFragment:(NSString *)fragment {
  [self.delegate referenceViewController:self
      didSelectInternalReferenceWithFragment:fragment];
}
...
- (void)referenceViewController:(ReferenceVC *)referenceViewController
  ...
  [self.delegate referenceViewController:self
       didSelectExternalReferenceWithURL:url];
}
...
- (UIViewController *)pageViewController:
  ...
  return
      [self viewControllerAtIndex:(((ReferenceVC *)viewController).index - 1)];
}
...
- (void)topPageControlTapped:(id)sender {
  ...
  ReferenceVC *currentView =
      [self.pageController.viewControllers objectAtIndex:0];
  BOOL isTopPageControlAhead =
      (self.topPageControl.currentPage > currentView.index);
  ...
  UIPageViewControllerNavigationDirection dir =
      isTopPageControlAhead ? UIPageViewControllerNavigationDirectionForward
                            : UIPageViewControllerNavigationDirectionReverse;
...
- (void)prevButtonTap:(UITapGestureRecognizer *)recognizer {
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    ...
    UIPageViewControllerNavigationDirection dir =
        isRTL ? UIPageViewControllerNavigationDirectionForward
              : UIPageViewControllerNavigationDirectionReverse;

    [self setViewControllers:@[
      [self viewControllerAtIndex:(--self.topPageControl.currentPage)]
    ]
                   direction:dir
                    animated:YES
                  completion:nil];
...
- (void)nextButtonTap:(UITapGestureRecognizer *)recognizer {
  if (recognizer.state == UIGestureRecognizerStateEnded) {
    ...
    UIPageViewControllerNavigationDirection dir =
        isRTL ? UIPageViewControllerNavigationDirectionReverse
              : UIPageViewControllerNavigationDirectionForward;
