...
extension WKWebView {
    ...
    @objc public func wmf_loadArticle2(_ article: MWKArticle){
        ...
        let newJSLocalizedStrings = "new window.wmf.sectionTransformation.LocalizedStrings('\(infoboxTitle)', '\(tableTitle)', '\(closeBoxText)', '\(readMoreHeading)', '\(licenseString)', '\(licenseSubstitutionString)', '\(viewInBrowserString)', '\(menuHeading)', '\(menuLanguagesTitle)', '\(menuLastEditedTitle)', '\(menuTalkPageTitle)', '\(menuPageIssuesTitle)', '\(menuDisambiguationTitle)', '\(menuCoordinateTitle)')"
