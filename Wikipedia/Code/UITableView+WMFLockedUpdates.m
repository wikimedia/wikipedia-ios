...
- (void)wmf_performUpdates:(dispatch_block_t)updates
  ...
  [UIView performWithoutAnimation:^{
    ...
    if (self.contentSize.height <= self.frame.size.height) {
      DDLogVerbose(@"Content size too small after updates, not adjusting "
                   @"content offset.");
      return;
    }

    UITableViewCell *newLockedCell =
        [self cellForRowAtIndexPath:lockedIndexPath];
    if (!newLockedCell) {
      DDLogVerbose(
          @"Can't find cell to lock for %@ after updates, skipping adjustment.",
          lockedIndexPath);
      return;
    }
    ...
    // ???: if deleting from above selected/focused row, do we need to limit
    // auto-scrolling to contentInset?

    DDLogVerbose(@"Adjusting content offset to %@ to prevent updates from "
                 @"moving cell at %@.",
                 NSStringFromCGPoint(newContentOffset), lockedIndexPath);
