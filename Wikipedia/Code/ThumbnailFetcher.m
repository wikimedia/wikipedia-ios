...
- (void)fetchWithManager:(AFHTTPSessionManager *)manager {
  ...
  [manager GET:self.url
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        NSError *error = nil;
        if (![self isDataResponseValid:responseObject] || !self.url ||
            (self.url.length == 0)) {
          ...
          error = [NSError errorWithDomain:@"Thumbnail Fetcher"
                                  ...
                                  userInfo:@{
                                    NSLocalizedDescriptionKey :
                                        [@"Thumbnail not retrieved. URL: "
                                            stringByAppendingString:errorUrl]
                                  }];
        }

        [self finishWithError:error fetchedData:responseObject];
      }
