...
- (void)viewDidLoad {
  ...
  [self.reloadCaptchaButton setTitle:MWLocalizedString(@"captcha-reload", nil)
                            forState:UIControlStateNormal];

  [self.captchaTextBox
      setPlaceholder:MWLocalizedString(@"captcha-prompt", nil)];
  ...
  [self adjustConstraintsScaleForViews:@[
    self.captchaImageView, self.captchaTextBox, self.reloadCaptchaButton
  ]];

  self.reloadCaptchaButton.titleLabel.font =
      [UIFont systemFontOfSize:15.0 * MENUS_SCALE_MULTIPLIER];
  self.captchaTextBox.font =
      [UIFont systemFontOfSize:15.0 * MENUS_SCALE_MULTIPLIER];
}

- (void)reloadCaptchaPushed:(id)sender {
  if ([self.parentViewController
          respondsToSelector:@selector(reloadCaptchaPushed:)]) {
    [self.parentViewController
        performSelectorOnMainThread:@selector(reloadCaptchaPushed:)
                         withObject:nil
                      waitUntilDone:YES];
  }
}
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  // buttons are tapped.
  if ([self.parentViewController
          conformsToProtocol:@protocol(UITextFieldDelegate)]) {
    self.captchaTextBox.delegate =
        (id<UITextFieldDelegate>)self.parentViewController;
  }
}
