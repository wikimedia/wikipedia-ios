...
static NSString *const WMFNumericTransformerErrorDomain =
    @"WMFNumericTransformerErrorDomain";
...
+ (instancetype)wmf_numericValueTransformer {
  ...
  return [MTLValueTransformer transformerUsingForwardBlock:^id(
                                  id stringOrNumber, BOOL *success,
      ...
      if ([[NSScanner scannerWithString:stringOrNumber] scanDouble:&value]) {
        ...
        *success = NO;
        WMFSafeAssign(
            error,
            [NSError
                errorWithDomain:WMFNumericTransformerErrorDomain
                           code:WMFNumericTransformerErrorDomainInvalidString
                       userInfo:nil]);
        return nil;
      }
    } else {
      return [validatingNumberTransformer transformedValue:stringOrNumber
                                                   success:success
                                                     error:error];
    }
  }];
