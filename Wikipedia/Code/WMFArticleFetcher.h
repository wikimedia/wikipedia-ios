...
/**
 *  Fetch the latest version of @c URL, if the locally stored revision is not
 * the latest.
 *
 *  @param title    The title to fetch.
 *  @param progress Block which will be invoked with download progress.
 *
 *  @return A promise which resolves to an article object. If there was a cached
 * article, and an error was encountered,
 *          the error's @c userInfo will contain the cached article for the key
 * @c WMFArticleFetcherErrorCachedFallbackArticleKey.
 */
- (AnyPromise *)
...
/**
 *  Fetch the latest version of @c URL, if the locally stored revision is not
 * the latest. If forceDownload is passed, the latest version is always
 * downloaded ignoring any cahced data
 *
 *  @param title    The title to fetch.
 *  @param forceDownload If YES, the article will be downloaded even if it is
 * already cached.
 *  @param progress Block which will be invoked with download progress.
 *
 *  @return A promise which resolves to an article object. If there was a cached
