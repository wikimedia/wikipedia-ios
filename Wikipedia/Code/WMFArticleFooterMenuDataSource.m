...
- (instancetype)initWithArticle:(nullable MWKArticle *)article {
  ...
  if (self) {
    ...
    self.cellConfigureBlock =
        ^(WMFArticleFooterMenuCell *cell, WMFArticleFooterMenuItem *menuItem,
    ...
    self.tableActionBlock =
        ^BOOL(SSCellActionType action, UITableView *tableView,
...
- (void)updateItemsForArticle:(nullable MWKArticle *)article {
  ...
  WMFArticleFooterMenuItem * (^makeItem)(WMFArticleFooterMenuItemType,
                                         NSString *, NSString *, NSString *) =
      ^WMFArticleFooterMenuItem *(WMFArticleFooterMenuItemType type,
  ...
  NSMutableArray<WMFArticleFooterMenuItem *> *menuItems =
    ...
    [menuItems
        addObject:
            makeItem(
                ...
                [MWSiteLocalizedString(article.url,
                                       @"page-read-in-other-languages", nil)
  ...
  NSDate *lastModified =
      article.lastmodified ? article.lastmodified : [NSDate date];

  if (FBTweakValue(@"Article", @"Article Metadata Footer",
                   @"Show last edit timestamp", NO)) {
    [menuItems addObject:makeItem(WMFArticleFooterMenuItemTypeLastEdited,
                                  [lastModified mediumString],
                                  MWSiteLocalizedString(
                                      article.url, @"page-edit-history", nil),
                                  @"footer-edit-history")];
  } else {
    [menuItems
        addObject:
            makeItem(
                ...
                [MWSiteLocalizedString(article.url, @"page-last-edited", nil)
                    stringByReplacingOccurrencesOfString:@"$1"
                                              withString:
                                                  [NSString
                                                      stringWithFormat:
  ...
  if (article.disambiguationURLs.count > 0) {
    [menuItems addObject:makeItem(WMFArticleFooterMenuItemTypeDisambiguation,
                                  MWSiteLocalizedString(
                                      article.url, @"page-similar-titles", nil),
                                  nil, @"footer-similar-pages")];
  }
