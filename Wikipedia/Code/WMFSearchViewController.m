...
@interface WMFSearchViewController () <
    UISearchBarDelegate, WMFRecentSearchesViewControllerDelegate,
    UITextFieldDelegate, WMFArticleListTableViewControllerDelegate,
...
@property(nonatomic, strong)
    WMFSearchResultsTableViewController *resultsListController;
...
@property(strong, nonatomic) IBOutletCollection(UIButton)
    NSArray *languageButtons;

@property(nonatomic, strong) WMFSearchFetcher *fetcher;

@property(nonatomic, strong)
    IBOutlet NSLayoutConstraint *suggestionButtonHeightConstraint;
...
@property(nonatomic, assign, getter=isRecentSearchesHidden)
    BOOL recentSearchesHidden;

@property(nonatomic, assign) UIStatusBarStyle previousStatusBarStyle;

- (void)setRecentSearchesHidden:(BOOL)hidingRecentSearches
                       animated:(BOOL)animated;

/**
 *  Set the text of the search field programatically.
 *
 *  Sets the text on the receiver's @c searchField and updates the vertical
 * separator's visibility.  This is solely
 *  for cases when the user searches for something without typing it manually or
...
+ (instancetype)searchViewControllerWithDataStore:(MWKDataStore *)dataStore {
  ...
  WMFSearchViewController *searchVC =
      [self wmf_initialViewControllerFromClassStoryboard];
...
- (NSString *)searchSuggestion {
  return
      [[self.resultsListController.dataSource searchResults] searchSuggestion];
}
...
- (void)updateRecentSearchesVisibility:(BOOL)animated {
  BOOL hideRecentSearches =
      [self.searchField.text wmf_trim].length > 0 ||
      [self.dataStore.userDataStore.recentSearchList countOfEntries] == 0;
...
- (void)setRecentSearchesHidden:(BOOL)hidingRecentSearches
  ...
  [UIView animateWithDuration:animated ? [CATransaction animationDuration] : 0.0
                   ...
                   animations:^{
                     self.recentSearchesContainerView.alpha =
                         self.isRecentSearchesHidden ? 0.0 : 1.0;
                     self.resultsListContainerView.alpha =
                         1.0 - self.recentSearchesContainerView.alpha;
                   }
                   completion:nil];
}
...
- (void)configureRecentSearchList {
  self.recentSearchesViewController.recentSearches =
      self.dataStore.userDataStore.recentSearchList;
...
- (void)configureSearchField {
  ...
  [self.searchField
      setPlaceholder:MWLocalizedString(@"search-field-placeholder-text", nil)];
}
...
- (void)configureLanguageButtons {
  if ([[NSUserDefaults standardUserDefaults] wmf_showSearchLanguageBar]) {
    ...
    [self.searchContentContainer
        mas_remakeConstraints:^(MASConstraintMaker *make) {
          make.top.equalTo(self.languageBarContainer.mas_bottom);
        }];

    [self.languageButtons
        enumerateObjectsUsingBlock:^(UIButton *_Nonnull obj, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
          obj.tintColor = [UIColor wmf_blueTintColor];
        }];

    UIImage *buttonBackground =
        [UIImage wmf_imageFromColor:[UIColor whiteColor]];
    UIImage *highlightedButtonBackground =
        [UIImage wmf_imageFromColor:[UIColor colorWithWhite:0.9 alpha:1]];
    [self.otherLanguagesButton setBackgroundImage:buttonBackground
                                         forState:UIControlStateNormal];
    [self.otherLanguagesButton setBackgroundImage:highlightedButtonBackground
                                         forState:UIControlStateHighlighted];
    [self.otherLanguagesButton.layer setCornerRadius:2.0f];
    [self.otherLanguagesButton setClipsToBounds:YES];

    [self.otherLanguagesButton
        setTitle:MWLocalizedString(@"main-menu-title", nil)
        forState:UIControlStateNormal];
...
- (void)viewDidLoad {
  ...
  self.resultsListController.tableView.keyboardDismissMode =
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  [NSUserActivity
      wmf_makeActivityActive:[NSUserActivity wmf_searchViewActivity]];
}

- (void)viewWillDisappear:(BOOL)animated {
  [super viewWillDisappear:animated];
  [[UIApplication sharedApplication]
      setStatusBarStyle:self.previousStatusBarStyle
               animated:animated];

  if (!self.presentedViewController) {
    /*
       Only perform animations & search site sync if search is being modally
       dismissed (as opposed to having another
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if ([segue.destinationViewController
          isKindOfClass:[WMFArticleListTableViewController class]]) {
...
- (IBAction)textFieldDidChange {
  ...
  dispatchOnMainQueueAfterDelayInSeconds(0.4, ^{
    ...
    /**
     *  This check must performed before checking isEmpty and calling
     * didCancelSearch
     *  This is to work around a "feature" of Siri which sets the textfield.text
     * to nil
     *  when cancelling the Siri interface, and then immediately sets the text
     ...
     *  textFieldDidChange fires with textfield.text="" (This is the offending
     * event!)
     *  textFieldDidChange fires with textfield.text="Mountain"
     *
     *  The event setting the textfield.text == nil causes many side effects
     * which can cause crashes like:
     *  https://phabricator.wikimedia.org/T123241
     */
    if (![query isEqualToString:self.searchField.text]) {
      DDLogInfo(@"Aborting search for %@ since query has changed to %@", query,
  ...
  [self.fetcher fetchArticlesForSearchTerm:searchTerm
      ...
      .thenOn(
          ...
          ^id(WMFSearchResults *results) {
            ...
            /*
               HAX: must set dataSource before starting the animation since
               dataSource is _unsafely_ assigned to the
               collection view, meaning there's a chance the collectionView
               accesses deallocated memory during an animation
             */
            ...
            [NSUserActivity
                wmf_makeActivityActive:
                    [NSUserActivity
                        wmf_searchResultsActivitySearchSiteURL:url
                                                    searchTerm:
                                                        results.searchTerm]];
            ...
            if ([results.results count] <
              ...
              return [self.fetcher
                  fetchArticlesForSearchTerm:searchTerm
                                     siteURL:url
                                 resultLimit:WMFMaxSearchResultLimit
                              fullTextSearch:YES
        ...
        if ([searchTerm isEqualToString:results.searchTerm]) {
          if (results.results.count == 0) {
            dispatchOnMainQueueAfterDelayInSeconds(0.25, ^{
              // Without the delay there is a weird animation due to the table
              // also reloading simultaneously
              [self.resultsListController
                  wmf_showEmptyViewOfType:WMFEmptyViewTypeNoSearchResults];
            });
          }
        }

        // change recent search visibility if no prefix results returned, and
...
- (void)updateSearchSuggestion:(NSString *)searchSuggestion {
  NSAttributedString *title =
      [searchSuggestion length]
          ? [self getAttributedStringForSuggestion:searchSuggestion]
          : nil;
  [self.searchSuggestionButton setAttributedTitle:title
                                         forState:UIControlStateNormal];
  [self.view setNeedsUpdateConstraints];
  [self.view layoutIfNeeded];
}
...
- (void)updateViewConstraints {
  ...
  self.searchFieldHeight.constant =
      [self searchFieldHeightForCurrentTraitCollection];
...
- (NSAttributedString *)getAttributedStringForSuggestion:
    (NSString *)suggestion {
...
- (void)saveLastSearch {
  if ([self currentResultsSearchTerm]) {
    MWKRecentSearchEntry *entry = [[MWKRecentSearchEntry alloc]
        initWithURL:[self currentResultsSearchSiteURL]
...
- (MWKLanguageLink *)selectedLanguage {
  NSURL *siteURL =
      [[NSUserDefaults standardUserDefaults] wmf_currentSearchLanguageDomain];
  MWKLanguageLink *lang = nil;
  if (siteURL) {
    lang =
        [[MWKLanguageLinkController sharedInstance] languageForSiteURL:siteURL];
  } else {
...
- (void)setSelectedLanguage:(MWKLanguageLink *)language {
  [[NSUserDefaults standardUserDefaults]
      wmf_setCurrentSearchLanguageDomain:language.siteURL];
  [self updateLanguageBarLanguages];
...
- (NSArray<MWKLanguageLink *> *)languageBarLanguages {
  return [[MWKLanguageLinkController sharedInstance].preferredLanguages
      wmf_arrayByTrimmingToLength:3];
}

- (nullable MWKLanguageLink *)appLanguage {
  MWKLanguageLink *language =
      [[MWKLanguageLinkController sharedInstance] appLanguage];
  NSAssert(language, @"No app language data found");
  return language;
}

- (void)updateLanguageBarLanguages {
  [[self languageBarLanguages]
      enumerateObjectsUsingBlock:^(MWKLanguageLink *_Nonnull obj,
...
- (void)selectLanguageForURL:(NSURL *)url {
  ...
  [[self languageBarLanguages]
      enumerateObjectsUsingBlock:^(MWKLanguageLink *_Nonnull language,
                                   NSUInteger idx, BOOL *_Nonnull stop) {
        if ([[language siteURL] isEqual:url]) {
          UIButton *buttonToSelect = self.languageButtons[idx];
          [self.languageButtons
              enumerateObjectsUsingBlock:^(
                  UIButton *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
  ...
  if (![url isEqual:[self.resultsListController.dataSource searchSiteURL]] ||
      [query
          isEqualToString:[self.resultsListController.dataSource searchResults]
                              .searchTerm]) {
    [self searchForSearchTerm:query];
  }
}
...
- (IBAction)openLanguagePicker:(id)sender {
  WMFLanguagesViewController *languagesVC =
      [WMFPreferredLanguagesViewController preferredLanguagesViewController];
...
- (void)languagesController:(WMFLanguagesViewController *)controller
  ...
  // The selected preferred language will not be displayed becuase we only
  // dsiplay max 3 languages, move it to index 2
  if (index == NSNotFound) {
    [[MWKLanguageLinkController sharedInstance]
        reorderPreferredLanguage:language
                         toIndex:2];
    [self updateLanguageBarLanguages];
  }
...
- (void)listViewController:(WMFArticleListTableViewController *)listController
       didSelectArticleURL:(nonnull NSURL *)url {
  // log tap through done in table
  UIViewController *presenter = [self presentingViewController];
  [self dismissViewControllerAnimated:YES
                           completion:^{
                             [presenter wmf_pushArticleWithURL:url
...
- (UIViewController *)listViewController:
    ...
    viewControllerForPreviewingArticleURL:(nonnull NSURL *)url {
...
- (void)listViewController:(WMFArticleListTableViewController *)listController
  ...
  [self dismissViewControllerAnimated:YES
                           completion:^{
                             [presenter
                                 wmf_pushArticleViewController:
                                     (WMFArticleViewController *)viewController
                                                      animated:YES];
                           }];
}
