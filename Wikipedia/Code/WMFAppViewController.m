 ...
 *
 *  @warning Be sure to update `WMFAppTabCount` when these enums change, and
 * always initialize the first enum to 0.
 *
 *  @see WMFAppTabCount
 */
 ...
 *
 *  @warning Kept as a separate constant to prevent switch statements from being
 * considered inexhaustive. This means we
...
- (BOOL)isPresentingOnboarding {
  return [self.presentedViewController
      isKindOfClass:[WMFWelcomeViewController class]];
}
...
- (void)loadMainUI {
  ...
  UITabBarController *tabBar =
      [[UIStoryboard storyboardWithName:@"WMFTabBarUI" bundle:nil]
...
- (void)configureTabController {
  ...
  for (WMFAppTabType i = 0; i < WMFAppTabCount; i++) {
    UINavigationController *navigationController =
        [self navigationControllerForTab:i];
...
- (void)configureArticleListController:
    (WMFArticleListTableViewController *)controller {
...
- (void)endBackgroundTask {
  ...
  UIBackgroundTaskIdentifier backgroundTaskToStop =
...
+ (WMFAppViewController *)initialAppViewControllerFromDefaultStoryBoard {
  return [[UIStoryboard
      storyboardWithName:NSStringFromClass([WMFAppViewController class])
...
- (void)launchAppInWindow:(UIWindow *)window {
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(appWillEnterForegroundWithNotification:)
             name:UIApplicationWillEnterForegroundNotification
           object:nil];
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(appDidEnterBackgroundWithNotification:)
  ...
  if ([[NSProcessInfo processInfo]
    ...
    self.spotlightManager = [[WMFSavedPageSpotlightManager alloc]
        initWithDataStore:self.session.dataStore];
  }
  [self presentOnboardingIfNeededWithCompletion:^(BOOL didShowOnboarding) {
    @strongify(self)[self loadMainUI];
    [self hideSplashViewAnimated:!didShowOnboarding];
    [self resumeApp];
    [[PiwikTracker wmf_configuredInstance]
        wmf_logView:[self rootViewControllerForTab:WMFAppTabTypeExplore]];
  }];
}
...
- (void)resumeApp {
  ...
  [[WMFAuthenticationManager sharedInstance]
      loginWithSavedCredentialsWithSuccess:NULL
                                   failure:NULL];
  ...
  if (FBTweakValue(@"Alerts", @"General", @"Show error on launch", NO)) {
    [[WMFAlertManager sharedInstance]
               showErrorAlert:[NSError errorWithDomain:@"WMFTestDomain"
                                                  code:0
                                              userInfo:@{
  ...
  if (FBTweakValue(@"Alerts", @"General", @"Show warning on launch", NO)) {
    [[WMFAlertManager sharedInstance] showWarningAlert:@"You have been warned"
                                                sticky:NO
                                 dismissPreviousAlerts:NO
                                           tapCallBack:NULL];
  }
  ...
  if (FBTweakValue(@"Alerts", @"General", @"Show message on launch", NO)) {
    [[WMFAlertManager sharedInstance] showAlert:@"You have been notified"
                                         sticky:NO
                          dismissPreviousAlerts:NO
                                    tapCallBack:NULL];
  }
...
- (void)logImportantStatistics {
  NSUInteger historyCount =
      [self.session.dataStore.userDataStore.historyList countOfEntries];
  NSUInteger saveCount =
      [self.session.dataStore.userDataStore.savedPageList countOfEntries];
  NSUInteger exploreCount =
      [self.exploreViewController numberOfSectionsInExploreFeed];
  UINavigationController *navVC =
      [self navigationControllerForTab:self.rootTabBarController.selectedIndex];
...
- (void)processShortcutItem:(UIApplicationShortcutItem *)item
...
- (BOOL)processUserActivity:(NSUserActivity *)activity {
  ...
  case WMFUserActivityTypeExplore:
    ...
    [[self navigationControllerForTab:WMFAppTabTypeExplore]
        popToRootViewControllerAnimated:NO];
    break;
  case WMFUserActivityTypeSavedPages:
    [self.rootTabBarController setSelectedIndex:WMFAppTabTypeSaved];
    [[self navigationControllerForTab:WMFAppTabTypeSaved]
        popToRootViewControllerAnimated:NO];
    break;
  case WMFUserActivityTypeHistory:
    [self.rootTabBarController setSelectedIndex:WMFAppTabTypeRecent];
    [[self navigationControllerForTab:WMFAppTabTypeRecent]
        popToRootViewControllerAnimated:NO];
    break;
  case WMFUserActivityTypeSearch:
    [self.rootTabBarController setSelectedIndex:WMFAppTabTypeExplore];
    [[self navigationControllerForTab:WMFAppTabTypeExplore]
        popToRootViewControllerAnimated:NO];
    [[self rootViewControllerForTab:WMFAppTabTypeExplore]
        wmf_showSearchAnimated:NO];
    break;
  case WMFUserActivityTypeSearchResults:
    [self.rootTabBarController setSelectedIndex:WMFAppTabTypeExplore];
    [[self navigationControllerForTab:WMFAppTabTypeExplore]
    ...
    [[self rootViewControllerForTab:WMFAppTabTypeExplore]
        wmf_showSearchAnimated:NO];
    [[UIViewController wmf_sharedSearchViewController]
        setSearchTerm:[activity wmf_searchTerm]];
    break;
  case WMFUserActivityTypeArticle: {
...
- (void)showArticleForURL:(NSURL *)articleURL animated:(BOOL)animated {
  ...
  [self.rootTabBarController setSelectedIndex:WMFAppTabTypeExplore];
  [[self exploreViewController] wmf_pushArticleWithURL:articleURL
                                             dataStore:self.session.dataStore
                                 restoreScrollPosition:YES
                                              animated:animated];
}
...
- (BOOL)shouldShowExploreScreenOnLaunch {
  ...
  if (fabs([resignActiveDate timeIntervalSinceNow]) >=
      WMFTimeBeforeRefreshingExploreScreen) {
    return YES;
  }
  return NO;
}
...
- (BOOL)exploreViewControllerIsDisplayingContent {
  return [self navigationControllerForTab:WMFAppTabTypeExplore]
...
- (NSURL *)onscreenURL {
  UINavigationController *navVC =
      [self navigationControllerForTab:self.rootTabBarController.selectedIndex];
  if ([navVC.topViewController
          isKindOfClass:[WMFArticleViewController class]]) {
...
- (UINavigationController *)navigationControllerForTab:(WMFAppTabType)tab {
  return (
      UINavigationController *)[self.rootTabBarController viewControllers][tab];
}

- (UIViewController<WMFAnalyticsViewNameProviding> *)rootViewControllerForTab:
    (WMFAppTabType)tab {
...
- (SavedArticlesFetcher *)savedArticlesFetcher {
  if (!_savedArticlesFetcher) {
    _savedArticlesFetcher = [[SavedArticlesFetcher alloc]
        initWithSavedPageList:[[[SessionSingleton sharedInstance] userDataStore]
                                  savedPageList]];
  }
...
- (WMFExploreViewController *)exploreViewController {
  return (WMFExploreViewController *)[self
      rootViewControllerForTab:WMFAppTabTypeExplore];
}

- (WMFArticleListTableViewController *)savedArticlesViewController {
  return (WMFArticleListTableViewController *)[self
      rootViewControllerForTab:WMFAppTabTypeSaved];
}

- (WMFArticleListTableViewController *)recentArticlesViewController {
  return (WMFArticleListTableViewController *)[self
      rootViewControllerForTab:WMFAppTabTypeRecent];
}
...
- (BOOL)shouldShowOnboarding {
  if (FBTweakValue(@"Welcome", @"General",
                   @"Show on launch (requires force quit)", NO) ||
      [[NSProcessInfo processInfo] environment][@"WMFShowWelcomeView"]
          .boolValue) {
    return YES;
  }
  NSNumber *didShow =
      [[NSUserDefaults standardUserDefaults] objectForKey:WMFDidShowOnboarding];
  return !didShow.boolValue;
}

- (void)setDidShowOnboarding {
  [[NSUserDefaults standardUserDefaults] setObject:@YES
                                            forKey:WMFDidShowOnboarding];
  [[NSUserDefaults standardUserDefaults] synchronize];
}

- (void)presentOnboardingIfNeededWithCompletion:
    (void (^)(BOOL didShowOnboarding))completion {
  if ([self shouldShowOnboarding]) {
    WMFWelcomeViewController *vc =
        [WMFWelcomeViewController welcomeViewControllerFromDefaultStoryBoard];
    vc.completionBlock = ^{
...
- (void)hideSplashViewAnimated:(BOOL)animated {
  ...
  [UIView animateWithDuration:duration
      animations:^{
        self.splashView.layer.transform =
            CATransform3DMakeScale(10.0f, 10.0f, 1.0f);
...
- (BOOL)shouldShowLastReadArticleOnLaunch {
  ...
  if (FBTweakValue(@"Last Open Article", @"General", @"Restore on Launch",
                   YES)) {
    return YES;
  }
  ...
  if (fabs([resignActiveDate timeIntervalSinceNow]) <
      WMFTimeBeforeRefreshingExploreScreen) {
    if (![self exploreViewControllerIsDisplayingContent] &&
        [self.rootTabBarController selectedIndex] == WMFAppTabTypeExplore) {
      return YES;
    }
  }
...
- (void)showSearchAnimated:(BOOL)animated {
  [self.rootTabBarController setSelectedIndex:WMFAppTabTypeExplore];
  UINavigationController *exploreNavController =
      [self navigationControllerForTab:WMFAppTabTypeExplore];
...
- (void)showRandomArticleAnimated:(BOOL)animated {
  ...
  NSURL *siteURL =
      [[[MWKLanguageLinkController sharedInstance] appLanguage] siteURL];
  [self.randomFetcher fetchRandomArticleWithSiteURL:siteURL
      ...
      success:^(MWKSearchResult *result) {
        ...
        [[self exploreViewController]
            wmf_pushArticleWithURL:articleURL
                         dataStore:self.session.dataStore
                          animated:YES];
      }];
}
...
- (void)showNearbyListAnimated:(BOOL)animated {
  ...
  NSURL *siteURL =
      [[[MWKLanguageLinkController sharedInstance] appLanguage] siteURL];
  WMFNearbyListViewController *vc = [[WMFNearbyListViewController alloc]
      initWithSearchSiteURL:siteURL
...
- (void)downloadAssetsFilesIfNecessary {
  ...
  [[QueuesSingleton sharedInstance].assetsFetchManager
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController
  ...
  if (viewController == tabBarController.selectedViewController) {
    ...
    case WMFAppTabTypeExplore: {
      WMFExploreViewController *exploreViewController =
          (WMFExploreViewController *)[self exploreViewController];
      [exploreViewController scrollToTop];
    } break;
    case WMFAppTabTypeSaved: {
      WMFArticleListTableViewController *savedArticlesViewController =
          (WMFArticleListTableViewController *)
              [self savedArticlesViewController];
      [savedArticlesViewController
          scrollToTop:savedArticlesViewController.dataStore.userDataStore
                          .savedPageList.countOfEntries > 0];
    } break;
    case WMFAppTabTypeRecent: {
      WMFArticleListTableViewController *historyArticlesViewController =
          (WMFArticleListTableViewController *)
              [self recentArticlesViewController];
      [historyArticlesViewController
          scrollToTop:historyArticlesViewController.dataStore.userDataStore
                          .historyList.countOfEntries > 0];
    } break;
    }
  }
