...
@property(strong, nonatomic, nullable)
    WMFAuthManagerInfoFetcher *authManagerInfoFetcher;
@property(strong, nonatomic, nullable) WMFAuthManagerInfo *loginAuthManagerInfo;
@property(strong, nonatomic, nullable)
    WMFAuthManagerInfo *accountCreationAuthManagerInfo;
...
- (void)getAccountCreationCaptchaWithUsername:(NSString *)username
                                     password:(NSString *)password
                                        email:(nullable NSString *)email
                                      captcha:(WMFCaptchaHandler)captcha
                                      failure:(WMFErrorHandler)failure {
  if (self.successBlock || self.failBlock) {
    if (failure) {
      failure([NSError wmf_errorWithType:WMFErrorTypeFetchAlreadyInProgress
                                userInfo:nil]);
    }
    return;
  }
...
- (void)getAccountCreationCaptchaWithHandler:(WMFCaptchaHandler)captcha
                                     failure:(WMFErrorHandler)failure {
  if (self.authenticatingUsername.length == 0 ||
      self.authenticatingPassword.length == 0 || captcha == nil ||
      failure == nil) {
    if (failure) {
      failure([NSError wmf_errorWithType:WMFErrorTypeStringMissingParameter
                                userInfo:nil]);
    }
    return;
  }
  ...
  [self fetchCreationAuthManagerInfoWithSuccess:^(
            WMFAuthManagerInfo *_Nonnull info) {
    self.accountCreationAuthManagerInfo = info;
    [self fetchCreationTokensWithInfo:info
...
- (void)fetchCreationAuthManagerInfoWithSuccess:
            (nullable WMFAuthManagerInfoBlock)success
                                        failure:
                                            (nullable WMFErrorHandler)failure {
  [self.authManagerInfoFetcher
      fetchAuthManagerCreationAvailableForSiteURL:
...
- (void)fetchCreationTokensWithInfo:(WMFAuthManagerInfo *)info
                           username:(NSString *)username
                           password:(NSString *)password
                              email:(nullable NSString *)email {
  [[QueuesSingleton sharedInstance].accountCreationFetchManager
      wmf_cancelAllTasksWithCompletionHandler:^{
        (void)[[AccountCreationTokenFetcher alloc]
            initAndFetchTokenForDomain:[[MWKLanguageLinkController
                                            sharedInstance] appLanguage]
                                           .languageCode
                              userName:username
                              password:password
                                 email:email
                           withManager:[QueuesSingleton sharedInstance]
                                           .accountCreationFetchManager
                    thenNotifyDelegate:self];
      }];
}

- (void)createAccountWithCaptchaText:(NSString *)captchaText
                             success:(dispatch_block_t)success
                             captcha:(WMFCaptchaHandler)captcha
                             failure:(WMFErrorHandler)failure {
  if (self.authenticatingUsername.length == 0 ||
      self.authenticatingPassword.length == 0 || success == nil ||
      captcha == nil || failure == nil) {
    if (failure) {
      failure([NSError wmf_errorWithType:WMFErrorTypeStringMissingParameter
                                userInfo:nil]);
    }
  ...
  [self
      createAccountWithCaptchaID:self.accountCreationAuthManagerInfo.captchaID];
}

- (void)createAccountWithCaptchaID:(nullable NSString *)captchaID {
  (void)[[AccountCreator alloc]
      initAndCreateAccountForUserName:self.authenticatingUsername
                             realName:@""
                               domain:[[MWKLanguageLinkController
                                           sharedInstance] appLanguage]
                                          .languageCode
                             password:self.authenticatingPassword
                                email:self.email
...
- (void)loginWithSavedCredentialsWithSuccess:(nullable dispatch_block_t)success
  ...
  [self loginWithUsername:self.keychainCredentials.userName
                 password:self.keychainCredentials.password
                  success:success
                  failure:^(NSError *error) {
                    if (error.domain == WMFAccountLoginErrorDomain &&
                        error.code != LOGIN_ERROR_UNKNOWN &&
                        error.code != LOGIN_ERROR_API) {
                      [self logout];
                    }
                    if (failure) {
                      failure(error);
...
- (void)loginWithUsername:(NSString *)username
                 password:(NSString *)password
                  success:(nullable dispatch_block_t)success
                  failure:(nullable WMFErrorHandler)failure {
  if (self.successBlock || self.failBlock) {
    if (failure) {
      failure([NSError wmf_errorWithType:WMFErrorTypeFetchAlreadyInProgress
...
- (void)loginWithSuccess:(nullable dispatch_block_t)success
                 failure:(nullable WMFErrorHandler)failure {
  if (self.authenticatingUsername.length == 0 ||
      self.authenticatingPassword.length == 0) {
    if (failure) {
      failure([NSError wmf_errorWithType:WMFErrorTypeStringMissingParameter
                                userInfo:nil]);
    }
  ...
  [self fetchLoginAuthManagerInfoWithSuccess:^(
            WMFAuthManagerInfo *_Nonnull info) {
    self.loginAuthManagerInfo = info;
    [self fetchLoginTokensWithInfo:info
                          username:self.authenticatingUsername
                          password:self.authenticatingPassword];
  }
      failure:^(NSError *error) {
...
- (void)fetchLoginAuthManagerInfoWithSuccess:
            (nullable WMFAuthManagerInfoBlock)success
                                     failure:(nullable WMFErrorHandler)failure {
  [self.authManagerInfoFetcher
      fetchAuthManagerLoginAvailableForSiteURL:[[MWKLanguageLinkController
                                                    sharedInstance] appLanguage]
                                                   .siteURL
      success:^(WMFAuthManagerInfo *_Nonnull info) {
        success(info);
...
- (void)fetchLoginTokensWithInfo:(WMFAuthManagerInfo *)info
                        username:(NSString *)username
                        password:(NSString *)password {
  [[QueuesSingleton sharedInstance]
          .loginFetchManager wmf_cancelAllTasksWithCompletionHandler:^{
    (void)[[LoginTokenFetcher alloc]
        initAndFetchTokenForDomain:[[MWKLanguageLinkController sharedInstance]
                                       appLanguage]
                                       .languageCode
                          userName:username
                          password:password
...
- (void)login {
  (void)[[AccountLogin alloc]
      initAndLoginForDomain:[[MWKLanguageLinkController sharedInstance]
                                appLanguage]
                                .languageCode
                   userName:self.authenticatingUsername
                   password:self.authenticatingPassword
    ...
    case FETCH_FINAL_STATUS_FAILED:
      if (error.code == ACCOUNT_CREATION_ERROR_NEEDS_CAPTCHA) {
        if ([self isInitialAccountCreationAtempt]) {
          ...
          // This is because we want to get any errors back from the API about
          // duplicate user names before we present the captcha
          // In this case, the user name is fine and we can fire the block and
          // have them solve the captcha
          [self
              sendCaptchaBlockWithURLString:self.accountCreationAuthManagerInfo
                                                .captchaURLFragment];
        } else {
          // The user tried to solve the captch and failed
          // Get another captcha URL and have the user try again
          [self getAccountCreationCaptchaWithHandler:self.captchaBlock
...
- (void)cloneSessionCookies {
  // Make the session cookies expire at same time user cookies. Just remember
  // they still can't be
  // necessarily assumed to be valid as the server may expire them, but at least
  // make them last as
  // long as we can to lessen number of server requests. Uses user tokens as
  // templates for copying
  // session tokens. See "recreateCookie:usingCookieAsTemplate:" for details.

  NSString *domain =
      [[MWKLanguageLinkController sharedInstance] appLanguage].languageCode;
  ...
  [[NSHTTPCookieStorage sharedHTTPCookieStorage]
         wmf_recreateCookie:cookie1Name
      usingCookieAsTemplate:cookie2Name];

  [[NSHTTPCookieStorage sharedHTTPCookieStorage]
         wmf_recreateCookie:@"centralauth_Session"
