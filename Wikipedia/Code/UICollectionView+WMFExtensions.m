...
- (NSArray *)wmf_indexPathsForIndexes:(NSIndexSet *__nonnull)indexes
  ...
  for (NSInteger section = 0; section < sectionCount; section++) {
    ...
    for (NSInteger row = 0; row < rowCount; row++) {
      NSIndexPath *indexPath =
          [NSIndexPath indexPathForItem:row inSection:section];
      if (block) {
...
- (void)wmf_enumerateVisibleCellsUsingBlock:(WMFCellEnumerator)block {
  [self wmf_enumerateVisibleIndexPathsUsingBlock:^(NSIndexPath *path,
                                                   BOOL *stop) {
...
- (NSIndexPath *)wmf_indexPathBeforeIndexPath:(NSIndexPath *)indexPath {
  if (indexPath.item == 0) {
    ...
    return
        [NSIndexPath indexPathForItem:previousItem inSection:previousSection];
  }

  NSInteger previousItem = indexPath.item - 1;
  return
      [NSIndexPath indexPathForItem:previousItem inSection:indexPath.section];
}
...
- (void)wmf_setCollectionViewLayout:(UICollectionViewLayout *)layout
               ...
               alwaysFireCompletion:(void (^)(BOOL finished))completion {
...
- (NSArray *)wmf_visibleIndexPathsOfItemsBeforeIndexPath:
    (NSIndexPath *)indexPath {
...
- (NSArray *)wmf_visibleIndexPathsOfItemsAfterIndexPath:
    (NSIndexPath *)indexPath {
  return [[self indexPathsForVisibleItems] bk_select:^BOOL(NSIndexPath *obj) {
    if (obj.section < indexPath.section) {
      return NO;
...
- (UIView *)wmf_snapshotOfCellsAtIndexPaths:(NSArray *)indexPaths {
  __block CGRect snapShotRect =
      [self wmf_rectEnclosingCellsAtIndexPaths:indexPaths];

  if (CGRectIsEmpty(snapShotRect)) {
    return nil;
  } else {
    return [self resizableSnapshotViewFromRect:snapShotRect
...
- (UIView *)wmf_snapshotOfCellsBeforeIndexPath:(NSIndexPath *)indexPath {
  return
      [self wmf_snapshotOfCellsAtIndexPaths:
                [self wmf_visibleIndexPathsOfItemsBeforeIndexPath:indexPath]];
}

- (UIView *)wmf_snapshotOfCellsAfterIndexPath:(NSIndexPath *)indexPath {
  return [self wmf_snapshotOfCellsAtIndexPaths:
                   [self wmf_visibleIndexPathsOfItemsAfterIndexPath:indexPath]];
}
