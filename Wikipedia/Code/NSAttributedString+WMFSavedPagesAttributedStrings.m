...
+ (NSAttributedString *)wmf_attributedStringWithTitle:(NSString *)title
  ...
  NSDictionary *titleAttribs = @{
    NSFontAttributeName :
        [UIFont systemFontOfSize:titleSizeMultiplier * kTitleFontSize *
                                 MENUS_SCALE_MULTIPLIER],
    NSForegroundColorAttributeName :
        [UIColor wmf_colorWithHex:kTitleColor alpha:1.0f]
  };
  ...
  if (!descripAttribs) {
    ...
    descripAttribs = @{
      ...
      NSForegroundColorAttributeName :
          [UIColor wmf_colorWithHex:kDescriptionColor alpha:1.0f],
  ...
  if (!langAttribs) {
    NSMutableParagraphStyle *langParagraphStyle =
        [[NSMutableParagraphStyle alloc] init];
    langParagraphStyle.paragraphSpacingBefore = kLanguageSpaceAbove;
    langAttribs = @{
      NSFontAttributeName :
          [UIFont systemFontOfSize:kLanguageFontSize * MENUS_SCALE_MULTIPLIER],
      NSForegroundColorAttributeName :
          [UIColor wmf_colorWithHex:kLanguageColor alpha:1.0f],
  ...
  return [kFormatString
      attributedStringWithAttributes:@{}
                 substitutionStrings:@[
                   title, kLineBreak, description, descripLineBreak, language
                 ]
              substitutionAttributes:@[
                titleAttribs, @{}, descripAttribs, @{}, langAttribs
              ]];
}
