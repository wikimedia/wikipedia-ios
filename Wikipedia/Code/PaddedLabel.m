...
- (void)layoutSubviews {
  ...
  // This fixes problem with PaddedLabels used by UITableViewCells not sizing to
  // their text
...
- (CGSize)intrinsicContentSize {
  ...
  // take into account the padding. Needed because the padding can affect how
  // many
...
- (void)setPadding:(UIEdgeInsets)padding {
  ...
  padding =
      UIEdgeInsetsMake(ceil(((CGFloat)padding.top) * MENUS_SCALE_MULTIPLIER),
  ...
  if ([[UIApplication sharedApplication] wmf_isRTL]) {
    _padding = UIEdgeInsetsMake(padding.top, padding.right, padding.bottom,
                                padding.left);
  } else {
    _padding = padding;
  }
