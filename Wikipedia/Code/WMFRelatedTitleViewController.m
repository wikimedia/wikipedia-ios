...
- (void)viewDidLoad {
  ...
  self.tableView.estimatedRowHeight =
      [WMFArticlePreviewTableViewCell estimatedRowHeight];
}

- (void)setDataSource:(WMFRelatedTitleListDataSource *)dataSource {
  self.title = [MWLocalizedString(@"home-more-like-footer", nil)
      stringByReplacingOccurrencesOfString:@"$1"
                                withString:dataSource.url.wmf_title];
  ...
  dataSource.cellConfigureBlock =
      ^(WMFArticlePreviewTableViewCell *cell, MWKSearchResult *searchResult,
        ...
        @strongify(self);
        NSURL *articleURL =
            [self.dataSource.url wmf_URLWithTitle:searchResult.displayTitle];
        [cell setSaveableURL:articleURL
               savedPageList:self.dataSource.savedPageList];
