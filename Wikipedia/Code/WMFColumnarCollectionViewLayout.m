...
@property(nonatomic, readonly) id<WMFColumnarCollectionViewLayoutDelegate>
    delegate;
...
- (id<WMFColumnarCollectionViewLayoutDelegate>)delegate {
  assert(self.collectionView.delegate == nil ||
         [self.collectionView.delegate
             conformsToProtocol:@protocol(
                                    WMFColumnarCollectionViewLayoutDelegate)]);
  return (id<WMFColumnarCollectionViewLayoutDelegate>)
      self.collectionView.delegate;
}
...
layoutAttributesForElementsInRect:(CGRect)rect {
  ...
  [self.info enumerateSectionsWithBlock:^(WMFCVLSection *_Nonnull section,
                                          NSUInteger idx, BOOL *_Nonnull stop) {
    if (CGRectIntersectsRect(section.frame, rect)) {
      [section enumerateLayoutAttributesWithBlock:^(
                   WMFCVLAttributes *attributes, BOOL *stop) {
...
layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind
  ...
  return [self.info layoutAttributesForSupplementaryViewOfKind:elementKind
...
- (void)prepareLayout {
  if (self.info == nil) {
    ...
    self.metrics =
        [WMFCVLMetrics metricsWithBoundsSize:self.collectionView.bounds.size];
    [self.info layoutWithMetrics:self.metrics
...
invalidationContextForBoundsChange:(CGRect)newBounds {
  WMFCVLInvalidationContext *context = (WMFCVLInvalidationContext *)[super
  ...
  [self.info updateWithMetrics:self.metrics
            ...
            (UICollectionViewLayoutAttributes *)preferredAttributes
                                    withOriginalAttributes:
                                        (UICollectionViewLayoutAttributes *)
                                            originalAttributes {
  return !CGRectEqualToRect(preferredAttributes.frame,
                            originalAttributes.frame);
}
    ...
    (UICollectionViewLayoutAttributes *)preferredAttributes
                         withOriginalAttributes:
                             (UICollectionViewLayoutAttributes *)
                                 originalAttributes {
  WMFCVLInvalidationContext *context = (WMFCVLInvalidationContext *)[super
      invalidationContextForPreferredLayoutAttributes:preferredAttributes
