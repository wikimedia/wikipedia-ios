...
static NSString *const MWKPOTDImageInfoErrorDomain =
    @"MWKPOTDImageInfoErrorDomain";
...
static MWKImageInfoResolve
addPictureOfTheDayToDescriptionForDate(NSDate *date) {
  return ^id(MWKImageInfo *info) {
    NSString *dateString = [[NSDateFormatter wmf_mediumDateFormatterWithoutTime]
        stringFromDate:date];
    ...
    return
        [[MWKImageInfo alloc] initWithCanonicalPageTitle:info.canonicalPageTitle
...
- (AnyPromise *)fetchPicOfTheDaySectionInfoForDate:(NSDate *)date
                                  metadataLanguage:
                                      (nullable NSString *)metadataLanguage {
  return
      [self
          fetchPartialInfoForImagesOnPages:@[ [date wmf_picOfTheDayPageTitle] ]
...
- (AnyPromise *)fetchPicOfTheDayGalleryInfoForDate:(NSDate *)date
  ...
  return
      [self
          fetchGalleryInfoForImagesOnPages:@[ [date wmf_picOfTheDayPageTitle] ]
                               fromSiteURL:[NSURL wmf_wikimediaCommonsURL]
                          metadataLanguage:metadataLanguage]
          .then(selectFirstImageInfo(date))
