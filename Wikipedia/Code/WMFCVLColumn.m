...
- (void)enumerateSectionsWithBlock:
  ...
  [self.info.sections enumerateObjectsAtIndexes:self.sectionIndexes
                                        options:0
                                     usingBlock:block];
}
...
- (void)setSize:(CGSize)size
    forHeaderAtIndexPath:(NSIndexPath *)indexPath
  ...
  CGFloat deltaY = [section setSize:size
                   forHeaderAtIndex:indexPath.item
                invalidationContext:invalidationContext];
  [self offsetSectionsByDistance:deltaY
            startingAfterSection:section
             invalidationContext:invalidationContext];
...
- (void)setSize:(CGSize)size
    forFooterAtIndexPath:(NSIndexPath *)indexPath
     invalidationContext:(WMFCVLInvalidationContext *)invalidationContext {
  NSInteger sectionIndex = indexPath.section;

  WMFCVLSection *section = self.info.sections[sectionIndex];
  CGFloat deltaY = [section setSize:size
                   forFooterAtIndex:indexPath.item
                invalidationContext:invalidationContext];
  [self offsetSectionsByDistance:deltaY
            startingAfterSection:section
             invalidationContext:invalidationContext];
...
- (void)offsetSectionsByDistance:(CGFloat)deltaY
            startingAfterSection:(WMFCVLSection *)afterSection
             invalidationContext:
  ...
  [self enumerateSectionsWithBlock:^(WMFCVLSection *_Nonnull section,
    ...
    if (section == afterSection) {
      ...
      [section offsetByDeltaY:deltaY
          withInvalidationContext:invalidationContext];
    }
  }];
}
