 ...
 * @param sel The selector used to compare @c a to @c b, if <code>a == b</code>
 * is @c false.
 * @param b   Second object, can be `nil`.
 * @return `YES` if the objects are the same pointer or invoking @c sel returns
 * @c YES, otherwise @c NO.
 */
 ...
 * @see WMF_EQUAL
 */
#define WMF_EQUAL_PROPERTIES(a, prop, sel, b)                                  \
  WMF_EQUAL([(a)prop], sel, [(b)prop])

/// Convenience for `WMF_EQUAL_PROPERTIES` which passes `isEqual:` for the
/// equality selector.
#define WMF_IS_EQUAL_PROPERTIES(a, prop, b)                                    \
  WMF_EQUAL_PROPERTIES(a, prop, isEqual:, b)
...
/**
 * Compare if the values returned by @c prop are equal for @c self and @c rhs
 * using @c sel
 ...
 * @return @c YES if the values returned by @c prop for @c self and @c rhs are
 * equal.
 */
...
#define WMF_SYNTHESIZE_IS_EQUAL(CLASS_NAME, CLASS_EQ_SEL)                      \
  -(BOOL)isEqual : (id)obj {                                                   \
    return [super isEqual:obj] || [obj isKindOfClass:[CLASS_NAME class]]       \
               ? [self CLASS_EQ_SEL obj]                                       \
               : NO;                                                           \
  }
