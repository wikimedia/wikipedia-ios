 ...
 *
 *  It abstracts and encapsulates the other fetchers and model classes to
 * provide a
 *  simple interface for performing authentication tasks.
 *
 *  NOTE: This class is NOT STATELESS. Only one authentication operation shoud
 * be running at a given time
 *  The class will return an error if a second operation is requested while one
 * is in progress
...
/**
 *  Get a captcha for account creation. This operation will also check if the
...
/**
 *  Create an account for the username in the above method passing in the answer
 * to the captcha.
 *  If the answer is incorrect, the captcha block will be invoked with a new
 * captcha.
 ...
 *  @param success     The handler for success - at this point the user is
 * logged in
 *  @param captcha     The handler for the returned captcha URL
 *  @param failure     The handler for any errors
 */
 ...
 *  @param success  The handler for success - at this point the user is logged
 * in
 *  @param failure     The handler for any errors
 */
