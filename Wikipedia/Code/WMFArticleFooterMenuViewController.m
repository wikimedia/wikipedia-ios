...
@interface WMFArticleFooterMenuViewController () <
    UITableViewDelegate, WMFLanguagesViewControllerDelegate,
...
- (instancetype)initWithArticle:(MWKArticle *)article
  ...
  if (self) {
    self.footerDataSource =
        [[WMFArticleFooterMenuDataSource alloc] initWithArticle:self.article];
...
- (void)viewDidLoad {
  ...
  [self.tableView registerNib:[WMFArticleFooterMenuCell wmf_classNib]
       forCellReuseIdentifier:[WMFArticleFooterMenuCell identifier]];

  NSAssert(self.tableView.style == UITableViewStyleGrouped,
           @"Use grouped UITableView layout so we get separator above first "
           @"cell and below last cell without having to implement any special "
           @"logic");
...
- (void)tableView:(UITableView *)tableView
  ...
  switch ([(WMFArticleFooterMenuItem *)[self.footerDataSource
...
- (void)showDisambiguationItems {
  WMFDisambiguationPagesViewController *articleListVC =
      [[WMFDisambiguationPagesViewController alloc]
  ...
  UINavigationController *navController =
      [[UINavigationController alloc] initWithRootViewController:articleListVC];
...
- (void)showEditHistory {
  PageHistoryViewController *editHistoryVC =
      [PageHistoryViewController wmf_initialViewControllerFromClassStoryboard];
  editHistoryVC.article = self.article;
  [self presentViewController:[[UINavigationController alloc]
                                  initWithRootViewController:editHistoryVC]
                     animated:YES
                   completion:nil];
}
...
- (void)showLanguages {
  WMFArticleLanguagesViewController *languagesVC =
      [WMFArticleLanguagesViewController
          articleLanguagesViewControllerWithArticleURL:self.article.url];
...
- (void)languagesController:(WMFArticleLanguagesViewController *)controller
          didSelectLanguage:(MWKLanguageLink *)language {
  [self dismissViewControllerAnimated:YES
                           completion:^{
                             WMFArticleViewController *articleContainerVC =
                                 [[WMFArticleViewController alloc]
                                     initWithArticleURL:language.articleURL
                                              dataStore:self.dataStore];
                             [self.navigationController
                                 pushViewController:articleContainerVC
                                           animated:YES];
                           }];
}

- (void)showPageIssues {
  WMFPageIssuesViewController *issuesVC = [[WMFPageIssuesViewController alloc]
      initWithStyle:UITableViewStyleGrouped];
  issuesVC.dataSource =
      [[SSArrayDataSource alloc] initWithItems:self.article.pageIssues];
  [self presentViewController:[[UINavigationController alloc]
                                  initWithRootViewController:issuesVC]
                     animated:YES
                   completion:nil];
}
