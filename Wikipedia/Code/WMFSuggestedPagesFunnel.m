...
static NSString *const kInitWithArticleAssertVerbiage =
    @"Article title must be well formed and suggested titles non-empty";
static NSString *const kInitMustUseSpecificInitializer =
    @"Wrong initializer. Use - (id)initWithArticle:(MWKArticle*)article "
...
- (id)initWithArticle:(MWKArticle *)article
  ...
  if (!title || ![suggestedTitles count]) {
    NSAssert(false, @"%@...ARTICLE TITLE: %@ SUGGESTED: %@",
             kInitWithArticleAssertVerbiage, [article url].wmf_title,
             suggestedTitles);
    return nil;
  }
  ...
  if (self) {
    ...
    _pageTitle = title;
    _readMoreList =
        [suggestedTitles componentsJoinedByString:kReadMoreListDelimeter];
  }
  return self;
}
