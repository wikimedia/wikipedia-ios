...
static NSString *const WMFRevisionQueryResultsErrorDomain =
    @"WMFRevisionQueryResultsErrorDomain";
...
- (BOOL)validate:(NSError *__autoreleasing *)error {
  if (!self.titleText.length) {
    WMFSafeAssign(
        error, [NSError errorWithDomain:WMFRevisionQueryResultsErrorDomain
  ...
  if (!self.revisions.count) {
    WMFSafeAssign(error,
                  [NSError
                      errorWithDomain:WMFRevisionQueryResultsErrorDomain
...
+ (NSValueTransformer *)revisionsJSONTransformer {
  return [MTLJSONAdapter
      arrayTransformerWithModelClass:[WMFArticleRevision class]];
}

+ (NSDictionary *)JSONKeyPathsByPropertyKey {
  return @{
    WMF_SAFE_KEYPATH([WMFRevisionQueryResults new], titleText) : @"title",
