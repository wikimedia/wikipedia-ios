...
+ (NSArray<WMFArticlePhoto *> *)photosWithThumbnailImageObjects:
    (NSArray<MWKImage *> *)imageObjects {
...
- (nullable UIImage *)placeholderImage {
  ...
  if (url) {
    UIImage *image =
        [[WMFImageController sharedInstance] cachedImageInMemoryWithURL:url];
    if (!image) {
...
- (instancetype)initWithPhotos:(nullable NSArray<id<NYTPhoto>> *)photos
                  initialPhoto:(nullable id<NYTPhoto>)initialPhoto
                      delegate:(nullable id<NYTPhotosViewControllerDelegate>)
                                   delegate {
  ...
  if (self) {
    ...
    NSAssert([self respondsToSelector:@selector(updateOverlayInformation)],
             @"NYTPhoto implementation changed!");
    NSAssert([self respondsToSelector:@selector(currentPhotoViewController)],
             @"NYTPhoto implementation changed!");
    NSAssert([self respondsToSelector:@selector(currentImageView)],
             @"NYTPhoto implementation changed!");
    NSAssert(
        [self respondsToSelector:@selector(newPhotoViewControllerForPhoto:)],
        @"NYTPhoto implementation changed!");
...
- (void)didTapShareButton {
  ...
  [[WMFImageController sharedInstance] fetchImageWithURL:url
      ...
      success:^(WMFImageDownload *_Nonnull download) {
        ...
        WMFImageTextActivitySource *textSource =
            [[WMFImageTextActivitySource alloc] initWithInfo:info];
        [items addObject:textSource];

        WMFImageURLActivitySource *imageSource =
            [[WMFImageURLActivitySource alloc] initWithInfo:info];
...
- (UIView *_Nullable)photosViewController:
                         (NYTPhotosViewController *)photosViewController
                    referenceViewForPhoto:(id<NYTPhoto>)photo {
  return nil; // TODO: remove this and re-enable animations when tickets for
              // fixing anmimations are addressed
...
- (NSString *_Nullable)photosViewController:
                           (NYTPhotosViewController *)photosViewController
...
- (UIView *_Nullable)photosViewController:
  ...
  WMFImageGalleryDetailOverlayView *caption =
      [WMFImageGalleryDetailOverlayView wmf_viewFromClassNib];

  caption.imageDescription = [imageInfo.imageDescription
  ...
  @weakify(self) caption.ownerTapCallback = ^{
    @strongify(self)[self wmf_openExternalUrl:imageInfo.license.URL];
  };
  caption.infoTapCallback = ^{
    @strongify(self)[self wmf_openExternalUrl:imageInfo.filePageURL];
  };
...
- (nullable instancetype)initWithArticle:(MWKArticle *)article
                           selectedImage:(nullable MWKImage *)image {
  ...
  NSArray<WMFArticlePhoto *> *photos =
      [WMFArticlePhoto photosWithThumbnailImageObjects:items];
  ...
  if (self) {
    self.infoController =
        [[WMFImageInfoController alloc] initWithDataStore:article.dataStore
                                                batchSize:50];
    [self.infoController setUniqueArticleImages:items
                                  forArticleURL:article.url];
    [self.photos enumerateObjectsUsingBlock:^(WMFArticlePhoto *_Nonnull obj,
                                              NSUInteger idx,
                                              BOOL *_Nonnull stop) {
...
- (MWKImageInfo *)currentImageInfo {
  return
      [self imageInfoForPhoto:[self photoAtIndex:[self indexOfCurrentImage]]];
}

+ (nullable id<NYTPhoto>)photoWithImage:(MWKImage *)image
                               inPhotos:(NSArray<id<NYTPhoto>> *)photos {
...
+ (NSUInteger)indexOfImage:(MWKImage *)image
                  inPhotos:(NSArray<id<NYTPhoto>> *)photos {
  return [photos indexOfObjectPassingTest:^BOOL(WMFArticlePhoto *anImage,
                                                NSUInteger _, BOOL *stop) {
    if ([anImage.imageObject isVariantOfImage:image] ||
        [anImage.thumbnailImageObject isVariantOfImage:image]) {
      *stop = YES;
      return YES;
    }
    return NO;
...
- (void)fetchImageInfoForPhoto:(WMFArticlePhoto *)galleryImage {
  [self.infoController
      fetchBatchContainingIndex:[self indexOfPhoto:galleryImage]];
}
...
- (void)fetchImageForPhoto:(WMFArticlePhoto *)galleryImage {
  ...
  if (memoryCachedImage == nil) {
    ...
    [[WMFImageController sharedInstance]
        fetchImageWithURL:[galleryImage imageURL]
        failure:^(NSError *_Nonnull error) {
          // show error
        }
        success:^(WMFImageDownload *_Nonnull download) {
...
- (void)imageInfoController:(WMFImageInfoController *)controller
  ...
  [self.photos
      enumerateObjectsAtIndexes:fetchedIndexes
                        options:0
                     usingBlock:^(WMFArticlePhoto *_Nonnull obj, NSUInteger idx,
                                  BOOL *_Nonnull stop) {
                       MWKImageInfo *info =
                           [controller infoForImage:[obj imageObject]];
                       if (!info) {
                         info = [controller
                             infoForImage:[obj thumbnailImageObject]];
                       }
...
- (instancetype)initWithDates:(NSArray<NSDate *> *)imageDates
            selectedImageInfo:(nullable MWKImageInfo *)imageInfo {
...
- (void)fetchImageInfoForPhoto:(WMFPOTDPhoto *)galleryImage {
  ...
  @weakify(self);
  [self.infoFetcher
      fetchPicOfTheDayGalleryInfoForDate:date
                        metadataLanguage:[[NSLocale currentLocale]
...
- (void)fetchImageForPhoto:(WMFPOTDPhoto *)galleryImage {
  ...
  if (memoryCachedImage == nil) {
    [[WMFImageController sharedInstance]
        fetchImageWithURL:[galleryImage bestImageURL]
        failure:^(NSError *_Nonnull error) {
          // show error
        }
        success:^(WMFImageDownload *_Nonnull download) {
