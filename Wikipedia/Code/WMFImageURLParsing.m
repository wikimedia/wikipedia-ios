...
static NSRegularExpression *WMFImageURLParsingRegex() {
  ...
  dispatch_once(&onceToken, ^{
    // TODO: try to read serialized regex from disk to prevent needless pattern
    // compilation on next app run
    NSError *patternCompilationError;
    imageNameFromURLRegex = [NSRegularExpression
        regularExpressionWithPattern:
            @"^(lossy-|lossless-)?(page\\d+-)?\\d+px-(.*)"
                             options:0
...
NSString *WMFParseImageNameFromSourceURL(NSString *sourceURL)
  ...
  if (pathComponents.count < 2) {
    DDLogWarn(@"Unable to parse source URL with too few path components: %@",
              pathComponents);
    return nil;
  }
...
NSString *WMFParseUnescapedNormalizedImageNameFromSourceURL(NSString *sourceURL)
    __attribute__((overloadable)) {
...
NSString *WMFParseUnescapedNormalizedImageNameFromSourceURL(NSURL *sourceURL)
    __attribute__((overloadable)) {
  return WMFParseUnescapedNormalizedImageNameFromSourceURL(
      sourceURL.absoluteString);
}

NSInteger WMFParseSizePrefixFromSourceURL(NSURL *sourceURL)
    __attribute__((overloadable)) {
  return WMFParseSizePrefixFromSourceURL(sourceURL.absoluteString);
}

NSInteger WMFParseSizePrefixFromSourceURL(NSString *sourceURL)
    __attribute__((overloadable)) {
  if (!sourceURL) {
    return NSNotFound;
  }
  ...
  if (pxRange.location == NSNotFound) {
    ...
    NSRange lastDashRange =
        [stringBeforePx rangeOfString:@"-" options:NSBackwardsSearch];
    ...
    if (lastDashRange.location == NSNotFound) {
      ...
      NSString *stringAfterDash =
          [stringBeforePx substringFromIndex:lastDashRange.location + 1];
...
NSString *WMFOriginalImageURLStringFromURLString(NSString *URLString) {
  if ([URLString containsString:@"/thumb/"]) {
    URLString = [[URLString stringByDeletingLastPathComponent]
        stringByReplacingOccurrencesOfString:@"/thumb/"
                                  withString:@"/"];
  }
  return URLString;
}

NSString *WMFChangeImageSourceURLSizePrefix(NSString *sourceURL,
                                            NSUInteger newSizePrefix)
    __attribute__((overloadable)) {
  ...
  NSString *urlAfterWikipedia =
      [sourceURL substringFromIndex:wikipediaStringRange.location +
                                    wikipediaStringRange.length];
  ...
  NSString *site =
      [urlAfterWikipedia substringToIndex:rangeOfSlashAfterWikipedia.location];
  if (site.length == 0) {
    return sourceURL;
  }
  ...
  if (WMFParseSizePrefixFromSourceURL(sourceURL) == NSNotFound) {
    NSString *sizeVariantLastPathComponent =
        [NSString stringWithFormat:@"%lupx-%@", (unsigned long)newSizePrefix,
                                   lastPathComponent];

    NSString *lowerCasePathExtension =
        [[sourceURL pathExtension] lowercaseString];
    if ([lowerCasePathExtension isEqualToString:@"pdf"]) {
      sizeVariantLastPathComponent = [NSString
          stringWithFormat:@"page1-%@.jpg", sizeVariantLastPathComponent];
    } else if ([lowerCasePathExtension isEqualToString:@"tif"] ||
               [lowerCasePathExtension isEqualToString:@"tiff"]) {
      sizeVariantLastPathComponent = [NSString
          stringWithFormat:@"lossy-page1-%@.jpg", sizeVariantLastPathComponent];
    }

    NSString *urlWithSizeVariantLastPathComponent =
        [[sourceURL stringByAppendingString:@"/"]
            stringByAppendingString:sizeVariantLastPathComponent];

    NSString *urlWithThumbPath = [urlWithSizeVariantLastPathComponent
        stringByReplacingOccurrencesOfString:
            [NSString stringWithFormat:@"%@%@/", wikipediaString, site]
    ...
    NSRange rangeOfLastPathComponent = NSMakeRange(
        [sourceURL rangeOfString:lastPathComponent options:NSBackwardsSearch]
            .location,
        lastPathComponent.length);
    return [WMFImageURLParsingRegex()
        stringByReplacingMatchesInString:sourceURL
                                 options:NSMatchingAnchored
                                   range:rangeOfLastPathComponent
                            withTemplate:[NSString
                                             stringWithFormat:
                                                 @"$1$2%lupx-$3",
                                                 (unsigned long)newSizePrefix]];
  }
}
