...
- (void)uploadWithManager:(AFHTTPSessionManager *)manager {
  NSURL *url = [[SessionSingleton sharedInstance]
      urlForLanguage:self.articleURL.wmf_language];
  ...
  [manager POST:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (![responseObject isDict]) {
          responseObject =
              @{ @"error" : @{@"info" : @"WikiText upload data not found."} };
        }
        ...
        if (!error && !result) {
          ...
          errorDict[NSLocalizedDescriptionKey] =
              MWLocalizedString(@"wikitext-upload-result-unknown", nil);

          // Set error condition so dependent ops don't even start and so the
          // errorBlock below will fire.
          error = [NSError errorWithDomain:@"Upload Wikitext Op"
                                      code:WIKITEXT_UPLOAD_ERROR_UNKNOWN
                                  userInfo:errorDict];
        }
        ...
        if (!error && result && [result isEqualToString:@"Failure"]) {
          if (responseObject[@"edit"][@"captcha"]) {
            ...
            errorDict[NSLocalizedDescriptionKey] =
                (self.captchaWord && (self.captchaWord.length > 0))
            ...
            errorDict[@"captchaId"] =
                responseObject[@"edit"][@"captcha"][@"id"];
            errorDict[@"captchaUrl"] =
                responseObject[@"edit"][@"captcha"][@"url"];

            // Set error condition so dependent ops don't even start and so the
            // errorBlock below will fire.
            error = [NSError errorWithDomain:@"Upload Wikitext Op"
                                        code:WIKITEXT_UPLOAD_ERROR_NEEDS_CAPTCHA
                                    userInfo:errorDict];
          } else if (responseObject[@"edit"][@"code"]) {
            NSString *abuseFilterCode = responseObject[@"edit"][@"code"];
            WikiTextSectionUploaderErrors errorType =
                WIKITEXT_UPLOAD_ERROR_UNKNOWN;
            ...
            case WIKITEXT_UPLOAD_ERROR_ABUSEFILTER_OTHER: {
              ...
              // Set error condition so dependent ops don't even start and so
              // the errorBlock below will fire.
              error = [NSError errorWithDomain:@"Upload Wikitext Op"
                                          code:errorType
                                      userInfo:errorDict];
            } break;
        ...
        [self finishWithError:error fetchedData:resultDict];
      }
