    ...
    [manager POST:url.absoluteString
        ...
        success:^(NSURLSessionDataTask *operation, id responseObject) {
            ...
            if (!error && !result) {
                ...
                errorDict[NSLocalizedDescriptionKey] = NSLocalizedStringWithDefaultValue(@"wikitext-upload-result-unknown", nil, NSBundle.wmf_localizationBundle, @"Unable to determine wikitext upload result.", "Alert text shown when the result of saving section wikitext changes is unknown");
            ...
            if (!error && result && [result isEqualToString:@"Failure"]) {
                if (responseObject[@"edit"][@"captcha"]) {
                    ...
                    errorDict[NSLocalizedDescriptionKey] = (self.captchaWord && (self.captchaWord.length > 0)) ? NSLocalizedStringWithDefaultValue(@"wikitext-upload-captcha-error", nil, NSBundle.wmf_localizationBundle, @"CAPTCHA verification error.", "Alert text shown when section wikitext upload captcha fails")
                                                                                                               : NSLocalizedStringWithDefaultValue(@"wikitext-upload-captcha-needed", nil, NSBundle.wmf_localizationBundle, @"Need CAPTCHA verification.", "Alert text shown when section wikitext upload captcha is required");
