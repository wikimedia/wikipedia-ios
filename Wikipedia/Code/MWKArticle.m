...
@property(readwrite, strong, nonatomic)
    MWKProtectionStatus *protection;                   // required
...
@property(readwrite, copy, nonatomic) NSString
    *entityDescription; // optional; currently pulled separately via wikidata
...
- (instancetype)initWithURL:(NSURL *)url
                  dataStore:(MWKDataStore *)dataStore
          searchResultsDict:(NSDictionary *)dict {
  self = [self initWithURL:url dataStore:dataStore];
  if (self) {
...
- (BOOL)isDeeplyEqualToArticle:(MWKArticle *)article {
  return
      [self isEqual:article] && WMF_IS_EQUAL(self.sections, article.sections);
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"%@ %@", [super description], self.url.description];
}
...
- (id)dataExport {
  ...
  [dict wmf_maybeSetObject:[self iso8601DateString:self.lastmodified]
                    forKey:@"lastmodified"];
...
- (void)importMobileViewJSON:(NSDictionary *)dict {
  ...
  self.languagecount =
      [[self requiredNumber:@"languagecount" dict:dict] intValue];
  ...
  @try {
    ...
    self.protection = nil;
    DDLogWarn(@"Protection Status is not a dictionary, setting to nil: %@",
              [[dict valueForKey:@"protection"] description]);
  }
  ...
  NSArray *sectionsData =
      [dict[@"sections"] bk_map:^id(NSDictionary *sectionData) {
  ...
  if (mainPageValue == nil) {
    // field not present due to "empty string" behavior (see below), or we're
    // loading legacy cache data
    self.main = NO;
  } else if ([mainPageValue isKindOfClass:[NSString class]]) {
    // old mediawiki convention was to use a field w/ an empty string as "true"
    // and omit the field for "false"
    NSAssert([mainPageValue length] == 0,
             @"Assuming empty string for boolean field.");
    self.main = YES;
  } else {
    // proper JSON boolean types!
    NSAssert([mainPageValue isKindOfClass:[NSNumber class]],
             @"Expected main page to be a boolean. Got %@", dict);
    self.main = [mainPageValue boolValue];
  }

  if ([sectionsData count] > 0) {
    self.sections =
        [[MWKSectionList alloc] initWithArticle:self sections:sectionsData];
  }
}
...
- (MWKImage *)existingImageWithURL:(NSString *)url {
  NSString *imageCacheFolderPath =
      [self.dataStore pathForImageURL:url forArticleURL:self.url];
  if (!imageCacheFolderPath) {
    return nil;
  }

  BOOL imageCacheFolderPathExists =
      [[NSFileManager defaultManager] fileExistsAtPath:imageCacheFolderPath
                                           isDirectory:NULL];
...
- (NSString *)thumbnailURLFromImageURL:(NSString *)imageURL {
  return WMFChangeImageSourceURLSizePrefix(
      imageURL, [[UIScreen mainScreen] wmf_listThumbnailWidthForScale]
                    .unsignedIntegerValue);
}
...
- (MWKProtectionStatus *)requiredProtectionStatus:(NSString *)key
  ...
  if (obj == nil) {
    @throw [NSException
        exceptionWithName:@"MWKDataObjectException"
...
- (NSString *)debugDescription {
  return [NSString
      stringWithFormat:@"%@ { \n"
                        ...
                        "\tentityDescription: %@, \n"
                        "}",
                       self.description, self.lastmodifiedby, self.lastmodified,
                       self.articleId, self.languagecount, self.displaytitle,
                       self.protection, self.editable, self.thumbnailURL,
                       self.imageURL, self.sections.debugDescription,
...
- (NSArray<NSURL *> *)imageURLsForGallery {
  WMFImageTagList *tagList = [[[WMFImageTagParser alloc] init]
      imageTagListFromParsingHTMLString:self.articleHTML
                            withBaseURL:self.url
                           leadImageURL:self.leadImage.sourceURL];
...
- (NSSet<NSURL *> *)allImageURLs {
  ...
  // Note: use the 'imageURLsForGallery' and 'imageURLsForSaving' methods on
  // WMFImageTagList so we don't have to parse twice.
  [imageURLs addObjectsFromArray:[tagList imageURLsForGallery]];
  [imageURLs addObjectsFromArray:[tagList imageURLsForSaving]];

  NSArray<MWKImageInfo *> *infos =
      [self.dataStore imageInfoForArticleWithURL:self.url];

  NSArray<NSURL *> *lazilyFetchedHighResolutionGalleryImageURLs =
      [infos wmf_mapAndRejectNil:^id _Nullable(MWKImageInfo *_Nonnull obj) {
  ...
  [imageURLs
      addObjectsFromArray:[self
                              schemelessURLsRejectingNilURLs:
                                  lazilyFetchedHighResolutionGalleryImageURLs]];

  NSArray<NSURL *> *thumbURLs =
      [infos wmf_mapAndRejectNil:^id _Nullable(MWKImageInfo *_Nonnull obj) {
        if ([obj isKindOfClass:[MWKImageInfo class]]) {
  ...
  [imageURLs
      addObjectsFromArray:[self schemelessURLsRejectingNilURLs:thumbURLs]];

  NSURL *articleImageURL = [NSURL wmf_optionalURLWithString:self.imageURL];
  [imageURLs wmf_safeAddObject:articleImageURL];

  NSURL *articleThumbnailURL =
      [NSURL wmf_optionalURLWithString:self.thumbnailURL];
  [imageURLs wmf_safeAddObject:articleThumbnailURL];

#if DEBUG
  for (NSURL *url in imageURLs) {
    NSAssert(url.scheme == nil,
             @"Non nil image url scheme detected! These must be nil here or we "
             @"can get duplicate image urls in the set - ie the same url "
             @"twice - once with the scheme and once without. This is because "
             @"WMFImageTagParser returns urls without schemes because the "
             @"html it parses does not have schemes - but the MWKImageInfo "
             @"*does* have schemes in its 'canonicalFileURL' and "
             @"'imageThumbURL' properties because the api returns schemes.");
  }
#endif
...
static NSString *const WMFArticleReflistColumnSelector =
    @"/html/body/*[contains(@class,'reflist')]//*[contains(@class, "
    @"'references')]/li";
  ...
  if (!_citations) {
    ...
    [self.sections.entries
        enumerateObjectsWithOptions:NSEnumerationReverse
                         usingBlock:^(MWKSection *section, NSUInteger idx,
                                      BOOL *stop) {
                           referenceListItems =
                               [section elementsInTextMatchingXPath:
                                            WMFArticleReflistColumnSelector];
                           if (referenceListItems.count > 0) {
                             *stop = YES;
                           }
                         }];
    if (!referenceListItems) {
      DDLogWarn(@"Failed to parse reflist for %@ cached article: %@",
                self.isCached ? @"" : @"not", self);
      return nil;
    }
    _citations =
        [[referenceListItems bk_map:^MWKCitation *(TFHppleElement *el) {
          return [[MWKCitation alloc]
              initWithCitationIdentifier:el.attributes[@"id"]
                                 rawHTML:el.raw];
