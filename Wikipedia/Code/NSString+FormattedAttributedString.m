...
attributedStringWithAttributes:(NSDictionary *)attributes
  ...
  @synchronized(regexArray) {
      ...
      for (NSUInteger i = 0; i < regexCountNeeded; i++) {
        NSRegularExpression *newRegex = [NSRegularExpression
            regularExpressionWithPattern:
                [NSString
                    stringWithFormat:@"\\$%lu+",
                                     (unsigned long)i + regexCountPrevious + 1]
                                 options:0
                                   error:nil];
    ...
    BOOL neededRegexesArePresent =
        regexArray.count >= substitutionStrings.count;
    NSAssert(neededRegexesArePresent,
             @"Not enough NSRegularExpression objects.");
    ...
    for (NSUInteger i = 0; i < substitutionStrings.count; i++) {
      NSArray *matches = [regexArray[i]
          matchesInString:returnString.string
                  options:0
