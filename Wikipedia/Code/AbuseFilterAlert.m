...
- (id)initWithType:(AbuseFilterAlertType)alertType {
  ...
  if (self) {
    ...
    // Add just a bit of scrolling margin to bottom just in case the bottom of
    // the last
...
- (void)addTopMask {
  ...
  [self addConstraints:[NSLayoutConstraint
                           constraintsWithVisualFormat:@"H:|[topMask]|"
                                               options:0
                                               metrics:nil
                                                 views:views]];

  [self
      addConstraint:[NSLayoutConstraint constraintWithItem:topMask
...
- (void)setupSubViewData {
  [self.subViewData addObject:@{
    ...
    @"string" : ((self.alertType == ABUSE_FILTER_DISALLOW) ? WIKIGLYPH_X
                                                           : WIKIGLYPH_FLAG),
    @"backgroundColor" :
        ((self.alertType == ABUSE_FILTER_DISALLOW) ? WMF_COLOR_RED
                                                   : WMF_COLOR_ORANGE),
  ...
  for (NSMutableDictionary *viewData in self.subViewData) {
    ...
    case VIEW_TYPE_ICON:
      ...
      viewData[@"fontSize"] = @((self.alertType == ABUSE_FILTER_DISALLOW)
                                    ? (74.0 * MENUS_SCALE_MULTIPLIER)
                                    : (70.0 * MENUS_SCALE_MULTIPLIER));
      break;
    case VIEW_TYPE_HEADING:
      ...
      viewData[@"font"] =
          [UIFont boldSystemFontOfSize:23.0 * MENUS_SCALE_MULTIPLIER];
      break;
    case VIEW_TYPE_SUBHEADING:
      ...
      viewData[@"kearning"] = @0;
      viewData[@"font"] =
          [UIFont systemFontOfSize:16.0 * MENUS_SCALE_MULTIPLIER];
      break;
    case VIEW_TYPE_ITEM:
      viewData[@"topPadding"] = @0;
      viewData[@"bottomPadding"] =
          (self.alertType == ABUSE_FILTER_WARNING) ? @8 : @15;
      viewData[@"leftPadding"] =
          (self.alertType == ABUSE_FILTER_WARNING) ? @30 : @20;
      viewData[@"rightPadding"] = @20;
      viewData[@"lineSpacing"] = @6;
      viewData[@"kearning"] = @0;
      viewData[@"bulletType"] = (self.alertType == ABUSE_FILTER_WARNING)
                                    ? @(BULLET_TYPE_ROUND)
                                    : @(BULLET_TYPE_NONE);
      viewData[@"font"] =
          [UIFont systemFontOfSize:16.0 * MENUS_SCALE_MULTIPLIER];
  ...
  for (NSDictionary *viewData in self.subViewData) {
    ...
    default: {
      ...
      if (bulletType) {
        ...
        item.bulletLabel.padding = UIEdgeInsetsMake(
            topPadding.floatValue, leftPadding.floatValue, 0, 4);

        // Zero out left padding because we already have the left padding
        // applied
      ...
      item.titleLabel.padding =
          UIEdgeInsetsMake(topPadding.floatValue, leftPadding.floatValue,
                           bottomPadding.floatValue, rightPadding.floatValue);

      [self setText:MWLocalizedString(viewData[@"string"], nil)
             forLabel:item.titleLabel
          subViewData:viewData];
