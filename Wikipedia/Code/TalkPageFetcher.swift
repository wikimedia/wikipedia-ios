...
class TalkPageFetcher: Fetcher {
    ...
    ///   - topics: Expects an array of Strings containing the `name` value from `TalkPageItem`
    ///   - completion: Returns either an array with the `name` property of subscribed topics or an Error
    ...
    func postReply(talkPageTitle: String, siteURL: URL, commentId: String, comment: String, completion: @escaping (Result<Int?, Error>) -> Void) {
        guard let title = talkPageTitle.denormalizedPageTitle else {
            completion(.failure(RequestError.invalidParameters))
            return
        }
    ...
    func postTopic(talkPageTitle: String, siteURL: URL, topicTitle: String, topicBody: String, completion: @escaping (Result<Int?, Error>) -> Void) {
    ...
    fileprivate func evaluateResponse(_ error: Error?, _ result: [String : Any]?, completion: @escaping (Result<Int?, Error>) -> Void) {
        if let error = error {
            completion(.failure(error))
            return
        }
