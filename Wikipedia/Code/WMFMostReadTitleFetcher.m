...
- (AnyPromise *)fetchMostReadTitlesForSiteURL:(NSURL *)siteURL
  ...
  if (siteURL == nil) {
    NSError *error =
        [NSError wmf_errorWithType:WMFErrorTypeInvalidRequestParameters
                          userInfo:nil];
  ...
  NSString *dateString =
      [[NSDateFormatter wmf_englishUTCSlashDelimitedYearMonthDayFormatter]
          stringFromDate:date];
  if (dateString == nil) {
    DDLogError(@"Failed to format pageviews date URL component for date: %@",
               date);
    NSError *error = [NSError
        wmf_errorWithType:WMFErrorTypeInvalidRequestParameters
                 userInfo:@{WMFFailingRequestParametersUserInfoKey : date}];
    return [AnyPromise promiseWithValue:error];
  }

  NSString *path = [NSString
      stringWithFormat:@"/metrics/pageviews/top/%@.%@/all-access/%@",
                       siteURL.wmf_language, siteURL.wmf_domain, dateString];

  NSString *requestURLString = [WMFWikimediaRestAPIURLStringWithVersion(1)
      ...
      [AnyPromise promiseWithResolverBlock:^(PMKResolver resolve) {
        [self.operationManager GET:requestURLString
            ...
            success:^(NSURLSessionDataTask *operation,
              ...
              NSError *parseError;
              WMFMostReadTitlesResponse *titlesResponse =
                  [MTLJSONAdapter modelOfClass:[WMFMostReadTitlesResponse class]
                            fromJSONDictionary:responseObject
                                         error:&parseError];
              WMFMostReadTitlesResponseItem *firstItem =
                  titlesResponse.items.firstObject;

              NSCAssert([[NSCalendar wmf_utcGregorianCalendar]
                                  compareDate:date
                                       toDate:firstItem.date
                            toUnitGranularity:NSCalendarUnitDay] ==
                            NSOrderedSame,
                        @"Date for most-read articles (%@) doesn't match "
                        @"original fetch date: %@",
                        firstItem.date, date);
