...
@property(nonatomic, strong, nullable, readwrite)
    NSArray<MWKSearchResult *> *results;
...
- (instancetype)
initWithSearchTerm:(NSString *)searchTerm
...
- (void)setRedirectMappings:
    (nullable NSArray<MWKSearchRedirectMapping *> *)redirectMappings {
...
+ (NSValueTransformer *)resultsJSONTransformer {
  return [MTLValueTransformer
      transformerUsingForwardBlock:^id(NSDictionary *value, BOOL *success,
                                       NSError *__autoreleasing *error) {
        NSArray *pages = [value allValues];
        NSValueTransformer *transformer = [MTLJSONAdapter
            arrayTransformerWithModelClass:[MWKSearchResult class]];
        return [[transformer transformedValue:pages]
            sortedArrayUsingDescriptors:@[ [self indexSortDescriptor] ]];
      }];
}

+ (NSValueTransformer *)redirectMappingsJSONTransformer {
  return [MTLJSONAdapter
      arrayTransformerWithModelClass:[MWKSearchRedirectMapping class]];
}
...
+ (NSSortDescriptor *)indexSortDescriptor {
  ...
  dispatch_once(&onceToken, ^{
    indexSortDescriptor = [[NSSortDescriptor alloc]
        initWithKey:WMF_SAFE_KEYPATH(MWKSearchResult.new, index)
          ascending:YES];
  });
...
+ (NSDictionary *)JSONKeyPathsByPropertyKey {
  return @{
    ...
    WMF_SAFE_KEYPATH(WMFSearchResults.new, searchSuggestion) :
...
- (void)mergeRedirectMappingsFromModel:(WMFSearchResults *)searchResults {
  NSArray *newMappings = [searchResults.redirectMappings
      bk_reject:^BOOL(MWKSearchRedirectMapping *mapping) {
        return [self.redirectMappings containsObject:mapping];
      }];
  self.redirectMappings =
      [self.redirectMappings arrayByAddingObjectsFromArray:newMappings];
}

- (void)mergeResultsFromModel:(WMFSearchResults *)searchResults {
  NSArray *newResults =
      [searchResults.results bk_reject:^BOOL(MWKSearchResult *obj) {
...
- (void)replaceResultsAtIndexes:(NSIndexSet *)indexes
                    withResults:(NSArray *)entrieArray {
