...
class ReadingListsViewController: ColumnarCollectionViewController {
    ...
    func setupFetchedResultsController() {
        ...
        if let names = readingLists?.flatMap({ $0.name }) {
            ...
            request.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: [basePredicate, NSPredicate(format:"isDefault == NO && SUBQUERY(articles, $a, $a == %@).@count == 0", article)])
        } else {
