 ...
 * @note This will also implicitly remove objects with the same key, as
 * determined by @c index, with the last object
 *       with said key will be in the resulting dictionary.
 * @return A dictionary where keys are the values returned by @c index and the
 ...
 * @param keypath A keypath which is KVC-compliant for all objects in the
 * receiver.
 * @see -bk_index:
 */
