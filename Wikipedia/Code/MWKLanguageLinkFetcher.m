...
- (void)fetchLanguageLinksForArticleURL:(NSURL *)url
  ...
  if (!url.wmf_title.length) {
    NSError *error =
        [NSError errorWithDomain:WMFNetworkingErrorDomain
  ...
  NSURL *apiURL =
      [[SessionSingleton sharedInstance] urlForLanguage:url.wmf_language];
  NSDictionary *params = @{
    @"action" : @"query",
    @"prop" : @"langlinks",
  ...
  [self.manager GET:apiURL.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation,
                NSDictionary *indexedLanguageLinks) {
        ...
        NSArray *languageLinksForTitle =
            [[indexedLanguageLinks allValues] firstObject];
        [self finishWithError:nil
                  fetchedData:languageLinksForTitle
                        block:success];
      }
