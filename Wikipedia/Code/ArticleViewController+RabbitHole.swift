...
extension ArticleViewController {
    struct RabbitHoleData: Equatable {
        let title: String
        let leadImageUrlString: String?
    }

    @objc internal func rabbitHoleShare() {
        if (true) {
            guard let rabbitHoleImage = rabbitHoleImage else {
                return
            }

            let shareVC = ShareViewController(image: rabbitHoleImage, theme: theme)
...
//            rabbitHoleImageView.contentMode = .topLeft
    ...
    func setRabbitHoleStack(with imageURL: URL?) {
        if let nc = self.navigationController,
           ...
           let thisTitle = articleURL.wmf_title {
            ...
            let dataForVC = RabbitHoleData(title: thisTitle, leadImageUrlString: imageURL?.absoluteString ?? article.imageURLString)
            articleRabbitHole?.append(dataForVC)
        } else {
...
//            }).reversed()
            guard let articleTitle = articleURL.wmf_title else {
                assertionFailure("No title found to save")
                return
            }

            if stopVC is SearchViewController {//, let searchTerm = searchVC.searchTerm {
                // I think once I fix the other bug w/ Search not appearing in long press back buttons, I can remove this
                articleRabbitHole = [RabbitHoleData(title: CommonStrings.searchTitle, leadImageUrlString: nil)]
            } else if let baseItemTitle: String = stopVC.navigationItem.backButtonTitle {
                articleRabbitHole = [RabbitHoleData(title: baseItemTitle, leadImageUrlString: nil)]
            }

            let dataForVC = RabbitHoleData(title: articleTitle, leadImageUrlString: imageURL?.absoluteString ?? article.imageURLString)
            articleRabbitHole?.append(dataForVC)
        }
    }

    // TODO: Split this view out into a separate file
    private var rabbitHoleView: UIView? {
        let overallView = UIView()

        guard let articleLabels: [UIView] = articleRabbitHole?.map( {
            return getRabbitHoleChip(for: $0.title, with: $0.leadImageUrlString, hasArrow: !(articleRabbitHole?.last == $0))
        } ) else {
            return nil
        }
        ...
        overallView.setNeedsLayout()
        overallView.layoutIfNeeded()
        
        let rabbitHoleFrame = RabbitHolePresentationView(with: overallView)
        rabbitHoleFrame.translatesAutoresizingMaskIntoConstraints = false
        rabbitHoleFrame.layoutIfNeeded()
        return rabbitHoleFrame
    }
    ...
    private func getRabbitHoleChip(for title: String, with imageURLString: String?, hasArrow: Bool) -> UIView {
        ...
        if let imageURLString = imageURLString, let imageURL = URL(string: imageURLString) {
            leadImageView.wmf_setImage(with: imageURL, detectFaces: false, onGPU: false, failure: {
                                        error in
...
//            leadImageView.backgroundColor = .darkGray
        ...
        if hasArrow {
            let arrowImageView = UIImageView(image: #imageLiteral(resourceName: "moveArrowDown").withRenderingMode(.alwaysTemplate))
