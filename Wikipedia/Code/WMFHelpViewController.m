...
- (UIBarButtonItem *)sendEmailToolbarItem {
    if (!_sendEmailToolbarItem) {
        ...
        _sendEmailToolbarItem.accessibilityLabel = NSLocalizedStringWithDefaultValue(@"button-report-a-bug", nil, NSBundle.wmf_localizationBundle, @"Report a bug", "Button text for reporting a bug");
...
- (void)sendEmail {
    if ([MFMailComposeViewController canSendMail]) {
        ...
        [[WMFAlertManager sharedInstance] showErrorAlertWithMessage:NSLocalizedStringWithDefaultValue(@"no-email-account-alert", nil, NSBundle.wmf_localizationBundle, @"Please setup an email account on your device and try again.", "Displayed to the user when they try to send a feedback email, but they have never set up an account on their device") sticky:NO dismissPreviousAlerts:NO tapCallBack:NULL];
    }
}
