...
- (void)fetchWikiTextWithManager:(AFHTTPSessionManager *)manager {
  NSURL *url = [[SessionSingleton sharedInstance]
      urlForLanguage:self.section.url.wmf_language];

  NSDictionary *params = [self getParams];

  [[MWNetworkActivityIndicatorManager sharedManager] push];

  // Note: "Preview should probably stay as a post, since the wikitext chunk may
  // be
  ...
  [manager GET:url.absoluteString
      ...
      success:^(NSURLSessionDataTask *operation, id responseObject) {
        ...
        if (responseObject[@"error"]) {
          ...
          error = [NSError errorWithDomain:@"Wikitext Fetcher"
                                      code:WIKITEXT_FETCHER_ERROR_API
                                  userInfo:errorDict];
        }
        ...
        if (!error) {
          ...
          if (![output objectForKey:@"revision"] ||
              ![output objectForKey:@"userInfo"]) {
            NSMutableDictionary *errorDict = @{}.mutableCopy;
            errorDict[NSLocalizedDescriptionKey] =
                MWLocalizedString(@"wikitext-download-failed", nil);
            error = [NSError errorWithDomain:@"Wikitext Fetcher"
                                        code:WIKITEXT_FETCHER_ERROR_INCOMPLETE
                                    userInfo:errorDict];
          }
        }
