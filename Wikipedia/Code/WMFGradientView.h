 ...
 *
 *  This is only exposed for use in Interface Builder.  To set this
 * programmatically, use <code>-setStartColor:endColor:</code>.
 */
 ...
 *
 *  These must be set together due to the <code>-[CAGradientLayer
 * setColors:]</code> API forcing them to be set all at once.
 *  This API only allows a start/end color to match the IBDesignable properties
...
/**
 *  Set the point at which the gradient starts fading from @c startColor to @c
 * endColor.
 *
 *  Points should be normalized, and in the CoreGraphics coordinate space:
 * between <code>[0,1]</code> where
