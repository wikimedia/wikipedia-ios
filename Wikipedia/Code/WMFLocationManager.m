 ...
 *
 *  We need to keep track of these properties to ensure that the UI isn't
 * emptied if the location manager is restarted,
 *  which will temporarily set its @c location and @c heading properties to @c
 * nil.
 */
 ...
 *
 *  @note
 *  CLLocationmanager doesn't always immediately listen to the request for
...
/**
 *  Whether or not the receiver made the request for location authorization in
 * order to begin updating location.
 */
@property(nonatomic, assign, readwrite,
          getter=isRequestingAuthorizationAndStart)
    BOOL requestingAuthorizationAndStart;
...
+ (instancetype)fineLocationManager {
  return [[self alloc]
      initWithLocationManager:[CLLocationManager wmf_fineLocationManager]];
}

+ (instancetype)coarseLocationManager {
  return [[self alloc]
      initWithLocationManager:[CLLocationManager wmf_coarseLocationManager]];
}
...
- (NSString *)description {
  ...
  return [NSString
      stringWithFormat:@"<%@ manager: %@ delegate: %@ is updating: %d>",
                       [super description], _locationManager, delegateDesc,
                       self.isUpdating];
}
...
- (BOOL)requestAuthorizationIfNeeded {
  ...
  if (status == kCLAuthorizationStatusNotDetermined) {
    ...
    DDLogInfo(@"%@ is requesting authorization to access location when in use.",
              self);
    [self.locationManager requestWhenInUseAuthorization];
    return YES;
  }
  DDLogVerbose(@"%@ is skipping authorization request because status is %d.",
               self, status);
  return NO;
}
...
- (void)startMonitoringLocation {
  ...
  if ([self requestAuthorizationIfNeeded] ||
      [WMFLocationManager isDeniedOrDisabled] || self.isUpdating) {
    return;
  }
...
- (void)startHeadingUpdates {
  ...
  @weakify(self);
  self.orientationNotificationToken = [[NSNotificationCenter defaultCenter]
...
- (void)updateHeadingOrientation {
  ...
  // Force location manager to re-emit the current heading which will take into
  // account the current device orientation
...
- (void)stopHeadingUpdates {
  [[NSNotificationCenter defaultCenter]
      removeObserver:self.orientationNotificationToken];
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  case kCLAuthorizationStatusRestricted: {
    DDLogVerbose(@"Ignoring not determined status call, should have already "
                 @"requested authorization.");
    break;
  }

  case kCLAuthorizationStatusDenied: {
    if ([self.delegate respondsToSelector:@selector(nearbyController:
  ...
  case kCLAuthorizationStatusAuthorizedAlways: {
    DDLogInfo(@"%@ was granted access to location when in use, attempting to "
              @"monitor location.",
              self);
    if ([self.delegate respondsToSelector:@selector(nearbyController:
                                               didChangeEnabledState:)]) {
    ...
    if (self.isRequestingAuthorizationAndStart) { // only start if we requested
                                                  // as a part of a start
      [self startMonitoringLocation];
    }
    break;
  }
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  // ignore nil or innaccurate values values to keep last known heading on the
  // screen
...
- (void)locationManager:(CLLocationManager *)manager
  ...
  if (!self.isUpdating) {
    DDLogVerbose(@"Suppressing error received after call to stop monitoring "
                 @"location: %@",
                 error);
    return;
  }
