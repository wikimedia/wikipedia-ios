...
@property(nonatomic, strong, nullable)
    NSMutableArray<WMFCVLSection *> *sections;
@end
...
- (id)copyWithZone:(NSZone *)zone {
  ...
  copy.sections =
      [[NSMutableArray allocWithZone:zone] initWithArray:self.sections
                                               copyItems:YES];

  NSMutableArray *columns =
      [[NSMutableArray allocWithZone:zone] initWithCapacity:self.columns.count];
...
- (void)resetColumns {
  self.columns = nil;
  [self enumerateSectionsWithBlock:^(WMFCVLSection *_Nonnull section,
...
- (void)enumerateSectionsWithBlock:
    (nonnull void (^)(WMFCVLSection *_Nonnull section, NSUInteger idx,
                      BOOL *_Nonnull stop))block {
  [self.sections enumerateObjectsUsingBlock:block];
}

- (void)enumerateColumnsWithBlock:
    (nonnull void (^)(WMFCVLColumn *_Nonnull column, NSUInteger idx,
                      BOOL *_Nonnull stop))block {
  [self.columns enumerateObjectsUsingBlock:block];
}

- (nullable WMFCVLAttributes *)layoutAttributesForItemAtIndexPath:
...
- (void)updateContentSizeWithMetrics:(WMFCVLMetrics *)metrics {
  ...
  newSize.height = 0;
  [self enumerateColumnsWithBlock:^(WMFCVLColumn *_Nonnull column,
                                    NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)updateContentSizeWithMetrics:(WMFCVLMetrics *)metrics
                 invalidationContext:(WMFCVLInvalidationContext *)context {
  CGSize oldContentSize = self.contentSize;

  [self updateContentSizeWithMetrics:metrics];

  CGSize contentSizeAdjustment =
      CGSizeMake(self.contentSize.width - oldContentSize.width,
                 self.contentSize.height - oldContentSize.height);
  context.contentSizeAdjustment = contentSizeAdjustment;
}

- (void)updateWithMetrics:(WMFCVLMetrics *)metrics
      invalidationContext:(nullable WMFCVLInvalidationContext *)context
                 delegate:(id<WMFColumnarCollectionViewLayoutDelegate>)delegate
  ...
  if (context.boundsDidChange) {
    [self resetColumns];
    [self layoutWithMetrics:metrics
                   delegate:delegate
             collectionView:collectionView
        invalidationContext:nil];
  } else if (context.originalLayoutAttributes &&
             context.preferredLayoutAttributes) {
    UICollectionViewLayoutAttributes *originalAttributes =
        context.originalLayoutAttributes;
    ...
    if (originalAttributes.representedElementCategory ==
        UICollectionElementCategoryCell) {
      [invalidatedColumn setSize:sizeToSet
              forItemAtIndexPath:indexPath
             invalidationContext:context];
    } else if ([originalAttributes.representedElementKind
                   isEqualToString:UICollectionElementKindSectionHeader]) {
      [invalidatedColumn setSize:sizeToSet
            forHeaderAtIndexPath:indexPath
             invalidationContext:context];
    } else if ([originalAttributes.representedElementKind
                   isEqualToString:UICollectionElementKindSectionFooter]) {
      [invalidatedColumn setSize:sizeToSet
            forFooterAtIndexPath:indexPath
             invalidationContext:context];
    }
    [self updateContentSizeWithMetrics:metrics invalidationContext:context];

    if (self.columns.count == 1 &&
        originalAttributes.frame.origin.y < collectionView.contentOffset.y) {
      context.contentOffsetAdjustment =
          CGPointMake(0, context.contentSizeAdjustment.height);
    }
  } else {
    ...
    [self layoutWithMetrics:metrics
                   delegate:delegate
             collectionView:collectionView
        invalidationContext:nil]; // context is intentionally nil - apparently
                                  // .invalidateEverything and
                                  // .invalidateDataSourceCounts contexts
                                  // shouldn't be updated
  }
}

- (void)layoutWithMetrics:(nonnull WMFCVLMetrics *)metrics
                 delegate:(id<WMFColumnarCollectionViewLayoutDelegate>)delegate
           collectionView:(UICollectionView *)collectionView
      invalidationContext:(nullable WMFCVLInvalidationContext *)context {

  NSInteger numberOfSections = [collectionView.dataSource
  ...
  if (self.columns == nil) {
    CGFloat availableWidth = size.width - contentInsets.left -
                             contentInsets.right -
                             ((numberOfColumns - 1) * interColumnSpacing);
    ...
    for (WMFCVLColumn *column in self.columns) {
      ...
      CGFloat baselineColumnWidth = floor(availableWidth / numberOfColumns);
      CGFloat columnWidthToCheck =
          [columnWeights[column.index] doubleValue] * baselineColumnWidth;
  ...
  for (NSInteger sectionIndex = 0; sectionIndex < numberOfSections;
       sectionIndex++) {
    ...
    NSInteger currentColumnIndex =
        numberOfColumns == 1
            ...
            : [delegate collectionView:collectionView
                  prefersWiderColumnForSectionAtIndex:sectionIndex]
                  ? widestColumnIndex
                  : defaultColumnIndex;
    ...
    if (sectionIndex >= [_sections count]) {
      ...
      if (![column containsSectionWithSectionIndex:sectionIndex]) {
        if (section.columnIndex != NSNotFound &&
            section.columnIndex < _columns.count) {
    ...
    NSIndexPath *supplementaryViewIndexPath =
        [NSIndexPath indexPathForRow:0 inSection:sectionIndex];

    __block CGFloat headerHeight = 0;
    BOOL didCreateOrUpdate = [section
        addOrUpdateHeaderAtIndex:0
               withFrameProvider:^CGRect(BOOL wasCreated,
                                         CGRect existingFrame) {
                 if (wasCreated) {
                   headerHeight = [delegate collectionView:collectionView
                         estimatedHeightForHeaderInSection:sectionIndex
    ...
    for (NSInteger item = 0; item < numberOfItems; item++) {
      ...
      NSIndexPath *itemIndexPath =
          [NSIndexPath indexPathForItem:item inSection:sectionIndex];

      __block CGFloat itemHeight = 0;
      BOOL didCreateOrUpdate =
          [section addOrUpdateItemAtIndex:item
                        withFrameProvider:^CGRect(BOOL wasCreated,
                                                  CGRect existingFrame) {
                          if (wasCreated) {
                            itemHeight = [delegate collectionView:collectionView
                                estimatedHeightForItemAtIndexPath:itemIndexPath
                                                   forColumnWidth:columnWidth];
    ...
    didCreateOrUpdate = [section
        addOrUpdateFooterAtIndex:0
               withFrameProvider:^CGRect(BOOL wasCreated,
                                         CGRect existingFrame) {
                 if (wasCreated) {
                   footerHeight = [delegate collectionView:collectionView
                         estimatedHeightForFooterInSection:sectionIndex
                                            forColumnWidth:columnWidth];
    ...
    section.frame =
        (CGRect){sectionOrigin, CGSizeMake(columnWidth, sectionHeight)};

    [column updateHeightWithDelta:sectionHeight];
  }

  if (_sections.count > numberOfSections) {
    NSRange invalidSectionRange =
        NSMakeRange(numberOfSections, _sections.count - numberOfSections);
  ...
  [context
      invalidateSupplementaryElementsOfKind:UICollectionElementKindSectionHeader
                               atIndexPaths:invalidatedHeaderIndexPaths];
  [context invalidateItemsAtIndexPaths:invalidatedItemIndexPaths];
  [context
      invalidateSupplementaryElementsOfKind:UICollectionElementKindSectionFooter
                               atIndexPaths:invalidatedFooterIndexPaths];
  ...
  if (metrics.shouldMatchColumnHeights && countOfColumns > 1) {
    ...
    for (NSInteger columnIndex = 0; columnIndex < countOfColumns;
         columnIndex++) {
    ...
    while (lastSectionInTallestColumn &&
           lastSectionInTallestColumn.frame.origin.y >
               shortestColumn.frame.size.height) {
      ...
      CGFloat heightDelta =
          lastSectionInTallestColumn.frame.size.height + interSectionSpacing;
      [tallestColumn updateHeightWithDelta:-1 * heightDelta];
      [shortestColumn
          updateHeightWithDelta:heightDelta]; // this is potentially error prone
                                              // - the height will be different
                                              // for different width columns
  ...
  if (needsAnotherLayoutPass) {
    ...
    [self layoutWithMetrics:newMetrics
                   delegate:delegate
             collectionView:collectionView
        invalidationContext:context];
  } else {
    ...
    for (NSIndexSet *set in indexes) {
      for (NSIndexSet *otherSet in indexes) {
        if (set != otherSet) {
          [set enumerateIndexesUsingBlock:^(NSUInteger idx,
                                            BOOL *_Nonnull stop) {
    ...
    for (WMFCVLColumn *column in self.columns) {
      ...
      [column
          enumerateSectionsWithBlock:^(WMFCVLSection *_Nonnull section,
                                       NSUInteger idx, BOOL *_Nonnull stop) {
            assert(section.frame.origin.x == column.frame.origin.x);
            [section enumerateLayoutAttributesWithBlock:^(
                         WMFCVLAttributes *_Nonnull layoutAttributes,
                         BOOL *_Nonnull stop) {
