...
void _WMFRecordFixtureWithBlock(NSString *folder, NSString *filename,
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        if (![[NSUserDefaults standardUserDefaults]
                boolForKey:@"WMFFixtureRecordingEnabled"] ||
            NSClassFromString(@"XCTestCase")) {
          return;
        }
        const char *const fixtureDir = getenv(WMFFixtureDirectoryEnvKey);
        if (fixtureDir) {
          NSString *fixtureFolderPath =
              [NSString stringWithFormat:@"%s/%@", fixtureDir, folder];
          NSError *err;
          if ([[NSFileManager defaultManager]
                        createDirectoryAtPath:fixtureFolderPath
                  withIntermediateDirectories:YES
                                   attributes:nil
                                        error:&err]) {
            block([fixtureFolderPath stringByAppendingPathComponent:filename]);
          } else {
            DDLogError(@"Failed to create fixture directory: %@",
                       fixtureFolderPath);
          }
        }
      });
