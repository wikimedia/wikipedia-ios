...
/**
 * Creates an image URL by appending @c path to @c
 * MWKDataStoreValidImageSitePrefix.
 * @param path The relative path to an image <b>without the leading slash</b>.
 * For example,
...
- (NSString *)pathForSites; // Excluded from iCloud Backup. Includes every site,
                            // article, title.
 ...
 * @param url The @c NSURL for the MWKArticle which contains the desired image
 * info.
 * @return The path to the <b>.plist</b> file where image info for an article
 * would be stored.
 */
 ...
 * @param imageInfo An array of @c MWKImageInfo objects belonging to the
 * specified article.
 * @param url   The url for the article which contains the specified images.
 * @discussion Image info objects are stored under an article so they can be
 * easily referenced and removed alongside
 *             the article.
 */
 ...
 *
 *  This will check memory cache first, falling back to disk if necessary. If
 * data is read from disk, it is inserted
 *  into the memory cache before returning, allowing subsequent calls to this
 * method to hit the memory cache.
 *
 ...
 *
 *  If an article already exists for this title return it. Otherwise, create a
 * new object and return it without saving
 *  it.
 *
