...
- (void)observeSavedPages {
  ...
  [self.KVOControllerNonRetaining
      ...
      options:NSKeyValueObservingOptionInitial
        block:^(WMFSaveButtonController *observer, id object,
                NSDictionary *change) {
...
- (void)toggleSave:(id)sender {
  ...
  if (isSaved) {
    ...
    [[PiwikTracker wmf_configuredInstance]
        wmf_logActionSaveInContext:self.analyticsContext
                       contentType:self.analyticsContentType];
  } else {
    [self.savedPagesFunnel logDelete];
    [[PiwikTracker wmf_configuredInstance]
        wmf_logActionUnsaveInContext:self.analyticsContext
                         contentType:self.analyticsContentType];
  }
