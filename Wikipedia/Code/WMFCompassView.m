...
- (void)drawRect:(CGRect)rect {
  ...
  [self drawCompassLinesInContext:ctx
                           ...
                           radius:compassLineRadius
                             size:CGSizeMake(WMFCompassLineWidth / scale,
                                             compassLineLength)
  ...
  [self
      drawOppositeLineInContext:ctx
                         center:CGPointMake(radius, radius)
                         radius:compassLineRadius
                           size:CGSizeMake(WMFCompassOppositeLineWidth / scale,
                                           oppositeTickLength)];

  // Draw tick (arrow-like directional indicator).
  CGFloat tickPercentOfRectWidth = 0.115;
  CGFloat tickPercentOfRectHeight = 0.135;
  CGSize tickSize = CGSizeMake((rect.size.width - (WMFCompassPadding * 2.0f)) *
                                   tickPercentOfRectWidth,
                               (rect.size.height - (WMFCompassPadding * 2.0f)) *
                                   tickPercentOfRectHeight);
...
- (void)drawTickInContext:(CGContextRef)ctx
  ...
  // Determines how far down from the vertical center the dots forming the base
  // of the
  ...
  CGPoint p2 = CGPointMake(CGRectGetMinX(tickRect),
                           CGRectGetMidY(tickRect) + midpointDescent);
  CGPoint p3 = CGPointMake(CGRectGetMidX(tickRect), CGRectGetMinY(tickRect));
  CGPoint p4 = CGPointMake(CGRectGetMaxX(tickRect),
                           CGRectGetMidY(tickRect) + midpointDescent);
