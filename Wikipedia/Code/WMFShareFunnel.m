...
- (id)initWithArticle:(MWKArticle *)article {
  ...
  if (!title) {
    NSAssert(false, @"%@ : %@", kInitWithArticleAssertVerbiage, [article url]);
    return nil;
  }
  ...
  if (self) {
    ...
    _articleTitle =
        [title wmf_safeSubstringToIndex:WMFEventLoggingMaxStringLength_General];
...
- (NSDictionary *)preprocessData:(NSDictionary *)eventData {
  if (!eventData) {
    NSAssert(false, @"%@ : %@", kEventDataAssertVerbiage, eventData);
    return nil;
  }
...
- (void)logShareButtonTappedResultingInSelection:(NSString *)selection {
  if (!selection) {
    ...
    self.selection = [selection
        wmf_safeSubstringToIndex:WMFEventLoggingMaxStringLength_Snippet];
  }
