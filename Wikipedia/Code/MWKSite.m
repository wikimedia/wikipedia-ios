...
+ (instancetype)siteWithLocale:(NSLocale *)locale {
  return [self siteWithDomain:WMFDefaultSiteDomain
...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  if (self) {
    if (!self.URL) {
      NSString *domain =
          [self decodeValueForKey:@"domain" withCoder:coder modelVersion:0];
      NSString *language =
          [self decodeValueForKey:@"language" withCoder:coder modelVersion:0];
      if (domain) {
...
- (MWKTitle *)titleWithNormalizedTitle:(NSString *)normalizedTitle {
  return [[MWKTitle alloc] initWithSite:self
...
// Need to specify storage properties since domain & language are readonly,
// which Mantle interprets as transitory.
+ (MTLPropertyStorage)storageBehaviorForPropertyWithKey:
    (NSString *)propertyKey {
#define IS_MWKSITE_KEY(key)                                                    \
  [propertyKey isEqualToString:WMF_SAFE_KEYPATH([MWKSite new], key)]
