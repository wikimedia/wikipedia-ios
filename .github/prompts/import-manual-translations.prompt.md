---
description: "Normalize messy translation CSVs from the translations team and insert the translations directly into the appropriate Localizable.strings files. Drop raw CSV files into scripts/manual translations/ and run this prompt."
agent: "agent"
---

You are normalizing raw translation CSV files from the Wikipedia iOS translations team and inserting the translations directly into the appropriate `Wikipedia/Localizations/{lang}.lproj/Localizable.strings` files.


## Step 0: Resolve the input

Process all `.csv` files found in `scripts/manual translations/`. Read `Wikipedia/Localizations/en.lproj/Localizable.strings` **once** before processing any files — reuse the same lookup table for all files.
...
## Step 6: Insert into Localizable.strings

For each matched row (Key is non-empty, Translation is non-empty):

1. Open `Wikipedia/Localizations/{lproj}/Localizable.strings`
2. **Skip** the row if the key already exists in the file (record it for the duplicate section of the summary)
3. **Insert** the entry alphabetically among the existing keys in the format:
   ```
   "key" = "translation";
   ```
4. Preserve all existing content exactly — do not reformat, reorder, or remove any lines, including comments

Rows with a blank Key are silently skipped (not inserted).

Repeat Steps 1–6 for every file in the input list. For multi-language files, process each translation column as a separate language.
...
```
de:  ✅ 47 inserted  ⏭️ 3 already existed  ⚠️ 6 needs review  ℹ️ 14 skipped (no app key)  🔇 243 blank rows
```
...
Keys that already exist in the target `.strings` file and were not overwritten.
Format: `[lang] key` — one per line.
