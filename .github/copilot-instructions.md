...
- Enable Swift 6 language mode in all new Swift packages and targets. Do not suppress concurrency warnings with `@preconcurrency` or `nonisolated(unsafe)` unless absolutely necessary and documented with a comment explaining why.
...
- Use `@MainActor` on classes or methods that interact with UIKit or SwiftUI (view models, hosting controllers, UI-related helpers). All `ObservableObject` view models should be annotated `@MainActor`.
...
- Singleton data controllers (e.g., `WMFImageDataController.shared`, `WMFArticleSummaryDataController.shared`) should be declared as `actor` or fully `@MainActor` depending on their access pattern.
