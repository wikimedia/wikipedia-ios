@objc public final class RemoteNotificationsController: NSObject {
    ...
    private func request(_ queryParameters: Query.Parameters) {
        let _ = Session.shared.requestWithCSRF(type: CSRFTokenJSONDecodableOperation.self, scheme: NotificationsAPI.scheme, host: NotificationsAPI.host, path: NotificationsAPI.path, method: .get, queryParameters: queryParameters, bodyEncoding: .form, tokenContext: tokenContext, operationCompletion: completion)
    }
    ...
    private lazy var tokenContext: CSRFTokenOperation<NotificationsAPIResult>.TokenContext = {
        return CSRFTokenOperation.TokenContext(tokenName: "token", tokenPlacement: .body, shouldPercentEncodeToken: false)
    }()

}

extension RemoteNotificationsController.NotificationsAPIResult.Error: LocalizedError {
