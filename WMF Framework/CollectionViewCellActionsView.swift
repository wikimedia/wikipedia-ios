...
@objc public protocol ActionDelegate: NSObjectProtocol {
    ...
    @objc optional func shouldPerformAction(_ action: Action) -> Bool
...
public enum ActionType {
    ...
    public func action(with target: Any?, indexPath: IndexPath) -> Action {
        switch self {
        case .delete:
            return Action(accessibilityTitle: CommonStrings.deleteActionTitle, icon: UIImage(named: "swipe-action-delete", in: Bundle.wmf, compatibleWith: nil), confirmationIcon: nil, type: .delete, indexPath: indexPath, target: target, selector: #selector(ActionDelegate.shouldPerformAction(_:)))
        case .save:
            return Action(accessibilityTitle: CommonStrings.saveTitle, icon: UIImage(named: "swipe-action-save", in: Bundle.wmf, compatibleWith: nil), confirmationIcon: UIImage(named: "swipe-action-unsave", in: Bundle.wmf, compatibleWith: nil), type: .save, indexPath: indexPath, target: target, selector: #selector(ActionDelegate.shouldPerformAction(_:)))
        case .unsave:
            return Action(accessibilityTitle: CommonStrings.accessibilitySavedTitle, icon: UIImage(named: "swipe-action-unsave", in: Bundle.wmf, compatibleWith: nil), confirmationIcon: UIImage(named: "swipe-action-save", in: Bundle.wmf, compatibleWith: nil), type: .unsave, indexPath: indexPath, target: target, selector: #selector(ActionDelegate.shouldPerformAction(_:)))
        case .share:
            return Action(accessibilityTitle: CommonStrings.shareActionTitle, icon: UIImage(named: "swipe-action-share", in: Bundle.wmf, compatibleWith: nil), confirmationIcon: nil, type: .share, indexPath: indexPath, target: target, selector: #selector(ActionDelegate.shouldPerformAction(_:)))
        }
    }
}
...
public class ActionsView: SizeThatFitsView, Themeable {
    ...
    func createSubviews(for actions: [Action]) {
        ...
        for (index, action) in actions.enumerated() {
            ...
            button.addTarget(self, action: #selector(shouldPerformAction(_:)), for: .touchUpInside)
    ...
    @objc func shouldPerformAction(_ sender: UIButton) {
        let action = actions[sender.tag]
        if let image = action.confirmationIcon {
            sender.setImage(image, for: .normal)
            DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(200)) {
                let _ = self.delegate?.shouldPerformAction?(action)
            }
        } else {
            let _ = delegate?.shouldPerformAction?(action)
        }
    }
