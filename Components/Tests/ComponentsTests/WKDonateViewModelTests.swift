...
final class WKDonateViewModelTests: XCTestCase {
    ...
    func testViewModelInstantiatesWithCorrectDefaultsUSD() {
        ...
        guard let viewModel = WKDonateViewModel(localizedStrings: .demoStringsForCurrencyCode("USD"), donateConfig: donateConfig, paymentMethods: paymentMethods, countryCode: "US", currencyCode: "USD",  languageCode: "EN", merchantID: merchantID, campaignUtmSource: "app_2023_enNL_iOS_control", delegate: nil) else {
    ...
    func testViewModelInstantiatesWithCorrectDefaultsUYU() {
        ...
        guard let viewModel = WKDonateViewModel(localizedStrings: .demoStringsForCurrencyCode("UYU"), donateConfig: donateConfig, paymentMethods: paymentMethods, countryCode: "UY", currencyCode: "UYU", languageCode: "ES", merchantID: merchantID, campaignUtmSource: "app_2023_enNL_iOS_control", delegate: nil) else {
            XCTFail("View model failed to instantiate")
            return
        }
        XCTAssertEqual(viewModel.buttonViewModels.count, 7)
