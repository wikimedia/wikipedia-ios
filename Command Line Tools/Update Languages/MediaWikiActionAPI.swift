...
/// Utility for updating prebuilt lists of information about different language Wikipedias
class WikipediaLanguagesAPI {
    ...
    func getSites() -> AnyPublisher<[Site], Error> {
        ...
        .map { jsonObject -> [Site] in
            /// See above as to why all of this is necessary instead of using codable
            guard let sitematrix = jsonObject["sitematrix"] as? [String: Any] else {
                return []
            }
            return sitematrix.compactMap { (kv) -> Site? in
                guard
                ...
                guard
                    ...
                    let url = wikipedia["url"] as? String,
                    let dbname = wikipedia["dbname"] as? String
                else {
                    return nil
                }
                print(wikipedia)
                return Site(languageCode: code, languageName: name, localName: localname, siteName: sitename, siteURL: url, dbName:dbname)
            }
        }.eraseToAnyPublisher()
    }
