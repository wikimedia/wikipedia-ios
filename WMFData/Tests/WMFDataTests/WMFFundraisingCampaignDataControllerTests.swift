...
final class WMFFundraisingCampaignDataControllerTests {
    ...
    func fetchConfigAndLoadAssetWithValidCountryValidDateValidWiki() async throws {
        try await fixture.withConfiguredEnvironment(configure: configureEnvironment) {
            ...
            #expect(enWikiAsset.actions[0].url == URL(string: "https://donate.wikimedia.org/?country=$country;&uselang=en;&appeal=SupportingWikipedia&wmf_medium=WikipediaApp&wmf_campaign=iOS&wmf_source=$formattedId;&app_install_id=$appInstallId;&app_version=$appVersion;"))
            ...
            #expect(nlWikiAsset.actions[0].url == URL(string: "https://donate.wikimedia.org/?country=$country;&uselang=nl&appeal=SupportingWikipedia&wmf_medium=WikipediaApp&wmf_campaign=iOS&wmf_source=$formattedId;&app_install_id=$appInstallId;&app_version=$appVersion;"))
