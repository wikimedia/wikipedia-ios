...
extension WMFForYouResponse {
    public static let colorTest: WMFForYouResponse = {
        let en = WMFLanguage(languageCode: "en", languageVariantCode: nil)
        let project = WMFProject.wikipedia(en)

        func article(_ title: String) -> WMFForYouArticle {
            WMFForYouArticle(title: title, project: project)
        }

        return WMFForYouResponse(
            interestTopicRandomArticles: [
                WMFForYouInterestTopicRandomArticles(topic: .architecture, articles: [
                    article("Apple Inc."),
                    article("Poland–Russia relations"),
                    article("List of roads in Ottawa"),
                    article("Wrist (Logic song)")
                ]),
                WMFForYouInterestTopicRandomArticles(topic: .biography, articles: [
                    article("Robert Mamątow"),
                    article("Lauren Bennett"),
                    article("Carmelo Cedrún"),
                    article("Gospodin Savršeni")
                ]),
                WMFForYouInterestTopicRandomArticles(topic: .mathematics, articles: [
                    article("Pythagorean theorem"),
                    article("Common logarithm"),
                    article("Transmission electron microscopy"),
                    article("Heart")
                ]),
                WMFForYouInterestTopicRandomArticles(topic: .history, articles: [
                    article("Murder of Henry Nowak"),
                    article("Pulse nightclub shooting"),
                    article("1968 Meckering earthquake"),
                    article("Brain")
                ]),
                WMFForYouInterestTopicRandomArticles(topic: .biology, articles: [
                    article("Black"),
                    article("Saturation diving"),
                    article("Technicolor"),
